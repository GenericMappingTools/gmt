...
EXTERN_MSC struct GMT_CTRL * GMT_begin (struct GMTAPI_CTRL *API, char *session, uint32_t pad);
...
EXTERN_MSC int GMT_init_scales (struct GMT_CTRL *GMT, uint32_t unit, double *fwd_scale, double *inv_scale, double *inch_to_unit, double *unit_to_inch, char *unit_name);
...
EXTERN_MSC double GMT_convert_units (struct GMT_CTRL *GMT, char *value, uint32_t from_default, uint32_t target_unit);
EXTERN_MSC enum GMT_enum_units GMT_get_unit_number (struct GMT_CTRL *GMT, char unit);
EXTERN_MSC uint32_t GMT_check_scalingopt (struct GMT_CTRL *GMT, char option, char unit, char *unit_name);
...
EXTERN_MSC int GMT_hash_lookup (struct GMT_CTRL *GMT, char *key, struct GMT_HASH *hashnode, uint32_t n, uint32_t n_hash);
EXTERN_MSC void GMT_syntax (struct GMT_CTRL *GMT, char option);
EXTERN_MSC void GMT_cont_syntax (struct GMT_CTRL *GMT, uint32_t indent, uint32_t kind);
...
EXTERN_MSC void GMT_label_syntax (struct GMT_CTRL *GMT, uint32_t indent, uint32_t kind);
EXTERN_MSC void GMT_dist_syntax (struct GMT_CTRL *GMT, char option, char *string);
EXTERN_MSC void GMT_vector_syntax (struct GMT_CTRL *GMT, uint32_t mode);
...
EXTERN_MSC int GMT_hash_init (struct GMT_CTRL *GMT, struct GMT_HASH *hashnode , char **keys, uint32_t n_hash, uint32_t n_keys);
...
EXTERN_MSC void GMT_check_lattice (struct GMT_CTRL *GMT, double *inc, uint32_t *registration, bool *active);
...
EXTERN_MSC void GMT_set_pad (struct GMT_CTRL *GMT, uint32_t npad);
...
EXTERN_MSC void GMT_pickdefaults (struct GMT_CTRL *GMT, bool lines, struct GMT_OPTION *options);
...
EXTERN_MSC int GMT_parse_symbol_option (struct GMT_CTRL *GMT, char *text, struct GMT_SYMBOL *p, uint32_t mode, bool cmd);
EXTERN_MSC int GMT_message (struct GMT_CTRL *GMT, char *format, ...);
EXTERN_MSC int GMT_report_func (struct GMT_CTRL *GMT, uint32_t level, const char *source_line, const char *format, ...);
