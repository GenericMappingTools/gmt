...
struct SURFACE_INFO {	/* Control structure for surface setup and execution */
	...
	size_t n_alloc;
	COUNTER npoints;			/* Number of data points */
	COUNTER ij_sw_corner, ij_se_corner,ij_nw_corner, ij_ne_corner;
	COUNTER n_empty;		/* No of unconstrained nodes at initialization  */
	GMT_LONG nx;			/* Number of nodes in x-dir. */
	GMT_LONG ny;			/* Number of nodes in y-dir. (Final grid) */
	COUNTER nxny;			/* Total number of grid nodes without boundaries  */
	GMT_LONG mx;
	GMT_LONG my;
	COUNTER mxmy;			/* Total number of grid nodes with boundaries  */
...
void fill_in_forecast (struct SURFACE_INFO *C) {
	...
	COUNTER index_0, index_1, index_2, index_3, index_new;
	GMT_LONG ii, jj, i, j;
	char *iu = C->iu;
...
int compare_points (const void *point_1v, const void *point_2v)
{
		...
		*/
	COUNTER block_i, block_j, index_1, index_2;
...
void find_nearest_point (struct SURFACE_INFO *C) {
	COUNTER ij_v2, k, last_index, iu_index, briggs_index;
...
{	/*
	 ...
	 */
	COUNTER index_1, index_2, k, k_index;
...
void throw_away_unusables (struct GMT_CTRL *GMT, struct SURFACE_INFO *C)
{
	...
	*/
	...
	COUNTER last_index, n_outside, k;
...
void interp_breakline (struct GMT_CTRL *GMT, struct SURFACE_INFO *C, struct GMT_TABLE *xyzline) {
	...
	COUNTER n_tot = 0, this_ini = 0, this_end = 0, n_int = 0;
	COUNTER k = 0, n, kmax = 0, kmin = 0, seg, row;
	GMT_LONG i, j;
	size_t n_alloc;
