/*--------------------------------------------------------------------
 *	$Id: grdclip_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_grdclip_parse (struct GMTAPI_CTRL *C, struct GRDCLIP_CTRL *Ctrl, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case 'S':	/* Set limits */
				...
				case 'a':
					...
					if (n != 2) {
						GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -Sa option.  Correct syntax:\n\t-Sa<high>/<above>, <above> may be set to NaN\n");
						n_errors++;
					}
					else 
				...
				case 'b':
					...
					if (n != 2) {
						GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -Sb option.  Correct syntax:\n\t-Sb<low>/<below>, <below> may be set to NaN\n");
						n_errors++;
					}
					else
					...
					break;
				default:
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -S option.  Correct syntax:\n\t-Sa<high>/<above> or -Sb<low>/<below>\n");
					n_errors++;
				}
				break;
	...
	n_errors += GMT_check_condition (GMT, !Ctrl->S.mode, "GMT SYNTAX ERROR -S option:  Must specify at least one of -Sa, -Sb\n");
