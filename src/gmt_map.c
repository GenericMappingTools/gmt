...
{	/* Returns true if area/scale are large/small enough
	...
	if (s > 1.0e7) {	/* if s in 1:s exceeds 1e7 we do the quick thing */
		GMT_Report (C->parent, GMT_MSG_VERBOSE, "Warning: Using spherical projection with conformal latitudes\n");
		return (true);
	}
...
bool gmt_quicktm (struct GMT_CTRL *C, double lon0, double limit)
{	/* Returns true if the region chosen is too large for the
	...
	if (fabs (d_left) > limit || fabs (d_right) > limit) {
		GMT_Report (C->parent, GMT_MSG_VERBOSE, "Warning: Using spherical projection with authalic latitudes\n");
		return (true);
	}
	else /* Use full ellipsoidal terms */
		return (false);
...
bool gmt_wesn_outside (struct GMT_CTRL *C, double lon, double lat)
{
...
bool gmt_polar_outside (struct GMT_CTRL *C, double lon, double lat)
{
...
bool gmt_eqdist_outside (struct GMT_CTRL *C, double lon, double lat)
{
	double cc, s, c;
...
bool gmt_radial_outside (struct GMT_CTRL *C, double lon, double lat)
{
	double dist;
...
bool gmt_rect_outside (struct GMT_CTRL *C, double lon, double lat)
{
	double x, y;
...
bool gmt_rect_outside2 (struct GMT_CTRL *C, double lon, double lat)
...
bool gmt_is_wesn_corner (struct GMT_CTRL *C, double x, double y)
...
bool gmt_lon_inside (struct GMT_CTRL *C, double lon, double w, double e)
{
...
unsigned int gmt_wesn_crossing (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1, double *clon, double *clat, double *xx, double *yy, unsigned int *sides)
{
...
bool gmt_is_rect_corner (struct GMT_CTRL *C, double x, double y)
{	/* Checks if point is a corner */
	C->current.map.corner = -1;
...
unsigned int gmt_rect_crossing (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1, double *clon, double *clat, double *xx, double *yy, unsigned int *sides)
{
...
unsigned int gmt_radial_crossing (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2, double *clon, double *clat, double *xx, double *yy, unsigned int *sides)
{
...
int gmt_map_jump_x (struct GMT_CTRL *C, double x0, double y0, double x1, double y1)
{
...
#if 0
/* NOT USED ?? */
int gmt_ellipse_crossing (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2, double *clon, double *clat, double *xx, double *yy, int *sides)
{
...
/* NOT USED ?? */
int GMT_eqdist_crossing (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2, double *clon, double *clat, double *xx, double *yy, int *sides)
{
...
unsigned int gmt_map_crossing (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2, double *xlon, double *xlat, double *xx, double *yy, unsigned int *sides)
{
...
uint64_t GMT_clip_to_map (struct GMT_CTRL *C, double *lon, double *lat, uint64_t np, double **x, double **y)
{
	...
	for (i = out = out_x = out_y = 0; i < np; i++)  {
		...
		out += (abs (C->current.map.this_x_status) == 2 || abs (C->current.map.this_y_status) == 2);
	}
	if (out == 0) {		/* All points are inside map boundary; no clipping required */
		GMT_malloc2 (C, xx, yy, np, NULL, double);
...
uint64_t gmt_move_to_rect (struct GMT_CTRL *C, double *x_edge, double *y_edge, uint64_t j, uint64_t nx)
{
	uint64_t n = 0;
	int key;
	double xtmp, ytmp;
...
uint64_t gmt_rect_clip_old (struct GMT_CTRL *C, double *lon, double *lat, uint64_t n, double **x, double **y, uint64_t *total_nx)
{
	uint64_t i, j = 0;
	unsigned int nx, k;
...
uint64_t gmt_rect_clip (struct GMT_CTRL *C, double *lon, double *lat, uint64_t n, double **x, double **y, uint64_t *total_nx)
{
	uint64_t i, n_get, m;
	size_t n_alloc = 0;
	...
	*total_nx = 1;	/* So that calling program will not discard the clipped polygon */
	...
	if (m) {	/* Reallocate and return the array with the final clipped polygon */
		n_alloc = m;
		GMT_malloc2 (C, xtmp[0], ytmp[0], 0U, &n_alloc, double);
		*x = xtmp[0];
		*y = ytmp[0];
#ifdef DEBUG
		if (dump) {
...
int gmt_move_to_wesn (struct GMT_CTRL *C, double *x_edge, double *y_edge, double lon, double lat, double lon_old, double lat_old, uint64_t j, uint64_t nx)
{
	int n = 0, key;
...
uint64_t gmt_wesn_clip_old (struct GMT_CTRL *C, double *lon, double *lat, uint64_t n, double **x, double **y, uint64_t *total_nx)
{
	uint64_t i, j = 0, nx, k;
	unsigned int sides[4];
	size_t n_alloc = GMT_CHUNK;
...
uint64_t GMT_wesn_clip (struct GMT_CTRL *C, double *lon, double *lat, uint64_t n_orig, double **x, double **y, uint64_t *total_nx)
{
	char *x_type = NULL;
	size_t n_alloc = 0, n_x_alloc = 0, n_t_alloc = 0;
	uint64_t i, n_get, n, m, n_cross = 0, *x_index = NULL;
	unsigned int new_n, range, j, np, side, in = 1, out = 0;
	int way, cross = 0;
	...
	*total_nx = 1;	/* So that calling program will not discard the clipped polygon */
	...
	x_index = GMT_malloc (C, NULL, GMT_TINY_CHUNK, &n_x_alloc, uint64_t);
	...
	for (side = 0; side < 4; side++) {	/* Must clip polygon against a single border, one border at a time */
		...
		for (i = 1; i < n; i++) {	/* For each line segment */
			...
			if (polygon && cross && curved) {	/* When crossing in/out of a curved boundary we must eventually sample along the curve between crossings */
				...
				if (++n_cross == n_x_alloc) {
					x_index = GMT_malloc (C, x_index, n_cross, &n_x_alloc, uint64_t);
		...
		if (polygon && curved && n_cross) {	/* Must resample between crossing points */
			...
			if (n_cross%2 == 1) {	/* Should not happen with a polygon */
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Error in GMT_wesn_clip: odd number of crossings?");
			}
...
uint64_t gmt_radial_boundary_arc (struct GMT_CTRL *C, int this, double end_x[], double end_y[], double **xarc, double **yarc) {
	uint64_t n_arc, k, pt;
	...
	GMT_malloc2 (C, xx, yy, n_arc, NULL, double);
...
uint64_t gmt_radial_clip (struct GMT_CTRL *C, double *lon, double *lat, uint64_t np, double **x, double **y, uint64_t *total_nx)
{
	size_t n_alloc = 0;
	uint64_t n = 0, n_arc;
	unsigned int i, nx;
	...
	*total_nx = 0;	/* Keep track of total of crossings */
	...
	n_alloc = n;
	GMT_malloc2 (C, xx, yy, 0U, &n_alloc, double);
	*x = xx;
	*y = yy;
#ifdef DEBUG
...
bool gmt_rect_overlap (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1)
{
...
bool gmt_wesn_overlap (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1)
{
...
bool gmt_radial_overlap (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1)
{	/* Dummy routine */
	return (true);
}

bool gmt_genper_overlap (struct GMT_CTRL *C, double lon0, double lat0, double lon1, double lat1)
{
/* Dummy routine */
	if (C->current.proj.g_debug > 0) GMT_Report (C->parent, GMT_MSG_DEBUG, "genper_overlap: overlap called\n");
	return (true);
}
...
{	/* Set x/y parameters */
	...
	GMT_Report (C->parent, GMT_MSG_DEBUG, "Projected values in meters: %g %g %g %g\n", xmin, xmax, ymin, ymax);
...
{	/* Set [and rescale] parameters */
	...
	if (C->current.proj.g_debug > 1) {
		GMT_Report (C->parent, GMT_MSG_DEBUG, "xmin %7.3f xmax %7.3f ymin %7.4f ymax %7.3f scale %6.3f\n", xmin/1000, xmax/1000, ymin/1000, ymax/1000, scl);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "gave_map_width %d w %9.4e h %9.4e factor %9.4e\n", C->current.proj.gave_map_width, w, h, factor);
	}
...
double gmt_mean_radius (struct GMT_CTRL *C, double a, double f)
{
	...
	switch (C->current.setting.proj_mean_radius) {
		...
		default:	/* Cannot get here! Safety valve */
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Internal ERROR: GMT mean radius not specified\n");
			exit (EXIT_FAILURE);
			break;
	}
...
void GMT_set_spherical (struct GMT_CTRL *C, bool notify)
{
	...
	if (notify) GMT_Report (C->parent, GMT_MSG_VERBOSE, "Warning: spherical approximation used!\n");
...
double gmt_az_backaz_cartesian (struct GMT_CTRL *C, double lonE, double latE, double lonS, double latS, bool baz)
{
...
double gmt_az_backaz_cartesian_proj (struct GMT_CTRL *C, double lonE, double latE, double lonS, double latS, bool baz)
{
	/* Calculate azimuths or backazimuths.  Cartesian case.
	 * First point is considered "Event" and second "Station".
	 * Azimuth is direction from Station to Event.
...
double gmt_az_backaz_flatearth (struct GMT_CTRL *C, double lonE, double latE, double lonS, double latS, bool baz)
{
...
double gmt_az_backaz_sphere (struct GMT_CTRL *C, double lonE, double latE, double lonS, double latS, bool baz)
{
...
double gmt_az_backaz_geodesic (struct GMT_CTRL *C, double lonE, double latE, double lonS, double latS, bool back_az)
{
	...
	*/
	...
	if (n_iter > 50) {
		...
		GMT_Report (C->parent, GMT_MSG_NORMAL, "Near- or actual antipodal points encountered. Precision may be reduced slightly.\n");
	}
...
void GMT_auto_frame_interval (struct GMT_CTRL *C, unsigned int axis, unsigned int item) {
...
bool GMT_geo_to_xy (struct GMT_CTRL *C, double lon, double lat, double *x, double *y)
...
bool gmt_map_init_linear (struct GMT_CTRL *C) {
	bool positive;
	...
	switch ( (C->current.proj.xyz_projection[GMT_X]%3)) {	/* Modulo 3 so that GMT_TIME (3) maps to GMT_LINEAR (0) */
		...
		case GMT_LOG10:	/* Log10 transformation */
			if (C->common.R.wesn[XLO] <= 0.0 || C->common.R.wesn[XHI] <= 0.0) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error -Jx option:  Limits must be positive for log10 option\n");
...
bool gmt_map_init_polar (struct GMT_CTRL *C)
{
	...
	if (C->current.proj.got_elevations) {	/* Requires s >= 0 and n <= 90 */
		if (C->common.R.wesn[YLO] < 0.0 || C->common.R.wesn[YHI] > 90.0) {
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: -JP...r for elevation plots requires s >= 0 and n <= 90!\n");
			GMT_exit (EXIT_FAILURE);
		}
...
bool gmt_map_init_merc (struct GMT_CTRL *C) {
	...
	if (C->common.R.wesn[YLO] <= -90.0 || C->common.R.wesn[YHI] >= 90.0) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error -R option:  Cannot include south/north poles with Mercator projection!\n");
		GMT_exit (EXIT_FAILURE);
	}
...
bool gmt_map_init_cyleq (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_stereo (struct GMT_CTRL *C) {
	unsigned int i;
		...
		if (C->current.proj.polar) {	/* Polar aspect */
			if (C->current.proj.north_pole) {
				if (C->common.R.wesn[YLO] <= -90.0) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: South boundary cannot be -90.0 for north polar stereographic projection\n");
					GMT_exit (EXIT_FAILURE);
				}
				...
				if (C->common.R.wesn[YHI] >= 90.0) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: North boundary cannot be +90.0 for south polar stereographic projection\n");
					GMT_exit (EXIT_FAILURE);
				}
...
bool gmt_map_init_lambert (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
void gmt_get_origin (struct GMT_CTRL *C, double lon1, double lat1, double lon_p, double lat_p, double *lon2, double *lat2)
{
	...
	if (c != 90.0) {	/* Not true origin */
		...
		*lon2 = lon1 + d_atan2d (sind (d) * sind (az), cosd (lat1) * cosd (d) - sind (lat1) * sind (d) * cosd (az));
		GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "Warning: Correct projection origin = %g/%g\n", *lon2, *lat2);
	}
	else {
		*lon2 = lon1;
		*lat2 = lat1;
...
void gmt_get_rotate_pole (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2)
{
	...
	C->current.proj.o_beta = -beta;
	GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "Oblique Mercator pole is %.12g %.12g, with beta = %.12g\n", plon, plat, -beta);
}
...
unsigned int gmt_wrap_around_check_tm (struct GMT_CTRL *C, double *angle, double last_x, double last_y, double this_x, double this_y, double *xx, double *yy, unsigned int *sides)
{
...
bool gmt_this_point_wraps_tm (struct GMT_CTRL *C, double y0, double y1)
{
...
bool gmt_will_it_wrap_tm (struct GMT_CTRL *C, double *x, double *y, uint64_t n, uint64_t *start)
...
int gmt_map_jump_tm (struct GMT_CTRL *C, double x0, double y0, double x1, double y1)
{
...
bool gmt_map_init_tm (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_utm (struct GMT_CTRL *C) {
...
bool gmt_map_init_lambeq (struct GMT_CTRL *C) {
	unsigned int i;
	...
	if (C->common.R.oblique) {	/* Rectangular box given */
		...
		if (C->current.proj.polar) {	/* Polar aspect */
			if (C->current.proj.north_pole) {
				if (C->common.R.wesn[YLO] <= -90.0){
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: South boundary cannot be -90.0 for north polar Lambert azimuthal projection\n");
					GMT_exit (EXIT_FAILURE);
				}
				if (C->common.R.wesn[YHI] >= 90.0) C->current.proj.edge[2] = false;
			}
				...
				if (C->common.R.wesn[YHI] >= 90.0) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: North boundary cannot be +90.0 for south polar Lambert azimuthal projection\n");
					GMT_exit (EXIT_FAILURE);
				}
				if (C->common.R.wesn[YLO] <= -90.0) C->current.proj.edge[0] = false;
			}
...
bool gmt_map_init_ortho (struct GMT_CTRL *C) {
	unsigned int i;
	double xmin, xmax, ymin, ymax, dummy, radius;
		...
		if (C->current.proj.polar) {	/* Polar aspect */
			if (C->current.proj.north_pole) {
				if (C->common.R.wesn[YLO] < 0.0) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Warning: South boundary cannot be < 0 for north polar orthographic projection (reset to 0)\n");
				...
				if (C->common.R.wesn[YHI] > 0.0) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Warning: North boundary cannot be > 0 for south polar orthographic projection (reset to 0)\n");
...
bool gmt_map_init_genper (struct GMT_CTRL *C) {
	bool search;
	unsigned int i;
	...
	if (C->current.proj.g_debug > 1) {
		GMT_Report (C->parent, GMT_MSG_DEBUG, "genper: units_pr_degree %d\n", C->current.proj.units_pr_degree);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "genper: radius %f\n", radius);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "genper: scale %f units %f\n", scale, units);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "genper: x scale %f y scale %f\n", C->current.proj.scale[GMT_X], C->current.proj.scale[GMT_Y]);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "genper: gave_map_width %d \n",C->current.proj.gave_map_width);
	}
	...
	if (C->current.proj.g_width != 0.0) {
		...
		if (C->current.proj.g_debug > 0) GMT_Report (C->parent, GMT_MSG_DEBUG, "using windowed region\n");
		C->current.map.outside = &gmt_rect_outside2;
		C->current.map.crossing = &gmt_rect_crossing;
		C->current.map.overlap = &gmt_rect_overlap;
		...
		if (C->current.proj.g_debug > 0) GMT_Report (C->parent, GMT_MSG_DEBUG, "using global view\n");
		/* No annotations or tickmarks in global mode */
		for (i = 0; i < GMT_GRID_UPPER; i++)
			C->current.map.frame.axis[GMT_X].item[i].active = C->current.map.frame.axis[GMT_Y].item[i].active = false,
			C->current.map.frame.axis[GMT_X].item[i].interval = C->current.map.frame.axis[GMT_Y].item[i].interval = 0.0;
	...
	if (C->current.proj.g_debug > 0) GMT_Report (C->parent, GMT_MSG_DEBUG, "xmin %f xmax %f ymin %f ymax %f\n", xmin/1000, xmax/1000, ymin/1000, ymax/1000);
	...
	if (C->current.proj.g_debug > 0) {
		GMT_Report (C->parent, GMT_MSG_DEBUG, "x scale %e y scale %e\n", C->current.proj.scale[GMT_X], C->current.proj.scale[GMT_Y]);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "x center %f y center %f\n", C->current.proj.c_x0, C->current.proj.c_y0);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "x max %f y max %f\n", C->current.proj.rect[XHI], C->current.proj.rect[YHI]);
		GMT_Report (C->parent, GMT_MSG_DEBUG, "x0 %f y0 %f\n\n", C->current.proj.origin[GMT_X], C->current.proj.origin[GMT_Y]);
		fflush(NULL);
	}
...
bool gmt_map_init_azeqdist (struct GMT_CTRL *C) {
	unsigned int i;
	double xmin, xmax, ymin, ymax, dummy, radius;
...
bool gmt_map_init_mollweide (struct GMT_CTRL *C) {
...
bool gmt_map_init_hammer (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_eckert4 (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_eckert6 (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_sinusoidal (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_cassini (struct GMT_CTRL *C) {
	bool too_big;
	double xmin, xmax, ymin, ymax;
...
bool gmt_map_init_albers (struct GMT_CTRL *C) {
...
bool gmt_map_init_econic (struct GMT_CTRL *C) {
	double xmin, xmax, ymin, ymax, dy, az, x1, y1;
...
int gmt_horizon_search (struct GMT_CTRL *C, double w, double e, double s, double n, double xmin, double xmax, double ymin, double ymax) {
	...
	if (beyond) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "ERROR: Rectangular region for azimuthal projection extends beyond the horizon\n");
		GMT_Report (C->parent, GMT_MSG_NORMAL, "ERROR: Please select a region that is completely within the visible hemisphere\n");
		GMT_exit (EXIT_FAILURE);
	}
	return (GMT_NOERROR);
}

bool GMT_map_outside (struct GMT_CTRL *C, double lon, double lat)
...
unsigned int gmt_wrap_around_check_x (struct GMT_CTRL *C, double *angle, double last_x, double last_y, double this_x, double this_y, double *xx, double *yy, unsigned int *sides)
{
...
bool gmt_this_point_wraps_x (struct GMT_CTRL *C, double x0, double x1, double w_last, double w_this)
{
...
bool gmt_will_it_wrap_x (struct GMT_CTRL *C, double *x, double *y, uint64_t n, uint64_t *start)
...
uint64_t gmt_map_truncate_x (struct GMT_CTRL *C, double *x, double *y, uint64_t n, uint64_t start, int l_or_r)
...
uint64_t gmt_map_truncate_tm (struct GMT_CTRL *C, double *x, double *y, uint64_t n, uint64_t start, int b_or_t)
...
uint64_t GMT_map_truncate (struct GMT_CTRL *C, double *x, double *y, uint64_t n, uint64_t start, int side)
{	/* Truncates a wrapping polygon against left or right edge.
...
double GMT_distance_type (struct GMT_CTRL *C, double lonS, double latS, double lonE, double latE, unsigned int id)
...
double gmt_geodesic_dist_meter (struct GMT_CTRL *C, double lonS, double latS, double lonE, double latE)
{
	...
	*/
	...
	if (n_iter > VINCENTY_MAX_ITER) {
		C->current.proj.n_geodesic_approx++;	/* Count inaccurate results */
		GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "Near- or actual antipodal points encountered. Precision may be reduced slightly.\n");
		s = M_PI;
	}
	else {
...
uint64_t *GMT_split_line (struct GMT_CTRL *C, double **xx, double **yy, uint64_t *nn, bool add_crossings)
{	/* Accepts x/y array for a line in projected inches and looks for
	...
	for (n_seg = 0, i = 1; i < *nn; i++) {
		if ((l_or_r = gmt_map_jump_x (C, xin[i], yin[i], xin[i-1], yin[i-1]))) {
			if (n_seg == n_alloc) {
				GMT_malloc (C, pos, n_seg, &n_alloc, uint64_t);
				n_alloc = n_seg;
...
uint64_t GMT_graticule_path (struct GMT_CTRL *C, double **x, double **y, int dir, double w, double e, double s, double n)
{	/* Returns the path of a graticule (box of meridians and parallels) */
	...
	if (GMT_IS_RECT_GRATICULE(C)) {	/* Simple rectangle in this projection */
		GMT_malloc2 (C, xx, yy, 5U, NULL, double);
		...
		if (GMT_is_geographic (C, GMT_IN) && s == -90.0) {	/* No path, just a point */
			GMT_malloc2 (C, xx, yy, 1U, &n_alloc, double);
			xx[0] = px1;	yy[0] = -90.0;
		}
		else
		...
		if (GMT_is_geographic (C, GMT_IN) && n == 90.0) {	/* No path, just a point */
			add = 0;
			GMT_malloc2 (C, xtmp, ytmp, 1U, &add, double);
...
uint64_t GMT_map_path (struct GMT_CTRL *C, double lon1, double lat1, double lon2, double lat2, double **x, double **y)
{
...
uint64_t GMT_lonpath (struct GMT_CTRL *C, double lon, double lat1, double lat2, double **x, double **y)
{
	size_t n_alloc = 0;
	uint64_t n, k;
	int n_try, pos;
	...
	if (C->current.map.meridian_straight == 2) {	/* Special non-sampling for gmtselect/grdlandmask */
		GMT_malloc2 (C, tlon, tlat, 2U, NULL, double);
		tlon[0] = tlon[1] = lon;
...
uint64_t GMT_latpath (struct GMT_CTRL *C, double lat, double lon1, double lon2, double **x, double **y)
{
	size_t n_alloc = 0;
	uint64_t k, n;
	int n_try, pos;
	...
	if (C->current.map.parallel_straight) {	/* Easy, just a straight line connection via quarter points */
		GMT_malloc2 (C, tlon, tlat, 5U, NULL, double);
...
uint64_t GMT_geo_to_xy_line (struct GMT_CTRL *C, double *lon, double *lat, uint64_t n)
{
...
uint64_t GMT_compact_line (struct GMT_CTRL *C, double *x, double *y, uint64_t n, int pen_flag, int *pen)
...
int GMT_project_init (struct GMT_CTRL *C, struct GMT_GRID_HEADER *header, double *inc, unsigned int nx, unsigned int ny, unsigned int dpi, unsigned int offset)
{
	if (inc[GMT_X] > 0.0 && inc[GMT_Y] > 0.0) {
		...
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_project_init: Necessary arguments not set\n");
		GMT_exit (EXIT_FAILURE);
	}
	...
	GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "Grid projection from size %dx%d to %dx%d\n", nx, ny, header->nx, header->ny);
	return (GMT_NOERROR);
}


int GMT_grd_project (struct GMT_CTRL *C, struct GMT_GRID *I, struct GMT_GRID *O, bool inverse)
{
	...
	if (I->header->pad[XLO] < 2 || I->header->pad[XHI] < 2 || I->header->pad[YLO] < 2 || I->header->pad[YHI] < 2) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_grd_project: Input grid does not have sufficient (2) padding\n");
		GMT_exit (EXIT_FAILURE);
	}

	/* Precalculate grid coordinates */

	x_in  = GMT_grd_coord (C, I->header, GMT_X);
	y_in  = GMT_grd_coord (C, I->header, GMT_Y);
	x_out = GMT_grd_coord (C, O->header, GMT_X);
	y_out = GMT_grd_coord (C, O->header, GMT_Y);
...
int GMT_img_project (struct GMT_CTRL *C, struct GMT_IMAGE *I, struct GMT_IMAGE *O, bool inverse)
{
	...
	if (I->header->pad[XLO] < 2 || I->header->pad[XHI] < 2 || I->header->pad[YLO] < 2 || I->header->pad[YHI] < 2) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_img_project: Input image does not have sufficient (2) padding\n");
		GMT_exit (EXIT_FAILURE);
	}
...
unsigned int GMT_map_clip_path (struct GMT_CTRL *C, double **x, double **y, bool *donut)
{
	...
	if (C->common.R.oblique)	/* Rectangular map boundary */
		...
		switch (C->current.proj.projection) {
			...
			case GMT_POLYCONIC:
				...
				break;
			default:
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Bad case in GMT_map_clip_path (%d)\n", C->current.proj.projection);
				np = 0;
				break;
		}
	}
...
double GMT_lat_swap (struct GMT_CTRL *C, double lat, unsigned int itype)
{
	...
	if (itype >= GMT_LATSWAP_N) {
		...
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_lat_swap(): Invalid choice, programming bug.\n");
		return(lat);
	}
...
void GMT_init_ellipsoid (struct GMT_CTRL *C)
{
	...
	if (C->current.proj.ECC != 0) { /* avoid division by 0 */
...
void GMT_datum_init (struct GMT_CTRL *C, struct GMT_DATUM *from, struct GMT_DATUM *to, bool heights)
{
...
int GMT_set_datum (struct GMT_CTRL *C, char *text, struct GMT_DATUM *D)
{
	int i;
	double t;
	...
	if (text[0] == '\0' || text[0] == '-') {	/* Shortcut for WGS-84 */
		...
		if (sscanf (text, "%[^:]:%s", ellipsoid, dr) != 2) {
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Malformed <ellipsoid>:<dr> argument!\n");
			return (-1);
		}
		if (sscanf (dr, "%lf,%lf,%lf", &D->xyz[GMT_X], &D->xyz[GMT_Y], &D->xyz[GMT_Z]) != 3) {
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Malformed <x>,<y>,<z> argument!\n");
			return (-1);
		}
		if ((i = GMT_get_ellipsoid (C, ellipsoid)) >= 0) {	/* This includes looking for format <a>,<1/f> */
			...
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Ellipsoid %s not recognized!\n", ellipsoid);
			return (-1);
		}
	}
		...
		if (sscanf (text, "%d", &i) != 1) {
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Malformed or unrecognized <datum> argument (%s)!\n", text);
			return (-1);
		}
		if (i < 0 || i >= GMT_N_DATUMS) {
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Datum ID (%d) outside valid range (0-%d)!\n", i, GMT_N_DATUMS-1);
			return (-1);
		}
...
unsigned int GMT_map_latcross (struct GMT_CTRL *C, double lat, double west, double east, struct GMT_XINGS **xings)
{
	bool go = false;
...
unsigned int GMT_map_loncross (struct GMT_CTRL *C, double lon, double south, double north, struct GMT_XINGS **xings)
{
	bool go = false;
	...
	switch (C->current.proj.xyz_projection[GMT_Z]%3) {	/* Modulo 3 so that GMT_TIME (3) maps to GMT_LINEAR (0) */
		...
		case GMT_LOG10:	/* Log10 transformation */
			if (C->common.R.wesn[ZLO] <= 0.0 || C->common.R.wesn[ZHI] <= 0.0) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error for -Jz -JZ option: limits must be positive for log10 projection\n");
				GMT_exit (EXIT_FAILURE);
			}
...
int GMT_map_setup (struct GMT_CTRL *C, double wesn[])
{
	unsigned int i;
	...
	if (C->current.map.width > 400.0 && !C->PSL) {	/* PSL not active so we assume this is ***project calling */
		...
		GMT_Report (C->parent, GMT_MSG_DEBUG, "Warning: GMT_map_setup perimeter search skipped due to excessive (> 10m) plot size; probably a mapproject process.\n");
	}
...
{	/* Assigns pointers to the chosen distance and azimuth functions */
	...
	switch (mode) {	/* Set pointers to distance functions */
		case GMT_CARTESIAN_DIST:	/* Cartesian 2-D x,y data */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be Cartesian\n", type_name[type]);
			break;
		...
		case GMT_CARTESIAN_DIST_PROJ:	/* Cartesian distance after projecting 2-D lon,lat data */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be Cartesian after first projecting via -J\n", type_name[type]);
			break;
		...
		case GMT_DIST_M+GMT_FLATEARTH:	/* 2-D lon, lat data, but scale to Cartesian flat earth in meter */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be Flat Earth in meters\n", type_name[type]);
			break;
		case GMT_DIST_M+GMT_GREATCIRCLE:	/* 2-D lon, lat data, use spherical distances in meter */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using great circle approximation with %s auxiliary latitudes and %s radius = %.4f m.\n",
		...
		case GMT_DIST_M+GMT_GEODESIC:	/* 2-D lon, lat data, use geodesic distances in meter */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using %s geodesics in meters\n", type_name[type], GEOD_TEXT);
			break;
		case GMT_DIST_DEG+GMT_FLATEARTH:	/* 2-D lon, lat data, use Flat Earth distances in degrees */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_flatearth;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be Flat Earth in degrees\n", type_name[type]);
			break;
		case GMT_DIST_DEG+GMT_GREATCIRCLE:	/* 2-D lon, lat data, use spherical distances in degrees */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_sphere;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using great circle approximation with %s auxiliary latitudes and return lengths in degrees.\n",
		...
		case GMT_DIST_DEG+GMT_GEODESIC:	/* 2-D lon, lat data, use geodesic distances in degrees */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_geodesic;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using geodesics in degrees\n", type_name[type]);
			break;
		case GMT_DIST_COS+GMT_GREATCIRCLE:	/* 2-D lon, lat data, and Green's function needs cosine of spherical distance */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_sphere;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using great circle approximation with %s auxiliary latitudes and return cosine of spherical angles.\n",
				type_name[type], aux[choice]);
			break;
		case GMT_DIST_COS+GMT_GEODESIC:	/* 2-D lon, lat data, and Green's function needs cosine of geodesic distance */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_geodesic;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be using cosine of geodesic angle\n", type_name[type]);
			break;
		case GMT_DIST_M+GMT_LOXODROME:	/* 2-D lon, lat data, but measure distance along rhumblines in meter */
			...
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be along loxodromes in meters\n", type_name[type]);
			break;
		case GMT_DIST_DEG+GMT_LOXODROME:	/* 2-D lon, lat data, but measure distance along rhumblines in degrees */
			...
			C->current.map.azimuth_func = &gmt_az_backaz_loxodrome;
			GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "%s distance calculation will be along loxodromes with %s auxiliary latitudes and return lengths in degrees.\n",
				type_name[type], aux[choice]);
			break;
		default:	/* Cannot happen unless we make a bug */
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Mode (=%d) for distance function is unknown. Must be bug.\n", mode);
			exit (EXIT_FAILURE);
			break;
	}
...
unsigned int GMT_init_distaz (struct GMT_CTRL *C, char unit, unsigned int mode, unsigned int type)
{
	...
	if (strchr (GMT_LEN_UNITS, unit) && !GMT_is_geographic (C, GMT_IN)) {	/* Want geographic distance units but -fg (or -J) not set */
		...
		GMT_Report (C->parent, GMT_MSG_LONG_VERBOSE, "Your distance unit (%c) implies geographic data; -fg has been set.\n", unit);
	}
	...
	switch (unit) {
		...
		default:
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: Distance units must be one of %s\n", GMT_LEN_UNITS_DISPLAY);
			GMT_exit (EXIT_FAILURE);
			break;
	}
