...
int GMT_fclose (struct GMT_CTRL *GMT, FILE *stream)
{
	...
	if ((size_t)stream == (size_t)-GMT->current.io.ncid) {
		...
		GMT->current.io.ncid = GMT->current.io.nvars = GMT->current.io.ncols = 0;
...
static inline unsigned int gmt_n_cols_needed_for_gaps (struct GMT_CTRL *GMT, unsigned int n) {
...
static inline void gmt_update_prev_rec (struct GMT_CTRL *GMT, unsigned int n_use) {
...
int gmt_process_binary_input (struct GMT_CTRL *GMT, unsigned int n_read) {
...
void * gmt_nc_input (struct GMT_CTRL *GMT, FILE *fp, unsigned int *n, int *retval)
{
	...
	else if ((ns = *n) > GMT->current.io.ncols) {
		GMT_Report (GMT->parent, GMT_MSG_NORMAL, "gmt_nc_input is asking for %d columns, but file has only %d\n", *n, GMT->current.io.ncols);
	...
	do {	/* Keep reading until (1) EOF, (2) got a segment record, or (3) a valid data record */
		...
		for (i = j = 0; i < GMT->current.io.nvars && j < n_use; ++i) {
			unsigned n;
...
void GMT_io_banner (struct GMT_CTRL *GMT, unsigned int direction)
{	/* Write verbose message about binary record i/o format */
	...
	if (GMT->current.setting.verbose < GMT_MSG_VERBOSE) return;	/* Not in verbose mode anyway */
	if (!GMT->common.b.active[direction]) return;	/* Not using binary i/o */
...
int GMT_get_cols (struct GMT_CTRL *GMT, unsigned int direction)
{
	...
	if (direction == GMT_IN) {
		...
		unsigned int in_n_cols = (GMT->common.i.active) ? GMT->common.i.n_cols : GMT->common.b.ncol[GMT_IN];
...
int GMT_set_cols (struct GMT_CTRL *GMT, unsigned int direction, unsigned int expected)
...
void * gmt_ascii_input (struct GMT_CTRL *GMT, FILE *fp, unsigned int *n, int *status)
{
...
unsigned int gmt_bin_colselect (struct GMT_CTRL *GMT)
...
bool gmt_get_binary_input (struct GMT_CTRL *GMT, FILE *fp, unsigned int n) {
	...
	for (i = 0; i < n; i++) {
		...
		if (GMT->current.io.fmt[GMT_IN][i].io (GMT, fp, 1, &GMT->current.io.curr_rec[i]) != 1) {
...
void * gmt_bin_input (struct GMT_CTRL *GMT, FILE *fp, unsigned int *n, int *retval)
...
bool gmt_skip_output (struct GMT_CTRL *GMT, double *cols, unsigned int n_cols)
...
int gmt_bin_output (struct GMT_CTRL *GMT, FILE *fp, unsigned int n, double *ptr)
{
	int k;
...
int GMT_ascii_output_col (struct GMT_CTRL *GMT, FILE *fp, double x, unsigned int col)
...
int GMT_ascii_output (struct GMT_CTRL *GMT, FILE *fp, unsigned int n, double *ptr)
{
...
void GMT_ascii_format_col (struct GMT_CTRL *GMT, char *text, double x, unsigned int col)
...
void GMT_quad_reset (struct GMT_CTRL *GMT, struct GMT_QUAD *Q, unsigned int n_items)
...
struct GMT_QUAD * GMT_quad_init (struct GMT_CTRL *GMT, unsigned int n_items)
...
void GMT_add_to_record (struct GMT_CTRL *GMT, char *record, double val, unsigned int col, unsigned int sep)
...
void GMT_write_segmentheader (struct GMT_CTRL *GMT, FILE *fp, unsigned int n_cols)
{
...
void GMT_io_binary_header (struct GMT_CTRL *GMT, FILE *fp, unsigned int dir)
{
	...
	if (dir == GMT_IN) {	/* Use fread since we dont know if input is a stream or a file */
		for (k = 0; k < GMT->current.setting.io_n_header_items; k++) GMT_fread (&c, sizeof (char), 1U, fp);
	}
	else {
...
int gmt_A_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
...
int gmt_c_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_u_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_h_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* read int16_t */
	unsigned i;
	int16_t s;
...
int gmt_h_read_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_i_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* read int32_t */
	unsigned i;
	int32_t s;
...
int gmt_i_read_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_l_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* read int64_t */
	unsigned i;
	int64_t s;
...
int gmt_l_read_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_f_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* read float */
	unsigned i;
	float f;
...
int gmt_f_read_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_d_read (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* read double */
	unsigned i;
	for (i = 0; i < n; ++i) {
...
int gmt_d_read_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_a_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write ascii */
	unsigned i;
	for (i = 0; i < (n - 1); ++i) {
...
int gmt_c_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_u_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_h_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write int16_t */
	unsigned i;
	int16_t s;
...
int gmt_h_write_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_i_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write int32_t */
	unsigned i;
	int32_t s;
...
int gmt_i_write_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_l_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write int64_t */
	unsigned i;
	int64_t s;
...
int gmt_l_write_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_f_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write float */
	unsigned i;
	for (i = 0; i < n; ++i) {
...
int gmt_f_write_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
int gmt_d_write (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
	/* write double */
...
int gmt_d_write_swab (struct GMT_CTRL *GMT, FILE *fp, unsigned n, double *d)
{
...
void GMT_init_z_io (struct GMT_CTRL *GMT, char format[], bool repeat[], enum GMT_swap_direction swab, off_t skip, char type, struct GMT_Z_IO *r)
{
	bool first = true;
	unsigned int k;
...
void * GMT_z_input (struct GMT_CTRL *GMT, FILE *fp, unsigned int *n, int *status)
{
	if ((*status = GMT->current.io.read_item (GMT, fp, *n, GMT->current.io.curr_rec)) == -1) return (NULL);
	return (GMT->current.io.curr_rec);
}
	
int GMT_z_output (struct GMT_CTRL *GMT, FILE *fp, unsigned int n, double *data)
{
...
void GMT_write_newheaders (struct GMT_CTRL *GMT, FILE *fp, unsigned int n_cols)
...
void gmt_adjust_segment (struct GMT_CTRL *GMT, struct GMT_DATASEGMENT *S, unsigned int n_columns)
...
void gmt_adjust_table (struct GMT_CTRL *GMT, struct GMT_DATATABLE *T, unsigned int n_columns)
{
...
void GMT_adjust_dataset (struct GMT_CTRL *GMT, struct GMT_DATASET *D, unsigned int n_columns)
{
...
struct GMT_TEXTSET * GMT_create_textset (struct GMT_CTRL *GMT, unsigned int n_tables, uint64_t n_segments, uint64_t n_rows, bool alloc_only)
...
struct GMT_DATATABLE * gmt_alloc_table (struct GMT_CTRL *GMT, struct GMT_DATATABLE *Tin, unsigned int n_columns, uint64_t n_rows)
{
...
int GMT_alloc_segment (struct GMT_CTRL *GMT, struct GMT_DATASEGMENT *S, uint64_t n_rows, unsigned int n_columns, bool first)
...
struct GMT_DATATABLE * GMT_create_table (struct GMT_CTRL *GMT, uint64_t n_segments, unsigned int n_columns, uint64_t n_rows, bool alloc_only)
{
...
struct GMT_DATASET * GMT_create_dataset (struct GMT_CTRL *GMT, unsigned int n_tables, uint64_t n_segments, unsigned int n_columns, uint64_t n_rows, unsigned int geometry, bool alloc_only)
...
struct GMT_DATATABLE * GMT_read_table (struct GMT_CTRL *GMT, void *source, unsigned int source_type, bool greenwich, bool poly, bool use_GMT_io)
{
	...
	T = GMT_create_table (GMT, GMT_CHUNK, 0, 0, false);
...
struct GMT_DATASET * GMT_alloc_dataset (struct GMT_CTRL *GMT, struct GMT_DATASET *Din, unsigned int n_columns, uint64_t n_rows, unsigned int mode)
{
...
{	/* Make an exact replica, return geometry if not NULL */
	...
	uint64_t seg;
	struct GMT_DATASET *D = NULL;
	D = GMT_alloc_dataset (GMT, Din, Din->n_columns, 0, mode);
...
struct GMT_VECTOR * GMT_create_vector (struct GMT_CTRL *GMT, unsigned int n_columns)
...
struct GMT_MATRIX * GMT_create_matrix (struct GMT_CTRL *GMT, unsigned int layers)
...
double GMT_get_aspatial_value (struct GMT_CTRL *GMT, unsigned int col, struct GMT_DATASEGMENT *S)
{
...
int GMT_load_aspatial_string (struct GMT_CTRL *GMT, struct GMT_OGR *G, unsigned int col, char out[GMT_BUFSIZ])
{
