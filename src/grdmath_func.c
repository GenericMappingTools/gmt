...
GMT_LONG GMT_grdmath_parse (struct GMTAPI_CTRL *C, struct GRDMATH_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	if (missing_equal) {
		GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Usage is <operations> = [outfile]\n");
		n_errors++;
	}
	if (Ctrl->I.active && !GMT->common.R.active) {
		GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  -I requires the -R option\n");
		n_errors++;
	}
	if (Ctrl->I.active && (Ctrl->I.inc[GMT_X] <= 0.0 || Ctrl->I.inc[GMT_Y] <= 0.0)) {
		GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -I option.  Must specify positive increment(s)\n");
		n_errors++;
	}
...
/*OPERATOR: CAZ 2 1 Cartesian azimuth from grid nodes to stack x,y.  */
{
	...
	GMT_grd_padloop (info->G, row, col, node) {
...
/*OPERATOR: CURV 1 1 Curvature of A (Laplacian).  */
{
	...
	GMT_boundcond_param_prep (GMT, stack[last], &edgeinfo);	/* Set parameters for selected BCs */
	GMT_boundcond_set (GMT, stack[last], &edgeinfo);	/* Set the pad nodes using the BCs */
	...
	GMT_row_loop (info->G, row) cx[row] = (float)+0.5 / (info->dx[row] * info->dx[row]);
	...
	GMT_boundcond_set (GMT, stack[last], &edgeinfo);	/* Update the pad nodes using the BCs */
	GMT_free (GMT, z);
	GMT_free (GMT, cx);
}
...
/*OPERATOR: D2DX2 1 1 d^2(A)/dx^2 2nd derivative.  */
{
	...
	GMT_row_loop (info->G, row) {	/* Process d2/dx2 row by row since dx may change with row */
		...
		GMT_col_loop (info->G, row, col, node) {	/* Loop over cols; always save the next left before we update the array at that col */
			left = next_left;
...
/*OPERATOR: D2DY2 1 1 d^2(A)/dy^2 2nd derivative.  */
{
	...
	mx = info->G->header->mx;
	GMT_col_loop (info->G, 0, col, node) {	/* Process d2/dy2 column by column */
...
/*OPERATOR: D2DXY 1 1 d^2(A)/dxdy 2nd derivative.  */
{
	...
	GMT_boundcond_parse (GMT, &edgeinfo, "g");		/* Select BCs */
	GMT_boundcond_param_prep (GMT, stack[last], &edgeinfo);	/* Set parameters for selected BCs */
	GMT_boundcond_set (GMT, stack[last], &edgeinfo);	/* Set the nodes using the BCs */
	...
	GMT_row_loop (info->G, row) cx[row] = 0.5 / (info->dx[row] * info->dx[row]);
...
/*OPERATOR: LOWER 1 1 The lowest (minimum) value of A.  */
{
	...
	GMT_grd_loop (info->G, row, col, node) {	/* First we must find the lowest value in the grid */
		if (GMT_is_fnan (stack[last]->data[node])) continue;
...
/*OPERATOR: PDIST 1 1 Compute distance (in degrees if -fg) from points in ASCII file A.  */
{
	...
	GMT_grd_padloop (info->G, row, col, node) stack[last]->data[node] = (float)GMT_mindist_to_point (GMT, (double)info->grd_x[col], (double)info->grd_y[row], T, dummy);
...
/*OPERATOR: ROTX 2 1 Rotate A by the (constant) shift B in x-direction.  */
{
	...
	GMT_row_loop (info->G, row) {	/* For each row */
		GMT_col_loop (info->G, row, col, node) z[new_col[col]] = stack[prev]->data[node];	/* Copy one row of data to z with shift */
		node = GMT_IJP (info->G->header, row, 0);		/* First col */
...
GMT_LONG decode_grd_argument (struct GMT_CTRL *GMT, struct GMT_OPTION *opt, double *value, struct GMT_HASH *H)
{
	...
	GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR: %s is not a number, operator or file name\n", opt->arg);
	return GRDMATH_ARG_IS_BAD;
}
...
#include "grdmath_op.h"
	...
	for (opt = options; opt; opt = opt->next) {
		if (opt->option == GMTAPI_OPT_INFILE && !strcmp (opt->arg, "=")) {	/* Found the output sequence */
			if (opt->next) {
				...
				GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  No output file specified via = file mechanism\n");
				Return (EXIT_FAILURE);
			}
		}
	...
	if (G_in) {	/* We read a gridfile header above, now update columns */
		if (GMT->common.R.active && Ctrl->I.active) {
			GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Cannot use -R, -I when grid files are specified\n");
			GMT_Destroy_Data (API, GMT_ALLOCATED, (void **)&G_in);
			Return (EXIT_FAILURE);
		}
		else if  (GMT->common.r.active) {
			GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Cannot use -r when grid files are specified\n");
			GMT_Destroy_Data (API, GMT_ALLOCATED, (void **)&G_in);
			Return (EXIT_FAILURE);
		}
		...
		GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Expression must contain at least one grid file or -R, -I\n");
		Return (EXIT_FAILURE);
	}
	...
	for (k = 0, col = -info.G->header->pad[XLO]; k < info.G->header->mx; col++, k++) info.grd_x[k] = (float)GMT_grd_col_to_x (col, info.G->header);
	for (k = 0, row = -info.G->header->pad[YHI]; k < info.G->header->my; row++, k++) info.grd_y[k] = (float)GMT_grd_row_to_y (row, info.G->header);
	...
	for (opt = list, error = FALSE; !error && opt; opt = opt->next) {
		...
		if (op == GRDMATH_ARG_IS_SAVE) {	/* Time to save the current stack to output and pop the stack */
			if (nstack <= 0) {
				GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  No items on stack available for output!\n");
				Return (EXIT_FAILURE);
			}
			...
			if (n_items && new_stack < 0 && constant[nstack-1]) {	/* Only a constant provided, set grid accordingly */
				...
				GMT_grd_loop (info.G, row, col, node) stack[nstack-1]->data[node] = (float)factor[nstack-1];
			}
			this_stack = nstack - 1;
		...
		if ((new_stack = nstack - consumed_operands[op] + produced_operands[op]) >= GRDMATH_STACK_SIZE) {
			GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Stack overflow (%s)\n", opt->arg);
			Return (EXIT_FAILURE);
		}

		if (nstack < consumed_operands[op]) {
			GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Operation \"%s\" requires %ld operands\n", operator[op], consumed_operands[op]);
			Return (EXIT_FAILURE);
		}
