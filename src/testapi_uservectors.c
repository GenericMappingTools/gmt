...
int main (int argc, char *argv[]) {
	...
	for (in = GMT_CHAR; in <= GMT_DOUBLE; in++) {
		for (out = GMT_CHAR; out <= GMT_DOUBLE; out++) {
			if (!quiet) printf ("Test vectors/dataset/vectors(prealloc) for Input = index [%s], output = 10*input + 1 [%s] :", type[in], type[out]);
	...
	for (in = GMT_CHAR, bad = 0; in <= GMT_DOUBLE; in++) {
		for (out = GMT_CHAR; out <= GMT_DOUBLE; out++) {
			if (!quiet) printf ("Test vectors/dataset/vectors(GMT alloc) for Input = index [%s], output = 10*input + 1 [%s] :", type[in], type[out]);
