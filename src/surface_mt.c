 ...
 * Version:	5 API
 *
 * This version is a complete re-write that differs from the original code:
...
/* Go from data y to fractional row y: */
...
#define col_to_x(col,x0,x1,dx,nx) (((int)(col) == (int)((nx)-1)) ? (x1) : (x0) + (col) * (dx))
/* Go from row to y knowing it is a gridline-registered grid: */
#define row_to_y(row,y0,y1,dy,ny) (((int)(row) == (int)((ny)-1)) ? (y0) : (y1) - (row) * (dy))
/* Evaluate the change in LS plane from (0,0) to (xx,yy): */
#define evaluate_trend(C,xx,yy) (C->plane_sx * (xx) + C->plane_sy * (yy))
/* Evaluate the LS plane at location (xx,yy) (this includes the intercept): */
#define evaluate_plane(C,xx,yy) (C->plane_icept + evaluate_trend (C, xx, yy))
/* Extract col from index: */
#define index_to_col(index,nx) ((index) % (nx))
...
GMT_LOCAL void fill_in_forecast (struct GMT_CTRL *GMT, struct SURFACE_INFO *C) {
	...
	double delta_x, delta_y, a0, a1, a2, a3, r_prev_size, a0_plus_a2_dy, a1_plus_a3_dy;
	float *u = C->Grid->data;
	
	/* First restructure the grid to allow for more nodes. We do this by
	 * looping backwards from last node to first so that updated nodes
	 * are always have higher node number than their source.  The
	 * previous grid solution was previous_nx x previous_ny while the new
	 * grid is of dimension current_nx x current_ny.  */
	expand = C->previous_stride / C->current_stride;	/* Multiplicity of new nodes in x and y dimensions */
	...
	for (previous_row = C->previous_ny - 1; previous_row >= 0; previous_row--) {	/* Loop backward over old grid rows */
		row = previous_row * expand;	/* Old row becomes a new row in the new extended grid */
		for (previous_col = C->previous_nx - 1; previous_col >= 0; previous_col--) {	/* Loop backward over old grid cols */
			col = previous_col * expand;	/* Old col becomes a new col in the new extended grid */
			current_node  = row_col_to_node (row, col, C->current_mx);			/* New node index */
	 ...
	 * we just copied, not moved, the nodes) and zeros (since we expand the grid into new unused memory).
	...
	/* Precalculate fractional increments of rows and cols in-between the old constrained rows and cols */
	...
	/* Loop over 4-point "bin squares" from northwest bin to southeast bin */

	for (previous_row = 1; previous_row < C->previous_ny; previous_row++) {	/* Starts at row 1 since it is the base for bin extending to row 0 */
		row = previous_row * expand;	/* Corresponding row in the new extended grid */

		for (previous_col = 0; previous_col < (C->previous_nx-1); previous_col++) {	/* Stop 1 col short of east */
			col = previous_col * expand;	/* Corresponding col in the new extended grid */

			/* Get the indices of the bilinear square [00, 10, 11, 01], with 00 referring to the current node */
			...
			/* Get coefficients for interpolation z = a0 + a1 * delta_x + a2 * delta_y + a3 * delta_x * delta_y */
			a0 = u[index_00];
			a1 = u[index_10] - a0;
			a2 = u[index_01] - a0;
			a3 = u[index_11] - u[index_10] - a2;

			/* Find all possible new fill ins */

			for (jj = 0, first = 1; jj < expand; jj++) {
				a0_plus_a2_dy = a0 + a2 * C->fraction[jj];
				a1_plus_a3_dy = a1 + a3 * C->fraction[jj];
				index_new = index_00 - jj * C->current_mx + first;
				for (ii = first;  ii < expand; ii++, index_new++) {
			...
			status[index_00] = SURFACE_IS_CONSTRAINED;
		}
	}
	...
	for (previous_row = 1; previous_row < C->previous_ny; previous_row++) {
		...
		a2 = u[index_01] - u[index_00];				/* Gradient in u toward ymax (increasing jj) */
		index_new = index_00 - C->current_mx;			/* Since we start at jj = 1 */
	...
	for (previous_col = 0; previous_col < (C->previous_nx-1); previous_col++) {
		...
		a1 = u[index_10] - u[index_00];		/* Gradient in u toward xmax (increasing ii) */
	...
	/* Now set final northeast corner to fixed and our work is done */
...
GMT_LOCAL int compare_points (const void *point_1v, const void *point_2v) {
	...
	*/
	...
	row = index_to_row (point_1->index, GMT_Surface_Global.current_nx);
	col = index_to_col (point_1->index, GMT_Surface_Global.current_nx);
	x0 = col_to_x (col, GMT_Surface_Global.wesn[XLO], GMT_Surface_Global.wesn[XHI], GMT_Surface_Global.inc[GMT_X], GMT_Surface_Global.current_nx);
	y0 = row_to_y (row, GMT_Surface_Global.wesn[YLO], GMT_Surface_Global.wesn[YHI], GMT_Surface_Global.inc[GMT_Y], GMT_Surface_Global.current_ny);
		...
		if (C->data[k].index != last_index) {	/* Moving to the next node to address its nearest data constraint */
			...
			dy = y_to_frow (C->data[k].y, y0, C->r_inc[GMT_Y]);
	 		...
	 		if (fabs (dx) < C->small && fabs (dy) < C->small) {	/* Close enough to assign fixed value to node */
	 			 ...
	 			 * dx,dy in final grid fractions we must scale by current_stride. */
	 			 
	 			z_at_node = C->data[k].z + (float) (C->r_z_scale * C->current_stride * evaluate_trend (C, dx, dy));
	 			if (C->constrained) {
...
GMT_LOCAL void set_grid_parameters (struct SURFACE_INFO *C) {
	/* Let the previous setting become the new settings */
	...
	/* Set the grid indices of the 4 corners */
	...
	rfact = -4.5 / (C->radius*C->radius);
 	for (row = 0; row < C->current_ny; row++) {
		...
		for (col = 0; col < C->current_nx; col++) {
			...
			node = row_col_to_node (row, col+1, C->current_mx);
	 		if (sum_w == 0.0) {
...
GMT_LOCAL int read_data_surface (struct GMT_CTRL *GMT, struct SURFACE_INFO *C, struct GMT_OPTION *options) {
	...
	do {	/* Keep returning records until we reach EOF */
		...
		if (C->periodic && col == 0) {	/* Now we replicate information from the western to the eastern boundary */
			col = C->current_nx - 1;
			C->data[k].index = row_col_to_index (row, col, C->current_nx);
#ifdef DEBUG
			C->data[k].number = k + 1;
			...
			C->data[k].y = (float)in[GMT_Y];	C->data[k].z = (float)in[GMT_Z];
			C->z_mean += in[GMT_Z];
			if (++k == C->n_alloc) {
				C->n_alloc <<= 1;
				C->data = gmt_M_memory (GMT, C->data, C->n_alloc, struct SURFACE_DATA);
	...
	C->z_mean /= k;	/* Estimate mean data value */
	...
	for (end = LO; end <= HI; end++) {
		if (C->set_limit[end] > NONE) {
			if (C->set_limit[end] < SURFACE) {	/* Got a constant level for this end */
				...
				if (C->Bound[end]->header->nx != C->Grid->header->nx || C->Bound[end]->header->ny != C->Grid->header->ny) {
			...
			if (transform) {	/* Remove plane and normalize the bounding values */
				for (row = 0; row < C->Grid->header->ny; row++) {
					yy = (double)(C->Grid->header->ny - row - 1);
					for (col = 0; col < C->Grid->header->nx; col++) {
						node = row_col_to_node (row, col, C->current_mx);
						if (gmt_M_is_fnan (C->Bound[end]->data[node])) continue;
						C->Bound[end]->data[node] -= (float)evaluate_plane (C, col, row);
						C->Bound[end]->data[node] *= (float)C->r_z_scale;
					}
				}
			}
			C->constrained = true;
...
GMT_LOCAL int write_output_surface (struct GMT_CTRL *GMT, struct SURFACE_INFO *C, char *grdfile) {
	...
	if (GMT->common.r.active) {	/* Pixel registration request. Reset limits to the original extent */
	...
	if (C->periodic) {	/* Ensure exact periodicity of E-W boundaries */
		for (row = 0; row < C->current_ny; row++) {
...
{	/* Fill in auxiliary boundary values, see euqations (A-8,9,10) in the reference */
	uint64_t node, node_w, node_e;
	int col, row, *d_node = C->offset;	/* Relative changes in node index from present node */;
	...
	for (col = 0; col < C->current_nx; col++) {	/* set BC1 along south and north side */
		node = C->node_sw_corner + col;	/* Set south: u_{0-1} = 2 * u_{00} - u_{01};  */
		v = u[node+d_node[S1]] = (float)(y_0_const * u[node] + y_1_const * u[node+d_node[N1]]);
		node = C->node_nw_corner + col;	/* Set north: u_01 = 2 * u_00 - u_0-1;  */
		v = u[node+d_node[N1]] = (float)(y_0_const * u[node] + y_1_const * u[node+d_node[S1]]);
	}
	if (C->periodic) {	/* Set periodic boundary conditions in longitude at west and east boundaries */
		for (row = 0, node_w = C->node_nw_corner, node_e = C->node_ne_corner; row < C->current_ny; row++, node_w += C->current_mx, node_e += C->current_mx) {
			u[node_w+d_node[W1]] = u[node_e+d_node[W1]];	/* West side */
			u[node_e+d_node[E1]] = u[node_w+d_node[E1]];	/* East side */
			v = u[node_e] = u[node_w] = 0.5f * (u[node_e] + u[node_w]);	/* Set to average of east and west */
		}
	}
	else {	/* Regular natural BC */
		for (row = 0, node_w = C->node_nw_corner, node_e = C->node_ne_corner; row < C->current_ny; row++, node_w += C->current_mx, node_e += C->current_mx) {
			/* Set west: u_{-10} = 2 * u_{00} - u_{10};  */
			v = u[node_w+d_node[W1]] = (float)(x_1_const * u[node_w+d_node[E1]] + x_0_const * u[node_w]);
			/* Set east: u_{10} = 2 * u_{00} - u_{-10};  */
            		a =u[node_e+d_node[W1]]; c =u[node_e];
			v = u[node_e+d_node[E1]] = (float)(x_1_const * u[node_e+d_node[W1]] + x_0_const * u[node_e]);
		}
	}

	/* Now set d2[]/dxdy = 0 at each of the 4 corners */

	node = C->node_sw_corner;
	v = u[node+d_node[SW]]  = u[node+d_node[SE]] + u[node+d_node[W1]] - u[node+d_node[NE]];
	node = C->node_nw_corner;
	v = u[node+d_node[NW]]  = u[node+d_node[NE]] + u[node+d_node[SW]] - u[node+d_node[SE]];
	node = C->node_se_corner;
	v = u[node+d_node[SE]] = u[node+d_node[SW]]  + u[node+d_node[NE]] - u[node+d_node[NW]];
	node = C->node_ne_corner;
	v = u[node+d_node[NE]]  = u[node+d_node[NW]] + u[node+d_node[SE]] - u[node+d_node[SW]];

	/* Now set dC/dn = 0 at each edge */

	for (col = 0; col < C->current_nx; col++) {	/* set BC2 along south and north side */
		node = C->node_sw_corner + col;	/* South side */
		v = u[node+d_node[S2]] = (float)(u[node+d_node[N2]] + C->eps_m2*(u[node+d_node[NW]] + u[node+d_node[NE]]
			- u[node+d_node[SW]] - u[node+d_node[SE]]) + C->two_plus_em2 * (u[node+d_node[S1]] - u[node+d_node[N1]]));
		
		node = C->node_nw_corner + col;	/* North side */
		v = u[node+d_node[N2]] = -(float)(-u[node+d_node[S2]] + C->eps_m2 * (u[node+d_node[NW]] + u[node+d_node[NE]]
			- u[node+d_node[SW]] - u[node+d_node[SE]]) + C->two_plus_em2 * (u[node + d_node[S1]] - u[node + d_node[N1]]));
	}

	for (row = 0, node_w = C->node_nw_corner, node_e = C->node_ne_corner; row < C->current_ny; row++, node_w += C->current_mx, node_e += C->current_mx) {	/* set BC2 along west and east side */
		if (C->periodic) {	/* Set periodic boundary conditions in longitude */
			v = u[node_w+d_node[W2]] = u[node_e+d_node[W2]];	/* West side */
			v = u[node_e+d_node[E2]] = u[node_w+d_node[E2]];	/* East side */
		}
		else {	/* Natural BCs */
			/* West side */
			v = u[node_w+d_node[W2]] = u[node_w+d_node[E2]] + (float)(C->eps_p2 * (u[node_w+d_node[NE]] + u[node_w+d_node[SE]]
				-u[node_w+d_node[NW]] - u[node_w+d_node[SW]]) + C->two_plus_ep2 * (u[node_w+d_node[W1]] - u[node_w+d_node[E1]]));
			/* East side */
			v = u[node_e+d_node[E2]] = - (float)(-u[node_e+d_node[W2]] + C->eps_p2 * (u[node_e+d_node[NE]] + u[node_e+d_node[SE]]
				- u[node_e+d_node[NW]] - u[node_e+d_node[SW]]) + C->two_plus_ep2 * (u[node_e+d_node[W1]] - u[node_e+d_node[E1]]));
		}
	}
}
...
GMT_LOCAL uint64_t iterate (struct GMT_CTRL *GMT, struct SURFACE_INFO *C, int mode) {
	...
	double change, max_change = 0.0, busum, sum_node, was;
	double *b = NULL;
	...
	do {
		...
		max_change = -1.0;	/* Ensure max_change is < 0 for starters */
#endif
...
#pragma omp parallel for private(col,node,row,sum_node,k,b,busum,briggs_index,change) shared(C,status,u_new,u_old,p) reduction(max : max_change)
		...
		for (row = 0; row < C->current_ny; row++) {	/* Loop over rows */
			...
			for (col = 0; col < C->current_nx; col++, node++) {	/* Loop over all columns */
				...
				sum_node = 0.0;
				set = (status[node] == SURFACE_IS_UNCONSTRAINED) ? SURFACE_UNCONSTRAINED : SURFACE_CONSTRAINED;
				for (k = 0; k < 12; k++)	/* This is either equation (A-4) or the corresponding part of (A-7) */
					sum_node += (u_old[node+d_node[k]] * C->coeff[set][k]);
				...
				sum_node = u_old[node] * C->relax_old + sum_node * C->relax_new;

				if (C->constrained) {	/* Must check that we don't exceed any imposed limits.  */
					if (C->set_limit[LO] && !gmt_M_is_fnan (C->Bound[LO]->data[node]) && sum_node < C->Bound[LO]->data[node])
						sum_node = C->Bound[LO]->data[node];
					else if (C->set_limit[HI] && !gmt_M_is_fnan (C->Bound[HI]->data[node]) && sum_node > C->Bound[HI]->data[node])
						sum_node = C->Bound[HI]->data[node];
				}
				was = u_old[node];
				change = fabs (sum_node - u_old[node]);		/* Change in node value between iterations */
				u_new[node] = (float)sum_node;			/* Our new estimate at this node */
				if (change > max_change) max_change = change;	/* Keep track of max change across all nodes */
				//fprintf (stderr, "I = %d row = %d col = %d node = %d status = %d was = %g new = %g change = %g\n",
				//	iteration_count, row, col, node, (int)status[node], was, sum_node, change);
		...
		max_change *= C->z_scale;	/* Scale max_change back into original z units */
		GMT_Report (GMT->parent, GMT_MSG_LONG_VERBOSE, C->format,
			C->current_stride, C->mode_type[mode], iteration_count, max_change, current_limit, C->total_iterations);
#ifdef OPENMP_	/* Must impose the condition that # of iteration is even so that old_u (i.e. C->Grid->data) holds the final solution */
		finished = ((max_change <= current_limit || iteration_count >= current_max_iterations) && iteration_count%2 == 0);
#else		/* Does not matter here since u_old == u_new anyway */
		finished = (max_change <= current_limit || iteration_count >= current_max_iterations);
#endif
	} while (!finished);
...
GMT_LOCAL void check_errors (struct GMT_CTRL *GMT, struct SURFACE_INFO *C) {
	...
	for (k = 0; k < C->npoints; k++) {
		row = (int)index_to_row (C->data[k].index, C->nx);
		col = (int)index_to_col (C->data[k].index, C->nx);
		node = row_col_to_node (row, col, C->mx);
	 	if (status[node] == SURFACE_IS_CONSTRAINED) continue;
		/* Get coordinates of this node */
		x0 = col_to_x (col, h->wesn[XLO], h->wesn[XHI], h->inc[GMT_X], h->nx);
		y0 = row_to_y (row, h->wesn[YLO], h->wesn[YHI], h->inc[GMT_Y], h->ny);
		/* Get dx,dy of data point away from this node */
...
GMT_LOCAL void remove_planar_trend (struct GMT_CTRL *GMT, struct SURFACE_INFO *C) {
	 ...
	 * Note: Here, x and y are first converted to fractional grid spacings from 0 to {nx,ny}-1.
	...
	for (k = 0; k < C->npoints; k++) {	/* Sum up normal equation terms */
		...
		yy = y_to_frow (C->data[k].y, h->wesn[YLO], h->r_inc[GMT_Y]);
		zz = C->data[k].z;
	...
	for (k = 0; k < C->npoints; k++) {
		xx = x_to_fcol (C->data[k].x, h->wesn[XLO], h->r_inc[GMT_X]);
		yy = y_to_frow (C->data[k].y, h->wesn[YLO], h->r_inc[GMT_Y]);
		C->data[k].z -= (float)evaluate_plane(C, xx, yy);
	}
...
GMT_LOCAL void replace_planar_trend (struct SURFACE_INFO *C) {
	...
	gmt_M_grd_loop (GMT, C->Grid, row, col, node) {
	 	u[node] = (float)((u[node] * C->z_scale) + (evaluate_plane (C, col, row)));
	}
}
...
GMT_LOCAL int rescale_z_values (struct GMT_CTRL *GMT, struct SURFACE_INFO *C) {
	...
	C->z_scale = sqrt (ssz / C->npoints);	/* Set z_scale = rms(z) */
...
GMT_LOCAL void suggest_sizes (struct GMT_CTRL *GMT, struct GMT_GRID *G, unsigned int factors[], unsigned int nx, unsigned int ny) {
	...
	if (n_sug) {	/* We did find some suggestions, report them (up to the first 10 suggestions) */
		...
		for (k = 0; k < n_sug && k < 10; k++) {
			m = sug[k].nx - (G->header->nx - 1);	/* Additional nodes needed in x to give more factors */
			...
			m = sug[k].ny - (G->header->ny - 1);	/* Additional nodes needed in y to give more factors */
			...
			GMT_Report (GMT->parent, GMT_MSG_NORMAL, "Hint: Choosing %s [nx = %d, ny = %d] might cut run time by a factor of %.8g\n",
				region, sug[k].nx, sug[k].ny, sug[k].factor);
		}
		...
		GMT_Report (GMT->parent, GMT_MSG_NORMAL, "Cannot suggest any nx,ny better than your current -R -I settings.\n");
	return;
}
...
GMT_LOCAL void init_surface_parameters (struct SURFACE_INFO *C, struct SURFACE_CTRL *Ctrl) {
	...
	gmt_M_memcpy (GMT_Surface_Global.wesn, C->Grid->header->wesn, 4, double);
}
...
int GMT_surface_mt (void *V_API, int mode, void *args) {
	...
	if ((C.current_stride == 1 && gmt_M_is_verbose (GMT, GMT_MSG_VERBOSE)) || Ctrl->Q.active) suggest_sizes (GMT, C.Grid, C.factors, C.nx-1, C.ny-1);
	...
	mem_use = C.npoints * sizeof (struct SURFACE_DATA);
	GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Using %s for data arrays.\n", gmt_memory_use (mem_use));
	...
	/* Because threads run simultaneously we cannot access the Briggs array sequentially and need a helper index array */
	...
	/* Here is the main action loop over multigrids, enlarging the grid as we solve the smaller problem */
	...
	if ((C.set_limit[HI] > NONE && C.set_limit[HI] < SURFACE) && GMT_Destroy_Data (API, &C.Bound[HI]) != GMT_OK) {
		GMT_Report (API, GMT_MSG_NORMAL, "Failed to free C.Bound[HI]\n");
	}
