...
EXTERN_MSC void GMT_fft_set_k (struct GMT_CTRL *GMT, uint32_t mode, struct GMT_FFT_WAVENUMBER *K);
EXTERN_MSC int GMT_fft_set_wave (struct GMT_CTRL *GMT, uint32_t mode, struct GMT_FFT_WAVENUMBER *K);
EXTERN_MSC double GMT_fft_get_wave (uint64_t k, struct GMT_FFT_WAVENUMBER *K);
EXTERN_MSC double GMT_fft_any_wave (uint64_t k, uint32_t mode, struct GMT_FFT_WAVENUMBER *K);
EXTERN_MSC void GMT_suggest_fft_dim (struct GMT_CTRL *GMT, uint32_t nx, uint32_t ny, struct GMT_FFT_SUGGESTION *fft_sug, bool do_print);

/* gmt_grdio.c: */

EXTERN_MSC void GMT_grd_detrend (struct GMT_CTRL *GMT, struct GMT_GRID *Grid, uint32_t mode, double *a);
...
EXTERN_MSC struct GMT_GRID * GMT_duplicate_grid (struct GMT_CTRL *GMT, struct GMT_GRID *G, uint32_t mode);
...
EXTERN_MSC void GMT_grd_pad_on (struct GMT_CTRL *GMT, struct GMT_GRID *G, uint32_t *pad);
...
EXTERN_MSC int GMT_read_grd (struct GMT_CTRL *GMT, char *file, struct GMT_GRID_HEADER *header, float *grid, double *wesn, uint32_t *pad, int complex_mode);
EXTERN_MSC int GMT_write_grd (struct GMT_CTRL *GMT, char *file, struct GMT_GRID_HEADER *header, float *grid, double *wesn, uint32_t *pad, int complex_mode);
EXTERN_MSC int GMT_adjust_loose_wesn (struct GMT_CTRL *GMT, double wesn[], struct GMT_GRID_HEADER *header);
EXTERN_MSC int GMT_grd_setregion (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *h, double *wesn, uint32_t interpolant);
EXTERN_MSC int GMT_grd_RI_verify (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *h, uint32_t mode);
EXTERN_MSC int GMT_read_img (struct GMT_CTRL *GMT, char *imgfile, struct GMT_GRID *G, double *wesn, double scale, uint32_t mode, double lat, bool init);
EXTERN_MSC int GMT_conv_intext2dbl (struct GMT_CTRL *GMT, char *record, uint32_t ncols);
EXTERN_MSC bool GMT_grd_pad_status (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *header, uint32_t *pad);
EXTERN_MSC int GMT_set_outgrid (struct GMT_CTRL *GMT, char *file, struct GMT_GRID *G, struct GMT_GRID **Out);
EXTERN_MSC int GMT_change_grdreg (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *h, uint32_t registration);
EXTERN_MSC void GMT_grd_shift (struct GMT_CTRL *GMT, struct GMT_GRID *Grid, double shift);
EXTERN_MSC void GMT_grd_set_ij_inc (struct GMT_CTRL *GMT, uint32_t nx, int *ij_inc);
...
EXTERN_MSC int GMT_read_image (struct GMT_CTRL *GMT, char *file, struct GMT_IMAGE *I, double *wesn, 
			uint32_t *pad, uint32_t complex_mode);		/* Function to read true images via GDAL */
...
EXTERN_MSC void GMT_map_clip_on (struct GMT_CTRL *GMT, double rgb[], uint32_t flag);
EXTERN_MSC void GMT_plot_line (struct GMT_CTRL *GMT, double *x, double *y, uint32_t *pen, uint64_t n);
...
EXTERN_MSC void GMT_vertical_axis (struct GMT_CTRL *GMT, uint32_t mode);
...
EXTERN_MSC int GMT_draw_custom_symbol (struct GMT_CTRL *GMT, double x0, double y0, double size[], struct GMT_CUSTOM_SYMBOL *symbol, struct GMT_PEN *pen, struct GMT_FILL *fill, uint32_t outline);
...
EXTERN_MSC void GMT_write_label_record (struct GMT_CTRL *GMT, double x, double y, double angle, char *label, bool save_angle);
...
EXTERN_MSC uint32_t GMT_setfont (struct GMT_CTRL *GMT, struct GMT_FONT *F);
...
EXTERN_MSC void GMT_io_binary_header (struct GMT_CTRL *GMT, FILE *fp, uint32_t dir);
...
EXTERN_MSC uint32_t GMT_quad_finalize (struct GMT_CTRL *GMT, struct GMT_QUAD *Q);
...
EXTERN_MSC void GMT_ascii_format_col (struct GMT_CTRL *GMT, char *text, double x, uint32_t direction, uint64_t col);
EXTERN_MSC void GMT_lon_range_adjust (uint32_t range, double *lon);		/* Adjust the longitude given the desired range */
EXTERN_MSC void GMT_add_to_record (struct GMT_CTRL *GMT, char *record, double val, uint64_t col, uint32_t sep);
EXTERN_MSC int GMT_scanf (struct GMT_CTRL *GMT, char *p, uint32_t expectation, double *val);	/* Convert strings to double, handling special formats [Data records only ] */
EXTERN_MSC int GMT_scanf_arg (struct GMT_CTRL *GMT, char *p, uint32_t expectation, double *val);	/* Convert strings to double, handling special formats [ command line only ] */
...
EXTERN_MSC int GMT_set_cols (struct GMT_CTRL *GMT, uint32_t direction, uint64_t expected);
EXTERN_MSC uint64_t GMT_get_cols (struct GMT_CTRL *GMT, uint32_t direction);
EXTERN_MSC struct GMT_DATASET * GMT_create_dataset (struct GMT_CTRL *GMT, uint64_t n_tables, uint64_t n_segments, uint64_t n_rows, uint64_t n_columns, uint32_t geometry, bool alloc_only);
...
EXTERN_MSC struct GMT_TEXTSET * GMT_duplicate_textset (struct GMT_CTRL *GMT, struct GMT_TEXTSET *Din, uint32_t mode);
EXTERN_MSC void GMT_adjust_dataset (struct GMT_CTRL *GMT, struct GMT_DATASET *D, uint64_t n_columns);
EXTERN_MSC struct GMT_DATASET * GMT_alloc_dataset (struct GMT_CTRL *GMT, struct GMT_DATASET *Din, uint64_t n_rows, uint64_t n_columns, uint32_t mode);
EXTERN_MSC struct GMT_DATASET * GMT_duplicate_dataset (struct GMT_CTRL *GMT, struct GMT_DATASET *Din, uint32_t mode, uint32_t *geometry);
EXTERN_MSC struct GMT_DATATABLE * GMT_read_table (struct GMT_CTRL *GMT, void *source, uint32_t source_type, bool greenwich, uint32_t *geometry, bool use_GMT_io);
EXTERN_MSC int GMT_write_dataset (struct GMT_CTRL *GMT, void *dest, uint32_t dest_type, struct GMT_DATASET *D, bool use_GMT_io, int table);
EXTERN_MSC int GMT_write_table (struct GMT_CTRL *GMT, void *dest, uint32_t dest_type, struct GMT_DATATABLE *T, bool use_GMT_io, uint32_t io_mode);
...
EXTERN_MSC int GMT_alloc_univector (struct GMT_CTRL *GMT, union GMT_UNIVECTOR *u, uint32_t type, uint64_t n_rows);
...
EXTERN_MSC int GMT_parse_segment_header (struct GMT_CTRL *GMT, char *header, struct GMT_PALETTE *P, bool *use_fill, struct GMT_FILL *fill, struct GMT_FILL def_fill, bool *use_pen, struct GMT_PEN *pen, struct GMT_PEN def_pen, uint32_t def_outline, struct GMT_OGR_SEG *G);
...
EXTERN_MSC void GMT_init_io_columns (struct GMT_CTRL *GMT, uint32_t dir);
...
EXTERN_MSC uint32_t GMT_split_line_at_dateline (struct GMT_CTRL *GMT, struct GMT_DATASEGMENT *S, struct GMT_DATASEGMENT ***Lout);
EXTERN_MSC uint32_t GMT_split_poly_at_dateline (struct GMT_CTRL *GMT, struct GMT_DATASEGMENT *S, struct GMT_DATASEGMENT ***Lout);
...
EXTERN_MSC int GMT_grd_BC_set (struct GMT_CTRL *GMT, struct GMT_GRID *G, uint32_t direction);
EXTERN_MSC void GMT_inplace_transpose (float *A, uint32_t n_rows, uint32_t n_cols);
...
EXTERN_MSC bool GMT_row_col_out_of_bounds (struct GMT_CTRL *GMT, double *in, struct GMT_GRID_HEADER *h, uint32_t *row, uint32_t *col);
...
EXTERN_MSC int GMT_write_cpt (struct GMT_CTRL *GMT, void *dest, uint32_t dest_type, uint32_t cpt_flags, struct GMT_PALETTE *P);
EXTERN_MSC void GMT_cpt_transparency (struct GMT_CTRL *GMT, struct GMT_PALETTE *P, double transparency, uint32_t mode);
...
EXTERN_MSC void GMT_contlabel_init (struct GMT_CTRL *GMT, struct GMT_CONTOUR *G, uint32_t mode);
...
EXTERN_MSC char * GMT_make_filename (struct GMT_CTRL *GMT, char *template, uint32_t fmt[], double z, bool closed, uint32_t count[]);
...
EXTERN_MSC uint32_t * GMT_prep_nodesearch (struct GMT_CTRL *GMT, struct GMT_GRID *G, double radius, uint32_t mode, uint32_t *d_row, uint32_t *actual_max_d_col);
...
EXTERN_MSC uint32_t GMT_gcd_euclid (uint32_t a, uint32_t b);
EXTERN_MSC uint32_t GMT_optimal_dim_for_surface (struct GMT_CTRL *GMT, uint32_t factors[], uint32_t nx, uint32_t ny, struct GMT_SURFACE_SUGGESTION **S);
EXTERN_MSC int GMT_best_dim_choice (struct GMT_CTRL *GMT, uint32_t mode, uint32_t in_dim[], uint32_t out_dim[]);
...
EXTERN_MSC void GMT_format_calendar (struct GMT_CTRL *GMT, char *date, char *clock, struct GMT_DATE_IO *D, struct GMT_CLOCK_IO *W, bool upper, uint32_t kind, double dt);
...
EXTERN_MSC void GMT_auto_frame_interval (struct GMT_CTRL *GMT, uint32_t axis, uint32_t item);
...
EXTERN_MSC int GMT_project_init (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *header, double *inc, uint32_t nx, uint32_t ny, uint32_t dpi, uint32_t offset);
...
EXTERN_MSC uint32_t GMT_init_distaz (struct GMT_CTRL *GMT, char c, uint32_t mode, uint32_t type);
EXTERN_MSC bool GMT_near_lines (struct GMT_CTRL *GMT, double lon, double lat, struct GMT_DATATABLE *T, uint32_t return_mindist, double *dist_min, double *x_near, double *y_near);
EXTERN_MSC bool GMT_near_a_line (struct GMT_CTRL *GMT, double lon, double lat, uint64_t seg, struct GMT_DATASEGMENT *S, uint32_t return_mindist, double *dist_min, double *x_near, double *y_near);
...
EXTERN_MSC double GMT_lat_swap (struct GMT_CTRL *GMT, double lat, uint32_t itype);
EXTERN_MSC double GMT_mindist_to_point (struct GMT_CTRL *GMT, double lon, double lat, struct GMT_DATATABLE *T, uint64_t *id);
EXTERN_MSC bool GMT_UTMzone_to_wesn (struct GMT_CTRL *GMT, uint32_t zone_x, char zone_y, int hemi, double wesn[]);
...
EXTERN_MSC int GMT_get_shore_bin (struct GMT_CTRL *GMT, uint32_t b, struct GMT_SHORE *c);
EXTERN_MSC int GMT_get_br_bin (struct GMT_CTRL *GMT, uint32_t b, struct GMT_BR *c, uint32_t *level, uint32_t n_levels);
EXTERN_MSC void GMT_free_shore_polygons (struct GMT_CTRL *GMT, struct GMT_GSHHS_POL *p, uint32_t n);
...
EXTERN_MSC int GMT_prep_shore_polygons (struct GMT_CTRL *GMT, struct GMT_GSHHS_POL **p, uint32_t np, bool sample, double step, int anti_bin);
...
EXTERN_MSC void GMT_matrix_vect_mult (struct GMT_CTRL *GMT, uint32_t dim, double a[3][3], double b[3], double c[3]);
...
EXTERN_MSC void GMT_sort_array (struct GMT_CTRL *GMT, void *base, uint64_t n, uint32_t type);
...
EXTERN_MSC uint64_t GMT_contours (struct GMT_CTRL *GMT, struct GMT_GRID *Grid, uint32_t smooth_factor, uint32_t int_scheme, int orient, uint32_t *edge, bool *first, double **x, double **y);
...
EXTERN_MSC int GMT_getincn (struct GMT_CTRL *GMT, char *line, double inc[], uint32_t n);
...
EXTERN_MSC uint32_t GMT_inonout (struct GMT_CTRL *GMT, double x, double y, const struct GMT_DATASEGMENT *S);
EXTERN_MSC uint32_t GMT_inonout_sphpol (struct GMT_CTRL *GMT, double plon, double plat, const struct GMT_DATASEGMENT *P);
EXTERN_MSC int GMT_intpol (struct GMT_CTRL *GMT, double *x, double *y, uint64_t n, uint64_t m, double *u, double *v, int mode);
EXTERN_MSC int GMT_just_decode (struct GMT_CTRL *GMT, char *key, uint32_t def);
EXTERN_MSC uint32_t GMT_minmaxinc_verify (struct GMT_CTRL *GMT, double min, double max, double inc, double slop);
EXTERN_MSC uint32_t GMT_get_arc (struct GMT_CTRL *GMT, double x0, double y0, double r, double dir1, double dir2, double **x, double **y);
EXTERN_MSC uint32_t GMT_non_zero_winding (struct GMT_CTRL *GMT, double xp, double yp, double *x, double *y, uint64_t n_path);
EXTERN_MSC uint32_t GMT_getmodopt (struct GMT_CTRL *GMT, const char *string, const char *sep, uint32_t *pos, char *token);
EXTERN_MSC uint32_t GMT_verify_expectations (struct GMT_CTRL *GMT, uint32_t wanted, uint32_t got, char *item);
...
EXTERN_MSC struct GMT_PALETTE * GMT_read_cpt (struct GMT_CTRL *GMT, void *source, uint32_t source_type, uint32_t cpt_flags);
EXTERN_MSC void GMT_smart_justify (struct GMT_CTRL *GMT, int just, double angle, double dx, double dy, double *x_shift, double *y_shift, uint32_t mode);
...
EXTERN_MSC struct GMT_DATASET * GMT_resample_data (struct GMT_CTRL *GMT, struct GMT_DATASET *Din, double along_ds, uint32_t mode, uint32_t ex_cols, enum GMT_enum_track smode);
...
EXTERN_MSC uint32_t GMT_get_prime_factors (struct GMT_CTRL *GMT, uint64_t n, uint32_t *f);
...
EXTERN_MSC uint64_t GMT_fix_up_path (struct GMT_CTRL *GMT, double **a_lon, double **a_lat, uint64_t n, double step, uint32_t mode);
EXTERN_MSC int GMT_jacobi (struct GMT_CTRL *GMT, double *a, uint32_t n, uint32_t m, double *d, double *v, double *b, double *z, uint32_t *nrots);
EXTERN_MSC int GMT_gauss (struct GMT_CTRL *GMT, double *a, double *vec, uint32_t n, uint32_t nstore, bool itriag);
EXTERN_MSC int GMT_gaussjordan (struct GMT_CTRL *GMT, double *a, uint32_t n, uint32_t ndim, double *b, uint32_t m, uint32_t mdim);
EXTERN_MSC int GMT_svdcmp (struct GMT_CTRL *GMT, double *a, uint32_t m, uint32_t n, double *w, double *v);
EXTERN_MSC int GMT_solve_svd (struct GMT_CTRL *GMT, double *u, uint32_t m, uint32_t n, double *v, double *w, double *b, uint32_t k, double *x, double *cutoff, uint32_t mode);
