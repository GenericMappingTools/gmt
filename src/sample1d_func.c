/*--------------------------------------------------------------------
 *	$Id: sample1d_func.c,v 1.2 2011-03-15 02:06:37 guru Exp $
 *
...
GMT_LONG GMT_sample1d_parse (struct GMTAPI_CTRL *C, struct SAMPLE1D_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, Ctrl->T.col < 0, "GMT SYNTAX ERROR -T option: Column number cannot be negative\n");
	n_errors += GMT_check_condition (GMT, Ctrl->S.mode == 1 && Ctrl->S.stop <= Ctrl->S.start, "GMT SYNTAX ERROR -S option: <stop> must exceed <start>\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && Ctrl->I.active, "GMT SYNTAX ERROR: Specify only one of -N and -S\n");
	n_errors += GMT_check_condition (GMT, Ctrl->I.active && Ctrl->I.inc <= 0.0, "GMT SYNTAX ERROR -I option: Must specify positive increment\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && GMT_access (GMT, Ctrl->N.file, R_OK), "GMT SYNTAX ERROR -N. Cannot read file %s\n", Ctrl->N.file);
