...
"        currentpoint /PSL_text_y edef /PSL_text_x edef N PSL_text_x PSL_text_y M V char true charpath fs U V char false charpath S U char stringwidth pop 0 G\n"
"      } if\n"
"      PSL_font_OF { % Outline text then fill, if requested\n"
"        currentpoint /PSL_text_y edef /PSL_text_x edef N PSL_text_x PSL_text_y M V char false charpath S U V char true charpath fs U char stringwidth pop 0 G\n"
"      } if\n"
...
"  /PSL_font_F  psl_bits 1536 and 0 eq def   % 0 if just fill\n"
"  /PSL_font_FO psl_bits 512 and 512 eq def  % 512 if fill then outline, 1024 if outline then fill, 0 if just fill\n"
"  /PSL_font_OF psl_bits 1024 and 1024 eq def  % 1024 if outline then fill, 0 if just fill\n"
"  /PSL_n_labels_minus_1 PSL_n_labels 1 sub def	% Upper limit in loop over labels\n"
"  /PSL_usebox PSL_fillbox PSL_drawbox or def	% true if we need box outline for fill or stroke or both\n"
"  0 1 PSL_n_labels_minus_1			% Loop psl_k = 0 < PSL_n_labels\n"
"  { /psl_k exch def				% Current label index psl_k\n"
