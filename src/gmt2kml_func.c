...
GMT_LONG GMT_gmt2kml_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "\tinfiles (in ASCII or binary) have 2 or more columns with (lon,lat) or (lat,lon) in first columns.\n");
...
GMT_LONG GMT_gmt2kml_parse (struct GMTAPI_CTRL *C, struct GMT2KML_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, Ctrl->C.active && !Ctrl->C.file, "GMT SYNTAX ERROR -C.  Need to supply color palette name\n");
	n_errors += GMT_check_condition (GMT, Ctrl->D.active && access (Ctrl->D.file, R_OK), "GMT SYNTAX ERROR -D: Cannot open HTML description file %s\n", Ctrl->D.file);
	...
	n_errors += GMT_check_condition (GMT, Ctrl->R2.automatic && n_files > 1, "GMT SYNTAX ERROR.  -Ra without arguments only accepted for single table\n");
...
#define Return(code) {Free_gmt2kml_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}
...
GMT_LONG GMT_gmt2kml (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_gmt2kml_usage (API, GMTAPI_USAGE));/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmt2kml_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
