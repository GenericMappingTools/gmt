...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Usage (API, 0, "usage: %s [<table>] [-A] [-C<slopegrid>] [-Dx|y] [-E<empty>] [-G%s] [%s] [%s] [-L<indextable>[+b]] [-M] [-N] "
		"[-Q[n]] [%s] [-S[1]] [-T] [%s] [-Z] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s]\n", name, GMT_OUTGRID,
...
#else
	GMT_Usage (API, 0, "usage: %s [<table>] [-A] [-C<slopegrid>] [-Dx|y] [-E<empty>] [-G%s] [%s] [%s] [-L<indextable>[+b]] [-M] [-N] "
		"[-Q] [%s] [-S[1]] [-T] [%s] [-Z] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s]\n", name, GMT_OUTGRID, GMT_I_OPT, 
...
#endif
	GMT_Option (API, "R");
	GMT_Usage (API, 1, "\n-S Output triangle polygons as multiple segments separated by segment headers. Append 1 to "
		"report the polygon numbers start counting at 1 [Default counts from zero]. Cannot be used with -Q.");
		...
		if (Ctrl->M.active || Ctrl->Q.active) {	/* Must find unique edges to output only once */
			...
			if (triplets[GMT_OUT]) {
				...
				for (i = ij = 0; i < np; i++, ij += 3) {
					...
					sprintf (record, "Polygon %d-%d-%d -Z%.8g", link[ij]+one, link[ij+1]+one, link[ij+2]+one, z_mean);
				...
				for (i = ij = 0; i < np; i++, ij += 3) {
					sprintf (record, "Polygon %d-%d-%d -Z%" PRIu64, link[ij]+one, link[ij+1]+one, link[ij+2]+one, i+one);
					if (Ctrl->A.active) {	/* Compute and report area */
						double area = 0.5 * ((xx[link[ij]] - xx[link[ij+2]]) * (yy[link[ij+1]] - yy[link[ij]]) - (xx[link[ij]] - xx[link[ij+1]]) * (yy[link[ij+2]] - yy[link[ij]]));
						sprintf (area_txt, a_format, area);
						strcat (record, area_txt);
