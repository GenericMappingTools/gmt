		...
		switch (opt->option) {
			...
			case 'M':	/* Get arrow parameters */
				...
				if (strchr (opt->arg, '/') && !strchr (opt->arg, '+')) {	/* Old-style args */
					...
					if (n != 4 || GMT_getrgb (GMT, txt_d, Ctrl->M.S.v_fill.rgb)) {
						...
						Ctrl->M.S.h_length = GMT_to_inch (GMT, txt_b);
						Ctrl->M.S.h_width = GMT_to_inch (GMT, txt_c);
						Ctrl->M.S.v_angle = atan (0.5 * Ctrl->M.S.h_width / Ctrl->M.S.h_length);
						Ctrl->M.S.v_outline = Ctrl->M.S.v_paint = TRUE;
					}
				}
				else {
#endif
...
GMT_LONG GMT_psrose (struct GMTAPI_CTRL *API, GMT_LONG mode, void *args)
{
	...
	if (Ctrl->C.active) {
		...
		if (!Ctrl->M.active) {	/* Must supply defaults for the vector attributes */
			...
			Ctrl->M.S.v_width  = VECTOR_LINE_WIDTH * GMT->session.u2u[GMT_PT][GMT_INCH];	/* 9p */
			Ctrl->M.S.v_angle  = 30.0;
			Ctrl->M.S.v_outline = Ctrl->M.S.v_paint = TRUE;
			Ctrl->M.S.v_heads = 2;
			GMT_init_pen (GMT, &Ctrl->M.S.v_pen, VECTOR_LINE_WIDTH);
			GMT_init_fill (GMT, &Ctrl->M.S.v_fill, 0.0, 0.0, 0.0);		/* Default vector fill = black */
		}
		GMT_init_vector_param (GMT, &Ctrl->M.S);
		Ctrl->M.S.v_width = Ctrl->W.pen[1].width * GMT->session.u2u[GMT_PT][GMT_INCH];
		for (i = 0; i < n_modes; i++) {
			...
			dim[0] = xr, dim[1] = yr;
			dim[2] = Ctrl->M.S.v_width, dim[3] = Ctrl->M.S.h_length, dim[4] = Ctrl->M.S.h_width;
			dim[5] = GMT->current.setting.map_vector_shape, dim[6] = Ctrl->M.S.v_heads, dim[7] = Ctrl->M.S.v_side;
			if (Ctrl->M.S.v_outline) GMT_setpen (GMT, &Ctrl->W.pen[1]);
			if (Ctrl->M.S.v_paint) GMT_setfill (GMT, &Ctrl->M.S.v_fill, TRUE);       /* Use fill structure */
