...
#define LM(L,M) ((L)*L_WIDTH+(M))
void GMT_plm_bar_all (struct GMT_CTRL *C, int l, int m, double x, bool ortho, double *P_lm)
{
	int i;
	unsigned int L_WIDTH = l + 1;
	bool csphase = false;
	...
	P[0] = 1.0;	// pmm = 1.0;
	for (L = 1; L <= m; L++)
		P[LM(L,0)] = d_sqrt (1.0 + 0.5/i) * u * P[LM(L-1,0)];
