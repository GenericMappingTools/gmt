...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Option (API, "<");
	GMT_Usage (API, 1, "\n-A[f|m|p|r|R][+l]");
	GMT_Usage (API, -2, "Control how the input track in <table> is resampled when -C or -E are selected:");
	...
	GMT_Usage (API, 3, "1. <length>: The full-length of each cross profile. "
		...
		"Default unit is meter (geographic grids) or user unit (Cartesian grids).", GMT_LEN_UNITS_DISPLAY);
	GMT_Usage (API, 3, "2. <ds>: The sampling interval along the cross-profiles, in units of u.");
	...
	GMT_Usage (API, -2, "Output columns are x, y, dist, az, z1, z2, ...zn (n is number of grids).");
	GMT_Usage (API, 1, "\n-D<dfile>");
	...
	GMT_Usage (API, 3, "+n Negative trough case [Assume a positive center peak].");
	...
	GMT_Usage (API, 3, "a: Mean value [Default].");
	GMT_Usage (API, 3, "l: Lower value.");
	GMT_Usage (API, 3, "L: Lower of positive values only");
	GMT_Usage (API, 3, "m: Median value.");
	GMT_Usage (API, 3, "p: Mode value.");
	GMT_Usage (API, 3, "u: Upper value.");
	GMT_Usage (API, 3, "U: Upper of negative values only.");
	GMT_Usage (API, -2, "The modifiers control what is being written:");
	GMT_Usage (API, 3, "+a Append stacked values to all cross-profiles.");
	GMT_Usage (API, 3, "+c Compute envelope as +/- <fact>*deviation [2]");
	GMT_Usage (API, 3, "+d Append stack deviations to all cross-profiles.");
	...
	for (opt = options; opt; opt = opt->next) {
		switch (opt->option) {
			...
			case 'C':	/* Create cross profiles */
				...
				if (n_units) {
					GMT_Report (API, GMT_MSG_ERROR, "Option -C: Only <length> takes a unit which is shared with <ds> [and <spacing>]\n");
					n_errors++;
				}
				if (n_modes) {
					GMT_Report (API, GMT_MSG_ERROR, "Option -C: Cannot imply different distance modes for <length> and <ds> [and/or <spacing>]\n");
					n_errors++;
				}
				break;
				...
				if ((c = strstr (opt->arg, "+l"))) {	/* Gave +l<listofgrids> */
					...
					while (fgets (file, PATH_MAX, fp)) {
						...
						if (grdtrack_process_one (GMT, file, Ctrl, ng) == 0)
							...
							ng++;
						if (ng > MAX_GRIDS) {
							GMT_Report (API, GMT_MSG_ERROR, "Option -G: Too many grids given via file %s (max = %d)\n", &c[2], MAX_GRIDS);
							n_errors++;
						}
					}
					fclose (fp);
			...
			case 'S':
				...
				while (gmt_strtok (&opt->arg[1], "+", &pos, p)) {
					switch (p[0]) {
						...
						default:
							n_errors++;
							GMT_Report (API, GMT_MSG_ERROR, "Bad modifier (%s) given to -S.\n", p[0]);
							break;
					}
				}
				break;
...
EXTERN_MSC int GMT_grdtrack (void *V_API, int mode, void *args) {
	...
	for (g = 0; g < Ctrl->G.n_grids; g++) {
		...
		if (Ctrl->G.type[g] == 0) {	/* Regular GMT grids */
			if ((GC[g].G = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_CONTAINER_ONLY|GMT_GRID_NEEDS_PAD2, NULL, Ctrl->G.file[g], NULL)) == NULL) {	/* Get header only */
				gmt_M_free (GMT, GC);
				Return (API->error);
			}
			...
			if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_DATA_ONLY|GMT_GRID_NEEDS_PAD2, wesn, Ctrl->G.file[g], GC[g].G) == NULL) {	/* Get subset */
				gmt_M_free (GMT, GC);
				Return (API->error);
			}
	...
	if (Ctrl->C.active) {	/* Special case of requesting cross-profiles for given line segments */
		...
		if (gmt_M_is_cartesian (GMT, GMT_IN) && Ctrl->A.loxo) {
			GMT_Report (API, GMT_MSG_WARNING, "Loxodrome mode ignored for Cartesian data.\n");
			Ctrl->A.loxo = false;
		}
		if (!Ctrl->E.active) {
			...
			if (Ctrl->C.dist_mode == GMT_GEODESIC) {
				GMT_Report (API, GMT_MSG_WARNING, "Cannot use geodesic distances as path interpolation is spherical; changed to spherical\n");
		...
		if (some_outside) GMT_Report (API, GMT_MSG_WARNING, "Some points along your profiles were outside the grid domain(s).\n");
	...
	if (some_outside) GMT_Report (API, GMT_MSG_WARNING, "Some input points were outside the grid domain(s).\n");
	/* Clean up */
	for (g = 0; g < Ctrl->G.n_grids; g++) {
		GMT_Report (API, GMT_MSG_INFORMATION, "Sampled %" PRIu64 " points from grid %s (%d x %d)\n",
