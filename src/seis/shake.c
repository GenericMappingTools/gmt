...
GMT_LOCAL void Free_Ctrl (struct GMT_CTRL *GMT, struct SHAKE_CTRL *C) {	/* Deallocate control structure */
	if (!C) return;
	gmt_M_free(GMT, C);
}
...
GMT_LOCAL int parse (struct GMT_CTRL *GMT, struct SHAKE_CTRL *Ctrl, struct GMT_Z_IO *io, struct GMT_OPTION *options) {
	...
	n_errors += gmt_M_check_condition (GMT, !Ctrl->L.active, "-L option: Must provide a fault file name.\n");
...
/* --------------------------------------------------------------------------------- */
int GMT_shake (void *V_API, int mode, void *args) {
