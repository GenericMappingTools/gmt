...
{	/* Given the internal time representation dt and the formatting information
	...
	if (date && !D->skip) {	/* Not NULL, want to format this string */
		...
		if (D->mw_text)	{						/* Must write month or week name */
			if (D->iso_calendar)
				strncpy (text, GMT->current.time.language.week_name[kind], GMT_TEXT_LEN16);
			else
				strncpy (text, GMT->current.time.language.month_name[kind][ival[D->item_pos[1]]-1], GMT_TEXT_LEN16);
...
{	/* Assemble the annotation label given the formatting options presented */
	...
	switch (T->unit) {
		...
		case 'K':	/*  ISO Weekday name */
			if (T->upper_case) GMT_str_toupper (GMT->current.time.language.day_name[T->flavor][calendar.iso_d%7]);
			sprintf (string, "%s", GMT->current.time.language.day_name[T->flavor][calendar.iso_d%7]);
			break;
