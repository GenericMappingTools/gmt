/*--------------------------------------------------------------------
 *	$Id: gmt_calclock.c,v 1.75 2011-03-15 02:06:35 guru Exp $
 *
...
/* Private functions to this file: 
   ...
   GMT_LONG cal_imod (GMT_LONG x, GMT_LONG y);
   GMT_LONG gyear_from_rd (GMT_LONG date);
   void gcal_from_dt (struct GMT_CTRL *C, double t, struct GMT_gcal *cal);	Break internal time into calendar and clock struct info.
   GMT_LONG gmonth_length (struct GMT_CTRL *C, GMT_LONG year,  GMT_LONG month);	Get the number of days in a month by Gregorian leap rule
   void small_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, GMT_LONG step_secs, GMT_LONG init);  Aux to GMT_moment_interval
*/
...
GMT_LONG splitinteger (struct GMT_CTRL *C, double value, GMT_LONG epsilon, double *doublepart) {
...
*/
	if (n > 0)
		...
		return ((GMT_LONG) (7*n + kday_after (date, kday)));
}
...
/*	Given integer proleptic gregorian calendar year,
	...
	if (cal_imod (gyear, 4) != 0) return (FALSE);
	
	y400 = cal_imod (gyear, 400);
	if (y400 == 0) return (TRUE);
	if (cal_imod (y400, 100) == 0) return (FALSE);
...
/*	Given ISO calendar year, week, day of week, 
	...
	return (id + nth_kday (iw, 0, rdtemp));
}
...
/*	Given rata die integer day number, load calendar structure
	...
	gcal->day_w = cal_imod ((GMT_LONG)date, 7);
...
GMT_LONG GMT_g_ymd_is_bad (struct GMT_CTRL *C, GMT_LONG y, GMT_LONG m, GMT_LONG d) {
	...
	k = gmonth_length (C, y, m);	/* Number of day in the specified month */
...
GMT_LONG GMT_verify_time_step (struct GMT_CTRL *C, GMT_LONG step, char unit) {
	...
	if (step < 0) {
		GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps must be positive.\n");
		return (-1);
	}
	...
	switch (unit) {
...
#endif
			if (step > 60) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in seconds must be <= 60\n");
				retval = -1;
			}
			break;
		case 'm':
		case 'M':
			if (step > 60) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in minutes must be <= 60\n");
				retval = -1;
			}
			break;
		case 'h':
		case 'H':
			if (step > 24) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in hours must be <= 24\n");
				retval = -1;
			}
			break;
		...
		case 'D':
			...
			if (C->current.plot.calclock.date.day_of_year) {
				if (step > 365) {	/* This is probably an error.  */
					GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in year days must be <= 365\n");
					retval = -1;
				}
			}
			else {
				...
				if (step > 31) {
					GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in days of the month must be <= 31\n");
					retval = -1;
				}
			}
			break;
		...
		case 'K':
			if (step > 7) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in weekdays must be <= 7\n");
				retval = -1;
			}
			break;
		case 'r':	/* Gregorian week.  Special case:  since weeks aren't numbered on Gregorian
			...
			if (step != 1) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time step must be 1 for Gregorian weeks\n");
				retval = -1;
			}
			break;
		case 'u':	/* ISO week */
		case 'U':
			if (step > 52) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in weeks must be <= 52\n");
				retval = -1;
			}
			break;
		case 'o':
		case 'O':
			if (step > 12) {
				GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  time steps in months must be <= 12\n");
				retval = -1;
			}
			break;
		...
		case 'p':
			break;
		default:
			GMT_report (C, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Unrecognized time axis unit.\n");
			retval = -1;
			break;
	}
	return (retval);
	...
	switch (p->unit) {
...
#endif
			k = p->step;
			small_moment_interval (C, p, k, init);
			break;
		case 'm':
		case 'M':
		...
		case 'D':
			if (p->step == 1) {
				...
				kml = gmonth_length (C, p->cc[0].year, p->cc[0].month);
		...
		case 'Y':
			if (init) {
				...
				if (C->current.plot.calclock.date.iso_calendar) {
					p->sd[0] = 0.0;
					if (p->step > 1) p->cc[0].iso_y -= cal_imod (p->cc[0].iso_y, p->step);
					p->rd[0] = GMT_rd_from_iywd (C, p->cc[0].iso_y, 1, 1);
				}
				else {
					p->sd[0] = 0.0;
					if (p->step > 1) p->cc[0].year -= cal_imod (p->cc[0].year, p->step);
...
{	/* Given the internal time representation dt and the formatting information
	...
	gcal_from_dt (C, dt + step, &calendar);			/* Convert dt to a complete calendar structure */
...
{	/* Assemble the annotation label given the formatting options presented */
	...
	gcal_from_dt (C, t, &calendar);			/* Convert t to a complete calendar structure */
	...
	switch (T->unit) {
		...
		case 'c':
			...
			break;
		default:
			GMT_report (C, GMT_MSG_FATAL, "INTERNAL ERROR: wrong unit passed to GMT_get_time_label\n");
			sprintf (string, "NaN");
			break;
	}
}
