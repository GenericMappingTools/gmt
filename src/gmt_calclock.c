...
/* Private functions to this file: 
   ...
   void gmt_small_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, int step_secs, int init);  Aux to GMT_moment_interval
*/
...
int64_t GMT_rd_from_gymd (struct GMT_CTRL *C, int gy, int gm, int gd) {
...
int64_t GMT_rd_from_iywd (struct GMT_CTRL *C, int iy, int iw, int id) {
...
int GMT_y2_to_y4_yearfix (struct GMT_CTRL *C, unsigned int y2) {
...
int GMT_verify_time_step (struct GMT_CTRL *C, int step, char unit) {
	...
	if (step < 0) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps must be positive.\n");
		return (-1);
	}
	...
	switch (unit) {
		...
		case 'c':
		case 'C':
			GMT_Report (C->parent, GMT_MSG_COMPAT, "Warning: Unit c for seconds is deprecated; use s.\n");
#endif
		case 's':
		case 'S':
			if (step > 60) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in seconds must be <= 60\n");
				retval = -1;
			}
			break;
		case 'm':
		case 'M':
			if (step > 60) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in minutes must be <= 60\n");
				retval = -1;
			}
			break;
		case 'h':
		case 'H':
			if (step > 24) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in hours must be <= 24\n");
				retval = -1;
			}
			break;
		...
		case 'D':
			...
			if (C->current.plot.calclock.date.day_of_year) {
				if (step > 365) {	/* This is probably an error.  */
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in year days must be <= 365\n");
					retval = -1;
				}
			}
			else {
				...
				if (step > 31) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in days of the month must be <= 31\n");
					retval = -1;
				}
			}
			break;
		...
		case 'K':
			if (step > 7) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in weekdays must be <= 7\n");
				retval = -1;
			}
			break;
		case 'r':	/* Gregorian week.  Special case:  since weeks aren't numbered on Gregorian
			...
			if (step != 1) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time step must be 1 for Gregorian weeks\n");
				retval = -1;
			}
			break;
		case 'u':	/* ISO week */
		case 'U':
			if (step > 52) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in weeks must be <= 52\n");
				retval = -1;
			}
			break;
		case 'o':
		case 'O':
			if (step > 12) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: time steps in months must be <= 12\n");
				retval = -1;
			}
			break;
		...
		case 'p':
			break;
		default:
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Syntax error: Unrecognized time axis unit.\n");
			retval = -1;
			break;
	}
	return (retval);
...
void gmt_small_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, int step_secs, bool init) {
...
void GMT_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, double dt_in, bool init) {
	...
	*/
	...
	switch (p->unit) {
		...
		case 'U':
			if (init) {
				...
				if (GMT_iso_ywd_is_bad (p->cc[0].iso_y, p->cc[0].iso_w, 1) ) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_LOGIC_BUG:  bad ywd on floor (month) in GMT_init_moment_interval()\n");
					return;
				}
			}
		...
		case 'O':
			...
			if (init) {
				...
				if (GMT_g_ymd_is_bad (p->cc[0].year, p->cc[0].month, p->cc[0].day_m) ) {
					GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_LOGIC_BUG:  bad ymd on floor (month) in GMT_init_moment_interval()\n");
					return;
				}
		...
		default:
			...
			GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_LOGIC_BUG:  Bad unit in GMT_init_moment_interval()\n");
			break;
	}
}		

void GMT_format_calendar (struct GMT_CTRL *C, char *date, char *clock, struct GMT_DATE_IO *D, struct GMT_CLOCK_IO *W, bool upper, unsigned int kind, double dt)
...
{	/* Assemble the annotation label given the formatting options presented */
	...
	switch (T->unit) {
		...
		case 's':	/* 2-digit seconds */
			...
			break;
		default:
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Error: wrong unit passed to GMT_get_time_label\n");
			sprintf (string, "NaN");
			break;
	}
}
