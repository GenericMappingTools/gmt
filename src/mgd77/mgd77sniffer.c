...
int GMT_mgd77sniffer_usage (struct GMTAPI_CTRL *API, int level)
{
	...
	GMT_Message (API, GMT_TIME_NONE, "\t[-g<fieldabbrev>,<imggrid>,<scale>,<mode>[,<latmax>]] [-G<fieldabbrev>,<grid>] [-H] [-I<fieldabbrev>,<rec1>,<recN>] [-K]\n");
	GMT_Message (API, GMT_TIME_NONE, "\t[-L<custom_limits_file> ] [-N] [%s] [-Sd|s|t] [-T<gap>]\n",GMT_Rgeo_OPT);
	GMT_Message (API, GMT_TIME_NONE, "\t[-Wc|g|o|s|t|v|x] [-Wc|g|o|s|t|v|x] [%s] [%s] [%s]\n\n", GMT_V_OPT, GMT_bo_OPT, GMT_n_OPT);
...
#include "mgd77snifferdefaults.h"
	...
	for (argno = 0; argno < n_paths; argno++) {		/* Process each ID */
		...
		for (curr = 0; curr < nvalues; curr++) {
			...
			MGD77_rdc2dt (GMT, &M, GMT_rd_from_gymd(GMT, (int)lrint(mgd77snifferdefs[MGD77_YEAR].minValue), 1, 1),0.0)) {
				...
				if (M.bit_pattern[0] & (1 << i)) {
					switch (i) {
						...
						case (MGD77_NQC):
							...
							break;
						case (MGD77_BCC):
							bccCode = (int)lrint (D[curr].number[i]);
						...
						case (MGD77_DAY):	/* Separate case since # of days in a month varies */
							if (!GMT_is_dnan (D[curr].number[i])) {
								if ((E[curr].flags[E77_VALUE] & (1 << MGD77_YEAR)) || (E[curr].flags[E77_VALUE] & (1 << MGD77_MONTH)))
									last_day = (int)lrint (mgd77snifferdefs[i].maxValue);	/* Year or month has error so we use 31 as last day in this month */
								else
									last_day = (int)GMT_gmonth_length ((int)lrint(D[curr].number[MGD77_YEAR]), (int)lrint(D[curr].number[MGD77_MONTH]));			/* Number of day in the specified month */
...
void regress_lms (struct GMT_CTRL *GMT, double *x, double *y, unsigned int nvalues, double *stats, unsigned int col)
{
	...
	d_angle = 1.0;
	limit = 0.1;
	n_angle = (int)lrint ((180.0 - 2 * d_angle) / d_angle) + 1;
...
void read_grid (struct GMT_CTRL *GMT, struct MGD77_GRID_INFO *info, double wesn[], unsigned int interpolant, double threshold) {
	...
	if (info->format == 0) {	/* GMT geographic grid with header */
		...
		info->nx = (unsigned int)lrint ( (info->G->header->wesn[XHI] - info->G->header->wesn[XLO]) / info->G->header->inc[GMT_X]) + info->one_or_zero;
		info->ny = (unsigned int)lrint ( (info->G->header->wesn[YHI] - info->G->header->wesn[YLO]) / info->G->header->inc[GMT_Y]) + info->one_or_zero;
...
int decimate (struct GMT_CTRL *GMT, double *new, double *orig, unsigned int nclean, double min, double max, double delta, double **dec_new, double **dec_orig, unsigned int *extreme, char *fieldTest) {
	...
	n = (unsigned int)lrint ((max - min)/delta) + 1;
	...
	*extreme=0;
	for (j = 0; j < nclean; j++) {
		...
		if (orig[j] >= min && orig[j] <= max) {
			ship_bin = (unsigned int)lrint ((orig[j] - min)/delta);
			grid_bin = (unsigned int)lrint ((new[j] - min)/delta);
