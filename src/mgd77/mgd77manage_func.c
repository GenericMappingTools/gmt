...
#define Return(code) {Free_mgd77manage_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); GMT_exit (code);}

GMT_LONG GMT_mgd77manage (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
...
/* Change this--> */		check = MGD77_igrf10syn (GMT, 0, date, 1, 0.0, xvar[i], yvar[i], IGRF);
	...
	if (Ctrl->A.mode == MODE_g || Ctrl->A.mode == MODE_i) GMT_Destroy_Data (API, GMT_ALLOCATED, (void **)&G);
