/*--------------------------------------------------------------------
 *	$Id: pshistogram_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_pshistogram_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: pshistogram [file] %s -W<width> [%s] [-C<cpt>] [-F] [-G<fill>]\n", GMT_Jx_OPT, GMT_B_OPT);
	...
	GMT_message (GMT, "\t-I Will inquire about min/max x and y.  No plotting is done\n");
	...
	GMT_message (GMT, "\t-S Draws a stairs-step diagram [Default is bar histogram].\n");
...
GMT_LONG GMT_pshistogram_parse (struct GMTAPI_CTRL *C, struct PSHISTOGRAM_CTRL *Ctrl, struct GMT_OPTION *options)
{
...
#ifdef GMT_COMPAT
			case 'T':
				GMT_report (GMT, GMT_MSG_COMPAT, "GMT Warning: The -T option is deprecated; use -i instead.\n");
	...
	n_errors += GMT_check_condition (GMT, !Ctrl->I.active && !GMT_IS_LINEAR (GMT), "GMT SYNTAX ERROR -J option:  Only linear projection supported.\n");
	n_errors += GMT_check_condition (GMT, Ctrl->Z.mode < PSHISTOGRAM_COUNTS || Ctrl->Z.mode > PSHISTOGRAM_LOG10_FREQ_PCT, "GMT SYNTAX ERROR -Z option:  histogram type must be in 0-5 range\n");
	n_errors += GMT_check_condition (GMT, !Ctrl->W.active, "GMT SYNTAX ERROR -W option:  Must specify bin width\n");
	n_errors += GMT_check_condition (GMT, Ctrl->W.active && Ctrl->W.inc <= 0.0, "GMT SYNTAX ERROR -W option:  bin width must be nonzero\n");
...
#define Return(code) {Free_pshistogram_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_pshistogram (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_pshistogram_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_pshistogram_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	if (automatic && GMT->current.setting.verbose >= GMT_MSG_NORMAL) {
