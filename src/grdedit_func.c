...
#define Return(code) {Free_grdedit_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_grdedit (struct GMTAPI_CTRL *API, struct GMT_OPTION *options) {
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_grdedit_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_grdedit_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
