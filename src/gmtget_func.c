/*--------------------------------------------------------------------
 *	$Id: gmtget_func.c,v 1.4 2011-04-23 02:14:12 guru Exp $
 *
...
GMT_LONG GMT_gmtget_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "gmtget %s [API] - Get individual default parameters\n\n", GMT_VERSION);
...
#define Return(code) {Free_gmtget_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_gmtget (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	GMT_LONG error = 0;
	...
	if (options) {
		if (options->option == GMTAPI_OPT_USAGE) return (GMT_gmtget_usage (API, GMTAPI_USAGE));		/* Return the usage message */
		if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtget_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	}
