...
#define Return(code) {Free_gmtset_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_gmtset (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	GMT_LONG error = 0;
	...
	if (options) {
		if (options->option == GMTAPI_OPT_USAGE) return (GMT_gmtset_usage (API, GMTAPI_USAGE));		/* Return the usage message */
		if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtset_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	}
