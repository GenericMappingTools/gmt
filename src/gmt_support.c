...
int gmt_list_cpt (struct GMT_CTRL *GMT, char option) {
	...
	gmt_message (GMT, "\t   ---------------------------------------------------------------------------------------\n");
	for (unsigned int k = 0; k < GMT_N_CPT_MASTERS; k++) GMT_Usage (API, GMT_INDENT_2, "%s", GMT_CPT_master[k]);
	gmt_message (GMT, "\t   ---------------------------------------------------------------------------------------\n");
...
void gmt_list_custom_symbols (struct GMT_CTRL *GMT) {
	...
	gmt_message (GMT, "\t     ---------------------------------------------------------\n");
	while (fgets (buffer, GMT_BUFSIZ, fp)) if (!(buffer[0] == '#' || buffer[0] == 0)) GMT_Usage (API, GMT_INDENT_3, "%s", buffer);
	fclose (fp);
	gmt_message (GMT, "\t     ---------------------------------------------------------\n");
}
