/*-----------------------------------------------------------------
 *	$Id: x2sys_solve_func.c,v 1.9 2011-06-30 07:13:51 guru Exp $
 *
...
#define Return(code) {Free_x2sys_solve_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}
...
GMT_LONG GMT_x2sys_solve (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_x2sys_solve_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_x2sys_solve_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
