/*-----------------------------------------------------------------
 *	$Id: x2sys_report_func.c,v 1.11 2011-06-30 08:45:18 guru Exp $
 *
...
#define Return(code) {Free_x2sys_report_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_x2sys_report (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	char **trk_name = NULL;
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_x2sys_report_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_x2sys_report_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
