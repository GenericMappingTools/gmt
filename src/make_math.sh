#!/bin/sh
#	$Id: make_math.sh,v 1.9 2008-03-24 08:58:31 guru Exp $
...
echo "" >> ${prefix}math_def.h
if [ $1 = "gmt" ]; then
	awk '{printf "void table_%s(struct GMTMATH_INFO *info, double **stack[], BOOLEAN *constant, double *factor, int last, int start, GMT_LONG n);\t\t/* id = %d */\n", $2, NR-1}' $$.txt >> gmtmath_def.h
else
