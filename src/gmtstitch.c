...
GMT_LONG GMT_gmtstitch (struct GMTAPI_CTRL *API, GMT_LONG mode, void *args)
{
	...
	for (k = 0; k < D[GMT_IN]->n_tables; k++) {	/* For each data table */
		for (j = 0; j < D[GMT_IN]->table[k]->n_segments; j++) {	/* For each segment */
			...
			if (np > 2 && distance <= closed_dist) {	/* Already closed, just write out and forget in the rest of the program */
				...
				if (D[GMT_IN]->table[k]->segment[j]->header) T[CLOSED][out_seg]->header = strdup (D[GMT_IN]->table[k]->segment[j]->header);
				...
				if (D[GMT_IN]->table[k]->segment[j]->header) T[OPEN][n_open]->header = strdup (D[GMT_IN]->table[k]->segment[j]->header);
	...
	if (wrap_up) {	/* Write out results and return */
		if (wrap_up == 2) {	/* Write n_open segments to D[OUT] and n_closed to C */
			...
			if (GMT_Put_Data (API, GMT_IS_TEXTSET, GMT_IS_FILE, GMT_IS_POINT, NULL, 0, Ctrl->C.file, C)) Return (API->error);
			if (GMT_Put_Data (API, GMT_IS_TEXTSET, GMT_IS_FILE, GMT_IS_POINT, NULL, 0, Ctrl->Out.file, D[GMT_OUT])) Return (API->error);
		}
		if (Ctrl->Q.active) {
			...
			if (GMT_Put_Data (API, GMT_IS_TEXTSET, GMT_IS_FILE, GMT_IS_POINT, NULL, q_mode, Ctrl->Q.file, Q)) Return (API->error);
		}
		Return (GMT_OK);
	}
	...
	if (Ctrl->L.active) {	/* Write out the link information */
		...
		if (GMT_Put_Data (API, GMT_IS_TEXTSET, GMT_IS_FILE, GMT_IS_POINT, NULL, 0, Ctrl->L.file, LNK)) Return (API->error);
		if (GMT_Destroy_Data (API, GMT_ALLOCATED, &LNK)) Return (API->error);
	}
	...
	while (!done) {
		...
		while (!done && connect (seg, id, end_order, Ctrl->T.dist[0], Ctrl->T.active[1], Ctrl->T.dist[1])) {
			...
			if (id2 == id || n_steps > ns) {	/* Not good... */
				...
				id = id2;
				n_alloc_pts += seg[id].n;		/* Update length of combined polygon so far */
			}
			n_steps++;
		}
	...
	if (GMT_Put_Data (API, GMT_IS_DATASET, GMT_IS_FILE, GMT_IS_POINT, NULL, io_mode, ofile, D[GMT_OUT])) Return (API->error);
