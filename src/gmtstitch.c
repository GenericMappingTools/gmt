...
int GMT_gmtstitch (void *V_API, int mode, void *args)
{
	...
	if ((D[GMT_OUT] = GMT_Create_Data (GMT->parent, GMT_IS_DATASET, D[GMT_IN]->geometry, 0, dim_tscr, NULL, NULL, 0, 0, NULL)) == NULL) {
	...
	if (Ctrl->C.active) {	/* Wish to return already-closed polygons via a separate file */
		if ((C = GMT_Create_Data (GMT->parent, GMT_IS_DATASET, GMT_IS_POLY, 0, dim_tscr, NULL, NULL, 0, 0, NULL)) == NULL) {
		...
		for (jseg = iseg + 1; jseg < ns; jseg++) {
			...
			if ((segment[iseg].x_end[0] == segment[jseg].x_end[0] && segment[iseg].y_end[0] == segment[jseg].y_end[0]) ||
			    ...
			    (segment[iseg].x_end[1] == segment[jseg].x_end[1] && segment[iseg].y_end[1] == segment[jseg].y_end[1])) {
					...
					if (match) {
						GMT_report (GMT, GMT_MSG_VERBOSE, "Segments %" PRIu64 " and %" PRIu64 "are duplicates - Segment %" PRIu64 " will be ignored\n", iseg, jseg, jseg);
						skip[jseg] = true;
					}
				}
			}
	...
	if (Ctrl->L.active) {	/* Write out the link information */
		...
		if ((LNK = GMT_Create_Data (GMT->parent, GMT_IS_TEXTSET, GMT_IS_NONE, 0, dim_tscr, NULL, NULL, 0, 0, NULL)) == NULL) {
	...
	GMT_report (GMT, GMT_MSG_VERBOSE, "Assemble new segments\n");
