/*--------------------------------------------------------------------
*    $Id: gmtspatial_func.c,v 1.7 2011-04-02 21:37:14 guru Exp $
*
...
GMT_LONG GMT_gmtspatial_usage (struct GMTAPI_CTRL *C, GMT_LONG level) {
	...
	GMT_message (GMT, "\tinfiles (in ASCII or binary) have 2 or more columns with (x,y) or (y,x) in first columns.\n");
...
#endif
	GMT_message (GMT, "\t-M Measure area and handedness of polygon(s) or length of line segments.  If -fg is used\n");
...
GMT_LONG GMT_gmtspatial_parse (struct GMTAPI_CTRL *C, struct GMTSPATIAL_CTRL *Ctrl, struct GMT_OPTION *options) {
	...
	for (opt = options; opt; opt = opt->next) {
 	...
 	if (Ctrl->E.active) Ctrl->M.active = TRUE;
	
       if (GMT_native_binary (GMT, GMT_IN) && GMT->common.b.ncol[GMT_IN] == 0) GMT->common.b.ncol[GMT_IN] = 2;
	n_errors += GMT_check_condition (GMT, n_files[GMT_IN] == 0, "GMT SYNTAX ERROR:  No input file specified\n");
	...
	n_errors += GMT_check_condition (GMT, GMT_native_binary (GMT, GMT_IN) && GMT->common.b.ncol[GMT_IN] < 2, "GMT SYNTAX ERROR:  Binary input data (-bi) must have at least %d columns\n", 2);
	n_errors += GMT_check_condition (GMT, Ctrl->S.mode == POL_CLIP && !Ctrl->T.file && !GMT->common.R.active, "GMT SYNTAX ERROR:  -T without a polygon requires -R\n");
	n_errors += GMT_check_condition (GMT, Ctrl->C.active && !GMT->common.R.active, "GMT SYNTAX ERROR:  -C requires -R\n");
	n_errors += GMT_check_condition (GMT, Ctrl->L.active && !GMT->common.R.active, "GMT SYNTAX ERROR:  -L requires -R\n");
	n_errors += GMT_check_condition (GMT, Ctrl->L.active && Ctrl->L.s_cutoff < 0.0, "GMT SYNTAX ERROR:  -L requires a positive cutoff in meters\n");
	...
	n_errors += GMT_check_condition (GMT, Ctrl->T.active && Ctrl->T.file && GMT_access (GMT, Ctrl->T.file, R_OK), "GMT SYNTAX ERROR -T:  Cannot read file %s!\n", Ctrl->T.file);
...
GMT_LONG GMT_gmtspatial (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_gmtspatial_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtspatial_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	Ctrl = (struct GMTSPATIAL_CTRL *) New_gmtspatial_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	...
	if (Ctrl->S.active) Ctrl->I.active = external = TRUE;
	...
	if (Ctrl->M.active) {	/* Calculate centroid and polygon areas or line lengths and place in segment headers */
		...
		if ((error = GMT_Init_IO (API, GMT_IS_DATASET, mode, GMT_OUT, GMT_REG_DEFAULT, options))) Return (error);	/* Registers default output destination, unless already set */
	...
	if (Ctrl->I.active) {	/* Crossovers between polygons */
		...
		if (Ctrl->S.mode == POL_CLIP) {	/* Need to set up a separate table with the clip polygon */
			if (Ctrl->T.file) {	/* Get it via a file */
				if ((error = GMT_Begin_IO (API, GMT_IS_DATASET, GMT_IN, GMT_BY_SET))) Return (error);	/* Enables data input and sets access mode */
				if (GMT_Get_Data (API, GMT_IS_DATASET, GMT_IS_FILE, GMT_IS_POINT, NULL, 0, (void **)&Ctrl->T.file, (void **)&C)) Return ((error = GMT_DATA_READ_ERROR));
				if ((error = GMT_End_IO (API, GMT_IN, 0))) Return (error);	/* Disables further data input */
			}
			else {	/* Design a table based on -Rw/e/s/n */
				C = GMT_create_dataset (GMT, 1, 1, 2, 5);	/* An empty table with 2 cols and 5 rows */
		...
		if ((error = GMT_set_cols (GMT, GMT_OUT, C->n_columns))) Return (error);
	...
	if (Ctrl->D.active) {	/* Look for duplicates of lines or polygons */
		...
		if (Ctrl->D.file) {	/* Get trial features via a file */
			if ((error = GMT_Begin_IO (API, GMT_IS_DATASET, GMT_IN, GMT_BY_SET))) Return (error);	/* Enables data input and sets access mode */
			if (GMT_Get_Data (API, GMT_IS_DATASET, GMT_IS_FILE, GMT_IS_POINT, NULL, 0, (void **)&Ctrl->D.file, (void **)&C)) Return ((error = GMT_DATA_READ_ERROR));
			if ((error = GMT_End_IO (API, GMT_IN, 0))) Return (error);	/* Disables further data input */
			from = Ctrl->D.file;
		}
		else {
		...
		if ((error = GMT_Init_IO (API, GMT_IS_TEXTSET, GMT_IS_TEXT, GMT_OUT, GMT_REG_DEFAULT, options))) Return (error);	/* Registers default output destination, unless already set */
		...
		if ((error = GMT_End_IO (API, GMT_OUT, 0))) Return (error);	/* Disables further data output */
		GMT_Destroy_Data (API, GMT_ALLOCATED, (void **)&D);
		if (Ctrl->D.file) GMT_Destroy_Data (API, GMT_ALLOCATED, (void **)&C);
		Return (EXIT_SUCCESS);
	}
	...
	if (Ctrl->N.active) {	/* Report the polygons that contain the given features */
		...
		if (Ctrl->N.mode == 1) {	/* Just report on which polygon contains each feature */
			...
			if ((error = GMT_Init_IO (API, GMT_IS_DATASET, GMT_IS_LINE, GMT_OUT, GMT_REG_DEFAULT, options))) Return (error);	/* Registers default output destination, unless already set */
			if ((error = GMT_Begin_IO (API, GMT_IS_DATASET, GMT_OUT, GMT_BY_SET))) Return (error);	/* Enables data output and sets access mode */
		}
