...
/* gmt_coremodule.c populates the external array of GMT core module parameters such as name
 ...
 *   void gmt_coremodule_show_all(struct GMTAPI_CTRL *API); - Pretty print all core module names and their
 *           purposes
 *
 * DO NOT edit this file directly! Instead edit gmt_coremoduleinfo.txt and regenerate
 * this file with gmt_make_coremodule_src.sh. */
...
struct Gmt_moduleinfo g_coremodule[] = {
...
struct Gmt_alias gmt_corealias[] =
{	/* Alias:	Full name */
	{"2kml", 	"gmt2kml"},
	{"convert",	"gmtconvert"},
...
struct Gmt_alias gmt_coreoldname[] =
{	/* Old:		New: */
	{"gmtdp",	"gmtsimplify"},
	{NULL,		NULL}
};
...
const char * gmt_coreformal_name (struct GMTAPI_CTRL *API, const char *module) {
	unsigned int k = 0;
	while (gmt_corealias[k].alias != NULL) {
		if (!strcmp (module, gmt_corealias[k].alias)) return gmt_corealias[k].name;
		k++;
	}
	if (GMT_compat_check (API->GMT, 4)) {
		k = 0;
		while (gmt_coreoldname[k].alias != NULL) {
			if (!strcmp (module, gmt_coreoldname[k].alias)) return gmt_coreoldname[k].name;
			k++;
		}
	}
	return module;
...
void gmt_coremodule_show_all (struct GMTAPI_CTRL *API) {
	unsigned int module_id = 0;
	char module_name_comp[GMT_TEXT_LEN64], message[GMT_TEXT_LEN256];

	GMT_Message (API, GMT_TIME_NONE, "Program                Purpose of Program\n");
	while (g_coremodule[module_id].name != NULL) {
		snprintf (module_name_comp, GMT_TEXT_LEN64, "%s(%s)",
				g_coremodule[module_id].name, g_coremodule[module_id].component);
		sprintf (message, "%-22s %s\n",
				module_name_comp, g_coremodule[module_id].purpose);
