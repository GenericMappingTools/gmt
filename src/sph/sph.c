...
void stripack_lists (struct GMT_CTRL *C, uint64_t n_in, double *x, double *y, double *z, struct STRIPACK *T)
{
	...
	GMT_report (C, GMT_MSG_LONG_VERBOSE, "Call STRIPACK TRMESH subroutine...");
	trmesh_ (&n, x, y, z, list, lptr, lend, &lnew, iwk, &iwk[n], ds, &ierror);
	GMT_free (C, ds);
	GMT_free (C, iwk);
	GMT_report (C, GMT_MSG_LONG_VERBOSE, "OK\n");

	if (ierror == -2) {
		GMT_report (C, GMT_MSG_NORMAL, "STRIPACK: Error in TRMESH. The first 3 nodes are collinear.\n");
		GMT_exit (EXIT_FAILURE);
	}

	if (ierror > 0) {
		GMT_report (C, GMT_MSG_NORMAL, "STRIPACK: Error in TRMESH.  Duplicate nodes encountered.\n");
		GMT_exit (EXIT_FAILURE);
	}
	...
	GMT_report (C, GMT_MSG_LONG_VERBOSE, "Call STRIPACK TRLIST subroutine...");
	trlist_ (&n, list, lptr, lend, &nrow, &n_out, T->D.tri, &ierror);
	GMT_report (C, GMT_MSG_LONG_VERBOSE, "OK\n");
	T->D.n = n_out;

	if (ierror) {
		GMT_report (C, GMT_MSG_NORMAL, "STRIPACK: Error in TRLIST.\n");
		GMT_exit (EXIT_FAILURE);
	}
	...
	if (T->mode == VORONOI) {	/* Construct the Voronoi diagram */
		...
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "Call STRIPACK CRLIST subroutine...");
		...
		if (0 < ierror) {
			GMT_report (C, GMT_MSG_NORMAL, "STRIPACK: Error in CRLIST.  IERROR = %" PRId64 ".\n", ierror);
			GMT_exit (EXIT_FAILURE);
		}
	...
	if (mode == 0) {	 /* C-0 interpolation (INTRC0). */
		...
		for (row = 0; row < h->ny; row++) {
			for (col = 0; col < h->nx; col++) {
				...
				if (ierror > 0) nxp++;
	            		if (ierror < 0) {
					GMT_report (C, GMT_MSG_NORMAL, "Error in INTRC0: I = %d, J = %d, IER = %" PRId64 "\n", row, col, ierror);
					GMT_exit (EXIT_FAILURE);
	            		}
			}
		}
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "INTRC0: Number of evaluations = %" PRId64 ", number of extrapolations = %" PRId64 "\n", nm, nxp);
	}
		...
		for (k = 0; k < n; k++) {
			...
			if (ierror < 0) {
				GMT_report (C, GMT_MSG_NORMAL, "Error in GRADL: K = %" PRId64 " IER = %" PRId64 "\n", k1, ierror);
				GMT_exit (EXIT_FAILURE);
            		}
			sum += (double)ierror;
		}
		sum /= n;
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "GRADL: Average number of nodes used in the least squares fits = %g\n", sum);
	        if (vartens) {	/* compute tension factors sigma (getsig). */
			getsig_ (&n, x, y, z, w, P.I.list, P.I.lptr, P.I.lend, grad, &tol, sigma, &dsm, &ierror);
			if (ierror < 0) {
				GMT_report (C, GMT_MSG_NORMAL, "Error in GETSIG: IER = %" PRId64 "\n", ierror);
				GMT_exit (EXIT_FAILURE);
			}
			GMT_report (C, GMT_MSG_LONG_VERBOSE, "GETSIG: %" PRId64 " tension factors altered;  Max change = %g\n", ierror, dsm);
	        }
		...
		if (ierror < 0) {
			GMT_report (C, GMT_MSG_NORMAL, "Error in UNIF: IER = %" PRId64 "\n", ierror);
			GMT_exit (EXIT_FAILURE);
		}
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "UNIF: Number of evaluation points = %" PRId64 ", number of extrapolation points = %" PRId64 "\n", nm, ierror);
	}
		...
		for (iter = iflgs = 0; iter < itgs; iter++) {
			...
			if (ierror < 0) {
				GMT_report (C, GMT_MSG_NORMAL, "Error in GRADG (iteration %" PRId64 "): IER = %" PRId64 "\n", iter, ierror);
				GMT_exit (EXIT_FAILURE);
			}
			GMT_report (C, GMT_MSG_LONG_VERBOSE, "GRADG (iteration %" PRId64 "): tolerance = %g max change = %g  maxit = %" PRId64 " no. iterations = %" PRId64 " ier = %" PRId64 "\n",
			...
			if (vartens) {
				...
				getsig_ (&n, x, y, z, w, P.I.list, P.I.lptr, P.I.lend, grad, &tol, sigma, &dsm, &ierror);
				if (ierror < 0) {
					GMT_report (C, GMT_MSG_NORMAL, "Error in GETSIG (iteration %" PRId64 "): ier = %" PRId64 "\n", iter, ierror);
					GMT_exit (EXIT_FAILURE);
				}
				GMT_report (C, GMT_MSG_LONG_VERBOSE, "GETSIG (iteration %" PRId64 "): %" PRId64 " tension factors altered;  Max change = %g\n", iter, ierror, dsm);
			}
		}
		/* compute interpolated values on the uniform grid (unif). */
		...
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "UNIF: Number of evaluations = %" PRId64 ", number of extrapolations = %" PRId64 "\n", nm, ierror);
	}
		...
		smtol = sqrt (2.0 / sm);
		gstol = 0.05 * e;
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "SMSURF parameters:\n\texpected squared error = %g\n\tsmoothing parameter sm = %g\n", e, sm);
		GMT_report (C, GMT_MSG_LONG_VERBOSE, "\tgauss-seidel tolerance = %g\n\tsmoothing tolerance = %g\n\tweights = %g\n", gstol, smtol, wtk);
		...
		for (iter = iflgs = 0; iter < itgs; iter++) {
			...
			if (ierror < 0) {
				GMT_report (C, GMT_MSG_NORMAL, "Error in SMSURF (iteration %" PRId64 "): IER = %" PRId64 "\n", iter, ierror);
				GMT_exit (EXIT_FAILURE);
			}
			if (ierror == 1) GMT_report (C, GMT_MSG_LONG_VERBOSE, "Error in SMSURF: inactive constraint in SMSURF (iteration %" PRId64 ").  f is a constant function\n", iter);
