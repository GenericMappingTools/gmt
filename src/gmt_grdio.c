...
int gmt_grd_RI_verify (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *h, unsigned int mode) {
	...
	switch (gmt_minmaxinc_verify (GMT, h->wesn[XLO], h->wesn[XHI], h->inc[GMT_X], GMT_CONV4_LIMIT)) {
		...
		case 2:
			...
			if (gmt_M_is_geographic (GMT, GMT_IN)) GMT_Report (GMT->parent, GMT_MSG_NORMAL, "Make sure west < east for geographic coordinates\n");
			error++;
			break;
		case 1:
...
unsigned int grdio_free_grid_ptr (struct GMT_CTRL *GMT, struct GMT_GRID *G, bool free_grid) {
	...
	if (G->header) {	/* Free the header structure and anything allocated by it */
		gmt_str_free (G->header->ProjRefWKT);
		gmt_str_free (G->header->ProjRefPROJ4);
		gmt_str_free (G->header->pocket);
...
int gmt_read_image_info (struct GMT_CTRL *GMT, char *file, struct GMT_IMAGE *I) {
	...
	gmt_str_free (I->header->ProjRefPROJ4);		/* Make sure we don't leak due to a previous copy */
	gmt_str_free (I->header->ProjRefWKT);
	...
	gmt_free (GMT, from_gdalread->ColorMap);	/* Maybe we will have a use for this in future, but not yet */
...
int gmt_read_image (struct GMT_CTRL *GMT, char *file, struct GMT_IMAGE *I, double *wesn, unsigned int *pad, unsigned int complex_mode) {
	...
	if (to_gdalread->B.active) gmt_str_free (I->header->pocket);		/* It was allocated by strdup. Free it for an eventual reuse. */
