...
GMT_LOCAL void psmask_draw_clip_contours (struct GMT_CTRL *GMT, struct PSL_CTRL *PSL, double *xx, double *yy, uint64_t nn, double rgb[], unsigned int id, unsigned int flag) {
	uint64_t i;
	double x, y;
...
GMT_LOCAL uint64_t psmask_trace_clip_contours (struct GMT_CTRL *GMT, struct PSMASK_INFO *info, char *grd, unsigned int *edge, struct GMT_GRID_HEADER *h, double inc2[], double **xx, double **yy, int i, int j, int kk, uint64_t *max) {
...
GMT_LOCAL uint64_t psmask_clip_contours (struct GMT_CTRL *GMT, struct PSMASK_INFO *info, char *grd, struct GMT_GRID_HEADER *h, double inc2[], unsigned int *edge, unsigned int first, double **x, double **y, uint64_t *max) {
	 ...
	 * psmask_trace_clip_contours will try to allocate more memory in blocks of GMT_CHUNK points.
			...
			for (i = i0; go_on && i < h->n_columns-1; i++, ij++) {	/* n_columns-1 since the last bin starts at n_columns-2 and ends at n_columns-1 */
				...
				if (!(edge[edge_word] & info->bit[edge_bit]) && ((grd[ij]+grd[ij-h->n_columns]) == 1)) { /* Start tracing contour */
					...
					*y[0] = gmt_M_grd_row_to_y (GMT, j, h) + 0.5 * h->inc[GMT_Y];
					edge[edge_word] |= info->bit[edge_bit];
					n = psmask_trace_clip_contours (GMT, info, grd, edge, h, inc2, x, y, i, j, 3, max);
					go_on = false;
					i0 = i + 1;
			...
			for (i = i0; go_on && i < h->n_columns-1; i++, ij++) {
				...
				if (!(edge[edge_word] & info->bit[edge_bit]) && ((grd[ij]+grd[ij+1]) == 1)) { /* Start tracing contour */
					...
					*y[0] = gmt_M_grd_row_to_y (GMT, j, h);
					edge[edge_word] |= info->bit[edge_bit];
					n = psmask_trace_clip_contours (GMT, info, grd, edge, h, inc2, x, y, i, j, 2, max);
					go_on = false;
					i0 = i + 1;
					j0 = j;	/* Return to finish this row later */
				}
...
GMT_LOCAL void psmask_orient_contours (struct GMT_CTRL *GMT, struct GMT_GRID_HEADER *h, char *grd, double *x, double *y, uint64_t n, int orient) {
...
GMT_LOCAL void psmask_shrink_clip_contours (double *x, double *y, uint64_t np, double w, double e, double s, double n) {
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Usage (API, 1, "\n-D<template>");
	GMT_Usage (API, -2, "Dump clip polygons as data polygons; no plotting takes place. "
		"Append filename <template> which may contain a C-format specifier. "
		"If no filename is given we write all polygons to standard output. "
		"If filename has no specifiers then we write all polygons to a single file. "
		"If an integer format (e.g., %%06d) is found we substitute a running segment count "
		"and write all polygons to individual files; see module documentation for more examples.");
	GMT_Usage (API, -2, "Note: Cannot be used with -T; see -Q to eliminate small polygons.");
	GMT_Usage (API, 1, "\n-F[l|r]");
	GMT_Usage (API, -2, "Force clip contours to be oriented so that the higher z-values "
	...
	GMT_Option (API, "K");
	GMT_Usage (API, 1, "\n-L<grdfile>[+i|o]");
	GMT_Usage (API, -2, "Save internal on/off grid to <grdfile> for testing [no grid saved]. Optionally, add a modifier:");
	GMT_Usage (API, 3, "+i Change inside nodes to NaNs.");
	GMT_Usage (API, 3, "+o Change outside nodes to NaNs.");
	GMT_Usage (API, 1, "\n-N Invert the sense of the clipping [or tiling].");
	GMT_Option (API, "O,P");
	GMT_Usage (API, 1, "\n-Q<min>");
	GMT_Usage (API, -2, "Do not dump contours with less than <min> points [Dump all contours]. "
		"Note: Ignored unless -D is set.");
	gmt_dist_syntax (API->GMT, "S" GMT_RADIUS_OPT, "Set search radius to identify inside points.");
	GMT_Usage (API, -2, "Grid nodes inside circles of <radius> centered on the input "
		"data points are considered to be reliable estimates of the surface. "
...
EXTERN_MSC int GMT_psmask (void *V_API, int mode, void *args) {
	...
	if (Ctrl->C.active) {	/* Just undo previous polygon clip-path */
		...
		if (!Ctrl->T.active) {	/* Must trace the outline of ON/OFF values in grd */
			...
			section = 0;
			first = 1;
			while ((n = psmask_clip_contours (GMT, &info, grd, Grid->header, inc2, edge, first, &x, &y, &max_alloc_points)) > 0) {
				closed = false;
				psmask_shrink_clip_contours (x, y, n, Grid->header->wesn[XLO], Grid->header->wesn[XHI], Grid->header->wesn[YLO], Grid->header->wesn[YHI]);
				if (Ctrl->F.active) psmask_orient_contours (GMT, Grid->header, grd, x, y, n, Ctrl->F.value);
				...
				if (make_plot) psmask_draw_clip_contours (GMT, PSL, x, y, n, Ctrl->G.fill.rgb, section, first);
				first = 0;
				section++;
			}

			if (make_plot) psmask_draw_clip_contours (GMT, PSL, x, y, 0, Ctrl->G.fill.rgb, section, 2);	/* Activate clip-path */
