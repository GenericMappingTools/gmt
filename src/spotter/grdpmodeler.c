...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Usage (API, 0, "usage: %s %s %s [-F<polygontable>] [-G<outgrid>] [%s] [%s] [-N<upper_age>] "
		"[-SadrswxyXY] [-T<time>] [%s] [%s] [%s] [%s] [%s] [%s]\n",
		name, GMT_INGRID, SPOTTER_E_OPT, GMT_Id_OPT, GMT_Rgeo_OPT, GMT_V_OPT, GMT_b_OPT, GMT_d_OPT, GMT_h_OPT, GMT_r_OPT, GMT_PAR_OPT);
