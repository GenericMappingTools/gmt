...
#include "gmt_dev.h"
#include "longopt/grdpmodeler_inc.h"
#include "spotter.h"
...
struct GRDROTATER_CTRL {	/* All control options for this program (except common args) */
	...
	struct GRDPMODELER_I {	/* -I (for checking only) */
		bool active;
	} I;
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Usage (API, 0, "usage: %s %s %s [-F<polygontable>] [-G%s] [%s] [%s] [-N<upper_age>] "
		...
		name, GMT_INGRID, SPOTTER_E_OPT, GMT_OUTGRID, GMT_I_OPT, GMT_Rgeo_OPT, GMT_V_OPT, GMT_b_OPT, GMT_d_OPT, GMT_h_OPT, GMT_r_OPT, GMT_PAR_OPT);
	...
	GMT_Usage (API, -2, "Default writes separate grids for adrswxyXY.");
	GMT_Usage (API, 1, "\n-T<time>");
	GMT_Usage (API, -2, "Set fixed time of reconstruction to override age grid.");
	...
	for (opt = options; opt; opt = opt->next) {
		switch (opt->option) {
			...
			case 'T':
				...
				n_errors += gmt_get_required_double (GMT, opt->arg, opt->option, 0, &Ctrl->T.value);
				break;
