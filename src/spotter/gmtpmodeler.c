...
void *New_gmtpmodeler_Ctrl (struct GMT_CTRL *GMT) {	/* Allocate and initialize a new control structure */
...
int GMT_gmtpmodeler (void *V_API, int mode, void *args)
{
	...
	if (mode == GMT_MODULE_PURPOSE) return (GMT_gmtpmodeler_usage (API, GMT_MODULE_PURPOSE));	/* Return the purpose of program */
	options = GMT_Create_Options (API, mode, args);	if (API->error) return (API->error);	/* Set or get option list */

	if (!options || options->option == GMT_OPT_USAGE) bailout (GMT_gmtpmodeler_usage (API, GMT_USAGE));	/* Return the usage message */
	if (options->option == GMT_OPT_SYNOPSIS) bailout (GMT_gmtpmodeler_usage (API, GMT_SYNOPSIS));	/* Return the synopsis */
	...
	Ctrl = New_gmtpmodeler_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	if ((error = GMT_gmtpmodeler_parse (GMT, Ctrl, options)) != 0) Return (error);
