/*--------------------------------------------------------------------
 *	$Id: hotspotter_func.c,v 1.5 2011-04-12 13:06:44 remko Exp $
 *
...
GMT_LONG GMT_hotspotter_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: hotspotter [<files>] -E[+]<euler.d> -G<CVAgrid> %s\n", GMT_Id_OPT);
	...
	GMT_message (GMT, "\t<files> is one or more seamount (x,y,z,r,t) files\n");
	...
	GMT_message (GMT, "\t-N sets upper age in m.y. for seamounts whose plate age is NaN [180]\n");
...
#define Return(code) {Free_hotspotter_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_hotspotter (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_hotspotter_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_hotspotter_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
