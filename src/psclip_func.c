/*--------------------------------------------------------------------
 *	$Id: psclip_func.c,v 1.4 2011-03-22 01:10:46 guru Exp $
 *
...
GMT_LONG GMT_psclip_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: psclip -C[s|c] [-K] [-O]  OR\n");
	...
	GMT_message (GMT, "\t   Append s to plot text previously used to build a straight-text clip path.\n");
	GMT_message (GMT, "\t   Append c to plot text previously used to build a curved clip path.\n");
	...
	GMT_message (GMT, "\t-N Use the outside of the polygons as clip paths\n");
	...
	for (opt = options; opt; opt = opt->next) {	/* Process all the options given */
		...
		switch (opt->option) {
			...
			case 'C':	/* Turn clipping off */
				Ctrl->C.active = TRUE;
				if (opt->arg[0] == 's') Ctrl->C.mode = 1;
				if (opt->arg[0] == 'c') Ctrl->C.mode = 2;
				break;
