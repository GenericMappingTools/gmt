/*--------------------------------------------------------------------
 *	$Id: gmt_init.c,v 1.273 2007-01-30 20:37:08 pwessel Exp $
 *
...
void *New_GMT_Ctrl ();
void Free_GMT_Ctrl (struct GMT_COMMON *C);
...
void GMT_explain_option (char option)
{
	...
	switch (option) {
		...
		case 'J':	/* Map projection option */
			...
			fprintf (stderr, "\t   -Ju<zone>/<scale> OR -JU<zone>/<width> (GMT_UTM)\n");
		...
		case 'j':	/* Condensed version of J */
			...
			fprintf (stderr, "\t   -Ju|U<zone>/<scale|width> (GMT_UTM)\n");
...
void GMT_get_history (int argc, char ** argv)
{
	 ...
	 *
	 * In GMT 4.1.5 GMT_get_history and GMT_put_history got consolidated in an
	 * attempt to lower the chances that in a pipe one program writes to .gmtcommands4
...
void GMT_put_history (int argc, char **argv)
{
	...
	if (fcntl (GMT_fd_history, F_SETLK, &lock)) {
		fprintf (stderr, "%s: Error returned by fcntl [F_UNLCK]\n", GMT_program);
		exit (EXIT_FAILURE);
	}
#endif
...
void GMT_get_history (int argc, char ** argv)
{
	...
	if (fcntl (GMT_fd_history, F_SETLKW, &lock)) {	/* Will wait for file to be ready for reading */
		fprintf (stderr, "%s: Error returned by fcntl [F_WRLCK]\n", GMT_program);
		exit (EXIT_FAILURE);
	}
...
void *New_GMT_Ctrl () {	/* Allocate and initialize a new common control structure */
	...
	C->H.n_recs = gmtdefs.n_header_recs;
	/* [7]  -P */
	C->P.active = gmtdefs.portrait;
	/* [9]  -U */
	C->U.active = gmtdefs.unix_time;
	C->U.x = gmtdefs.unix_time_pos[0];
	C->U.y = gmtdefs.unix_time_pos[1];
	/* [10]  -V */
	C->V.active = gmtdefs.verbose;
	/* [11]  -X */
	C->X.off = gmtdefs.x_origin;
	/* [12] -Y */
	C->Y.off = gmtdefs.y_origin;
	/* [13]  -c */
	C->c.copies = gmtdefs.n_copies;
	/* [14]  -:[i|o] */
