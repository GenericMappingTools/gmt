...
GMT_LOCAL int gmtinit_gmtinit_get_psl_encoding (const char *encoding) {
...
GMT_LOCAL int gmtinit_gmtinit_compare_terms (const void *term_1v, const void *term_2v) {
...
GMT_LOCAL int gmtinit_parse_model1d (struct GMT_CTRL *GMT, char option, char *in_arg, struct GMT_MODEL *M) {
	...
	qsort (M->term, n_model, sizeof (struct GMT_MODEL_TERM), gmtinit_gmtinit_compare_terms);
...
GMT_LOCAL void gmtinit_gmtinit_trim_off_any_slash_at_end (char *dir) {
...
GMT_LOCAL int gmtinit_set_env (struct GMT_CTRL *GMT) {
	...
	gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.SHAREDIR);
	...
	if (GMT->session.HOMEDIR) {	/* Mostly to keep Coverity happy */
		...
		gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.HOMEDIR);
	...
	if (GMT->session.USERDIR) {
		...
		gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.USERDIR);
	}
	...
	if (GMT->session.CACHEDIR) {
		...
		gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.CACHEDIR);
	}
	...
	if (API->session_dir) {
		...
		gmtinit_gmtinit_trim_off_any_slash_at_end (API->session_dir);
	}
	...
	if (GMT->session.DATASERVER)
		gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.DATASERVER);
	...
	if ((this_c = getenv ("GMT_TMPDIR")) != NULL) {		/* GMT_TMPDIR was set, check it */
		if (access (this_c, R_OK|W_OK|X_OK)) {
			...
			gmtinit_gmtinit_trim_off_any_slash_at_end (GMT->session.TMPDIR);
		}
	}
	return GMT_OK;
}
...
GMT_LOCAL int gmtinit_load_encoding (struct GMT_CTRL *GMT) {
	...
	if ((k = gmtinit_gmtinit_get_psl_encoding (symbol)) == -1) {
...
GMT_LOCAL void gmtinit_gmtinit_explain_R_geo (struct GMT_CTRL *GMT) {
...
void gmtlib_explain_options (struct GMT_CTRL *GMT, char *options) {
	...
	for (k = 0; k < s_length; k++) {
		...
		case 'G':	/* Geographic Region option */
			...
			gmtinit_gmtinit_explain_R_geo (GMT);
			break;
		...
		case 'U':	/* Plot time mark and [optionally] command line */
			...
			gmt_message (GMT, "\t   Set the justification point via +j<code> [BL] and an optional offset [-54p/-54p].\n");
		...
		case 'V':	/* Verbose */
			...
			gmt_message (GMT, "\t   Choose among 7 levels; each level adds more messages:\n");
...
GMT_LOCAL int gmtinit_gmtinit_get_current_panel (struct GMTAPI_CTRL *API, int fig, int *row, int *col, double gap[], char *tag, unsigned int *first) {
	...
	if (access (file, F_OK))	{	/* Panel selection file not available so we are not doing subplots */
		...
		GMT_Report (API, GMT_MSG_DEBUG, "gmtinit_gmtinit_get_current_panel: No current panel selected so not in subplot mode\n");
	...
	GMT_Report (API, GMT_MSG_DEBUG, "gmtinit_gmtinit_get_current_panel: Current panel is (%d, %d)\n", *row, *col);
	return GMT_NOERROR;
}
...
int gmt_get_next_panel (struct GMTAPI_CTRL *API, int fig, int *row, int *col) {
	...
	if (*col != INT_MAX && gmtinit_gmtinit_get_current_panel (API, fig, row, col, NULL, NULL, NULL)) {	/* Not good */
		API->error = GMT_RUNTIME_ERROR;
		return GMT_RUNTIME_ERROR;
	}
...
struct GMT_SUBPLOT *gmt_subplot_info (struct GMTAPI_CTRL *API, int fig) {
	...
	API->error = GMT_RUNTIME_ERROR;
	if (gmtinit_gmtinit_get_current_panel (API, fig, &row, &col, gap, tmp, &first))	/* No panel or there was an error */
		return NULL;
...
GMT_LOCAL bool gmtinit_gmtinit_is_PS_module (struct GMTAPI_CTRL *API, const char *name, const char *keys, struct GMT_OPTION **in_options) {
...
GMT_LOCAL unsigned int gmtinit_gmtinit_strip_R_from_E_in_pscoast (struct GMT_CTRL *GMT, struct GMT_OPTION *options, char r_code[]) {
...
GMT_LOCAL bool gmtinit_gmtinit_is_region_geographic (struct GMT_CTRL *GMT, struct GMT_OPTION *options, const char *module) {
...
GMT_LOCAL int gmtinit_gmtinit_set_modern_mode_if_oneliner (struct GMTAPI_CTRL *API, struct GMT_OPTION **options) {
	...
	for (opt = *options; opt; opt = opt->next) {	/* Loop over all options */
		...
		if (opt->next && opt->next->option == GMT_OPT_INFILE) {	/* Found a -ext[,ext,ext,...] <prefix> pair */
			...
			if (GMT_Delete_Option (API, opt->next, options) || GMT_Delete_Option (API, opt, options)) {
				GMT_Report (API, GMT_MSG_ERROR, "Unable to remove -ext <prefix> options in gmtinit_gmtinit_set_modern_mode_if_oneliner.\n");
				return GMT_NOTSET;
			}
			...
			if ((error = GMT_Call_Module (API, "begin", GMT_MODULE_CMD, session))) {
				GMT_Report (API, GMT_MSG_ERROR, "Unable to call module begin from gmtinit_gmtinit_set_modern_mode_if_oneliner.\n");
				return GMT_NOTSET;
			}
...
GMT_LOCAL bool gmtinit_gmtinit_build_new_J_option (struct GMTAPI_CTRL *API, struct GMT_OPTION *opt_J, struct GMT_SUBPLOT *P, struct GMT_INSET *I, bool is_psrose) {
...
GMT_LOCAL void gmtinit_gmtinit_panel_B_set (struct GMTAPI_CTRL *API, int fig, int row, int col) {
...
GMT_LOCAL bool gmtinit_gmtinit_panel_B_get (struct GMTAPI_CTRL *API, int fig, int row, int col) {
...
struct GMT_CTRL *gmt_init_module (struct GMTAPI_CTRL *API, const char *lib_name, const char *mod_name, const char *keys, const char *in_required, struct GMT_KEYWORD_DICTIONARY *this_module_kw, struct GMT_OPTION **options, struct GMT_CTRL **Ccopy) {
	...
	if (options && !strncmp (mod_name, "pscoast", 7U) && (E = GMT_Find_Option (API, 'E', *options)) && (opt = GMT_Find_Option (API, 'R', *options)) == NULL) {
		...
		E_flags = gmtinit_gmtinit_strip_R_from_E_in_pscoast (GMT, *options, r_code);
	...
	is_PS = gmtinit_gmtinit_is_PS_module (API, mod_name, keys, options);	/* true if module will produce PS */
	if (is_PS) {
		if (gmtinit_gmtinit_set_modern_mode_if_oneliner (API, options))	/* Look out for modern -png mymap and similar specs */
			return NULL;
	}
	...
	if (GMT->current.setting.run_mode == GMT_MODERN) {	/* Make sure options conform to this mode's harsh rules: */
		...
		if (GMT->current.ps.active) {	/* Only explore -c settings, etc for plot modules */
			...
			if (strncmp (mod_name, "inset", 5U) && GMT->current.plot.inset.active && got_J) {	/* Map inset and gave -J */
				if (gmtinit_gmtinit_build_new_J_option (API, opt_J, NULL, &GMT->current.plot.inset, is_psrose))	/* Found ?-mark(s) and replaced them */
		...
		if (GMT->current.ps.active && !exceptionp && (P = gmt_subplot_info (API, fig))) {	/* Yes, so set up current panel settings */
			...
			char *c = NULL;
			int row = 0, col = 0;
			gmtinit_gmtinit_get_current_panel (API, fig, &row, &col, NULL, NULL, NULL);
			B_set = gmtinit_gmtinit_panel_B_get (API, fig, row, col);
			if (exceptionb == 0 && P->first == 1 && !B_set) {
				...
				gmtinit_gmtinit_panel_B_set (API, fig, row, col);
			}
			if (GMT->hidden.func_level == GMT_CONTROLLER) {	/* Top-level function called by subplot needs to handle positioning and possibly set -J */
				...
				if ((*options = GMT_Append_Option (API, opt, *options)) == NULL) return NULL;	/* Failure to append option */
				if (gmtinit_gmtinit_build_new_J_option (API, opt_J, P, NULL, is_psrose))	/* Found ?-mark(s) and replaced them */
					GMT->current.plot.panel.no_scaling = 0;
		...
		if (got_J == false && strchr (required, 'J')) {	/* Need a projection but no -J was set */
			...
			if (got_J == false) {	/* No history, apply default projection, but watch out for subplots */
				...
				unsigned int geo = gmtinit_gmtinit_is_region_geographic (GMT, *options, mod_name);
...
#ifdef HAVE_GDAL
GMT_LOCAL int gmtinit_gmtinit_parse_proj4 (struct GMT_CTRL *GMT, char *item, char *dest) {
		...
		case 'J':
			if (item && (item[0] == 'Z' || item[0] == 'z')) {	/* -JZ or -Jz */
				...
				if (!two) {		/* A single CRS */
					...
					error = gmtinit_gmtinit_parse_proj4 (GMT, item, dest);
				}
				else {
					size_t len, k = 3;
					len = strlen(pch);
					pch[0] = '\0';
					error  = gmtinit_gmtinit_parse_proj4 (GMT, item, source);
					if (pch[3] == ' ' || pch[3] == '\t')
					...
					error += gmtinit_gmtinit_parse_proj4 (GMT, &pch[k], dest);
				}
...
GMT_LOCAL bool gmtinit_gmtinit_check_if_we_must_download (struct GMT_CTRL *GMT, const char *file, unsigned int kind) {
...
#endif
	return (gmtinit_gmtinit_check_if_we_must_download (GMT, file, *kind));
}
...
GMT_LOCAL FILE *gmtinit_gmtinit_open_figure_file (struct GMTAPI_CTRL *API, unsigned int mode, int *err) {
...
/*! . */
GMT_LOCAL void gmtinit_gmtinit_get_session_name_format (struct GMTAPI_CTRL *API, char prefix[GMT_LEN256], char formats[GMT_LEN256]) {
...
GMT_LOCAL int gmtinit_read_figures (struct GMT_CTRL *GMT, unsigned int mode, struct GMT_FIGURE **figs) {
	...
	if ((fp = gmtinit_gmtinit_open_figure_file (GMT->parent, 0, &err)) == NULL && mode < 2) {	/* No such file, nothing to do */
		return 0;
	}
	...
	if (mode == 2) {	/* Insert default session figure */
		gmtinit_gmtinit_get_session_name_format (GMT->parent, fig[0].prefix, fig[0].formats);
		fig[0].ID = 0;	k = 1;
	}
...
GMT_LOCAL int gmtinit_gmtinit_put_session_name (struct GMTAPI_CTRL *API, char *arg) {
...
GMT_LOCAL int gmtinit_gmtinit_get_graphics_formats (struct GMT_CTRL *GMT, char *formats, char fmt[], int gcode[]) {
...
GMT_LOCAL bool gmtinit_gmtinit_check_if_autosize (struct GMTAPI_CTRL *API, int ID) {
...
GMT_LOCAL bool gmtinit_gmtinit_A_was_given (char *text) {
...
GMT_LOCAL int gmtinit_gmtinit_process_figures (struct GMTAPI_CTRL *API, char *show) {
	...
	for (k = 0; k < n_figs; k++) {
		...
		nf = gmtinit_gmtinit_get_graphics_formats (API->GMT, fig[k].formats, fmt, gcode);
		...
		for (f = 0; f < nf; f++) {	/* Loop over all desired output formats */
			...
			auto_size = gmtinit_gmtinit_check_if_autosize (API, fig[k].ID);	/* Determine if the PostScript file has auto size enabled */
			if (fig[k].options[0]) {	/* Append figure-specific psconvert settings */
				...
				if (not_PS && auto_size && !gmtinit_gmtinit_A_was_given (fig[k].options))	/* Must always add -A if not PostScript unless when media size is given, unless crop is off via +n */
					strcat (cmd, " -A");
			}
				...
				if (not_PS && auto_size && !gmtinit_gmtinit_A_was_given (API->GMT->current.setting.ps_convert))	/* Must always add -A if not PostScript unless when media size is given */
					strcat (cmd, " -A");
			}
	...
	*subplot = gmtinit_subplot_status (API, *fig);	/* Get information about subplot, if active */
	...
	if ((*subplot) & GMT_SUBPLOT_ACTIVE) {	/* subplot is active */
		if (((*subplot) & GMT_PANEL_NOTSET) == 0) {
			int row, col;
			if (gmtinit_gmtinit_get_current_panel (API, *fig, &row, &col, NULL, NULL, NULL) == 0)	/* panel set */
...
int gmt_add_figure (struct GMTAPI_CTRL *API, char *arg) {
	...
	if (found && !(formats[0] == '-' || n == 1)) {
		...
		if ((fp = gmtinit_gmtinit_open_figure_file (API, 1, &err)) == NULL)	/* Failure to open existing or create new file */
			return GMT_ERROR_ON_FOPEN;
...
GMT_LOCAL void gmtinit_gmtinit_set_legend_filename (struct GMTAPI_CTRL *API, char *file) {
...
int gmt_legend_file (struct GMTAPI_CTRL *API, char *file) {
	...
	gmtinit_gmtinit_set_legend_filename (API, file);
...
GMT_LOCAL void gmtinit_gmtinit_draw_legend_line (struct GMTAPI_CTRL *API, FILE *fp, struct GMT_LEGEND_ITEM *item, unsigned int code) {
...
void gmt_add_legend_item (struct GMTAPI_CTRL *API, struct GMT_SYMBOL *S, bool do_fill, struct GMT_FILL *fill, bool do_line, struct GMT_PEN *pen, struct GMT_LEGEND_ITEM *item) {
	...
	gmtinit_gmtinit_set_legend_filename (API, file);	/* Get the legend filename given current scope */
	...
	if ((item->draw & GMT_LEGEND_DRAW_D) && ((item->draw & GMT_LEGEND_DRAW_V) == 0 || item->pen[GMT_LEGEND_PEN_V][0] == '\0'))
		gmtinit_gmtinit_draw_legend_line (API, fp, item, GMT_LEGEND_DRAW_D);	/* Draw horizontal line, if requested, before symbol */
	...
	if (((item->draw & GMT_LEGEND_DRAW_V) && item->pen[GMT_LEGEND_PEN_V][0] == '\0'))	/* Initialize the vertical line setting */
		gmtinit_gmtinit_draw_legend_line (API, fp, item, GMT_LEGEND_DRAW_V);
	/* Get the symbol size */
	...
	if (item->draw & GMT_LEGEND_DRAW_V && item->pen[GMT_LEGEND_PEN_V][0]) {	/* Must end with horizontal, then vertical line */
		gmtinit_gmtinit_draw_legend_line (API, fp, item, GMT_LEGEND_DRAW_D);		/* Draw horizontal line, if requested, after last symbol */
		gmtinit_gmtinit_draw_legend_line (API, fp, item, GMT_LEGEND_DRAW_V);
	}
	fclose (fp);
}
...
int gmt_manage_workflow (struct GMTAPI_CTRL *API, unsigned int mode, char *text) {
	...
	switch (mode) {
		case GMT_BEGIN_WORKFLOW:	/* Must create a new temporary directory */
			...
			error = gmtinit_gmtinit_put_session_name (API, text);		/* Store session name, possibly setting psconvert options */
		...
		case GMT_END_WORKFLOW:
			...
			if ((error = gmtinit_gmtinit_process_figures (API, text)))
				GMT_Report (API, GMT_MSG_ERROR, "gmtinit_gmtinit_process_figures returned error %d\n", error);
