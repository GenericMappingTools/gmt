/*--------------------------------------------------------------------
 *	$Id: nearneighbor_func.c,v 1.3 2011-03-25 22:17:41 guru Exp $
 *
...
GMT_LONG GMT_nearneighbor_parse (struct GMTAPI_CTRL *C, struct NEARNEIGHBOR_CTRL *Ctrl, struct GMT_EDGEINFO *edgeinfo, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case 'E':	/* NaN value */
				...
				if (opt->arg[0])
					...
					n_errors++;
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -E option:  Must specify value or NaN\n");
				}
				break;
			case 'G':	/* Output file */
				Ctrl->G.active = TRUE;
	...
	n_errors += GMT_check_condition (GMT, !Ctrl->S.active, "GMT SYNTAX ERROR:  Must specify -S option\n");
	n_errors += GMT_check_condition (GMT, !Ctrl->G.file, "GMT SYNTAX ERROR option -G:  Must specify output file\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.sectors <= 0, "GMT SYNTAX ERROR -N option:  Must specify a positive number of sectors\n");
