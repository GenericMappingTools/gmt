/*--------------------------------------------------------------------
 *	$Id: grdpaste_func.c,v 1.8 2011-05-14 00:04:06 guru Exp $
 *
...
#define Return(code) {Free_grdpaste_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_grdpaste (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_grdpaste_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_grdpaste_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	Ctrl = (struct GRDPASTE_CTRL *) New_grdpaste_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	...
	if (GMT_Get_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, GMT_GRID_HEADER, (void **)&(Ctrl->In.file[0]), (void **)&A)) Return (GMT_DATA_READ_ERROR);	/* Get header only */
	if (GMT_Get_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, GMT_GRID_HEADER, (void **)&(Ctrl->In.file[1]), (void **)&B)) Return (GMT_DATA_READ_ERROR);	/* Get header only */
