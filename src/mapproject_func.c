/*--------------------------------------------------------------------
*	$Id: mapproject_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
*
...
GMT_LONG GMT_mapproject_parse (struct GMTAPI_CTRL *C, struct MAPPROJECT_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	for (opt = options; opt; opt = opt->next) {
		switch (opt->option) {
			...
			case 'A':
				...
				if (n < 1) {
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR.  Expected -Ab|B|f|F[<lon0>/<lat0>]\n");
					n_errors++;
				}
				else {
					switch (c) {
						...
						case 'o':
							...
							break;
						default:
							GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR.  Expected -Ab|B|f|F|o|O[<lon0>/<lat0>]\n");
							n_errors++;
							break;
					}
					if (n == 3) {
			...
			case 'C':
				...
				if (opt->arg[0]) {	/* Also gave shifts */
					n_errors += GMT_check_condition (GMT, sscanf (opt->arg, "%lf/%lf", &Ctrl->C.easting, &Ctrl->C.northing) != 2, "GMT SYNTAX ERROR.  Expected -C[<false_easting>/<false_northing>]\n");
					Ctrl->C.shift = TRUE;
				}
				break;
			case 'D':
			...
			case 'G':
				...
				if (n_slash == 2 || n_slash == 1) {	/* Got -Glon0/lat0[/[+|-]units] */
					...
					n_errors += GMT_check_condition (GMT, n < 2, "GMT SYNTAX ERROR.  Expected -G<lon0>/<lat0>[/[-|+]%s|c|C]\n", GMT_LEN_UNITS_DISPLAY);
					...
					if (n_slash == 2) {
						...
						Ctrl->G.unit = (c == '-' || c == '+') ? d : c;
						n_errors += GMT_check_condition (GMT, !strchr (GMT_LEN_UNITS "cC", (int)Ctrl->G.unit), "GMT SYNTAX ERROR.  Expected -G<lon0>/<lat0>[/[-|+]%s|c|C]\n", GMT_LEN_UNITS_DISPLAY);
					}
				}
			...
			case 'L':
				...
				if (k >= 0 && (Ctrl->L.file[k] == '/' || Ctrl->L.file[k] == '-' || Ctrl->L.file[k] == '+')) {	/* Gave [-|+]unit */
					...
					Ctrl->L.file[slash] = 0;
					n_errors += GMT_check_condition (GMT, !strchr (GMT_LEN_UNITS "cC", (int)Ctrl->L.unit), "GMT SYNTAX ERROR.  Expected -L<file>[/[-|+]%s|c|C][+]\n", GMT_LEN_UNITS_DISPLAY);
				}
			...
			case 'N':
				...
				switch (opt->arg[0]) {
					...
					default:
						GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR.  Expected -N[a|c|g|m]\n");
						n_errors++;
				}
				break;
			case 'Q':
			...
			case 'T':
				...
				n_errors += GMT_check_condition (GMT, GMT_set_datum (GMT, to, &Ctrl->T.to) == -1 || GMT_set_datum (GMT, from, &Ctrl->T.from) == -1, "GMT SYNTAX ERROR -T: Usage -T[h]<from>[/<to>]\n");
				break;
	...
	n_errors += GMT_check_condition (GMT, Ctrl->T.active && (Ctrl->G.mode + Ctrl->E.active + Ctrl->L.active) > 0, "GMT SYNTAX ERROR: -T cannot work with -E, -G or -L\n");
	/* Can only do one of -A, -G and -L */
	n_errors += GMT_check_condition (GMT, (Ctrl->G.mode || Ctrl->A.active || Ctrl->L.active) > 1, "GMT SYNTAX ERROR:  Can only specify one of -A, -G and -L\n");
	n_errors += GMT_check_condition (GMT, geodetic_calc && Ctrl->I.active, "GMT SYNTAX ERROR: -A, -G, and -L cannot work with -I\n");
	/* Must have -J */
	n_errors += GMT_check_condition (GMT, !GMT->common.J.active && (Ctrl->G.mode || Ctrl->L.active) && Ctrl->G.unit == 'C', "GMT SYNTAX ERROR:  Must specify -J option with selected form of -G or -L\n");
	if (!GMT->common.R.active && GMT->current.proj.projection == GMT_UTM && Ctrl->C.active) {	/* Set default UTM region from zone info */
		n_errors += GMT_check_condition (GMT, !GMT->current.proj.utm_zoney && GMT->current.proj.utm_hemisphere == 0, "GMT SYNTAX ERROR:  -Ju need zone specification with latitude or hemisphere selection\n");
		if (GMT_UTMzone_to_wesn (GMT, GMT->current.proj.utm_zonex, GMT->current.proj.utm_zoney, GMT->current.proj.utm_hemisphere, GMT->common.R.wesn)) {
			GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Bad UTM zone\n");
			n_errors++;
		}
		else
	...
	n_errors += GMT_check_condition (GMT, !GMT->common.R.active && !(geodetic_calc || Ctrl->T.active || Ctrl->E.active || Ctrl->N.active), "GMT SYNTAX ERROR:  Must specify -R option\n");
	n_errors += GMT_check_binary_io (GMT, 2);
	n_errors += GMT_check_condition (GMT, (Ctrl->D.active + Ctrl->F.active) == 2, "GMT SYNTAX ERROR:  Can specify only one of -D and -F\n");
	n_errors += GMT_check_condition (GMT, ((Ctrl->T.active && GMT->current.proj.datum.h_given) || Ctrl->E.active) && GMT_native_binary (GMT, GMT_IN) && GMT_get_cols (GMT, GMT_IN) < 3, "GMT SYNTAX ERROR.  For -E or -T, binary input data (-bi) must have at least 3 columns\n");
