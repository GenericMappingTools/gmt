/*--------------------------------------------------------------------
 *	$Id: makecpt_func.c,v 1.6 2011-04-12 13:06:44 remko Exp $
 *
...
#define Return(code) {Free_makecpt_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_makecpt (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_makecpt_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_makecpt_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
