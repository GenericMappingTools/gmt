...
GMT_LONG GMT_img2grd (struct GMTAPI_CTRL *API, GMT_LONG mode, void *args)
{
	...
	Merc = GMT_Create_Data (API, GMT_IS_GRID, NULL);
	...
	if (GMT_Register_IO (API, GMT_IS_GRID, GMT_IS_REF, GMT_IS_SURFACE, GMT_IN, &Merc, NULL, &in_ID)) exit (EXIT_FAILURE);
	...
	if (Ctrl->E.active) {	/* Since we will resample again, register a memory location for the result */
		if (GMT_Register_IO (API, GMT_IS_GRID, GMT_IS_REF, GMT_IS_SURFACE, GMT_OUT, &Geo, NULL, &out_ID)) exit (EXIT_FAILURE);
	...
	if (Ctrl->E.active) {	/* Resample again using the given -R and the dx/dy in even minutes */
		...
		if (GMT_Register_IO (API, GMT_IS_GRID, GMT_IS_REF, GMT_IS_SURFACE, GMT_IN, &Geo, NULL, &in_ID)) exit (EXIT_FAILURE);
		GMT_Encode_ID (API, s_in_ID, in_ID);	/* Make filename with embedded object ID */
