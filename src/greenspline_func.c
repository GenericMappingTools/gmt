/*--------------------------------------------------------------------
 *	$Id: greenspline_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_greenspline_parse (struct GMTAPI_CTRL *C, struct GREENSPLINE_CTRL *Ctrl, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case 'R':	/* Normally processed internally but must be handled separately since it can take 1,2,3 dimensions */
				...
				if (!(n_items == 2 || n_items == 4 || n_items == 6)) {
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -R option:  Give 2, 4, or 6 coordinates\n");
					n_errors++;
				}
			...
			case 'Q':	/* Directional derivative */
				...
				if (strchr (opt->arg, '/')) {	/* Got 3-D vector components */
					...
					if (k != 3) {
						GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -Q option:  Append azimuth (2-D) or x/y/z components (3-D)\n");
						n_errors++;
					}
			...
			case 'S':	/* Spline selection */
				...
				switch (opt->arg[0]) {
					...
					case 'q':
						...
						break;
					default:
						GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -D option:  Append c|t|g|p|q\n");
						n_errors++;
					break;
				}
				break;
	...
	n_errors += GMT_check_condition (GMT, Ctrl->A.active && GMT_access (GMT, Ctrl->A.file, R_OK), "GMT SYNTAX ERROR -A:  Cannot read file %s!\n", Ctrl->A.file);
	n_errors += GMT_check_condition (GMT, !(GMT->common.R.active || Ctrl->N.active || Ctrl->T.active), "GMT SYNTAX ERROR.  No output locations specified (use either [-R -I], -N, or -T)\n");
	n_errors += GMT_check_condition (GMT, Ctrl->R3.mode && dimension != 2, "GMT SYNTAX ERROR.  The -R<gridfile> option only applies to 2-D gridding\n");
	n_errors += GMT_check_condition (GMT, Ctrl->R3.dimension != dimension, "GMT SYNTAX ERROR.  The -R and -D options disagree on the dimension\n");
	n_errors += GMT_check_binary_io (GMT, dimension + 1);
	n_errors += GMT_check_condition (GMT, Ctrl->S.value[0] < 0.0 || Ctrl->S.value[0] >= 1.0, "GMT SYNTAX ERROR -S option.  Tension must be in range 0 <= t < 1\n");
	n_errors += GMT_check_condition (GMT, !(Ctrl->S.mode == PARKER_1994 || Ctrl->S.mode == WESSEL_BECKER_2008) && Ctrl->D.mode == 3, "GMT SYNTAX ERROR -Sc|t|r option.  Cannot select -D3\n");
	n_errors += GMT_check_condition (GMT, Ctrl->I.active && (Ctrl->I.inc[GMT_X] <= 0.0 || (dimension > 1 && Ctrl->I.inc[GMT_Y] <= 0.0) || (dimension == 3 && Ctrl->I.inc[GMT_Z] <= 0.0)), "GMT SYNTAX ERROR -I option.  Must specify positive increment(s)\n");
	n_errors += GMT_check_condition (GMT, dimension == 2 && !Ctrl->N.active && !(Ctrl->G.active  || Ctrl->G.file), "GMT SYNTAX ERROR -G option.  Must specify output grid file name\n");
	n_errors += GMT_check_condition (GMT, Ctrl->C.active && Ctrl->C.value < 0.0 && !Ctrl->C.file, "GMT SYNTAX ERROR -C option.  Must specify file name for eigenvalues if cut < 0\n");
	n_errors += GMT_check_condition (GMT, Ctrl->T.active && !Ctrl->T.file, "GMT SYNTAX ERROR -T option.  Must specify mask grid file name\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && !Ctrl->N.file, "GMT SYNTAX ERROR -N option.  Must specify node file name\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && Ctrl->N.file && GMT_access (GMT, Ctrl->N.file, R_OK), "GMT SYNTAX ERROR -N:  Cannot read file %s!\n", Ctrl->N.file);
	n_errors += GMT_check_condition (GMT, (Ctrl->I.active + GMT->common.R.active) == 1 && dimension == 2, "GMT SYNTAX ERROR.  Must specify -R, -I, [-r], -G for gridding\n");
...
GMT_LONG GMT_greenspline (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (Ctrl->T.file) {	/* Existing grid that will have zeros and NaNs, only */
		...
		if (dimension > 1) {
			...
			GMT_grd_setpad (Grid->header, GMT->current.io.pad);	/* Assign default pad */
