...
GMT_LOCAL void *api_pass_object (struct GMTAPI_CTRL *API, struct GMTAPI_DATA_OBJECT *object, unsigned int family, double *wesn) {
	/* Passes back the object pointer after possibly some minor adjustments to metadata.
	 * For grids and images we must worry about any subset requests */
	...
	switch (family) {	/* Do family-specific prepping before passing back the object */
		case GMT_IS_PALETTE:
			...
			break;
		case GMT_IS_GRID:	/* Grids need to update the grdtype setting, possibly rotate geographic grids */
			G = api_get_grid_data (data);
			gmtlib_grd_get_units (API->GMT, G->header);
			G->header->grdtype = gmtlib_get_grdtype (API->GMT, G->header);
			if (wesn && G->data) {	/* Subset or global rotation */
				if (gmt_M_grd_is_global (API->GMT, G->header)) {	/* May have to rotate geographic grid since we are not reading from file */
			...
			break;
		case GMT_IS_IMAGE:	/* Grids need to update the grdtype setting, possibly rotate geographic grids */
			I = api_get_image_data (data);
			gmtlib_grd_get_units (API->GMT, I->header);
			I->header->grdtype = gmtlib_get_grdtype (API->GMT, I->header);
			if (wesn && I->data) {	/* Subset or global rotation */
				if (gmt_M_grd_is_global (API->GMT, I->header)) {	/* May have to rotate geographic grid since we are not reading from file */
...
GMT_LOCAL struct GMT_POSTSCRIPT * api_import_ps (struct GMTAPI_CTRL *API, int object_ID, unsigned int mode) {
	...
	switch (S_obj->method) {	/* File, array, stream etc ? */
		case GMT_IS_FILE:
			...
			if ((P_obj = gmtlib_read_ps (GMT, S_obj->filename, S_obj->method, mode)) == NULL) return_null (API, GMT_CPT_READ_ERROR);
			break;
		case GMT_IS_STREAM:
 			/* gmtlib_read_ps will report where it is reading from if level is GMT_MSG_LONG_VERBOSE */
			kind = (S_obj->fp == GMT->session.std[GMT_IN]) ? 0 : 1;	/* 0 if stdin, 1 otherwise for user pointer */
...
int gmt_write_matrix (struct GMT_CTRL *GMT, void *dest, unsigned int dest_type, unsigned int mode, struct GMT_MATRIX *M) {
	...
	if (dest_type == GMT_IS_FILE) {	/* dest is a file name */
		...
		if (fd && (fp = fdopen (*fd, "a")) == NULL) {
			GMT_Report (GMT->parent, GMT_MSG_NORMAL, "Cannot convert Matrix file descriptor %d to stream in gmt_write_matrix\n", *fd);
			return (GMT_ERROR_ON_FDOPEN);
		}
		...
		GMT_Report (GMT->parent, GMT_MSG_NORMAL, "Unrecognized source type %d in gmt_write_matrix\n", dest_type);
	...
	for (row = 0; row < M->n_rows; row++) {
		ij = GMT_2D_to_index (row, 0, M->dim);	/* Index into the user data matrix depends on layout (M->shape) */
		api_get_val (&(M->data), ij, &value);
		fprintf (fp, GMT->current.setting.format_float_out, value);
...
GMT_LOCAL int api_export_matrix (struct GMTAPI_CTRL *API, int object_ID, unsigned int mode, struct GMT_MATRIX *M_obj) {
	...
	switch (S_obj->method) {	/* File, array, stream etc ? */
		case GMT_IS_FILE:
			...
			if ((error = gmt_write_matrix (GMT, S_obj->filename, S_obj->method, mode, M_obj))) return (gmtapi_report_error (API, error));
			break;
	 	case GMT_IS_STREAM:
			/* gmt_write_matrix will report where it is writing from if level is GMT_MSG_LONG_VERBOSE */
			kind = (S_obj->fp == GMT->session.std[GMT_OUT]) ? 0 : 1;	/* 0 if stdout, 1 otherwise for user pointer */
			GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Write MATRIX to %s %s output stream\n", GMT_method[S_obj->method], GMT_stream[kind]);
			if ((error = gmt_write_matrix (GMT, S_obj->fp, S_obj->method, mode, M_obj))) return (gmtapi_report_error (API, error));
			break;
	 	case GMT_IS_FDESC:
			/* gmt_write_matrix will report where it is writing from if level is GMT_MSG_LONG_VERBOSE */
			kind = (*((int *)S_obj->fp) == GMT_OUT) ? 0 : 1;	/* 0 if stdout, 1 otherwise for user pointer */
...
int gmt_write_vector (struct GMT_CTRL *GMT, void *dest, unsigned int dest_type, unsigned int mode, struct GMT_VECTOR *V) {
	...
	for (row = 0; row < V->n_rows; row++) {
		api_get_val[0] (&(V->data[0]), row, &value);
		fprintf (fp, GMT->current.setting.format_float_out, value);
		for (col = 1; col < V->n_columns; col++) {
			api_get_val[col] (&(V->data[col]), row, &value);
			fprintf (fp, "%s", GMT->current.setting.io_col_separator);
			fprintf (fp, GMT->current.setting.format_float_out, value);
		}
		fprintf (fp, "\n");
		...
		switch (S_obj->method) {	/* File, array, stream etc ? */
			...
			case GMT_IS_REFERENCE:	/* Just pass memory location, so free up what we allocated earlier */
		 	...
		 	case GMT_IS_REFERENCE_VIA_MATRIX:
				...
				S = D_obj->table[D_obj->n_tables]->segment[0] = GMT_Alloc_Segment (API, GMT_IS_DATASET, M_obj->n_rows, n_columns, NULL, NULL);
				...
				if (seg)	/* Got more than one segment, finalize the reallocation of last segment to exact record count */
					S = GMT_Alloc_Segment (API, GMT_IS_DATASET, n_records, n_columns, NULL, S);	/* Reallocate to exact length */
				...
				for (row = seg = n_records = 0; row < V_obj->n_rows; row++) {	/* This loop may include NaN-records and data records */
					...
					if ((status = api_bin_input_memory (GMT, V_obj->n_columns, n_use)) < 0) {	/* Segment header found, finish the one we had and add more */
						...
						if (got_data) {	/* If first input segment has header then we already have a segment allocated */
							S = GMT_Alloc_Segment (API, GMT_IS_DATASET, n_records, n_columns, NULL, S);
							...
							D_obj->table[D_obj->n_tables]->segment[seg] = GMT_Alloc_Segment (API, GMT_IS_DATASET, V_obj->n_rows-n_records, n_columns, NULL, NULL);
	...
	switch (S_obj->method) {
		case GMT_IS_FILE:	/* Name of a image file on disk */
			...
			if (gmt_M_err_pass (GMT, gmtlib_read_image (GMT, S_obj->filename, I_obj, S_obj->wesn,
				...
				return_null (API, GMT_IMAGE_READ_ERROR);
			if (gmt_M_err_pass (GMT, gmtlib_image_BC_set (GMT, I_obj), S_obj->filename)) return_null (API, GMT_IMAGE_BC_ERROR);	/* Set boundary conditions */
...
int GMT_Create_VirtualFile (void *V_API, unsigned int family, unsigned int geometry, char *string) {
	...
	API = api_get_api_ptr (V_API);
	/* GMT_Create_Data will return error code if there are issues with the values of family, or geometry */
	/* Creating an empty object means it is intended to hold output [GMT_OUT] form a module */
...
void *GMT_Read_VirtualFile (void *V_API, const char *string) {
	...
	if ((object = GMT_Retrieve_Data (V_API, object_ID)) == NULL)
		return_null (V_API, GMT_OBJECT_NOT_FOUND);
	return object;
}
			...
			if ((method == GMT_IS_DUPLICATE || method == GMT_IS_REFERENCE)) {	/* GMT_Put_Record: Must realloc last segment and the tables segment array */
				if (S_obj->actual_family == GMT_IS_DATASET) {	/* Dataset type */
					...
					if (D_obj && D_obj->table && D_obj->table[0]) {
						...
						gmtlib_assign_segment (GMT, T_obj->segment[p[GMT_SEG]], p[GMT_ROW], T_obj->n_columns);	/* Allocate and place arrays into segment */
					...
					if (D_obj && D_obj->table && D_obj->table[0]) {
						...
						if (p[GMT_SEG] > 0) T_obj->segment[p[GMT_SEG]]->data = gmt_M_memory (GMT, T_obj->segment[p[GMT_SEG]]->data, T_obj->segment[p[GMT_SEG]]->n_rows, char *);	/* Last segment */
			...
			case GMT_IS_REFERENCE:	/* Only for textsets and datasets */
				...
				if (S_obj->family == GMT_IS_DATASET) {
					...
					switch (status) {
						case GMT_IO_DATA_RECORD:	/* Got a data record */
							...
							for (col = 0; col < n_columns; col++) {	/* Copy from row to curr_rec */
								col_pos = api_pick_in_col_number (GMT, (unsigned int)col);
								GMT->current.io.curr_rec[col] = DS_obj->table[p[GMT_TBL]]->segment[p[GMT_SEG]]->data[col_pos][p[GMT_ROW]];
							}
					...
					switch (status) {
						case GMT_IO_DATA_RECORD:	/* Got a data record */
							S_obj->status = GMT_IS_USING;		/* Mark this resource as currently being read */
							record = strncpy (GMT->current.io.current_record, DT_obj->table[p[GMT_TBL]]->segment[p[GMT_SEG]]->data[p[GMT_ROW]], GMT_BUFSIZ-1);	/* Copy record */
			...
			if (S_obj->family == GMT_IS_DATASET) {
				...
				switch (mode) {
					...
					case GMT_WRITE_SEGMENT_HEADER:	/* Export a segment header record; write NaNs if binary  */
						if (p[GMT_SEG] > 0) {	/* Must first copy over records for the previous segments; last segment done by GMT_End_IO */
							gmtlib_assign_segment (API->GMT, T_obj->segment[p[GMT_SEG]-1], p[GMT_ROW], T_obj->n_columns);	/* Allocate and place arrays into segment */
				...
				switch (mode) {
					...
					case GMT_WRITE_SEGMENT_HEADER:	/* Export a segment header record; write NaNs if binary  */
						p[GMT_SEG]++, p[GMT_ROW] = 0;	/* Go to next segment */
						if (p[GMT_SEG] > 0)
							T_obj->segment[p[GMT_SEG]-1]->data = gmt_M_memory (API->GMT, T_obj->segment[p[GMT_SEG]-1]->data,
					...
					case GMT_WRITE_TEXT:		/* Export a record */
						...
						if (p[GMT_ROW] == T_obj->segment[p[GMT_SEG]]->n_alloc) {	/* Allocate more records */
							...
							T_obj->segment[p[GMT_SEG]]->data = gmt_M_memory (API->GMT, NULL, T_obj->segment[p[GMT_SEG]]->n_alloc, char *);
						}
						T_obj->segment[p[GMT_SEG]]->data[p[GMT_ROW]] = strdup (record);
	...
	switch (family) {	/* dataset, cpt, text, grid , image, vector, matrix */
		...
		case GMT_IS_IMAGE:	/* GMT image, allocate header but not data array */
			if ((mode & GMT_GRID_DATA_ONLY) == 0) {	/* Create new image unless we only ask for data only */
				if (data) return_null (API, GMT_PTR_NOT_NULL);	/* Error if data is not NULL */
	 			if ((new_obj = gmtlib_create_image (API->GMT)) == NULL) return_null (API, GMT_MEMORY_ERROR);	/* Allocation error */
				if (pad >= 0) gmt_set_pad (API->GMT, pad);	/* Change the default pad; give -1 to leave as is */
		...
		case GMT_IS_DATASET:	/* GMT dataset, allocate the requested tables, segments, rows, and columns */
			...
			if ((new_obj = gmtlib_create_dataset (API->GMT, this_dim[GMT_TBL], this_dim[GMT_SEG], this_dim[GMT_ROW], this_dim[GMT_COL], geometry, false)) == NULL) return_null (API, GMT_MEMORY_ERROR);	/* Allocation error */
			break;
		case GMT_IS_TEXTSET:	/* GMT text dataset, allocate the requested tables, segments, and rows */
