...
int GMT_initconf(struct GMT_CTRL *GMT) {
	...
	// FONT_ANNOT_PRIMARY:
	if (GMT_getfont (GMT, "12p,Helvetica,black", &GMT->current.setting.font_annot[GMT_PRIMARY])) error = true;
	// FONT_ANNOT_SECONDARY:
	if (GMT_getfont (GMT, "14p,Helvetica,black", &GMT->current.setting.font_annot[GMT_SECONDARY])) error = true;
	// FONT_TITLE:
	if (GMT_getfont (GMT, "24p,Helvetica,black", &GMT->current.setting.font_title)) error = true;
	// FONT_LABEL:
	if (GMT_getfont (GMT, "16p,Helvetica,black", &GMT->current.setting.font_label)) error = true;
	// FONT_LOGO:
	if (GMT_getfont (GMT, "8p,Helvetica,black", &GMT->current.setting.font_logo)) error = true;

		/* MAP GROUP */

	// MAP_ANNOT_OFFSET_PRIMARY:
	GMT->current.setting.map_annot_offset[GMT_PRIMARY] = GMT_to_inch (GMT, "5p");
	// MAP_ANNOT_OFFSET_SECONDARY:
	GMT->current.setting.map_annot_offset[GMT_SECONDARY] = GMT_to_inch (GMT, "5p");
	// MAP_ANNOT_OBLIQUE:
	GMT->current.setting.map_annot_oblique = 1;
	// MAP_ANNOT_MIN_ANGLE:
	GMT->current.setting.map_annot_min_angle = 20;
	// MAP_ANNOT_MIN_SPACING:
	GMT->current.setting.map_annot_min_spacing = GMT_to_inch (GMT, "0p");
	// MAP_ANNOT_ORTHO:
	...
	// MAP_FRAME_WIDTH:
	dval = GMT_to_inch (GMT, "5p");
	GMT->current.setting.map_frame_width = dval;
	// MAP_GRID_CROSS_SIZE_PRIMARY:
	dval = GMT_to_inch (GMT, "0p");
	...
	// MAP_LABEL_OFFSET:
	GMT->current.setting.map_label_offset = GMT_to_inch (GMT, "8p");
	// MAP_LINE_STEP:
	GMT->current.setting.map_line_step = GMT_to_inch (GMT, "0.75p");
	// MAP_LOGO:
	...
	// MAP_ORIGIN_X:
	GMT->current.setting.map_origin[GMT_X] = GMT_to_inch (GMT, "1i");
	// MAP_ORIGIN_Y:
	GMT->current.setting.map_origin[GMT_X] = GMT_to_inch (GMT, "1i");
	// MAP_POLAR_CAP:
	...
	// MAP_SCALE_HEIGHT:
	GMT->current.setting.map_scale_height = GMT_to_inch (GMT, "5p");
	...
	// MAP_TITLE_OFFSET:
	GMT->current.setting.map_title_offset = GMT_to_inch (GMT, "14p");
	// MAP_VECTOR_SHAPE:
	...
	// IO_LONLAT_TOGGLE:
	if (!gmt_true_false_or_error ("false", &GMT->current.setting.io_lonlat_toggle[GMT_IN]))
