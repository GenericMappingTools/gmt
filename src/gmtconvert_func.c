/*--------------------------------------------------------------------
 *	$Id: gmtconvert_func.c,v 1.5 2011-04-12 03:05:18 remko Exp $
 *
...
GMT_LONG GMT_gmtconvert_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: gmtconvert [files] [-A] [-D[<template>]] [-E[f|l]]\n");
	GMT_message (GMT, "\t[-I] [-L] [-N] [-Q<seg>] [-S[~]\"search string\"] [-T] [%s]\n\t[%s] [%s] [%s]\n", GMT_V_OPT, GMT_a_OPT, GMT_b_OPT, GMT_f_OPT);
...
GMT_LONG GMT_gmtconvert_parse (struct GMTAPI_CTRL *C, struct GMTCONVERT_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, Ctrl->D.active && Ctrl->D.name && !strstr (Ctrl->D.name, "%"), "Syntax error: Output template must contain %%d\n");
