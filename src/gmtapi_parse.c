...
int GMTAPI_Parse_Args (int n_args, char *args[], struct GMT_OPTION **list)
{
	...
	for (arg = 0; arg < n_args; arg++) {	/* Loop over all command arguments */
		...
		new = GMTAPI_Create_Option (args[arg]);	/* Create the new option structure */
...
int GMTAPI_Generate_Args (char ***args, struct GMT_OPTION *head)
{
	...
	txt = (char **) GMT_memory (VNULL, n_alloc, sizeof (char *), "GMTAPI_Generate_Args");
	...
	for (current = head; current; current = current->next) {
		...
		txt[arg] = (char *) GMT_memory (VNULL, strlen (buffer)+1, sizeof (char), "GMTAPI_Generate_Args");
		strcpy (txt[arg], buffer);
#else
		txt[arg] = (char *) GMT_memory (VNULL, strlen (current->arg)+1, sizeof (char), "GMTAPI_Generate_Args");
		strcpy (txt[arg], current->arg);
#endif		
		arg++;
		if (arg == n_alloc) {
			n_alloc += GMT_SMALL_CHUNK;
			txt = (char **) GMT_memory ((void *)txt, n_alloc, sizeof (char *), "GMTAPI_Generate_Args");
		}
	}
	if (arg < n_alloc) txt = (char **) GMT_memory ((void *)txt, (size_t)arg, sizeof (char *), "GMTAPI_Generate_Args");
...
void GMTAPI_Free_Args (int argc, char *args[])
{
	/* Just deallocate the space taken by the list of arguments */
	while (argc) GMT_free ((void *)args[argc--]);
}

struct GMT_OPTION *GMTAPI_Find_Option (struct GMT_OPTION *head, char option)
{
	...
	return (current);	/* Will be NULL if option was not found */
}

int GMTAPI_Update_Option (struct GMT_OPTION *head, char *arg)
{
	/* If the option exists we will update the arguemnts, otherwise
	 * we create a new option and append it to the list */

	struct GMT_OPTION *current, *new;

	new = GMTAPI_Create_Option (arg);	/* What the updated option should be */
	
	current = GMTAPI_Find_Option (head, arg[0]);	/* Try to see if it already existst */
	if (current) {	/* Option is present - replace structure with the one we created */
		if (current->previous) current->previous->next = new;
		if (current->next) current->next->previous = new;
		if (current->arg) GMT_free ((void *)current->arg);
		GMT_free ((void *)current);		
	}
	else {	/* Just hook a new option at the end of the list */
...
struct GMT_OPTION *GMTAPI_Create_Option (char *arg)
{
	...
	new = (struct GMT_OPTION *) GMT_memory (VNULL, 1, sizeof (struct GMT_OPTION), "GMTAPI_Parse_Args");
	new->option = (arg[0] == '-') ? arg[1] : '*';	/* Assign which option character was used (* for file) */
#ifdef WNEN_READY
	if (arg[1] && arg[2]) {		/* Allocate space for the argument and copy to structure */
		new->arg = (char *) GMT_memory (VNULL, strlen (arg) - 1, sizeof (char), "GMTAPI_Parse_Args");
