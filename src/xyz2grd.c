...
#define THIS_MODULE_OPTIONS "-:JRVbdefhiqrsw" GMT_OPT("FH")
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	if (level == GMT_MODULE_PURPOSE) return (GMT_NOERROR);
		GMT_Usage (API, 0, "usage: %s [<table>] -G<outgrid> %s %s [-A[d|f|l|m|n|r|S|s|u|z]] [%s] [%s] [-S[<zfile>]] [%s] "
			"[-Z[<flags>]] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s]", name, GMT_I_OPT, GMT_Rgeo_OPT,
	...
	GMT_Usage (API, 3, "%s A  ASCII (multiple floating point values per record).", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s a  ASCII (one value per record).", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s c  int8_t, signed 1-byte character.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s u  uint8_t, unsigned 1-byte character.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s h  int16_t, signed 2-byte integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s H  uint16_t, unsigned 2-byte integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s i  int32_t, signed 4-byte integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s I  uint32_t, unsigned 4-byte integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s l  int64_t, signed long (8-byte) integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s L  uint64_t, unsigned long (8-byte) integer.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s f  4-byte floating point single precision.", GMT_LINE_BULLET);
	GMT_Usage (API, 3, "%s d  8-byte floating point double precision.", GMT_LINE_BULLET);
