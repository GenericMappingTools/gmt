...
static struct Gmt_moduleinfo g_core_module[] = {
	...
	{"grd2kml", "grd2kml", "core", "Create KML image quadtree from single grid", "<G{,CC(,IG("},
...
#else
	{"basemap", "psbasemap", "core", "Plot base maps and frames", ">X},>DA@AD)", &GMT_basemap, &GMT_psbasemap},
	{"begin", "begin", "core", "Initiate a new GMT modern mode session", "", &GMT_begin, &GMT_begin},
	{"blockmean", "blockmean", "core", "Block average (x,y,z) data tables by mean estimation", "<D{,>D},GG),A->", &GMT_blockmean, &GMT_blockmean},
	{"blockmedian", "blockmedian", "core", "Block average (x,y,z) data tables by median estimation", "<D{,>D},GG),A->", &GMT_blockmedian, &GMT_blockmedian},
	{"blockmode", "blockmode", "core", "Block average (x,y,z) data tables by mode estimation", "<D{,>D},GG),A->", &GMT_blockmode, &GMT_blockmode},
	{"clear", "clear", "core", "Delete current default settings, or the cache, data or sessions directories", "", &GMT_clear, &GMT_clear},
	{"clip", "psclip", "core", "Initialize or terminate polygonal clip paths", "<D{,>X},C-(", &GMT_clip, &GMT_psclip},
	{"coast", "pscoast", "core", "Plot continents, countries, shorelines, rivers, and borders", ">?}", &GMT_coast, &GMT_pscoast},
	{"colorbar", "psscale", "core", "Plot gray scale or color scale bar", "CC{,>X},ZD(", &GMT_colorbar, &GMT_psscale},
	{"contour", "pscontour", "core", "Contour table data by direct triangulation", "<D{,AD)=t,CC(,ED(,DDD,G?(=1,>X}@<D{,AD)=t,CC(,ED(,DD),G?(=1", &GMT_contour, &GMT_pscontour},
	{"dimfilter", "dimfilter", "core", "Directional filtering of grids in the space domain", "<G{,GG},>DQ", &GMT_dimfilter, &GMT_dimfilter},
	{"docs", "docs", "core", "Show HTML documentation of specified module", "", &GMT_docs, &GMT_docs},
	{"end", "end", "core", "Terminate GMT modern mode session and produce optional graphics", "", &GMT_end, &GMT_end},
	{"events", "psevents", "core", "Plot event symbols and labels for a moment in time", "<D{,CC(,>X}", &GMT_events, &GMT_psevents},
	{"figure", "figure", "core", "Set attributes for the current modern mode session figure", "", &GMT_figure, &GMT_figure},
	{"filter1d", "filter1d", "core", "Time domain filtering of 1-D data tables", "<D{,>D},FD(1", &GMT_filter1d, &GMT_filter1d},
	{"fitcircle", "fitcircle", "core", "Find mean position and great [or small] circle fit to points on sphere", "<D{,>D},>DF", &GMT_fitcircle, &GMT_fitcircle},
	{"gmt2kml", "gmt2kml", "core", "Convert GMT data table to Google Earth KML file", "<D{,>D},CC(", &GMT_gmt2kml, &GMT_gmt2kml},
	{"gmtconnect", "gmtconnect", "core", "Connect individual lines whose end points match within tolerance", "<D{,>D},CD),LD),QD)", &GMT_gmtconnect, &GMT_gmtconnect},
	{"gmtconvert", "gmtconvert", "core", "Convert, paste, or extract columns from data tables", "<D{,>D}", &GMT_gmtconvert, &GMT_gmtconvert},
	{"gmtdefaults", "gmtdefaults", "core", "List current GMT default settings", "", &GMT_gmtdefaults, &GMT_gmtdefaults},
	{"gmtget", "gmtget", "core", "Get individual GMT default settings", ">D}", &GMT_gmtget, &GMT_gmtget},
	{"gmtinfo", "gmtinfo", "core", "Get information about data tables", "<D{,>D}", &GMT_gmtinfo, &GMT_gmtinfo},
	{"gmtlogo", "gmtlogo", "core", "Plot the GMT logo", ">X}", &GMT_gmtlogo, &GMT_gmtlogo},
	{"gmtmath", "gmtmath", "core", "Reverse Polish Notation (RPN) calculator for data tables", "<D{,AD(=,TD(,>D}", &GMT_gmtmath, &GMT_gmtmath},
	{"gmtread", "gmtread", "core", "Read GMT objects into external API", "-T-,<?{,>?}", &GMT_gmtread, &GMT_gmtread},
	{"gmtregress", "gmtregress", "core", "Linear regression of 1-D data sets", "<D{,>D}", &GMT_gmtregress, &GMT_gmtregress},
	{"gmtselect", "gmtselect", "core", "Select data table subsets based on multiple spatial criteria", "<D{,CD(=,FD(,LD(=,>D},GG(", &GMT_gmtselect, &GMT_gmtselect},
	{"gmtset", "gmtset", "core", "Change individual GMT default settings", "", &GMT_gmtset, &GMT_gmtset},
	{"gmtsimplify", "gmtsimplify", "core", "Line reduction using the Douglas-Peucker algorithm", "<D{,>D}", &GMT_gmtsimplify, &GMT_gmtsimplify},
	{"gmtspatial", "gmtspatial", "core", "Geospatial operations on points, lines and polygons", "<D{,DD(=f,ND(=,TD(,>D}", &GMT_gmtspatial, &GMT_gmtspatial},
	{"gmtvector", "gmtvector", "core", "Operations on Cartesian vectors in 2-D and 3-D", "<D{,AD(,>D}", &GMT_gmtvector, &GMT_gmtvector},
	{"gmtwhich", "gmtwhich", "core", "Find full path to specified files", ">D}", &GMT_gmtwhich, &GMT_gmtwhich},
	{"gmtwrite", "gmtwrite", "core", "Write GMT objects from external API", "-T-,<?{,>?}", &GMT_gmtwrite, &GMT_gmtwrite},
	{"grd2cpt", "grd2cpt", "core", "Make linear or histogram-equalized color palette table from grid", "<G{+,>C}", &GMT_grd2cpt, &GMT_grd2cpt},
	{"grd2kml", "grd2kml", "core", "Create KML image quadtree from single grid", "<G{,CC(,IG(", &GMT_grd2kml, &GMT_grd2kml},
	{"grd2xyz", "grd2xyz", "core", "Convert grid to data table", "<G{+,>D}", &GMT_grd2xyz, &GMT_grd2xyz},
	{"grdblend", "grdblend", "core", "Blend several partially overlapping grids into one larger grid", "<G{+,GG}", &GMT_grdblend, &GMT_grdblend},
	{"grdclip", "grdclip", "core", "Clip the range of grid values", "<G{,GG}", &GMT_grdclip, &GMT_grdclip},
	{"grdcontour", "grdcontour", "core", "Make contour map using a grid", "<G{,AD)=t,CC(,DDD,>X},G?(=1@<G{,AD)=t,CC(,DD),G?(=1", &GMT_grdcontour, &GMT_grdcontour},
	{"grdconvert", "grdconvert", "core", "Convert between different grid formats", "<G{,>G}", &GMT_grdconvert, &GMT_grdconvert},
	{"grdcut", "grdcut", "core", "Extract subregion from a grid", "<G{,GG}", &GMT_grdcut, &GMT_grdcut},
	{"grdedit", "grdedit", "core", "Modify header or content of a grid", "<G{,ND(,GG}", &GMT_grdedit, &GMT_grdedit},
	{"grdfft", "grdfft", "core", "Mathematical operations on grids in the spectral domain", "<G{+,GG},GDE", &GMT_grdfft, &GMT_grdfft},
	{"grdfill", "grdfill", "core", "Interpolate across holes in a grid", "<G{,>G}", &GMT_grdfill, &GMT_grdfill},
	{"grdfilter", "grdfilter", "core", "Filter a grid in the space (or time) domain", "<G{,FG(=1,GG}", &GMT_grdfilter, &GMT_grdfilter},
	{"grdgradient", "grdgradient", "core", "Compute directional gradients from a grid", "<G{,AG(,GG},SG)", &GMT_grdgradient, &GMT_grdgradient},
	{"grdhisteq", "grdhisteq", "core", "Perform histogram equalization for a grid", "<G{,GG},DD)", &GMT_grdhisteq, &GMT_grdhisteq},
	{"grdimage", "grdimage", "core", "Project and plot grids or images", "<G{+,CC(,IG(,>X},>IA,<ID@<G{+,CC(,IG(,AI),<ID", &GMT_grdimage, &GMT_grdimage},
	{"grdinfo", "grdinfo", "core", "Extract information from grids", "<G{+,>D}", &GMT_grdinfo, &GMT_grdinfo},
	{"grdlandmask", "grdlandmask", "core", "Create a \"wet-dry\" mask grid from shoreline data base", "GG}", &GMT_grdlandmask, &GMT_grdlandmask},
	{"grdmask", "grdmask", "core", "Create mask grid from polygons or point coverage", "<D{,GG}", &GMT_grdmask, &GMT_grdmask},
	{"grdmath", "grdmath", "core", "Reverse Polish Notation (RPN) calculator for grids (element by element)", "<G(,=G}", &GMT_grdmath, &GMT_grdmath},
	{"grdpaste", "grdpaste", "core", "Join two grids along their common edge", "<G{2,GG}", &GMT_grdpaste, &GMT_grdpaste},
	{"grdproject", "grdproject", "core", "Forward and inverse map transformation of grids", "<G{,GG}", &GMT_grdproject, &GMT_grdproject},
	{"grdsample", "grdsample", "core", "Resample a grid onto a new lattice", "<G{,GG}", &GMT_grdsample, &GMT_grdsample},
	{"grdtrack", "grdtrack", "core", "Sample grids at specified (x,y) locations", "<D{,DD),E-<,GG(,>D},SD)=s", &GMT_grdtrack, &GMT_grdtrack},
	{"grdtrend", "grdtrend", "core", "Fit trend surface to grids and compute residuals", "<G{,DG),TG),WG(,WG)", &GMT_grdtrend, &GMT_grdtrend},
	{"grdvector", "grdvector", "core", "Plot vector field from two component grids", "<G{2,CC(,>X}", &GMT_grdvector, &GMT_grdvector},
	{"grdview", "grdview", "core", "Create 3-D perspective image or surface mesh from a grid", "<G{,CC(,GG(,IG(,>X}", &GMT_grdview, &GMT_grdview},
	{"grdvolume", "grdvolume", "core", "Calculate grid volume and area constrained by a contour", "<G{,>D}", &GMT_grdvolume, &GMT_grdvolume},
	{"greenspline", "greenspline", "core", "Interpolate using Green's functions for splines in 1-3 dimensions", "<D{,AD(=,ED),ND(,TG(,CD)=f,G?},GDN", &GMT_greenspline, &GMT_greenspline},
	{"histogram", "pshistogram", "core", "Calculate and plot histograms", "<D{,CC(,>X},>D),>DI@<D{,ID)", &GMT_histogram, &GMT_pshistogram},
	{"image", "psimage", "core", "Plot raster or EPS images", "<I{,>X}", &GMT_image, &GMT_psimage},
	{"inset", "inset", "core", "Manage figure inset setup and completion", ">X}", &GMT_inset, &GMT_inset},
	{"kml2gmt", "kml2gmt", "core", "Extract GMT table data from Google Earth KML files", ">D}", &GMT_kml2gmt, &GMT_kml2gmt},
	{"legend", "pslegend", "core", "Plot a legend", "<D{,>X}", &GMT_legend, &GMT_pslegend},
	{"makecpt", "makecpt", "core", "Make GMT color palette tables", ">C},ED(,SD(,TD(,<D(", &GMT_makecpt, &GMT_makecpt},
	{"mapproject", "mapproject", "core", "Forward and inverse map transformations, datum conversions and geodesy", "<D{,LD(=,>D},W-(", &GMT_mapproject, &GMT_mapproject},
	{"mask", "psmask", "core", "Clip or mask map areas with no data table coverage", "<D{,DDD,C-(,>X},LG)@<D{,DD),C-(,LG)", &GMT_mask, &GMT_psmask},
	{"movie", "movie", "core", "Create animation sequences and movies", "<D(", &GMT_movie, &GMT_movie},
	{"nearneighbor", "nearneighbor", "core", "Grid table data using a \"Nearest neighbor\" algorithm", "<D{,GG}", &GMT_nearneighbor, &GMT_nearneighbor},
	{"plot", "psxy", "core", "Plot lines, polygons, and symbols in 2-D", "<D{,CC(,T-<,>X},S?(=2", &GMT_plot, &GMT_psxy},
	{"plot3d", "psxyz", "core", "Plot lines, polygons, and symbols in 3-D", "<D{,CC(,T-<,>X},S?(=2", &GMT_plot3d, &GMT_psxyz},
	{"project", "project", "core", "Project data onto lines or great circles, or generate tracks", "<D{,>D},G-(", &GMT_project, &GMT_project},
	{"psconvert", "psconvert", "core", "Convert [E]PS file(s) to other formats using Ghostscript", "<X{+,FI)", &GMT_psconvert, &GMT_psconvert},
	{"rose", "psrose", "core", "Plot a polar histogram (rose, sector, windrose diagrams)", "<D{,CC(,ED(,>X},>D),>DI@<D{,ID),CC(", &GMT_rose, &GMT_psrose},
	{"sample1d", "sample1d", "core", "Resample 1-D table data using splines", "<D{,ND(,>D}", &GMT_sample1d, &GMT_sample1d},
	{"solar", "pssolar", "core", "Plot day-light terminators and other sunlight parameters", ">X},>DI,>DM@ID),MD)", &GMT_solar, &GMT_pssolar},
	{"spectrum1d", "spectrum1d", "core", "Compute auto- [and cross-] spectra from one [or two] time series", "<D{,>D},T-)", &GMT_spectrum1d, &GMT_spectrum1d},
	{"sph2grd", "sph2grd", "core", "Compute grid from spherical harmonic coefficients", "<D{,GG}", &GMT_sph2grd, &GMT_sph2grd},
	{"sphdistance", "sphdistance", "core", "Create Voronoi distance, node, or natural nearest-neighbor grid on a sphere", "<D{,ND(,QD(,GG},Q-(", &GMT_sphdistance, &GMT_sphdistance},
	{"sphinterpolate", "sphinterpolate", "core", "Spherical gridding in tension of data on a sphere", "<D{,GG}", &GMT_sphinterpolate, &GMT_sphinterpolate},
	{"sphtriangulate", "sphtriangulate", "core", "Delaunay or Voronoi construction of spherical data", "<D{,>D},ND)", &GMT_sphtriangulate, &GMT_sphtriangulate},
	{"splitxyz", "splitxyz", "core", "Split xyz[dh] data tables into individual segments", "<D{,>D}", &GMT_splitxyz, &GMT_splitxyz},
	{"subplot", "subplot", "core", "Manage modern mode figure subplot configuration and selection", "", &GMT_subplot, &GMT_subplot},
	{"surface", "surface", "core", "Grid table data using adjustable tension continuous curvature splines", "<D{,DD(,LG(,GG}", &GMT_surface, &GMT_surface},
	{"ternary", "psternary", "core", "Plot data on ternary diagrams", "<D{,>X},>DM,C-(@<D{,MD),C-(", &GMT_ternary, &GMT_psternary},
	{"text", "pstext", "core", "Plot or typeset text", "<D{,>X}", &GMT_text, &GMT_pstext},
	{"trend1d", "trend1d", "core", "Fit [weighted] [robust] polynomial/Fourier model for y = f(x) to xy[w] data", "<D{,>D}", &GMT_trend1d, &GMT_trend1d},
	{"trend2d", "trend2d", "core", "Fit [weighted] [robust] polynomial for z = f(x,y) to xyz[w] data", "<D{,>D}", &GMT_trend2d, &GMT_trend2d},
	{"triangulate", "triangulate", "core", "Delaunay triangulation or Voronoi partitioning and gridding of Cartesian data", "<D{,CG(,>D},GG)", &GMT_triangulate, &GMT_triangulate},
	{"wiggle", "pswiggle", "core", "Plot z = f(x,y) anomalies along tracks", "<D{,>X}", &GMT_wiggle, &GMT_pswiggle},
	{"xyz2grd", "xyz2grd", "core", "Convert data table to a grid", "<D{,SD),GG}", &GMT_xyz2grd, &GMT_xyz2grd},
...
void *gmt_core_module_lookup (void *API, const char *candidate) {
	...
	/* Match actual_name against g_module[module_id].cname */
	while (g_core_module[module_id].cname != NULL && (m = strcmp (&candidate[4], g_core_module[module_id].mname)) && strcmp (&candidate[4], g_core_module[module_id].cname))
		++module_id;

	/* Return Module function or NULL */
	return ((m) ? g_core_module[module_id].c_func : g_core_module[module_id].m_func);
}
#endif
