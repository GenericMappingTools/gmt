/*--------------------------------------------------------------------
 *    $Id: psvelo_func.c,v 1.2 2011-03-15 02:06:37 guru Exp $
 *
...
GMT_LONG GMT_psvelo_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT,"usage: psvelo <infiles> %s %s [%s]\n", GMT_J_OPT, GMT_Rgeo_OPT, GMT_B_OPT);
...
GMT_LONG GMT_psvelo_parse (struct GMTAPI_CTRL *C, struct PSVELO_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, n_set > 1, "GMT SYNTAX ERROR:  Only one -S setting is allowed.\n");
	n_errors += GMT_check_condition (GMT, !no_size_needed && (Ctrl->S.symbol > 1 && Ctrl->S.scale <= 0.0), "GMT SYNTAX ERROR:  Must specify symbol size.\n");
	n_errors += GMT_check_condition (GMT, Ctrl->D.active && ! (Ctrl->S.readmode == READ_ELLIPSE || Ctrl->S.readmode == READ_WEDGE), "GMT SYNTAX ERROR:  -D requres -Se|w.\n");

	return (n_errors ? GMT_PARSE_ERROR : GMT_OK);
}

#define Return(code) {Free_psvelo_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_psvelo (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_psvelo_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_psvelo_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	Ctrl = (struct PSVELO_CTRL *)New_psvelo_Ctrl (GMT);	/* Allocate and initialize a new control structure */
