...
int GMT_gdalwrite (struct GMT_CTRL *C, char *fname, struct GDALWRITE_CTRL *prhs) {
	...
	if (!strcmp(prhs->type,"byte")) {		/* This case arrives here via grdimage */
		...
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_gdalwrite: Unsuported input data class!\n");
		return(-1);
	}
	...
	if (prhs->P.active) {
		...
		if( OSRImportFromProj4( hSRS_2, prhs->P.ProjectionRefPROJ4) == CE_None ) {
			...
			GMT_Report (C->parent, GMT_MSG_NORMAL, "Warning: GMT_gdalwrite failed to convert the proj4 string\n%s\n to WKT\n", 
	...
	if( hDriverOut == NULL ) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GMT_gdalwrite: Output driver %s not recognized\n", pszFormat );
		/* The following is s bit idiot. The loop should only be executed is verbose so requires */
		GMT_Report (C->parent, GMT_MSG_VERBOSE, "The following format drivers are configured and support output:\n" );
		for (i = 0; i < GDALGetDriverCount(); i++) {
			hDriver = GDALGetDriver(i);
			if ( GDALGetMetadataItem( hDriver, GDAL_DCAP_CREATE, NULL ) != NULL || 
			     GDALGetMetadataItem( hDriver, GDAL_DCAP_CREATECOPY, NULL ) != NULL )
				GMT_Report (C->parent, GMT_MSG_VERBOSE, "  %s: %s\n", 
	...
	if (hDstDS == NULL) {
		GMT_Report (C->parent, GMT_MSG_NORMAL, "GDALOpen failed - %d\n%s\n", CPLGetLastErrorNo(), CPLGetLastErrorMsg());
		return(-1);
	}
	...
	for (i = 0; i < n_bands; i++) {
		...
		if( i == 1 && hColorTable != NULL ) {
			if (GDALSetRasterColorTable( hBand, hColorTable ) == CE_Failure)
				GMT_Report (C->parent, GMT_MSG_NORMAL, "\tERROR creating Color Table");
