 ...
 * +s : Save data constraints & breakline table after surface_throw_away_unusables is done.
...
#ifdef DEBUG_SURF
GMT_LOCAL void surface_find_mean_constraint (struct GMT_CTRL *GMT, struct SURFACE_INFO *C) {
	...
	GMT_Report (GMT->parent, GMT_MSG_INFORMATION, "Determine mean data constraints and set Briggs coefficients [stride = %d]\n", C->current_stride);
	...
	*/
	...
	for (k = 0; k < C->npoints; k++) {
		if (C->data[k].index == last_index) {	/* Same node but further away than our guy */
			...
			if (C->data[k].kind != last_kind)
				GMT_Report (GMT->parent, GMT_MSG_WARNING, "The %s point %" PRId64 " has been replaced by a %s point.\n", point_type[C->data[k].kind], int64_abs(C->data[k].number), point_type[last_kind]);
			else {
				GMT_Report (GMT->parent, GMT_MSG_WARNING, "The %s point %" PRId64 " will be ignored.\n", point_type[C->data[k].kind], int64_abs(C->data[k].number));
				n_ignored++;
			}
	...
	if (n_outside) {	/* Sort again; this time the SURFACE_OUTSIDE points will be sorted to end of the array */
		...
		if (C->save) {	/* Debug saving the final data constraints */
			...
			GMT_Report (GMT->parent, GMT_MSG_ERROR, "Writing final usable data constraints to file surface.data.\n");
...
GMT_LOCAL void surface_interpolate_add_breakline (struct GMT_CTRL *GMT, struct SURFACE_INFO *C, struct GMT_DATATABLE *T, char *file) {
	int srow, scol;
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Message (API, GMT_TIME_NONE, "\t   +s : Save data constraints & breakline table after surface_throw_away_unusables is done\n");
#endif
...
int GMT_surface_mt (void *V_API, int mode, void *args) {
	...
	if (Ctrl->D.active) {	/* Append breakline dataset */
		...
		surface_interpolate_add_breakline (GMT, &C, Lin->table[0], file);	/* Pass the single table since we read a single file */
	}
...
#ifdef DEBUG_SURF
	if (average)
		surface_find_mean_constraint (GMT, &C);		/* Assign mean data value to nodes and evaluate Briggs coefficients */
	else
#endif
