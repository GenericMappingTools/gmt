...
void GMT_xy_axis (double x0, double y0, double length, double val0, double val1, struct PLOT_AXIS *A, int below, int anotate)
{
	...
	double angle[2];		/* Angles of anotation */
	...
	angle[0] = GMT_set_label_offsets (axis, val0, val1, A, below, anot_off, &label_off, justify, &label_justify, format);
	angle[1] = (A->item[GMT_ANOT_LOWER].active) ? angle[0] : 0.0;
	...
	for (k = 0; k < GMT_GRID_UPPER; k++) {	/* For each one of the 5 axis items (gridlines are done separately) */
		...
		if (do_anot) {	/* Then do anotations too */
			...
			for (i = 0; k < 4 && i < (nx - is_interval); i++) {
				...
				ps_text (x, anot_off[anot_pos], font_size, string, angle[anot_pos], justify[anot_pos], 0);		/* Plot anotation */
			}
		}
	...
	if (axis == 0) {
		...
		switch (project_info.xyz_projection[axis]) {
			...
			case TIME:
				...
				GMT_get_coordinate_label (text_l, &GMT_plot_calclock, format, T, val0);		/* Get time anotation string */
				GMT_get_coordinate_label (text_u, &GMT_plot_calclock, format, T, val1);		/* Get time anotation string */
				break;
		}
...
struct CUSTOM_SYMBOL * GMT_init_custom_symbol (char *name) {
	...
	while (fgets (buffer, BUFSIZ, fp)) {
		...
		if (col[2][0] == 'M') {	/* Set new anchor point */
			if (nc < 3 || nc > 5) error++;
			s->action = ACTION_MOVE;
			if (nc > 3 && col[3][0] == '-' && col[3][1] == 'G') fill_p = &col[3][2], do_fill = TRUE;
			if (nc > 3 && col[3][0] == '-' && col[3][1] == 'W') pen_p = &col[3][2], do_pen = TRUE;
			if (nc > 4 && col[4][0] == '-' && col[4][1] == 'G') fill_p = &col[4][2], do_fill = TRUE;
			if (nc > 4 && col[4][0] == '-' && col[4][1] == 'W') pen_p = &col[4][2], do_pen = TRUE;
		}
		else if (col[2][0] == 'D') {	/* Draw to next point */
			if (!nc == 3) error++;
			...
			s->action = ACTION_ARC;
			s->r = atof (col[2]);
			s->dir1 = atof (col[3]) * D2R;	/* Convert to radians here */
			s->dir2 = atof (col[4]) * D2R;
		}
		else
			error++;
