 ...
 *	GMT_geo_polygon :	Plots polygon in lon/lat on maps, takes care of periodicity jumps
...
void GMT_map_symbol (struct GMT_CTRL *C, struct PSL_CTRL *P, double *xx, double *yy, GMT_LONG *sides, double *line_angles, char *label, GMT_LONG nx, GMT_LONG type, GMT_LONG annotate, GMT_LONG level, GMT_LONG form);
void GMT_map_symbol_ew (struct GMT_CTRL *C, struct PSL_CTRL *P, double lat, char *label, double west, double east, GMT_LONG annotate, GMT_LONG level, GMT_LONG form);
void GMT_map_symbol_ns (struct GMT_CTRL *C, struct PSL_CTRL *P, double lon, char *label, double south, double north, GMT_LONG annotate, GMT_LONG level, GMT_LONG form);
...
void GMT_map_latline (struct GMT_CTRL *C, struct PSL_CTRL *P, double lat, double west, double east);
void GMT_map_lonline (struct GMT_CTRL *C, struct PSL_CTRL *P, double lon, double south, double north);
...
void GMT_fancy_frame_extension (struct GMT_CTRL *C, double angle, double extend[2]);
...
void GMT_geoplot (struct GMT_CTRL *C, struct PSL_CTRL *P, double lon, double lat, int pen);
void GMT_geosegment (struct GMT_CTRL *C, struct PSL_CTRL *P, double lon1, double lat1, double lon2, double lat2);
GMT_LONG set_do_seconds (struct GMT_CTRL *C, double inc);
...
void GMT_linear_map_boundary (struct GMT_CTRL *C, struct PSL_CTRL *P, double w, double e, double s, double n)
{
	...
	if (C->current.map.frame.side[W_SIDE]) GMT_xy_axis (C, P, C->current.proj.rect[XLO], C->current.proj.rect[YLO], y_length, s, n,
		&C->current.map.frame.axis[GMT_Y], TRUE,  C->current.map.frame.side[W_SIDE] & 2);	/* West or left y-axis */
	if (C->current.map.frame.side[E_SIDE]) GMT_xy_axis (C, P, C->current.proj.rect[XHI], C->current.proj.rect[YLO], y_length, s, n,
		&C->current.map.frame.axis[GMT_Y], FALSE, C->current.map.frame.side[E_SIDE] & 2);	/* East or right y-axis */
	if (C->current.map.frame.side[S_SIDE]) GMT_xy_axis (C, P, C->current.proj.rect[XLO], C->current.proj.rect[YLO], x_length, w, e,
		&C->current.map.frame.axis[GMT_X], TRUE,  C->current.map.frame.side[S_SIDE] & 2);	/* South or lower x-axis */
	if (C->current.map.frame.side[N_SIDE]) GMT_xy_axis (C, P, C->current.proj.rect[XLO], C->current.proj.rect[YHI], x_length, w, e,
	...
	form = GMT_setfont (C, P, &C->current.setting.font_title);
...
void GMT_fancy_frame_extension (struct GMT_CTRL *C, double angle, double extend[2])
{
	...
	double s, c;
	sincosd (angle, &s, &c);
	extend[0] = (C->current.setting.map_frame_type == GMT_IS_ROUNDED) ? 0.0 : C->current.setting.map_frame_width * c;
	extend[1] = (C->current.setting.map_frame_type == GMT_IS_ROUNDED) ? 0.0 : C->current.setting.map_frame_width * s;
}
...
void GMT_map_latline (struct GMT_CTRL *C, struct PSL_CTRL *P, double lat, double west, double east)		/* Draws a line of constant latitude */
{
	...
	if (C->current.plot.n > 1) {	/* Need at least 2 points for a line */
		...
		GMT_plot_line (C, P, C->current.plot.x, C->current.plot.y, C->current.plot.pen, C->current.plot.n);
	}
	GMT_free (C, llon);
	GMT_free (C, llat);
}
...
void GMT_map_gridlines (struct GMT_CTRL *C, struct PSL_CTRL *P, double w, double e, double s, double n)
{
	...
	for (k = 0; k < 2; k++) {
		...
		GMT_setpen (C, P, &C->current.setting.map_grid_pen[k]);
...
void GMT_map_gridcross (struct GMT_CTRL *C, struct PSL_CTRL *P, double w, double e, double s, double n)
{
	...
	GMT_map_clip_on (C, P, C->session.no_rgb, 3);
...
void GMT_map_annotate (struct GMT_CTRL *C, struct PSL_CTRL *P, double w, double e, double s, double n)
{
	...
	for (k = 0; k < 1 + dual; k++) {
		if (dx[k] > 0.0 && (GMT_x_is_lon (C, GMT_IN) || C->current.proj.projection == GMT_POLAR)) {	/* Annotate the S and N boundaries */
			...
			do_seconds = set_do_seconds (C, dx[k]);
		...
		if (dy[k] > 0.0 && (GMT_y_is_lat (C, GMT_IN) || C->current.proj.projection == GMT_POLAR)) {	/* Annotate W and E boundaries */
			...
			if (GMT_y_is_lat (C, GMT_IN)) {
				...
				do_seconds = set_do_seconds (C, dy[k]);
				lonlat = 1;
			}
...
void GMT_map_basemap (struct GMT_CTRL *C, struct PSL_CTRL *P)
{
	...
	GMT_vertical_axis (C, P, C->current.plot.mode_3D);
	...
	if (C->current.setting.map_frame_type == GMT_IS_INSIDE) {
		GMT_map_clip_on (C, P, C->session.no_rgb, 3);	/* Must clip to ensure things are inside */
		clip_on = TRUE;
	...
	if (clip_on) GMT_map_clip_off (C, P);
...
void GMT_vertical_axis (struct GMT_CTRL *C, struct PSL_CTRL *P, GMT_LONG mode)
{
	...
	if (fore && C->current.map.frame.side[Z_SIDE]) {
		GMT_plane_perspective (C, P, -1, 0.0);
		...
		GMT_xy_axis (C, P, 0.0, -C->common.R.wesn[ZLO], C->current.proj.zmax - C->current.proj.zmin, C->common.R.wesn[ZLO],
...
void gmt_vertical_wall (struct GMT_CTRL *C, struct PSL_CTRL *P, GMT_LONG quadrant, double *nesw)
{
	GMT_plane_perspective (C, P, (quadrant + 1) % 2, nesw[quadrant % 4]);
...
GMT_LONG GMT_setfont (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_FONT *F)
{	/* Set all attributes of the selected font in the PS */
	GMT_LONG outline;
	
	PSL_setfont (P, F->id);	/* Set the current font ID */
	if (F->form & 2) {	/* Outline font requested; set pen and fill (rgb[0] == -1 means no fill) */
		GMT_setpen (C, P, &F->pen);		/* Stroke the text outline with this pen */
		GMT_setfill (C, P, &F->fill, TRUE);	/* Use this color or pattern (if any) for the text fill */
		...
		PSL_setcolor (P, F->fill.rgb, PSL_IS_FONT);
...
void GMT_plot_line (struct GMT_CTRL *C, struct PSL_CTRL *P, double *x, double *y, int *pen, GMT_LONG n)
{
...
void GMT_draw_map_scale (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_MAP_SCALE *ms)
{
	...
	if (ms->fancy) {	/* Fancy scale */
		...
		if (ms->boxdraw || ms->boxfill) {	/* Draw a rectangle beneath the scale */
			...
			if (ms->boxdraw) GMT_setpen (C, P, &ms->pen);
...
void GMT_draw_map_rose (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_MAP_ROSE *mr)
{
...
{	/* Magnetic compass rose */
	...
	for (level = 0; level < 2; level++) {	/* Outer and inner angles */
		...
		form = GMT_setfont (C, P, &C->current.setting.font_annot[level]);
...
void GMT_setpen (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_PEN *pen)
{
	/* GMT_setpen issues PostScript code to set the specified pen. */

	if (!pen) return;
	PSL_setlinewidth (P, pen->width);
	PSL_setdash (P, pen->style, pen->offset);
	PSL_setcolor (P, pen->rgb, PSL_IS_STROKE);
}

GMT_LONG custum_failed_bool_test (struct GMT_CTRL *C, struct GMT_CUSTOM_SYMBOL_ITEM *s, double size[])
{
	GMT_LONG result;
...
void GMT_draw_custom_symbol (struct GMT_CTRL *C, struct PSL_CTRL *P, double x0, double y0, double size[], struct GMT_CUSTOM_SYMBOL *symbol, struct GMT_PEN *pen, struct GMT_FILL *fill, GMT_LONG outline)
{
	...
	while (s) {
		if (s->conditional > 1) {	/* Process if/elseif/else and } by updating level and skip array, then go to next item */
			if (s->conditional == 2) {	/* Beginning of if branch. If we are inside an earlier branch whose test false then all is FALSE */
				skip[level+1] = (level > 0 && skip[level]) ? TRUE : custum_failed_bool_test (C, s, size), level++;
			...
			if (s->conditional == 8) {	/* Skip if prior if/elseif was TRUE, otherwise evaluate */
				skip[level] = (skip[level]) ? custum_failed_bool_test (C, s, size) : TRUE;
		...
		if (s->conditional == 1 && custum_failed_bool_test (C, s, size)) {	/* Done here, move to next item */
			s = s->next;
			continue;
		}
		...
		switch (s->action) {
			...
			case GMT_SYMBOL_YDASH:
				...
				p = (s->pen)  ? s->pen  : current_pen;
				this_outline = (p && p->rgb[0] == -1) ? FALSE : outline;
				if (this_outline) GMT_setpen (C, P, p);
				GMT_setfill (C, P, f, this_outline);
...
void GMT_flush_symbol_piece (struct GMT_CTRL *C, struct PSL_CTRL *P, double *x, double *y, GMT_LONG *n, struct GMT_PEN *p, struct GMT_FILL *f, GMT_LONG outline, GMT_LONG *flush)
{
	...
	if (draw_outline) GMT_setpen (C, P, p);
	if (draw_outline == 2) {	/* Stroke path only */
		...
		GMT_setfill (C, P, f, draw_outline);
...
void GMT_contlabel_debug (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_CONTOUR *G)
{
	...
	GMT_setpen (C, P, &C->current.setting.map_default_pen);
	if (G->fixed) {	/* Place a small open circle at each fixed point */
		...
		for (j = 0; j < G->xp->n_segments; j++) {
			...
			GMT_plot_line (C, P, G->xp->segment[j]->coord[GMT_X], G->xp->segment[j]->coord[GMT_Y], pen, G->xp->segment[j]->n_rows);
			GMT_free (C, pen);
		}
	}
}
...
void GMT_contlabel_clippath (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_CONTOUR *G, GMT_LONG mode)
{
	...
	if (G->curved_text) {		/* Do it via the labeling PSL function */
		...
		GMT_textpath_init (C, P, &L->pen, G->rgb, &G->pen, L->rgb);
		form = (G->box & 4) ? 16 : 0;
...
{
	PSL_defpen (P, "PSL_setlinepen", LP->width, LP->style, LP->offset, LP->rgb);
	PSL_defpen (P, "PSL_setboxpen", LP->width, LP->style, LP->offset, LP->rgb);

	PSL_defcolor (P, "PSL_setboxrgb", Brgb);
	PSL_defcolor (P, "PSL_settxtrgb", Frgb);
}
...
struct EPS *GMT_epsinfo (struct GMT_CTRL *C)
{
	...
	GMT_sort_array ((void *)fno, id, GMT_LONG_TYPE);
...
void GMT_geo_line (struct GMT_CTRL *C, struct PSL_CTRL *P, double *lon, double *lat, GMT_LONG n)
{
	...
	GMT_plot_line (C, P, C->current.plot.x, C->current.plot.y, C->current.plot.pen, C->current.plot.n);	/* Separately plot the outline */
}

void GMT_geo_polygon (struct GMT_CTRL *C, struct PSL_CTRL *P, double *lon, double *lat, GMT_LONG n)
{
...
#define JUMP_R 1
	...
	GMT_LONG outline, jump, i, k, first, jump_dir = JUMP_L;
	double *xp = NULL, *yp = NULL;
	PFD x_on_border[2] = {NULL, NULL};

	outline = P->current.outline;
	if (GMT_eq (P->current.rgb[PSL_IS_FILL][0], -1.0)) {
		...
		if (C->current.plot.n == n)	{		/* No clipping took place, OK to fill and outline polygon in one go */
			PSL_plotpolygon (P, xp, yp, C->current.plot.n);	/* Fill Cartesian polygon and possibly draw outline */
			outline = FALSE;	/* Meaning we already drew the outline (if it was TRUE) */
		}
		else {	/* Must fill with outline = FALSE then draw the outline separately (if outline = TRUE) */
			if (outline) PSL_command (P, "O0\n");
			PSL_plotpolygon (P, xp, yp, C->current.plot.n);	/* Fill Cartesian polygon but do not draw outline */
			if (outline) PSL_command (P, "O1\n");	/* Restore outline if it was set before */
		}
		...
		if (outline) PSL_command (P, "O0\n");	/* Temporarily switch off outline */
	...
	if (outline) GMT_geo_line (C, P, lon, lat, n);
}
...
void GMT_geo_polygon_segment (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_LINE_SEGMENT *S, GMT_LONG add_pole)
{
	...
	if (add_pole) {	/* Must detour to the N or S pole, then resample the path */
		double step = C->current.setting.map_line_step / C->current.proj.scale[GMT_X] / C->current.proj.M_PR_DEG;
		...
		n = GMT_fix_up_path (C, &plon, &plat, n, step, 0);
	}
	GMT_geo_polygon (C, P, plon, plat, n);	/* Plot filled polygon [no outline if add_pole is true] */
...
void GMT_geo_polygons (struct GMT_CTRL *C, struct PSL_CTRL *P, struct GMT_LINE_SEGMENT *S)
{	/* Deal with plotting of one or more polygons that may wrap across the map.
 	 * Multi-polygons occer if composed of a perimeter and one or more holes.
	 ...
	 * we must lay down path twice (once for fill; once for line) since the
	...
	if (add_pole) {	/* Must do fill and outline separately */
		PSL_comment (P, "Perimeter polygon and polar cap\n");
		...
		PSL_comment (P, "Perimeter polygon\n");
	if (S->next == NULL) {				/* No holes following this perimeter */
		GMT_geo_polygon_segment (C, P, S, add_pole);	/* Lay down perimeter, add pole excursion if add_pole == TRUE */
		P->current.outline = outline;		/* Reset outline to what it was before */
		if (P->current.outline) {		/* Switch on outline again and redraw lines */
			PSL_command (P, "O1\n");	/* Turn outline on in the PS again */
			GMT_geo_line (C, P, S->coord[GMT_X], S->coord[GMT_Y], S->n_rows);	/* Draw the outline only */
		}
		return;
	}
	...
	for (S2 = S->next; S2->next; S2 = S2->next) {	/* Process all the holes except the last one */
		PSL_comment (P, "Hole polygon\n");
		GMT_geo_polygon_segment (C, P, S2, FALSE);	/* Add this hole to the path */
	}
	PSL_command (P, "/FO {fs os}!\n");		/* Reset FO to its original settings */
	GMT_geo_polygon_segment (C, P, S2, FALSE);		/* Lay down final hole and fill/stroke combined polygon */
	if (add_pole) {
		PSL_comment (P, "Perimeter polygon and polar cap outline\n");
		P->current.outline = outline;		/* Reset outline to what it was */
		if (P->current.outline) {
			PSL_command (P, "O1\n");	/* Switch on outline again and redraw lines */
			PSL_comment (P, "Hole polygons outline\n");
			for (S2 = S; S2; S2 = S2->next) GMT_geo_line (C, P, S2->coord[GMT_X], S2->coord[GMT_Y], S2->n_rows);
		}
	}
}

void GMT_geo_ellipse (struct GMT_CTRL *C, struct PSL_CTRL *P, double lon, double lat, double major, double minor, double azimuth)
{
...
void GMT_draw_fence (struct GMT_CTRL *C, struct PSL_CTRL *P, double x[], double y[], GMT_LONG n, struct GMT_FRONTLINE *f)
{
...
void GMT_plane_perspective (struct GMT_CTRL *C, struct PSL_CTRL *P, GMT_LONG plane, double level)
{
