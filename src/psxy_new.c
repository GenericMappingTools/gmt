...
struct PSXY_CTRL {
	...
	struct PSXY_L {	/* -L[+xl|r|x0][+yb|t|y0][+e|E][+p<pen>] */
		...
		bool draw;		/* true when -L<pen> is given and -S is also used */
...
GMT_LOCAL void plot_x_errorbar (struct GMT_CTRL *GMT, struct PSL_CTRL *PSL, double x, double y, double delta_x[], double error_width2, int line, int kind) {
	...
	if (gmt_M_is_dnan (x_1)) {
		GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "X error bar exceeded domain near line %d. Reset to x_min\n", line);
		x_1 = GMT->current.proj.rect[XLO];
		tip1 = false;
	}
	if (gmt_M_is_dnan (x_2)) {
		GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "X error bar exceeded domain near line %d. Reset to x_max\n", line);
...
GMT_LOCAL void plot_y_errorbar (struct GMT_CTRL *GMT, struct PSL_CTRL *PSL, double x, double y, double delta_y[], double error_width2, int line, int kind) {
	...
	if (gmt_M_is_dnan (y_1)) {
		GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "Y error bar exceeded domain near line %d. Reset to y_min\n", line);
		y_1 = GMT->current.proj.rect[YLO];
		tip1 = false;
	}
	if (gmt_M_is_dnan (y_2)) {
		GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "Y error bar exceeded domain near line %d. Reset to y_max\n", line);
...
GMT_LOCAL void plot_x_whiskerbar (struct GMT_CTRL *GMT, struct PSL_CTRL *PSL, double x, double y, double hinge[], double error_width2, double rgb[], int line, int kind) {
	...
	for (i = 0; i < 4; i++) {	/* for 0, 25, 75, 100% hinges */
		...
		if (gmt_M_is_dnan (xx[i])) {
			GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "X %d %% hinge exceeded domain near line %d\n", q[i], line);
...
GMT_LOCAL void plot_y_whiskerbar (struct GMT_CTRL *GMT, struct PSL_CTRL *PSL, double x, double y, double hinge[], double error_width2, double rgb[], int line, int kind) {
	...
	for (i = 0; i < 4; i++) {	/* for 0, 25, 75, 100% hinges */
		...
		if (gmt_M_is_dnan (yy[i])) {
			GMT_Report (GMT->parent, GMT_MSG_VERBOSE, "Y %d %% hinge exceeded domain near line %d\n", q[i], line);
...
GMT_LOCAL int plot_decorations (struct GMT_CTRL *GMT, struct GMT_DATASET *D) {
	...
	GMT_Report (GMT->parent, GMT_MSG_LONG_VERBOSE, "Calling psxy with args %s\n", buffer);
...
GMT_LOCAL int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Message (API, GMT_TIME_NONE, "\t[%s] %s[%s] [%s]\n\t[%s] [%s] [%s]\n\t[%s] [%s]\n\t[%s] [%s] [%s]\n\n", GMT_bi_OPT, API->c_OPT, GMT_di_OPT, GMT_e_OPT, \
		GMT_f_OPT, GMT_g_OPT, GMT_h_OPT, GMT_i_OPT, API->P_OPT, GMT_tv_OPT, GMT_colon_OPT, GMT_PAR_OPT);
	...
	GMT_Message (API, GMT_TIME_NONE, "\t   If symbols are selected then -L<pen> can be used to connect the points with a line [no line].\n");
...
int GMT_psxy (void *V_API, int mode, void *args) {
	...
	bool polygon, penset_OK = true, plot_symbols, plot_line, old_is_world;
	...
	uint64_t row, seg, seg_out = 0, n_new, n_cols = 2;

	char s_args[GMT_BUFSIZ] = {""};

	double direction, length, dx, dy, d, in[GMT_MAX_SYMBOL_COLS];
	...
	struct GMT_DATASET *D_orig = NULL;	/* Pointer to original un-resampled GMT multisegment table(s) */
	...
	else GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Processing input table data\n");
	...
	plot_symbols = !(S.symbol == GMT_SYMBOL_FRONT || S.symbol == GMT_SYMBOL_QUOTED_LINE || S.symbol == GMT_SYMBOL_DECORATED_LINE || S.symbol == GMT_SYMBOL_LINE);
	plot_line = (!plot_symbols || Ctrl->L.draw);
	...
	get_rgb = (plot_symbols && Ctrl->C.active);	/* Need to read z-vales from input data file */
	...
	else if (plot_symbols)	/* Here we have the usual x y [z] [size] [other args] [symbol] record */
	...
	else if (plot_symbols && (Ctrl->N.mode == PSXY_CLIP_REPEAT || Ctrl->N.mode == PSXY_CLIP_NO_REPEAT))	/* Only set clip if plotting symbols and -N not used */
		clip_set = true;
	...
	if (plot_symbols && !outline_active && !fill_active && !get_rgb) outline_active = true;	/* If no fill nor outline for symbols then turn outline on */

	if (Ctrl->D.active) PSL_setorigin (PSL, Ctrl->D.dx, Ctrl->D.dy, 0.0, PSL_FWD);	/* Shift plot a bit */

	old_is_world = GMT->current.map.is_world;
	geometry = plot_symbols ? GMT_IS_POINT : ((polygon) ? GMT_IS_POLY: GMT_IS_LINE);
	...
	if (S.read_symbol_cmd)	/* Must prepare for a rough ride */
		...
		GMT_Set_Columns (API, GMT_IN, n_needed, GMT_COL_FIX_NO_TEXT);
	else if ((error = GMT_Set_Columns (API, GMT_IN, (unsigned int)n_needed, GMT_COL_FIX)) != GMT_NOERROR) {
	...
	else if (Ctrl->F.active) {	/* Do network connections */
		struct GMT_DATASET *Dtmp = NULL;	/* Pointer to GMT multisegment table(s) */
		if ((Dtmp = GMT_Read_Data (API, GMT_IS_DATASET, GMT_IS_FILE, 0, GMT_READ_NORMAL, NULL, NULL, NULL)) == NULL) {
			Return (API->error);
		}
		if (plot_symbols)
			D_orig = GMT_Duplicate_Data (API, GMT_IS_DATASET, GMT_DUPLICATE_DATA, Dtmp);
		if (Dtmp->n_records) {
	...
	if (plot_symbols && D_orig == NULL)
		D_orig = GMT_Duplicate_Data (API, GMT_IS_DATASET, GMT_DUPLICATE_DATA, D);
	if (plot_line) {	/* Line/polygon part */
	...
	if (plot_symbols) {	/* Symbol part (not counting GMT_SYMBOL_FRONT, GMT_SYMBOL_QUOTED_LINE, GMT_SYMBOL_DECORATED_LINE) */
		bool periodic = false, delayed_unit_scaling = false;
		unsigned int n_warn[3] = {0, 0, 0}, warn, item, n_times, k;
		...
		for (tbl = 0; tbl < D_orig->n_tables; tbl++) {

			for (seg = 0; seg < D_orig->table[tbl]->n_segments; seg++) {	/* For each segment in the table */
				L = D_orig->table[tbl]->segment[seg];	/* Shorthand */
				PSL_comment (PSL, "Segment header: %s\n", L->header);
				(void)gmt_parse_segment_header (GMT, L->header, P, &fill_active, &current_fill, &default_fill, &outline_active, &current_pen, &default_pen, default_outline, NULL);
				if (Ctrl->I.active) {
					gmt_illuminate (GMT, Ctrl->I.value, current_fill.rgb);
					gmt_illuminate (GMT, Ctrl->I.value, default_fill.rgb);
				}
				if (gmt_parse_segment_item (GMT, L->header, "-S", s_args)) {	/* Found -Sargs */
				...
				for (row = 0; row < L->n_rows; row++) {	/* Data record to process */
			
					for (k = 0; k < L->n_columns; k++) in[k] = L->data[k][row];
					n_total_read++;

					if (S.read_symbol_cmd) {	/* Must do special processing depending on variable symbol */
						char X[6][32];
						int n_read, kk;
						/* The problem here is that we have read the entire file and n_colums gets set by the first record.
						 * In the case of variable symbols then the number of items vary per record and some of the data
						 * are now in the text string.  Thus, we convert the words in the string to numbers and the number of
						 * successful scans are the missing floating point columns.  The next holds the symbol code and more.
						 * Thus, we parse that and continue on. */
						
						/* First establish the symbol type given at the end of the record */
						n_read = sscanf (L->text[row], "%s %s %s %s %s %s", X[0], X[1], X[2], X[3], X[4], X[5]) - 1;
						for (kk = 0; kk < n_read; kk++) in[k++] = atof (X[kk]);	/* Add in other columns */
						//if (S.read_symbol_cmd == 1) gmt_parse_symbol_option (GMT, L->text[row], &S, 0, false);
						if (S.read_symbol_cmd == 1) gmt_parse_symbol_option (GMT, X[kk], &S, 0, false);
					...
					if (gmt_M_is_dnan (plot_x)) {	/* Transformation of x yielded a NaN (e.g. log (-ve)) */
						GMT_Report (API, GMT_MSG_VERBOSE, "Data point with x = NaN near line %d\n", n_total_read);
						continue;
					}
