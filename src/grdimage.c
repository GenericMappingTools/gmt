...
EXTERN_MSC bool gmtlib_remote_file_is_tiled (struct GMTAPI_CTRL *API, const char *file);
...
EXTERN_MSC int GMT_grdimage (void *V_API, int mode, void *args) {
	...
	if (use_intensity_grid) {	/* Illumination grid desired */
		if (Ctrl->I.derive) {	/* Illumination grid must be derived */
			...
			if (gmtlib_remote_file_is_tiled (API, Ctrl->In.file[0])) {	/* Must read and stitch the tiles first */
