...
/* gmt_supplmodule.c populates the external array of GMT suppl module parameters such as name
 ...
 *   void gmt_supplmodule_show_all(struct GMTAPI_CTRL *API); - Pretty print all suppl module names and their
 *           purposes
 *
 * DO NOT edit this file directly! Instead edit gmt_supplmoduleinfo.txt and regenerate
 * this file with gmt_make_supplmodule_src.sh. */
...
struct Gmt_moduleinfo g_supplmodule[] = {
...
struct Gmt_alias gmt_supplalias[] =
{	/* Alias:	Full name */
...
struct Gmt_alias gmt_supploldname[] =
{	/* Old:		New: */
	{NULL,		NULL}
};

/* Look out for suppl modules given by their aliases */
const char * gmt_supplformal_name (struct GMTAPI_CTRL *API, const char *module) {
	unsigned int k = 0;
	while (gmt_supplalias[k].alias != NULL) {
		if (!strcmp (module, gmt_supplalias[k].alias)) return gmt_supplalias[k].name;
		k++;
	}
	if (GMT_compat_check (API->GMT, 4)) {
		k = 0;
		while (gmt_supploldname[k].alias != NULL) {
			if (!strcmp (module, gmt_supploldname[k].alias)) return gmt_supploldname[k].name;
			k++;
		}
	}
	return module;
...
void gmt_supplmodule_show_all (struct GMTAPI_CTRL *API) {
	unsigned int module_id = 0;
	char module_name_comp[GMT_TEXT_LEN64], message[GMT_TEXT_LEN256];

	GMT_Message (API, GMT_TIME_NONE, "Program                Purpose of Program\n");
	while (g_supplmodule[module_id].name != NULL) {
		snprintf (module_name_comp, GMT_TEXT_LEN64, "%s(%s)",
				g_supplmodule[module_id].name, g_supplmodule[module_id].component);
		sprintf (message, "%-22s %s\n",
				module_name_comp, g_supplmodule[module_id].purpose);
