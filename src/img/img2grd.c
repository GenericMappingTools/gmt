	...
	for (opt = options; opt; opt = opt->next) {	/* Process all the options given */
		...
		switch (opt->option) {
			...
			case 'W':
				...
				if (gmt_get_required_double (GMT, opt->arg, opt->option, 0, &Ctrl->W.value)) {
					n_errors++;
...
EXTERN_MSC int GMT_img2grd (void *V_API, int mode, void *args) {
	...
	if (Ctrl->M.active) {
		...
		snprintf (Merc->header->remark, GMT_GRID_REMARK_LEN160, "%s%.12g/%.12g/%.12g/%.12g", GMT_IMG_REMARK, wesn[XLO], wesn[XHI], south2, north2);
	}
	else {
