...
GMT_LONG GMT_gmtaverage_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: gmtaverage [infile(s)] %s -Te|m|n|o|s|w|<q> %s\n", GMT_I_OPT, GMT_Rgeo_OPT);
	GMT_message (GMT, "\t[-C] [-E[b]] [-Q] [%s] [-W[i][o]]\n\t[%s] [%s] [%s] [%s] [%s] [%s] [%s]\n\n",
...
#define Return(code) {Free_gmtaverage_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_gmtaverage (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_gmtaverage_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtaverage_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
