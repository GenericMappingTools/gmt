...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Message (API, GMT_TIME_NONE, "\t[-C<cpt>] [-D<sigscale>] [-G<fill>] %s[-L[<pen>][+c[f|l]]] [-N] %s%s[-S<symbol>[<scale>][</args>][+f<font>]]\n", API->K_OPT, API->O_OPT, API->P_OPT);
	...
	GMT_Message (API, GMT_TIME_NONE, "\t-D Multiply uncertainties by <sigscale> (Se and Sw only).\n");
...
EXTERN_MSC int GMT_psvelo (void *V_API, int mode, void *args) {
	...
	double z_val, e_val, value, scale;
	...
	if (Ctrl->S.read) {	/* Read symbol size from file */
		...
		scol = Ctrl->S.n_cols - 1;	/* Column ID with scales */
