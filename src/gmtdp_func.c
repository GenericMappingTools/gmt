/*
 *	$Id: gmtdp_func.c,v 1.4 2011-04-12 03:05:18 remko Exp $
 *
...
GMT_LONG GMT_gmtdp_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: gmtdp <infiles> -T<tolerance>[<unit>] [-G<file>] [%s]\n", GMT_V_OPT);
	GMT_message (GMT, "\t[%s] [%s] [%s]\n\t[%s] [%s] [%s] [%s]\n\n",
		GMT_b_OPT, GMT_f_OPT, GMT_g_OPT, GMT_h_OPT, GMT_i_OPT, GMT_o_OPT, GMT_colon_OPT);

	if (level == GMTAPI_SYNOPSIS) return (EXIT_FAILURE);

	GMT_message (GMT, "\tinfiles (in ASCII or binary) have 2 or more columns with (x,y) [or (y,x); see -:] in the first columns.\n");
...
#define Return(code) {Free_gmtdp_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_gmtdp (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_gmtdp_usage (API, GMTAPI_USAGE));/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtdp_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
