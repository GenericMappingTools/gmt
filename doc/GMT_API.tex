...
API simple: In addition to the \GMT\ modules, there are only 16 public functions to become familiar with.
...
you will use the \texttt{GMT\_Get\_Data} or \texttt{GMT\_Read\_Data}. In the case of data and text tables, you may also consider the
...
phase and are ready to start the actual data import.  This is only required when reading one record at the
...
The \texttt{nfields} pointer will return the number of fields read; pass NULL if this is not
of interest.
...
FORTRAN 77 developers who wish to use the \GMT\ API may use the same 16 API functions as discussed in Chapter 2.
...
\end{itemize}
The list of the basic 16 FORTRAN prototype functions thus becomes
\begin{verbatim}
