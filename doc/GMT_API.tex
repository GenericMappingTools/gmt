...
\texttt{	unsigned int id;}	&       /* \emph{An identification number} */ \\
\texttt{	unsigned int n\_columns;}	&       /* \emph{Number of vectors}  */\\
\texttt{	uint64_t n\_rows;}	&       /* \emph{Number of rows in each vector} */ \\
\texttt{	enum GMT_enum_alloc alloc\_mode;}	&       /* \emph{Determines if we may free the vectors or not}  */\\
\texttt{	enum GMT_enum_type *type;}		&       /* \emph{Array with data type for each vector}  */\\
...
\texttt{	unsigned int id;}		&       /* \emph{An identification number} */ \\
\texttt{	unsigned int n\_rows;}		&       /* \emph{Number of rows in the matrix} */ \\
\texttt{	unsigned int n\_columns;}	&       /* \emph{Number of columns in the matrix}  */\\
\texttt{	unsigned int n\_layers;}	&       /* \emph{Number of layers in a 3-D matrix}  */\\
\texttt{	unsigned int shape;}		&       /* \emph{0 = C (rows) and 1 = Fortran (cols)}  */\\
\texttt{	unsigned int registration;}	&       /* \emph{0 for gridline and 1 for pixel registration}  */\\
\texttt{	size_t dim;}		&       /* \emph{Length of dimension for row (C) or column (Fortran)}  */\\
\texttt{	size_t size;}		&       /* \emph{Byte length of data}  */\\
\texttt{	enum GMT_enum_alloc alloc\_mode;}	&       /* \emph{Determines if we may free the vectors or not}  */\\
\texttt{	enum GMT_enum_type type;}		&       /* \emph{The matrix data type}  */\\
...
\begin{verbatim}
int GMT_Register_IO (struct GMTAPI_CTRL *API, unsigned int family, unsigned int method, \
...
\begin{verbatim}
int GMT_Init_IO (struct GMTAPI_CTRL *API, unsigned int family, unsigned int geometry, \
...
Next, \texttt{n_args} is 0 if \texttt{args} is the head of a linked list of options, otherwise \texttt{args} is an array of \texttt{n_args} strings (i.e., char *argv[])
...
\begin{verbatim}
void * GMT_Read_Data (struct GMTAPI_CTRL *API, unsigned int family, unsigned int method, \
    unsigned int geometry, unsigned int mode, double wesn[], char *input, void *data)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Get_Data (struct GMTAPI_CTRL *API, int ID, unsigned int mode, \
	void *data)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Get_Record (struct GMTAPI_CTRL *API, unsigned int mode, int *nfields)
\end{verbatim}
...
\begin{verbatim}
int GMT_Write_Data (struct GMTAPI_CTRL *API, unsigned int family, unsigned int method, \
...
\begin{verbatim}
int GMT_Put_Record (struct GMTAPI_CTRL *API, unsigned int mode, void *rec)
\end{verbatim}
...
\begin{verbatim}
int GMT_Destroy_Data (struct GMTAPI_CTRL *API, unsigned int mode, void *data)
\end{verbatim}
