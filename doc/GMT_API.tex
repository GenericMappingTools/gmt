...
to control if a time stamp should preface the message and if selected how it should be formatted.
...
\label{sec:parsopt}
As you develop new code you may need to rely on some of the \GMT\ common options in your program.
...
that may require you to parse user text into dimensions, length, coordinates, time, etc. The API
provides several functions to simplify these tasks

\subsection{Display syntax for a GMT common option}
...
own custom options in an overall alphabetical order.
...
The API provide a limited window into the full \GMT\ machinery accessible to the modules.
You can determine if a particular option has been parsed and in some case determine the
values with
...
For instance, to determine what the \Opt{R} resulted in you may call
...
Note: Dimensions given with units of inches, cm, or points are converted to the
...
\subsection{Inquiring about a GMT default value}
...
Depending on what parameter you selected you could obtain a numerical value
via \texttt{GMT\_Get\_Value} or just do a text comparison.
...
have standardized on using `N').  The \texttt{dim} sets the dimension of the transform, currently 1 or 2,
while the \texttt{string} is the 1-line message that goes with the option; you should tailor this to
...
which accepts the user's string option via \texttt{args}; the other arguments repeats those from above.
...
Before your can take any transforms you must initialize the process. This involves a series of steps
that are conveniently performed by

\begin{verbatim}
void * GMT_FFT_Create (void *API, void *X, unsigned int dim, unsigned int subdivide, \
...
(1 or 2), \texttt{subdivide} is not used by the API, \texttt{mode} passes various flags to the setup,
and \texttt{F} is the opaque pointer returned by \texttt{GMT\_FFT\_Parse}.  Depending on the options
you chose to pass to \texttt{GMT\_FFT\_Parse}, the data will be detrended, reflected by various symmetries,
padded to desired transform dimensions, and possibly
with temporary files written out before the transform takes place.

\section{Taking the FFT}
\index{FFT}

Now that everything has been set up you can call
...
specify if we pass a real or complex data set, and \texttt{K} is the opaque pointer returned
by \texttt{GMT\_FFT\_Create}.  The transform is performed in place.  When done with your
manipulations you can call it again with the inverse flag.

\section{Wavenumber calculations}
\index{Wavenumber calculations}

As your data has been transformed to the wavenumber domain you will wish to visit the various
