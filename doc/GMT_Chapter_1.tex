%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.140 2010-01-08 00:00:34 guru Exp $
%
...
involve new options and enhancements.  Beginning with \GMT 4.5.3 there will only be bug-fixes
...
for some time after \GMT 5 is released later this year
...
\begin{description}
	...
	\item [\GMTprog{gmt\_init.c}]:	Now guards against getting a negative hash value, which happened
		...
		Bug in the parsing of \Opt{Jg}{\it lon/lat/radius/lat}.
	...
	\item [\GMTprog{pslib.c}]:	Ellipse was wrongly dimensioned by semi-major and semi-
		minor axes, instead of major and minor axes.  Also, memory never got freed by \GMTfunc{ps\_free}.
	...
	\item [\GMTprog{x2sys/x2sys\_init.c}]:	Did not accept \Opt{m}, only the old \Opt{M}.
...
\begin{enumerate}
	\item \GMTprog{fitcircle.c} will let \Opt{S} optionally take a latitude instead of finding
		the best-fitting latitude [Default].
	\item \GMTprog{gmtdefaults.c} has new BASEMAP\_TYPE = graph option for linear projections that wish to
	...
	\item \GMTprog{psxyz.c} also has the new symbol \Opt{Sm} option but does not draw the arrow heads yet.
