%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.129 2009-09-11 02:14:14 guru Exp $
%
...
\subsection{Overview of \gmt\ 4.5.1 [Sept-15, 2009]}

This is almost entirely a but-fix release where we address 64-bit incompatibilities and
rebuild the netcdf GSHHS library to include some attributes from GSHHS that are needed
by new options in \GMTprog{pscoast}.  Note that GSHHS polygons have not changed (still
...
projection (\Opt{JPoly}) and experimental support for grid and image imports via GDAL
...
\begin{description}
	\item [\GMTprog{configure.ac}]:	Now use --enable-flock to enable file locking, instead of
		...
		Switch -DGMT\_QSORT is now on only for OSX prior to Snow Leopard as the latter has
...
\begin{enumerate}
	\item \GMTprog{gmt\_customio.c} now has optional GDAL read interface as format 22 (code gd).
	...
	\item \GMTprog{grdimage.c} has optional support that allow a minimalist image import via GDAL.
	\item \GMTprog{nc2xy.c} now has \Opt{bo} option.
\end{enumerate}
