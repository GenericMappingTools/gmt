%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.88 2008-04-25 00:42:22 guru Exp $
%
...
support for \htmladdnormallinkfoot{Octave}{http://www.octave.org}, all source code is now fully
64-bit compliant, we have added an isolation mode option (if {\bf GMT\_TMPDIR} is defined, write
...
(similar to \htmladdnormallinkfoot{proj4}{http://proj.maptools.org}).  Finally, binary table
data can now be COARDS-compliant netCDF files. As for documentation, we have now switched from
C shell to Bourne shell.

The following lists specific enhancements or new program options:

% Enhancements:
\begin{enumerate}
\item \GMTprog{gmt\_grdio.c} is modified so the grd i/o support the {\bf GMT\_[DATA$|$IMG$|$GRID]DIR} environment settings.
\item \GMTprog{gmt\_init.c} was enhanced so \Opt{U} can now interpret a justification (e.g., {\it just/dx/dy})
and we introduce a new default parameter {\bf UNIX\_TIME\_FORMAT} which controls the formatting
of the timestamp.
\item \GMTprog{gmt\_io.c} now implements \Opt{b}[{\bf i}$|${\bf o}]{\bf c}[{\it var1/...}] option to indicate input is netCDF.
\item \GMTprog{gmtmath} recognizes new constants {\bf TMIN}, {\bf TMAX}, {\bf TINC}, and {\bf N}.
...
\item \GMTprog{grdmath} recognizes new constants {\bf XMIN}, {\bf XMAX}, {\bf XINC}, {\bf NX}, and similarly
{\bf YMIN}, {\bf YMAX}, {\bf YINC}, {\bf NY}.
...
\item \GMTprog{mgd77/mgd77list}	added \Opt{Ga}$|${\bf b}{\it rec} to limit output to a certain record range.
...
A long list of bugs have been squashed since the last release, the most important are listed below:
...
\item [\GMTprog{gmt\_io.c}]: Now skips blank lines that has leading whitespace.  Function
\item [\GMTprog{gmt\_init.c}]: {\bf c} for seconds was not recognized as {\bf TIME\_UNIT} (expected {\bf s},
...
\item [\GMTprog{gmt\_plot.c}]: 3-D basemap axis did not use {\bf LABEL\_OFFSET}.
...
of {\it size\_t} variables in expressions that could yield a negative value.
...
properly honor the {\it ddd.xx} setting. Now implements annotation for Gnomonic maps.
...
Fixed error in applying \GMTprog{pstext}'s \Opt{Dj}{\it dx/dy} shift in paragraph mode (\Opt{M}).
Redefine rect symbol to be less prone to round-off.
Used internal {\it point\_code} before it was initialized.  This
caused {\bf PAGE\_COLOR} not to work (wrote $<$NUL$>$ rather than C).
\item [\GMTprog{gmtmath.c}]: Implemented Welford (1962) algorithm in {\bf KURT}, {\bf SKEW} and {\bf STD} operators
...
\item [\GMTprog{grd2xyz.c}]: Do not abort with \Opt{R} exceeds grid; simply output common region.
...
Needed to allow for some slop when comparing shrink to 0.0 since sometimes the result of {\it acos} is 1e-14 or thereabouts.
\item [\GMTprog{grdmath.c}]: Implemented Welford (1962) algorithm in {\bf KURT}, {\bf SKEW} and {\bf STD} operators
for more precise one-pass computation of mean and sum of squares.
...
and {\bf OUTPUT\_DEGREE\_FORMAT} could interfere with result.
...
in hue.
\item \GMTprog{originator} ({\it spotter} supplement) has new option \Opt{Q} to specify constant r/t for (x,y,z) data only.
\end{enumerate}
...
\item [DLL]: The {\it meca} supplements now work with Windows DLL by using the GMT i/o functions.
...
\item [\GMTprog{mgd77manage.c}]: \Opt{Q}{\it value} changed to \Opt{Q}[-]{\bf b$|$c$|$l$|$n}[/{\it threshold}] to
...
\item [\GMTprog{mgd77.c}]: Added mechanism to search directories for files ({\it mgd77} supplement).
\item [\GMTprog{mgd77list}]: Activated \Opt{X} option and associated machinery for applying data corrections  ({\it mgd77} supplement).
\end{description}
