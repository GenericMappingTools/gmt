%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.100 2009-01-10 00:12:18 guru Exp $
%
...
% Complete up to 2009-1-9:

\subsection{Overview of \gmt\ 4.4.0 [Jan-15, 2009]}
...
\begin{description}
	...
	\item [\GMTprog{grdreformat.c}]: Since there is no longer a \filename{share/conf/gmt\_formats.conf} the usage message crashed.
		Did not initialize grid header structure and could getnetCDF error ``Named variable does not exist in file''.  This
...
\item \GMTprog{pslib.c} internals now measure paper size in double precision points instead
	...
	Finally, we use a new implementation of \GMTfunc{ps\_textdim} to ensure proper alignment
...
\item \GMTprog{ps2raster.c} now will scan for the optional comment \verb+%%HiResBoundingBox+
	...
	set the level of anti-aliasing for graphics and text, respectively.  Finally, we added a new
	option \Opt{F} to force a specified output file name.
