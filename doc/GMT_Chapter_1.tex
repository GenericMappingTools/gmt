%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.35 2006-05-08 01:35:12 pwessel Exp $
%
...
\subsection{Overview of \gmt\ 4.1.2}

On the surface, changes in \GMT\ 4.1.1 are relatively minor.  Most of the work has involved
...
\item Optionally append {\bf z} to the \Opt{Jp} projection to annotate depths (north-y) rather than radius.
\item Two new tools added to the {\bf misc} supplement for digitizing lines (\GMTprogi{gmtdigitize.c} and
to stitch digitized lines into continuous lines or polygons (\GMTprogi{gmtstitch.c}.
\item Extended \Opt{M} option to take modifiers {\bf i} or {\bf o} for input or output.
\item Added support for custom grd format AGC from Atlantic Geoscience Center.  Given the code {\bf af} [21].
\end{enumerate}

A few programs or options have received minor updates and new features, such as

\begin{description}
\item [\GMTprogi{blockmean.c}]: Added \Opt{E} for reporting standard deviation, min, and max values per block.
\item [\GMTprogi{blockmedian.c}]: Added \Opt{E} for reporting L1 scale, min, and max values per block.
Also added \Opt{Q} to specify a particular quartile [Default {\it q} = 0.5 = median].
\item [\GMTprogi{blockmode.c}]: Added \Opt{E} for reporting LMS scale, min, and max values per block.
\item [\GMTprogi{configure}]: Added explicit options to bypass the installation of supplements
that require Matlab (\verbX--disable-mexX) and X11 (\verbX--disable-xgridX).
\item [\GMTprogi{gmtconvert.c}]: Added \Opt{D} option to write segments to individual output files.
\item [\GMTprogi{gmtdefaults.c}]: Support for new default {\bf PS\_VERBOSE} which controls the
...
\item [\GMTprogi{gmtmath.c}]: Added option \Opt{M} to indicate the program can now handle multisegment files.
Added {\bf CPOISS} for cumulative Poisson distribution.
\item [\GMTprogi{grdmath.c}]: Added {\bf CPOISS} for cumulative Poisson distribution.
\item [\GMTprogi{minmax.c}]: \Opt{D} made obsolete by improved range checking for longitudes (but available for
backwards compatibility).
\item [\GMTprogi{psscale.c}]: Enhanced \Opt{I} option for asymmetrical intensity ranges from {\it low} to {\it high}.
\item [\GMTprogi{psxy.c}]: Added \Opt{SW} for wedges defined by azimuths rather than directions.  Polygons of large
lontigudional extent now clip correctly.
\item [\GMTprogi{splitxyz.c}]: New option \Opt{Q} to specify the output columns and their order.
\end{description}
...
\begin{description}
\item [\GMTprogi{gmt\_plot.c}]: The 3-D perspective plotting of text was not scaled correctly.
\item [\GMTprogi{gmt\_support.c}]: Parsing of \Opt{L} option used in \GMTprogi{psbasemap} and \GMTprogi{pscoast}
failed to get correct unit when ddd:mm:ss syntax was used for the position.
\item [\GMTprogi{gmt2rgb.c}]: Did not check name template properly, and did not initialize region.
\item [\GMTprogi{gmtselect.c}]: Option \Opt{F} insisted on using spherical testing for Cartesian {\it x,y} data.
\item [\GMTprogi{grd2xyz.c}]: The \Opt{E} option had the {\it y}-direction reversed.
\item [\GMTprogi{grdfilter.c}]: Needed the \Opt{f} option to process \Opt{R}ddd:mm syntax.
\item [\GMTprogi{grdimage.c}]: Would hang in {\it stdin} if \Opt{C} not given when one grid is plotted.
\item [\GMTprogi{grdmask.c}]: Did not explicitly close polygons before using them.
\item [\GMTprogi{grdmath.c}]: Command {\bf INSIDE} for Cartesian data had bug (passed {\it y} where {\it x} was expected).
\item [\GMTprogi{grdsample.c}]: Failed when \Opt{I} was specified.
\item [\GMTprogi{project.c}: Cartesian projections gave incorrect results for {\it p,w,r,s}.
Removed 0--360 restriction on azimuth.  \Opt{G} was susceptible to round-off and thus
sometimes reissued the final point.
\item [\GMTprogi{psxy.c}]: \Opt{SV} and \Opt{SE} for \Opt{JX} did not convert azimuths to directions.
Would get confused when distances between successive labels were smaller than the line's point spacing.
\item [\GMTprogi{mgd77/mgd77manage.c}]: Did no properly close the file after ingesting E77 information.
\item [\GMTprogi{pslib.c}]: \GMTfunc{ps\_load\_raster} did not use open mode {\bf rb} and hence failed under Windows.
\item [\GMTprogi{xyz2grd.c}]: The \Opt{E} option had the {\it y}-direction reversed.
\item [\GMTprogi{x2sys/x2sys\_get.c}]: \Opt{N} did not work properly (now fixed and tested).
\end{description}
...
\item \GMTprogi{spotter/grdrotater}: New tool that rotates grids given a specified finite rotation.
...
\item [\GMTprogi{psxyz.c}]: \Opt{S}{\bf O}$|${\bf U} imitate \Opt{S}{\bf o}$|${\bf u} but without the 3-D color shading.
\end{description}
...
\item [\GMTprogi{ISO-8859-9.ps}]: Added /dotlessi per Onur Tan.
