%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.164 2011-03-15 02:06:29 guru Exp $
%
...
{\bf MAP\_ANNOT\_ORTHO}, which controls whether axes annotations for Cartesian plots are horizontal or
...
\begin{enumerate}
	\item All programs now use consistent, standardized choices for plot dimension units ({\bf c}m, {\bf i}nch, or {\bf p}oint;
		we no longer consider {\bf m}eter a plot length unit), and projected distances (choose spherical arc lengths in {bf d}egree,
		{\bf m}inute, and {\bf s}econd [was {\bf c}], or distances in m{\bf e}ter [Default], {\bf f}eet [new], 
		{\bf k}m, {\bf M}iles [was sometimes {\bf i} or {\bf m}], or {\bf n}autical miles).
	...
	\item All text can now optionally be filled with patterns and/or drawn with outline pens.  In the past, only
		\GMTprog{pstext} could plot outline fonts via \Opt{S}{\it pen}.  Now, any text can be an outline text
		by manipulating the corresponding FONT defaults (e.g., {\bf FONT\_TITLE}).
	\item All color or fill specifications may append @{\it transparency} to change the PDF transparency level for that item.
	...
	\item \GMTprog{mapproject} has a new \Opt{N} option to do geodetic/geocentric conversions; it combines with \Opt{I}
		for inverse conversions.  Also, we have extended \Opt{A} to accept \Opt{A}{\bf o}$|${\bf O} to compute line orientations (-90/90).
	...
	\item \GMTprog{grdvolume} has enhanced \Opt{T}, now \Opt{T}[{\bf c}$|${\bf h}] for ORS estimates based on max curvature or height.
	...
	\item \GMTprog{psxy.c} and \GMTprog{psxyz.c} can take symbol modifier {\bf +s}{\it scale}[{\it unit}][/{\it origin}][{ \bf l}]
		in \Opt{S} to adjust scales read from files.  This is used when you have data in the third column that should be
		used for symbol size but they need to be offset (by {\it origin}) and scaled by ({\it scale}) first; {\bf l} means
	...
	\item \GMTprog{pstext.c} can take simplified input via new option \Opt{F} to set fixed font (including size), angle, and justification.  If
		these parameters are fixed for all the text strings then the input can simply be {\it x y text}.
