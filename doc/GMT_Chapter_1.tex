%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.111 2009-04-22 05:37:31 guru Exp $
%
...
\subsection{Overview of \gmt\ 4.4.1 [May-1, 2009]}

This is another significant update of the official distribution and hence has a mix of bug fixes
...
spherical surface.  The hard work is done by the original work of Robert Renka who developed
the Fortran-77 SSRFPACK and STRIPACK libraries; these are here supplied via a \progname{f2c}
translation and requires the \progname{f2c} library and include file to be installed.
...
Finally, \GMTprog{ps2raster.c} has evolved further and can now be used to create KML files for Google Earth,
assuming \progname{gdal} is installed on your system.

A major new enhancement is the global option \Opt{g} which is used to control if excessive spacing (``gaps'')
between data points (defined in a variety of ways) should be used to segment an otherwise continuous
line.  We expect to enable \Opt{g} in several programs during the next revision.
...
allows us to promote this option to global status (same meaning in all \gmt\ programs).
Use of \Opt{M} will remain valid for the rest of \gmt\ 3.x but results in a warning about the new usage.
Related to this is the introduction of a new parameter ({\bf NAN_RECORDS}) that determines if NaNs in
key columns (such as longitude, latitude) should constitute a line break or bad data.

We have revised how ellipsoids are specified.  When importing an ellipsoid file, allow a,b,f as ellipsoid parameters,
where b or f could both be zero.  If file does not exist, attempt to read name as a[/[b=|f=]f],
meaning <semi-major axis>, b=<semi-minor axis>, f=<flattening>, or <inverse flattening>.  We have also
added parameters for the Moon and planets (IAU2000).

FInally, our configure script continuous to evolve and now better supports installation on 64-bit systems and
...
\begin{description}
	...
	Fixed inability to determine projection pole when central_meridian and western boundary are the same (Albers, Conic equidistant).
	...
	\item [\GMTprog{gmt_support.c}]: Exceeded array bounds in \GMTfunc{GMT\_read\_cpt} for hsv conversion.
	...
	\item [\GMTprog{gshhs/gshhs.c}]: Checked wrong variable to test if a file was given.
	\item [\GMTprog{meca/util\_meca.c}]: Needed to convert the output from d_atan2() to degrees.
	\item [\GMTprog{mgd77/mgd77.c}]: Was failing to read Windows terminated ASCII files.
	\item [\GMTprog{mgd77/mgd77list.c}]: Used GMT\_LONG in memset for an array allocated as int.
...
\begin{enumerate}
	...
	\item \GMTprog{gmt_nc.c} will properly handle netCDF that have LatLon = 0, 1: flip x and y.
	\item \GMTprog{gmt_support.c} enables GMT_intpol to handle NaNs by treating them as segment
	...
	\item \GMTprog{gmtmath.c} added MOD function (remainder after floored division,
		Knuth style). This is in contrast to FMOD which gives the remainder after (horror) truncated division.
	...
	\item \GMTprog{grdmath.c} added MOD function (remainder after floored division).
	...
	\item \GMTprog{mgd77/mgd77list.c} added -Fytime which gives decimal absolute year for time output.
...
\begin{description}
	...
	\item [\GMTprog{spotter/backtracker.c}]: The \Opt{W} option always assumed the reverse rotation,
		i.e. from hotspot to seamount.  Now obeys the \Opt{D} option.
...
The box option \Opt{W}[\emph{fill}][o$|$O$|$c$|$C][\emph{pen}]] is now \Opt{W}[\emph{fill},][o$|$O$|$c$|$C][\emph{pen}]], i.e., we now use
...
\item Optionally append \textbf{z} to the \Opt{Jp} projection to annotate depths (i.e., ``north-y'') rather than radius.
