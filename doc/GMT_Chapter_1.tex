%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.43 2006-10-19 08:08:08 pwessel Exp $
%
...
Changes in \GMT\ 4.1.4 are relatively minor and predominantly bug fixes.  However, a few enhancements
...
\item \GMTprog{misc/ps2raster} Many improvements; added EPS output; high-quality PDF output.
ALso removed -dDOINTERPOLATE option which caused inversion of colour map and had no benefits.
Did not look in current dir for *.def files\item New environment variable {\bf GMT\_USERDIR} [by default ~/.gmt] points to a directory
which can contain user initialization files such as default settings.
\end{enumerate}
...
Did not consider if absolute coordinates were given in \Opt{X} and \Opt{Y}.
...
\item [\GMTprog{xyz2grd}]: Had \Opt{Az} as default rather than no \Opt{A}.
