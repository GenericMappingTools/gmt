%------------------------------------------
%	$Id: GMT_Appendix_Q.tex,v 1.2 2011-03-15 02:06:29 guru Exp $
%
...
possible different versions in future.  It is indicated by the {\bf @V} sequence.

\begin{table}[h]
\small
\centering
\begin{tabular}{|l|l|l|} \hline
V & GMT-1.0 & Data in this file is stored using v1.0 of the OGR/GMT data format \\ \hline
\end{tabular}
\label{tbl:Q1}
\end{table} 
...
(preceded by the {\bf @G} code sequence on
...
region will be stored in \GMT\ \Opt{R} format (i.e.: \Opt{R}{\it W/E/S/N},
where the {\it W/E/S/N} values represent the extent of features); the
{\bf @R} code sequence marks the domain information.
...
prefixed by {\bf @J} (J being the \GMT\ character for projection information.
The {\bf @J} code will be followed by a character identifying the format, as shown
...
Note that an OGR-generated file will not have a {\bf @Jg} string, as OGR does
...
\end{table} 
\noindent
Any name containing a space must be quoted.   The {\bf @N} selection must be combined with a matching string specifying the data type
for each of the named fields, using the {\bf @T} prefix.
\begin{table}[H]
\small
\centering
\begin{tabular}{|l|l|l|} \hline
...
specification, including {\bf string}, {\bf integer}, {\bf double},
{\bf datetime}, and {\bf logical} (boolean).
...
\begin{verbatim}
# @VGMT-1.0 @GPOLYGON @Nname|depth|id @Tstring|double|integer
\end{verbatim}
...
line will be a comment flag (``\#'') followed by {\bf @D}, followed by a string of
...
the {\bf @N} or {\bf @D} specifiers must be enclosed in double quotes.
...
\begin{verbatim}
# @VGMT-1.0 @GPOINT @Nname|depth|id @Tstring|double|integer
# @D"Point 1"|-34.5|1
\end{verbatim}
\noindent
...
outer perimeter, or an inner ring (hole). This is done using a {\bf @P} or {\bf @H}
on the data comment preceding the polygon coordinates. The {\bf @P} specifies
a new feature boundary (perimeter), any following {\bf @H} polygons are
holes, and must be within the preceding {\bf @P} polygon (as described in the
shapefile specification). Any {\bf @H} polygons will NOT have any {\bf @D} values,
as the aspatial attribute data pertain to the entire feature, the {\bf @H}
...
\begin{verbatim}
# @VGMT-1.0 @GPOINT @Nname|depth|id
...
\begin{verbatim}
# @VGMT-1.0 @GLINESTRING @Nname|depth|id
# @Tstring|double|integer
...
\begin{verbatim}
# @VGMT-1.0 @GPOLYGON @N"Polygon name"|substrate|id @Tstring|string|integer
# @R178.1/178.6/-48.7/-45.6
