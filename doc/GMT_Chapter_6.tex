%------------------------------------------
%	$Id: GMT_Chapter_6.tex,v 1.21 2009-08-17 16:42:47 remko Exp $
%
...
(although you may change this with {\bf MAP\_SCALE\_FACTOR}).
...
the {\bf MAP\_SCALE\_FACTOR} parameter.
...
when a world map is given (indicated by \Opt{R}{\it 0/360/s/n}), the arguments
...
\begin{figure}[htb]
   \includegraphics[width=\textwidth]{scripts/GMT_utm_zones}
...
(although a standard, you can change this with {\bf MAP\_SCALE\_FACTOR}).
...
into rectangular {\it x}/{\it y} coordinates: 
...
\gmt\ uses the interpolant selected by the parameter {\bf INTERPOLANT}
in the \filename{.gmtdefaults4} file.}.  The scale is true
...
use \Opt{JKf} ({\bf f} for ``four'') while Eckert VI is selected with
\Opt{JKs} ({\bf s} for ``six'').  If no modifier is given it defaults to
Eckert VI.
...
The same script, with {\bf s} instead of {\bf f}, yields the Eckert VI map:
