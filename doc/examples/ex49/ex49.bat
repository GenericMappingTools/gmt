...
gmt begin ex49
	...
	gmt blockmode -R0/100/-10000/0 -I5/10000 -r -E -Q depth-age.bin -bi3f -o0,2,3 > modal.txt
	...
	echo S 0.5c - 0.9c - 4p,green 1.1c Parsons ^& Sclater (1977) > tmp.txt
	echo S 0.5c - 0.9c - 4p,white 1.1c Stein ^& Stein (1992)	>> tmp.txt
	echo S 0.5c s 0.4c blue - 1.1c Modal depth estimates		>> tmp.txt
	gmt legend tmp.txt -DjRT+w6c+o0.25c -F+p1p+gbeige+s
	echo S 0.5c - 0.9c - 1p 0.75c	> tmp.txt
	echo S 0.5c - 0.9c - 1p 0.75c	>> tmp.txt
	echo S 0.5c s 0.1c white - 0.75c	>> tmp.txt
	gmt legend tmp.txt -DjRT+w6c+o0.25c
