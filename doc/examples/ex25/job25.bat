...
REM		GMT EXAMPLE 25
REM
REM		$Id: job25.bat,v 1.2 2004-04-27 22:34:33 pwessel Exp $
REM
...
gmtmath -bi1s -Ca -S key.b SUM UPPER RINT = | gawk "{printf "S 0.15i s 0.2i blue 0.25p 0.3i Terrestrial Antipodes [%d %%]\n", $1}" >> key.txt 
grdmath key.grd -1 EQ 0 NAN scale.grd MUL = tmp.grd
grd2xyz tmp.grd -S -ZTLf > key.b
...
grd2xyz tmp.grd -S -ZTLf > key.b
gmtmath -bi1s -Ca -S key.b SUM UPPER RINT = | gawk "{printf "S 0.15i s 0.2i blue 0.25p 0.3i Mixed Antipodes [%d %%]\n", $1}" >> key.txt 
...
pslegend -R0/9/0/0.5 -Jx1i/-1i -O -Dx4.5/0/6i/0.3i/TC -Y-0.2i -Fthick key.txt >> example_25.ps
del *.grd
del key.*
del .gmt*
if %master%==y cd ..
