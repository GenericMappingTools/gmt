...
# First we convert the shapefile to something more GMT friendly. Here we make
# into a large ASCII file. Human-readable but not very efficient. GDALs ogr2ogr
# is used with the "-F GMT" option indicating that the output should have the
# native GMT format. Input is the downloaded land_polygons.shp, and the output
# is named land_polygons_osm_planet.gmt. The resulting file is very large 
# (>1 GB) but already usable. But wait, there is more ...

# ogr2ogr -F GMT land_polygons_osm_planet.gmt land_polygons.shp
...
# file.
# Lets have a closer look at -bo2f: 
...
gmt select land_polygons_osm_planet.bf2 \
 -bi2f -bo2f \
 -R-25.14/16.75/-24.8/16.95r \
 > land_polygons_osm_caboverde.bf2
...
gmt begin ex51
  ...
  gmt coast -JL-24.9/16.55/16.3/16.7/15c -R-25.14/16.75/-24.8/16.95+r \
    -Wred -Ba10mg10m
