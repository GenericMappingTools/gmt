...
gmt begin ex52 ps
  ...
  slat=$(gmt solar -C -o1 -I+d2000-06-22T24:00+z-10)
  ...
  gmt grdmath -Rd -I0.1 -r --PROJ_ELLIPSOID=sphere $slon $slat SDIST KM2DEG 90 SUB NEG 2 DIV ATAN DUP UPPER DIV 1 ADD 2 DIV = w.grd

  # We will create an intensity grid based on the 6m DEM so that we can see structures in the oceans
  gmt grdgradient @earth_relief_06m -Nt0.5 -A45 -Gintens.grd
  ...
  gmt grdmix @BlueMarble_06m.tif @BlackMarble_06m.tif -Ww.grd -Iintens.grd -Gnewmap.tif
