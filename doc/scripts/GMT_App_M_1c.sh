...
plot () {
	...
	gmt colorbar -Dx${2}i/${y}i+w2.70i/0.125i+h+jTC -Ctt.1.cpt -B0
	gmt colorbar -Dx${2}i/${y2}i+w2.70i/0.125i+h+jTC -Ctt.2.cpt -Bf0.25
...
dy=0.6
y0=$(gmt math -Q $n 2 ADD $dy MUL 0.5 MUL 0.25 ADD =)
...
gmt makecpt -H -Cgmt/categorical -T1/13/1 > tt.1.cpt
gmt makecpt -H -Cgmt/paired -T1/13/1 > tt.2.cpt
gmt colorbar -Dx1.55i/${y}i+w2.70i/0.125i+h+jTC -Ctt.1.cpt -B0
gmt colorbar -Dx4.50i/${y}i+w2.70i/0.125i+h+jTC -Ctt.2.cpt -B0
gmt text -D0/0.05i -F+f9p,Helvetica-Bold+jBC <<-END
1.55 $y gmt/categorical
4.50 $y gmt/paired
END
