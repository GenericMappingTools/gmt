...
*nrows*\ **x**\ *ncols*
    ...
    subplots per row you will need to stack separate subplots.  Note: You are not
    required to place a plot in each panel.
...
Note: Any **-J** option passed when plotting subplots must give ? as scale or width
...
fill the subplot by using unequal scales].
