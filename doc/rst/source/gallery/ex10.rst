...
linear projection for the basemap and add the multi-band column and text on top.
The different bands in the columns indicate how commonly the languages are used, from institutional languages to
languages threatened by extinction.  We use awk to sum up the total languages per continent.
