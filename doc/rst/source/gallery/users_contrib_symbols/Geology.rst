...
In order to use the symbols in GMT you need to use the program :doc:`psxy </psxy>`, using the custom
...
the symbol as in any symbol plotted with :doc:`psxy </psxy>`. The table lists the parameters needed for
...
.. _tbl-Geology:
    ...
    be used in :doc:`psxy </psxy>` **-Sk**\ *symbolname*\ /*size*. The extra parameters needed for each symbol are
