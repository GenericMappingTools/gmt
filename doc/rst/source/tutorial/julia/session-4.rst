...
#. Create color tables based on a master CPT color table and the histogram-equalized distribution of *z*-values
   in a gridded data file (:doc:`/grd2cpt`)

One can also make these files manually. Here we will limit our discussion to :doc:`/makecpt`. 
...
We can plot these color tables with :doc:`/colorbar`;
...
If you run :doc:`/makecpt` in modern mode then you usually
...
#. Redo the :doc:`/makecpt` exercise using the master table *hot* and redo the bar plot.
...
Color images are made with :doc:`/grdimage` which takes the usual common command options
(by default the **region** is taken from the data set) and a CPT; the main other options are:
...
derive the required intensities from the gradients of the topography in the N90°E direction using :doc:`/grdgradient`.
...
#. Rerun :doc:`/gragradient` with **norm**\ =1.
...
produces perspective views of gridded data files is :doc:`/grdview`.
