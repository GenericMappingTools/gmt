...
\section{Options to change grid coordinates}
\index{grid file!coordinates}
...
\begin{itemize}
	\item You have a geographic grid (i.e., in longitude and latitude).  Simply supply the
	      ...
	      via a ``Flat Earth'' approximation on the currently selected ellipsoid.
	\item You have a Cartesian grid but the units are not meters (e.g., they may perhaps be
	      ...
	      to save original grid with distances in meters using some other unit.
\end{itemize}
This modifier is only allowed when map projections are not selected (or are Cartesian).
