%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.12 2001-09-26 02:07:39 pwessel Exp $
%
...
\subsection{Projections -- The \Opt{J?} options}
\index{\Opt{J}}
...
\subsection{Map frame and axes -- The \Opt{B} option}
\index{\Opt{B}}
\index{Tickmarks}
\index{Anotations}
\index{Gridlines}
...
to another format it will be used directly.  Note that for these axes you may use the {\it unit} setting to
...
\subsection{Header records -- The \Opt{H} option}
\index{\Opt{H}}
...
\subsection{Portrait orientation -- The \Opt{P} option} 
...
\subsection{Timestamps -- The \Opt{U} option} 
\index{\Opt{U}}
\index{Timestamp}
...
\subsection{Plot positioning -- The \Opt{X} \Opt{Y} options}
\index{\Opt{X}}
\index{\Opt{Y}}
\index{Plot!offset}
\index{Offset, plot}
...
\subsection{lat and Lon -- The \Opt{:} option}
\index{\Opt{:}}
\index{lat/lon input}
...
\filename{.gmtdefaults} file which by default is set to {\it yyyy-mm-dd}.  Y2K-challenged input data such as 29/05/89T08.30pm can be processed by setting {\bf INPUT\_DATE\_FORMAT}
