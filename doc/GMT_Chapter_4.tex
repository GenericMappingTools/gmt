%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.88 2006-10-30 15:41:43 remko Exp $
%
...
\emph{DarkGreen}). Capitalizing the color name may be required when the option that takes the pen attribute allows an
additional lower case specifier. For example, use \GMTprog{grdcontour} \Opt{W}Azure, instead of \Opt{W}azure, to avoid
accidentally switching on the annotated contours \Opt{Wa}.
\end{enumerate}
...
those conventions (provided the grid is 2- or 3-, or 4-dimensional) can be read directly by \GMT\ and the netCDF grids
...
\GMT\ will take case of this. The user is, however, still responsible for setting the variable name and
