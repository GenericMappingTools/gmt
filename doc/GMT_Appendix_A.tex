...
\section{gshhg: GSHHG data extractor}

This package contains \GMTprog{gshhg} which you can use to extract
shoreline polygons from the Global Self-consistent Hierarchical High-resolution
Shorelines (GSHHG) available separately from \htmladdnormallinkfoot{NGDC}{http://www.ngdc.noaa.gov/mgg/shorelines/gshhs.html}
or the \htmladdnormallinkfoot{GSHHG home page}{http://www.soest.hawaii.edu/pwessel/gshhg}
(GSHHG is the polygon data base from which
the \GMT\ coastlines derive).  It also contains \GMTprog{gshhg\_dp}
for cleverly decimating a shoreline, and \GMTprog{gshhgtograss} to convert shoreline
...
This package consists of the program \GMTprog{img2grd} to
...
At the moment, this package contains the programs \GMTprog{gravfft}, which
performs gravity, isostasy, and admittance calculation for grids, \GMTprog{grdredpol}, which
compute the continuous reduction to the pole, AKA differential RTP for magnetic data,
\GMTprog{grdseamount}, which computes synthetic bathymetry over various seamount shapes,
and \GMTprog{xyzokb}, which computes the gravity or magnetic anomaly of a body by
