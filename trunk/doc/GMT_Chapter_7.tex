%------------------------------------------
%	$Id: GMT_Chapter_7.tex,v 1.52 2008-06-12 22:06:19 guru Exp $
%
...
are available; only the \progname{sh}-scripts are discussed here) and support
...
volcano symbol and \GMTprog{psxy}. We do something similar with the bullseye symbols.
...
grids but instead tied to a spherical Mercator grid.  The \GMT\ supplement imgsrc has tools to
...
avoiding to re-project the data twice since it is already in a Mercator projection.  This example
shows how this is accomplished.  We use the \Opt{M} option in \GMTprog{img2grd} to pull out the
grid in Mercator units (i.e., do \emph{not} invert the Mercator grid) and then simply plot the
...
Finally, we present a case where we wish to plot a data set given in UTM coordinates and make sure
it is properly registered with overlying geographic data such as coastlines or data points.  The
number one rookie error most \GMT\ users commit is to specify the UTM projection with their UTM
data.  However, that data have already been projected and is now in linear meters.  The only way
sensible to plot such data is with a linear projection, yielding a UTM map.  In this step one can
choose to annotate or tick the map in UTM meters as well.  To plot geographic (lon/lat) data on
the same map there are a few things you must remember:
\begin{enumerate}
