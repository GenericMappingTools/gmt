%------------------------------------------
%	$Id: GMT_Chapter_7.tex,v 1.13 2004-04-27 22:34:33 pwessel Exp $
%
...
days) we must change the default input/output formats used by \GMT. Here is
...
example will run even if you do not have \progname{wget}; remove the comment to
...
aircraft pilots have known from the start that only one great-circle path connect the
...
these cities with their names and distances (in km) from Rome, all ontop of a beautiful
world map.
...
\begin{enumerate}
\item Lies in the ocean and not on land
\item Is within 3000 km of Hobart
\item Is more than 1000 km away from the International Dateline
\end{enumerate}
...
passes our criteria.  Most of the work here is done by \GMTprog{gmtselect}; the rest
are carried out by the usual \GMTprog{pscoast} and \GMTprog{psxy} workhorses.  Note
...
equation, note that we first calculate a grid that is +1 when a point and its
antipode is on land, -1 if both are in the ocean, and 0 elsewhere.  We then
seek to calculate the area distribution or dry antipodes by only pulling out the nodes
that equal +1.  As each point represent an area $\Delta \phi \times \Delta \lambda$
and the $\Delta \lambda$ terms actual dimension depends on $\cos (\phi)$, we need
to allow for that shrinkage and compare that to the whole area of the Earth.  Since
the $\Delta \lambda$, $\Delta \phi$ terms appear twice they cancel out, leaving the
intractable expressions below:
