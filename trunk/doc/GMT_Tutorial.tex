%------------------------------------------
%	$Id: GMT_Tutorial.tex,v 1.1.1.1 2000-12-28 01:23:45 gmt Exp $
%
...
\title{The Generic Mapping Tools Version 3.3.6---A Map-making Tutorial}
...
3.3.1, 3.3.2, 3.3.3, 3.3.4, 3.3.5, and 3.3.6) were released in Novenber 1998, March 1999, July 1999,
July 1999, September, December 1999, March 2000, July 2000, and October 2000, respectively. \GMT\ now is
...
\index{.gmtdefaults@\filename{.gmtdefaults}}

\item Implicitly using \GMT\ defaults for a variety of parameters
(stored in \filename{.gmtdefaults}).
...
Numerous minor options (more than 50) can only be changed by
modifying the \GMT\ defaults settings.  These settings control
such aspects of \GMT\ as font types and sizes, pen thickness used
for basemaps, linear interpolants used when interpolation is needed
and many more (Figures \ref{fig:GMT_Defaults_1a}, \ref{fig:GMT_Defaults_1b},
and \ref{fig:GMT_Defaults_1c} show the parameters that affect plots).
The \GMT\ defaults reside in a file named
\filename{.gmtdefaults}.  A user will typically have a ``master''
\filename{.gmtdefaults} file in the home directory, and possibly
more specialized \filename{.gmtdefaults} files in certain sub-
directories.  If no such file exist in the current directory,
\GMT\ will attempt to open the user'’s master defaults file.  If
it is not present then the site-specific \GMT\ defaults are used.
These come preset from the \GMT\ developers but may be modified
prior to \GMT\ installation.  One typical change at this level
is to select SI units rather than the default US/British units.
It is recommended not to modify the system \GMT\ defaults
substantially since some applications may rely on the presence of
a standard set of default values.  Users may create a new
\filename{.gmtdefaults} file with the current \GMT\ preset values
using the \GMTprog{gmtdefaults} utility.
...
\section{\gmt\ Common Command Line Options}

\GMT\ has 13 options that are identical to all programs.  It is vital that
you understand how to use these options, which we here present
alphabetically.
...
Given as \Opt{–B}{\it xinfo}[/{\it yinfo}][:."title string":][{\bf W}$|${\bf w}][{\bf E}$|${\bf e}][{\bf S}$|${\bf s}][{\bf N}$|${\bf n}],
this switch specifies a map boundary to be plotted by using the
selected tick-mark intervals. {\it xinfo} and {\it yinfo} are of the form \\

\par [{\bf a}]{\it tick}[{\bf m}$|${\bf c}][{\bf f}{\it tick}[{\bf m}$|${\bf c}]][{\bf g}{\it tick}[{\bf m}$|${\bf c}]][{\bf l}$|${\bf p}][:"axis label":][:,"unit label":] \\

where {\bf a}, {\bf f}, and {\bf g} stand for annotation, frame, and grid interval.
The {\bf m}$|${\bf c} indicates minutes ({\bf m}) or seconds ({\bf c}).  By
...
\begin{figure}[h]
\centering
\epsfig{figure=eps/GMT_-B_geo.eps}
\caption{Geographic map border using separate selections for anotation,
frame, and grid intervals.  Formatting of the anotation is controlled by
the parameter DEGREE\_FORMAT in your \filename{.gmtdefaults} file.}
\label{fig:GMT_-B_geo}
\end{figure}
\begin{figure}[h]
\centering
\epsfig{figure=eps/GMT_-B_linear.eps}
\caption{Linear Cartesian projection axis.  Long tickmarks accompany
...
\begin{figure}[h]
\centering
\epsfig{figure=eps/GMT_-B_log.eps}
\caption{Logarithmic projection axis using separate values for anotation,
...
\begin{figure}[h]
\centering
\epsfig{figure=eps/GMT_-B_pow.eps}
\caption{Exponential or power projection axis.  (top) Using an exponent of 0.5
...
\item Edit your \filename{.gmtdefaults} file and change DEGREE\_FORMAT
to a number in the 0–--4 range.
...
\Opt{–S[symbol]}[{\it size}] & Select one of 16 symbols (See Table~\ref{tbl:psxysymbols}) \\ \hline
...
symbols may be provided by the input data.  The 15 symbols available to
us are
...
\Opt{–SE} & {\bf e}llipse; {\it azimuth} (CW from vertical), {\it major}, and {\it minor} axes in kilometers \\
 & are read from the input file\\ \hline
\Opt{–Sf}{\it gap}/{\it tick}[{\bf l$|$L$|$r$|$R}] & {\bf f}ault; {\it gap} and {\it tick} set length of and distance between ticks. \\
 & If {\it gap} $<$ 0 it means the number of ticks.  {\bf l} or {\bf r} will draw ticks to left or right \\
 & side of line [Default is centered] \\
 & Use upper case {\bf L} or {\bf R} to draw triangles instead of ticks \\ \hline
...
set WANT\_EURO\_FONT to TRUE in your \filename{.gmtdefaults} file.
