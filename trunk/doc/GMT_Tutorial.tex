%------------------------------------------
%	$Id: GMT_Tutorial.tex,v 1.21 2004-01-10 02:34:54 pwessel Exp $
%
...
January 2004, culminating in the January 2004 introduction of 4.0$\beta$.
\GMT\ now is used by $\sim$6,000 users worldwide in a broad range of disciplines.
...
\end{tabular}
\caption{The symbol option in \GMTprog{psxy}.  Lower case symbols ({\bf a, c, d, h, i, n, s, t, x})
will fit inside a circle of given diameter.  Upper case symbols ({\bf A, C, D, H, I, N, S, T, X}) will have area equal to that of a circle of given diameter.}
...
\item[$\rightarrow$]{\it Width} is by default measured in units of thecurrent device resolution (i.e., the value assigned to the parameterDOTS\_PR\_INCH in your\filename{.gmtdefaults4} file).  Thus, if the dpi is set to 300 this unitis 1/300th of an inch.  Append {\bf c}, {\bf i}, {\bf p}, or {\bf m} to specify pen width in
cm, inch, points (1/72 of an inch)\footnote{\PS\ definition.  In the typesetting industry
...
\item[$\rightarrow$]The {\it color} can be specified in four different ways:
\begin{enumerate}
...
two take on the range 0--1.  To use this color system you must also set {\bf COLOR\_MODEL} to HSV.  You probably will want
to read Appendix I as well.
...
({\it r}/{\it g}/{\it b}, {\it h}/{\it s}/{\it v}, or {\it c}/{\it m}/{\it y}/{\it k}), similar to the pen color
