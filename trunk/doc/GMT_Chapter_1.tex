%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.18 2004-08-26 03:50:47 pwessel Exp $
%
...
\Opt{E} for returning extreme records, and the \Opt{I} option was extended to handle any number of columns if \Opt{C} is used.
...
(with \Opt{Sk}; use any of the 30 that come with \GMT\ or design your own), many new symbols (horizontal and vertical dashes,
...
\begin{itemize}
\item New shorthand to repeat the most recent projection (\Opt{J}).
...
\item Now choose from 50 ellipsoids and 207 datums, plus add additional custom values.
...
\item New hidden files \filename{.gmtdefaults4} and \filename{.gmtcommands4} to ensure peaceful coexistance with \GMT\ 3-series.
\item Data files in directories pointed to by environmental parameters {\bf \$GMT\_DATADIR}, {\bf \$GMT\_GRIDDIR},
and {\bf \$GMT\_IMGDIR} can be specified without their full path names.
