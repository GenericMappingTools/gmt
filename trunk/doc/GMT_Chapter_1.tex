%------------------------------------------
%	$Id: GMT_Chapter_1.tex,v 1.34 2006-05-08 00:04:18 pwessel Exp $
%
...
realignment of code and parsing of arguments to to simplify the upcoming port to \GMT\ 5;
...
that require Matlab (\verb X--disable-mexX) and X11 (\verb X--disable-xgridX).
...
\item [\GMTprogi{gmtselect.c}]: Option \Opt{F} insisted on using spherical testing for Cartesian x,y data.
\item [\GMTprogi{grd2xyz.c}]: The \Opt{E} option had the y-direction reversed.
...
\item [\GMTprogi{grdmath.c}]: {\bf INSIDE} for Cartesian data had bug (used {\it y} where {\it x} was expected).
...
\item [\GMTprogi{pslib.c}]: \GMTfunc{ps\_load\_raster} did not use open mode rb and hence failed under Windows.
\item [\GMTprogi{xyz2grd.c}]: The \Opt{E} option had the y-direction reversed.
\item [\GMTprogi{x2sys/x2sys_get.c}]: \Opt{N} did not work properly (now fixed and tested).
\end{description}
