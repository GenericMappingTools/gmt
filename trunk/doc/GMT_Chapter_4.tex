%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.117 2009-04-16 19:38:18 guru Exp $
%
...
\filename{.gmtdefaults4.doc} from the directory \filename{doc/scripts} to \filename{.gmtdefaults4}.
As a result the commands \GMTprog{gmtset} of other scripts were overall, reverting to a
``virgin'' of parameters set in \filename{.gmtdefaults4.doc}. The graphs in Chapter~\ref{ch:7} were created
using \filename{.gmtdefaults4.doc} from the directory \filename{examples} after which the graphs were
scaled down by 50\%.
...
\index{\Opt{m} (multiple segment files)}%
...
\GMT\ has several mechanisms to determine line segmentation.  Typically, data segments are
separated by multiple segment header records (see \Opt{m} below).  However, if key data
columns contain a NaN we may also use that information to break lines into multiple
segments.  This behavior is modified by the default parameter {\bf NAN\_RECORDS} which
by default is set to {\it skip}, meaning such records are considered bad and are simply
skipped.  If you wish such records to indicate a segment boundary then set this parameter
to {\pass}.  Finally, you may wish to indicate gaps based on the data values themselves.
The \Opt{g} option is used to detect gaps based on one or more criteria (use \Opt{g+} if
\emph{all} the criteria must be met; otherwise only one of the specified criteria need
to be met to signify a data gap).  Gaps can be based on excessive jumps in the {\fi x}- or
{\fi y}-coordinates (\Opt{gx} or \Opt{gy}), or on the distance between points (\Opt{gd}).
Append the {\it gap} distance and optionally a unit for actual distances.  For programs
that maps data to map coordinates you can optionally specify these criteria to apply to
...
For projected data , choose from {\bf i}nch, {\bf c}entimeter, {\bf m}eter, or {\bf p}oints.
[Default unit is controlled by {\bf MEASURE\_UNIT}].
Note: For \Opt{gx} or \Opt{gy} with time data the unit is instead controlled by {\bf TIME\_UNIT}.

\subsection{Multiple segment data: The \Opt{m} option}
\label{sec:multsegments}
\index{Table!format}
\index{Input!format \Opt{mi}}
\index{Output!format \Opt{mo}}
\index{\Opt{mi} (set input segment data)}
\index{\Opt{mo} (set output segment data)}

The \Opt{m} option states that the input and output table data will contain special records
that marks the start of new line segments.  These records are identified by their first
character, which can be specified as an argument to \Opt{m} [The default is ``>''].
...
data sets such as lines, points, and text plotable directly with \GMTprog{psxy}
...
to dd/mm/yy.  A complete description of possible formats is given in the \GMTprog{gmtdefaults}
...
\emph{key}-values = NaN) are all defined in the \filename{.gmtdefaults4}
...
\begin{enumerate}
\item To write a native binary float grid file, specify the name as \filename{my\_file.grd=bf}.
