%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.23 2003-02-19 19:10:45 pwessel Exp $
%
...
\texttt{gmtset ANNOT\_FONT Times-Bold ANOT\_FONT\_SIZE 12} \\
...
These values are simple converted as they are to internal representation.  One exception is the concept
...
\subsection{Map frame and axes anotations: The \Opt{B} option}
\index{\Opt{B}}
\index{Tickmarks}
\index{Anotations}
\index{Gridlines}
...
{\bf i}[{\it v}]	&	(upper) interval  annotation spacing\\ \hline
{\bf I}[{\it v}]	&	lower interval  annotation spacing\\ \hline
...
{\bf D}	&	date & Format anotation using {\bf PLOT\_DATE\_FORMAT} \\
{\bf d}	&	day & Plot day of month (1--31) or day of year (1--366) {see \bf PLOT\_DATE\_FORMAT} \\
...
the parameter {\bf PLOT\_DEGREE\_FORMAT} in your \filename{.gmtdefaults} file.}

\subsubsection{Cartesian linear axes}

For non-geographic axes, the {\bf BASEMAP\_TYPE} setting is implicitly set to plain.  Other than than,
cartesian linear axes are very similar to geographic axes.  The anotation format may be controlled with
...
add a unit string to each anotation (see Figure~\ref{fig:GMT_-B_linear}).
...
abbreviated names of the weekdays.   The upper uses Gregorian weeks (which start at the day chosen by {\bf TIME\_WEEK\_START}); they
do not have numbers.
...
Our seventh and final example illustrates anotation of year-days.  Unless we specify the formatting with a leading hyphen
...
\filename{.gmtdefaults} file which by default is set to {\it yyyy-mm-dd}.  Y2K-challenged input data such as 29/05/89 can be processed by setting {\bf INPUT\_DATE\_FORMAT}
...
in the 0--360, 0--1, 0--1 range), or CMYK color ({\it cyan}/{\it magenta}/{\it yellow}/{\it black},
each ranging from 0--100\%); in that respect it is similar to specifying the pen color
...
\filename{.gmtdefaults}).  Several tools such as \GMTprog{xyz2grd}, \GMTprog{gmtmath}, and
