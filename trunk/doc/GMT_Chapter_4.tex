...
Note: There are two additional \gmt\ defaults that control how great circle distances are computed. One concerns the selection
of the ``mean radius''.  This is selected by {\bf PROJ\_MEAN\_RADIUS}, which selects one of several possible
mean radii.  The second is {\bf PROJ\_AUX\_LATITUDE}, which converts geodetic latitudes into one of
...
experts users can choose from a range of options as detailed in the \filename{gmt.conf} man page.
...
projection and are given with a leading length emph{unit}, (e.g., k-200/200/-300/300 for a 400 by 600 km
...
\index{Verbose (\Opt{V})}
\label{sec:verbose}
\Opt{V} selects verbose mode, which will send progress reports to standard error.
