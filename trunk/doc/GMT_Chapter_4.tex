%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.144 2011-05-17 03:02:06 guru Exp $
%
...
[\textbf{l}][\textbf{s}{\it scale}][\textbf{o}{\it offset}] to each column (or range
of columns).  All items are optional: The \textbf{l} implies we should first take
$\log_{10}$ of the data [leave as is]. Next, we may scale the result by the given {\it scale} [1].
Finally, we add in the specified {\it offset} [0].
...
i.e., only output the records whose {\it z}-value equals NaN.  Use \Opt{sa} to suppress
