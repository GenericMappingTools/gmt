%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.47 2004-05-06 00:07:25 pwessel Exp $
%
...
format (e.g., 123:27:15W).  Note that \Opt{Rg} and \Opt{Rg} are shorthands for ``global domain'' \Opt{R}{\it 0}/{\it 360}/{\it -90}/{\it 90}
...
appear at multiple of {\it stride}; you can phase-shift this by appending $\pm${\it phase}.
\begin{table}[H]
\centering
...
``secondary'' annotation interval is larger (e.g., months).
...
If filenames are given for reading, the programs will first look in the
current directory.  If the file is not found, the programs
will look in three other directories pointed to by environmental
parameters.  These are {\bf GMT\_DATADIR}, {\bf GMT\_GRIDDIR}, and
...
versatile subset consists of the 29 choices {\it white}, {\it black}, and [light$|$dark]\{{\it red, orange, yellow, green,
...
or meters (see width discussion). 
\end{description} 
...
here the comment \# COLOR\_MODEL = HSV must be present in the cpt file).
