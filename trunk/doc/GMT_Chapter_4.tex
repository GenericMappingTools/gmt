%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.95 2007-08-29 17:28:40 guru Exp $
%
...
given epoch. The parameter \textbf{TIME\_SYSTEM}, or a combination of the parameters \textbf{TIME\_EPOCH} and
\textbf{TIME\_UNIT} define the epoch and time unit. Denote relative time coordinates by appending the optional lower case
...
Color palette tables (CPT) comes in two flavors: Ones designed to work with
categorical data (e.g., data where interpolation of values is undefined) and
regular, continuously-varying data.

\subsection{Categorical tables}
...
each category is assigned a unique integer key, a color or pattern, and an optional
label (usually the category name).  Keys must be monotonically increasing but do not
need to be consequtive.  The format is
...
the name of the grdfile.  However, if you write another format you must
...
\item To read a Golden Software ``surfer'' format 6 grd file, just pass the file name,
