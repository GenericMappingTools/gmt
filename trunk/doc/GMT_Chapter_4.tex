%------------------------------------------
%	$Id: GMT_Chapter_4.tex,v 1.69 2005-12-17 05:59:21 pwessel Exp $
%
...
Due to the logarithmic nature of annotation spacings, the {\it stride} parameter takes on specific
...
\filename{.gmtdefaults4} file; it is by default set to {\it yyyy-mm-dd}.  Y2K-challenged input data such as 29/05/89 can be processed by setting {\bf INPUT\_DATE\_FORMAT}
...
of the colorscale when plotted using \GMTprog{psscale}.  The optional code {\bf A} may be {\bf L},
...
colon followed by a text label will make \GMTprog{psscale}, when used with the \Opt{L} option,
...
By default, \GMT\ will create new gridfiles using the {\bf nf} format; however,
...
When the netCDF file contains more than one 2-D or 3-D variable, the user can select the
...
In case a variable is 3-dimensional and a layer other than the first is required, either add
``[\emph{index}]'' or ``(\emph{level})'', where \emph{index} is
...
When you supply the numerical value for the layer, \GMT\ will pick the layer closest to that value.
