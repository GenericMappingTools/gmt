%------------------------------------------
%	$Id: GMT_Chapter_8.tex,v 1.1 2001-09-21 20:57:54 pwessel Exp $
%
...
designing one’s own symbols.  The following recipe is used
...
$x_0$ $y_0$ $r$ $\Delta\alpha$ {\bf C} [ \Opt{G}{\it fill} ] [ \Opt{W}{\it pen} ] \=Draw \kill
...
$x_0$ $y_0$ $r$ $\Delta\alpha$ {\bf C} [ \Opt{G}{\it fill} ] [ \Opt{W}{\it pen} ] \> Draw
single circle of radius $r$ around $x_0$, $y_0$ in steps of $\Delta\alpha$\DS \\

$x_0$ $y_0$ $r$ $\alpha_1$ $\alpha_2$ $\Delta\alpha$ {\bf A} \> Draw
arc segment of radius $r$ from angle $\alpha_1$ to $\alpha_2$
every $\Delta\alpha$\DS
...
definition file we use the supplied \progname{\$AWK}-script \filename{make\_symbol}
to create a custom \progname{\$AWK}-script that will read locations and
...
be plotted with \GMTprog{psxy} or \GMTprog{psxyz}.
...
\progname{\$AWK}-script \filename{volcano.awk} and \GMTprog{psxy}.
Note that you must first transform the coordinates using
\GMTprog{mapproject} prior to running the \progname{\$AWK}-script.
