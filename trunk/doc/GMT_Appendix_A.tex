%-----------------------------------------
%	$Id: GMT_Appendix_A.tex,v 1.2 2001-04-04 01:23:56 pwessel Exp $
%
...
\section{cps: complete \PS\ en-/decoder}

This package contains \GMTprog{cpsencode} which reads a shell
script, compresses and encodes it and all data files referenced
therein as \PS\ comments, and appends them to the \PS\ plot that the shell
script generates.  You may reverse the process with
\GMTprog{cpsdecode} and extract the original data and shell script from
the \PS\ file. The package is mainained by Paul Wessel\footnote{wessel@soest.hawaii.edu}.
...
segments to the GRASS database format.
...
\section{mex: Matlab 5--\gmt\ interface}

Here you will find the mex files \progname{grdinfo}, \progname{grdread},
and \progname{grdwrite}, which can be used in Matlab 5 to read and write
...
It is now mainained by Paul Wessel.

\section{mgg: MGD77 extractor and plotting tools}
...
data files (MGD-77†\footnote{These data are available on CD-ROM from NGDC
...
and a set of absolute plate motion stage poles, and \GMTprog{originator},
which associates seamounts with the most likely hotspot origins.  The package
