...
LL=`mapproject -hi tmp.txt -Ju5Q/1:1 -F -C -I --FORMAT_GEO_OUT=ddd:mm:ss.x -i1,3 -o0,1 | \
	$AWK '{printf "%s/%s\n", $1, $2}'`
UR=`mapproject -hi tmp.txt -Ju5Q/1:1 -F -C -I --FORMAT_GEO_OUT=ddd:mm:ss.x -i2,4 -o0,1 | \
	$AWK '{printf "%s/%s\n", $1, $2}'`
...
Rkm=`grdinfo Kilauea.utm.nc -C | cut -f2,3,4,5 | gmtconvert -hi -i0-3s0.001 --IO_COL_SEPARATOR=/`
