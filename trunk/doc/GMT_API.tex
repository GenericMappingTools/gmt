%------------------------------------------
%       $Id: GMT_API.tex,v 1.11 2011-03-15 02:06:28 guru Exp $
%
...
the command line or via special program options (e.g., \Opt{I}{\it intensity.nc}). The output of
...
\texttt{ptr} is the address of an opaque pointer to the named resource.
...
holds a single command line argument (e.g., ``\Opt{R}{\it 120:30/134:45/8S/3N}'') as well as the count \texttt{argc}
...
\end{description}
\item [Text table]: The \texttt{mode} is used the same was as for data tables.
...
function GMT_Create_Data (family, geometry, ipar, data)
