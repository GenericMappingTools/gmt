%------------------------------------------
%	$Id: GMT_Appendix_O.tex,v 1.7 2004-06-22 20:00:28 pwessel Exp $
%
...
are taken from the \GMTprog{pstext} justification keys [{\bf L$|$C$|$R}][{\bf B$|$M$|$T}] so that the first
...
\item [+$\hat{ }$:]  Similar in most regards to {\bf +u} but applies instead to a label \emph{prefix} which
you must append.
\end{description}
For contours, the label will be the value of the contour (possibly modified by {\bf +u} or {\bf +$\hat{ }$}).
...
from the files header record).  We first scan the header for an embedded
...
multi-segment lines specified in the \Opt{GX} option.}
...
\GMTfig[H]{GMT_App_O_6}{Labels attributes are controlled with the arguments to the \Opt{Sq} option.}
