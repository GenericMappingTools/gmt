%------------------------------------------
%	$Id: GMT_Appendix_I.tex,v 1.16 2008-03-09 21:01:00 remko Exp $
%
...
\begin{figure}[b]
   ...
   \caption{Chartreuse in \progname{GIMP}. (\emph{a}) Sliders indicate how the color is altered when changing the H, S, V, R, G, or B levels. (\emph{b}) For a constant hue (here 90\DS) value increases to the right and saturation increases up, so the ``pure'' color is on the top right.}
   \label{fig:gimp}
\end{figure}

Is chocolate your favorite color, but you do not know the RGB equivalent values? Then look them up in the \filename{rgb.txt} file in your X-windows system directory or in Figure~\ref{fig:RGBchart}. It's 210/105/30. But \GMT\ makes it easy on you: you can specify pen, fill, and palette colors by any of the more than 550 unique names found in that file.

\begin{figure}
   \includegraphics[width=\textwidth]{scripts/GMT_RGBchart_a4.pdf}
...
defined in the \filename{.gmtdefaults4} file and are usually
