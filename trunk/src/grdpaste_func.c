/*--------------------------------------------------------------------
 *	$Id: grdpaste_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_grdpaste_parse (struct GMTAPI_CTRL *C, struct GRDPASTE_CTRL *Ctrl, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case '<':	/* Input files */
				if (n_in == 0)
					...
					n_errors++;
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR:  Only two files may be pasted\n");
				}
				break;
	...
	n_errors += GMT_check_condition (GMT, !Ctrl->In.file[0] || !Ctrl->In.file[1], "GMT SYNTAX ERROR:  Must specify two input files\n");
