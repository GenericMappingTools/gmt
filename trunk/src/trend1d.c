...
void transform_x_1d (struct TREND1D_DATA *data, uint64_t n_data, unsigned int model_type, double xmin, double xmax)
{
	uint64_t i;
	double offset, scale;
...
void untransform_x_1d (struct TREND1D_DATA *data, uint64_t n_data, unsigned int model_type, double xmin, double xmax)
{
	uint64_t i;
	double offset, scale;
...
void load_g_row_1d (double x, int n, double *gr, unsigned int type)
{
...
void calc_m_and_r_1d (struct TREND1D_DATA *data, uint64_t n_data, double *model, unsigned int n_model, unsigned int m_type, double *grow)
{
...
void load_gtg_and_gtd_1d (struct TREND1D_DATA *data, uint64_t n_data, double *gtg, double *gtd, double *grow, unsigned int n_model, unsigned int mp, unsigned int m_type)
{
...
int GMT_trend1d_usage (struct GMTAPI_CTRL *C, int level)
{
...
int GMT_trend1d_parse (struct GMTAPI_CTRL *C, struct TREND1D_CTRL *Ctrl, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case 'F':
				...
				for (j = 0; opt->arg[j]; j++) {
					if (j < TREND1D_N_OUTPUT_CHOICES)
						...
						GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error -F option: Too many output columns selected: Choose from -Fxymrw|p\n");
						n_errors++;
					}
				}
				break;
			...
			case 'N':
				...
				if (opt->arg[j])
					...
					GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error -N option: No model specified\n");
					n_errors++;
				}
				break;
			case 'W':
...
int GMT_trend1d (struct GMTAPI_CTRL *API, int mode, void *args)
{
	...
	if (xmin == xmax) {
		GMT_report (GMT, GMT_MSG_NORMAL, "Error: Min and Max value of input data are the same.\n");
		Return (EXIT_FAILURE);
	}
	if (n_data == 0) {
	...
	if (n_data < (uint64_t)Ctrl->N.value) GMT_report (GMT, GMT_MSG_NORMAL, "Warning: Ill-posed problem; n_data < n_model_max.\n");
	...
	if (GMT_is_verbose (GMT, GMT_MSG_VERBOSE)) {
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, format, n_data, xmin, xmax);
	...
	if (Ctrl->I.active) {
		...
		while (n_model < Ctrl->N.value && significant) {
			...
			calc_m_and_r_1d (data, n_data, c_model, n_model, Ctrl->N.mode, workb);
			c_chisq = get_chisq_1d (data, n_data, n_model);
			GMT_report (GMT, GMT_MSG_VERBOSE, format, n_model, rank, c_chisq, 1.00);
			if (Ctrl->N.robust) {
				do {
					recompute_weights_1d (GMT, data, n_data, work, &scale);
					move_model_a_to_b_1d (c_model, w_model, n_model, &c_chisq, &w_chisq);
	...
	if (GMT_is_verbose (GMT, GMT_MSG_VERBOSE)) {
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Model Coefficients  (Chebyshev):");
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Model Coefficients (Polynomial):");
		for (i = 0; i < n_model; i++) GMT_message (GMT, format, c_model[i]);
		GMT_message (GMT, "\n");
	}
