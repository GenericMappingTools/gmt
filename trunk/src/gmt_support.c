/*--------------------------------------------------------------------
 *	$Id: gmt_support.c,v 1.235 2006-03-25 03:56:52 pwessel Exp $
 *
...
L380:
	theta = -twopi * (float) non2 / (float) (mmax <<  
2);
	if (*ksign >= 0) {
	    goto L390;
	} else {
...
/* C-callable wrapper for fourtd_ DOUBLE PRECISION version */

void GMT_fourtd (double *data, int *nn, int ndim, int ksign, int iform, double *work)
            	/* Data array */
         	/* Dimension array */
         	/* Number of dimensions */
          	/* Forward(-1) or Inverse(+1) */
          	/* Real(0) or complex(1) data */
            	/* Work array */
{
	int fourtd_ (double *data, int *nn, int *ndim, int *ksign, int *iform, double *work);
	(void) fourtd_ (data, nn, &ndim, &ksign, &iform, work);
}
...
int fourtd_ (double *data, int *nn, int *ndim, int *ksign, int *iform, double *work)
{
    ...
    static double theta, oldsi, tempi, oldsr, sinth, difi, difr, sumi, sumr, tempr, twopi;
    static double wstpi, wstpr, twowr, wi, wr, u1i, w2i, w3i, u2i, u3i, u4i, t2i, u1r;
    static double u2r, u3r, w2r, w3r, u4r, t2r, t3r, t3i, t4r, t4i;
    static double wrd, wid;
...
L380:
	theta = -twopi * (double) non2 / (double) (mmax << 2);
	if (*ksign >= 0) {
	    goto L390;
	} else {
	    goto L400;
...
L615:
	...
	for (j2 = j2min; i__4 < 0 ? j2 >= i__5 : j2 <= i__5; j2 += i__4) {
	    theta = -twopi * (double) (j2 - 1) / (double)  np2;
	    if (*ksign >= 0) {
		goto L620;
	    } else {
		goto L625;
...
L625:
		...
		for (i1 = j1; i1 <= i__6; i1 += 2) {
		    i__8 = ntot;
		    i__7 = np2;
		    for (i3 = i1; i__7 < 0 ? i3 >= i__8 : i3 <= i__8; i3 += 

			    i__7) {
			j3max = i3 + ifp2 - np1;
			i__9 = j3max;
			i__10 = np1;
			for (j3 = i3; i__10 < 0 ? j3 >= i__9 : j3 <= i__9; j3 
...
L640:
	theta = -twopi / (double) ifact[if_ - 1];
	if (*ksign >= 0) {
	    goto L645;
	} else {
	    goto L650;
...
L650:
	...
	for (i1 = 1; i1 <= i__2; i1 += 2) {
		...
		i__5 = krang;
		i__10 = kstep;
		for (kmin = 1; i__10 < 0 ? kmin >= i__5 : kmin <= i__5; kmin 
...
L710:
	...
	i__3 = np2;
	for (i = imin; i__3 < 0 ? i >= i__4 : i <= i__4; i += i__3) {
	    sumr = 0.5 * (data[i] + data[j]);
	    sumi = 0.5 * (data[i + 1] + data[j + 1]);
	    difr = 0.5 * (data[i] - data[j]);
	    difi = 0.5 * (data[i + 1] - data[j + 1]);
	    tempr = wr * sumi + wi * difr;
	    tempi = wi * sumi - wr * difr;
	    data[i] = sumr + tempr;
	    data[i + 1] = difi + tempi;
