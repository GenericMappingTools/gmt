...
void GMT_fourt (float *data, int *nn, int ndim, int ksign, int iform, float *work)
         	...
         	/* Number of dimensions */
          	...
          	/* Real(0) or complex(1) data */
            	/* Work array */
{
	int fourt_ (real *data, integer *nn, integer *ndim, integer *ksign, integer *iform, real *work);
...
int fourt_ (real *data, integer *nn, integer *ndim, integer *ksign, integer *iform, real *work)
{
    ...
    integer i__1, i__2, i__3, i__4, i__5, i__6, i__7, i__8, i__9, i__10, i__11, i__12;
    ...
    static integer j1rg2, idiv, irem, ipar, kmin, imin, jmin, lmax, mmax, imax, jmax;
    static integer ntwo, j1cnj, np1hf, np2hf, j1min, i1max, i1rng, j1rng, j2min, j3max;
    static integer j1max, j2max, i2max, non2t, j2stp, i, j, k, l, m, n, icase, ifact[1];
    static integer nhalf, krang, kconj, kdif, idim, ntot, kstep, k2, k3, k4, nprev, iquot;
    static integer i2, i1, i3, j3, k1, j2, j1, if_, np0, np1, np2, ifp1, ifp2, non2;
    
    static real theta, oldsi, tempi, oldsr, sinth, difi, difr, sumi, sumr, tempr, twopi;
    static real wstpi, wstpr, twowr, wi, wr, u1i, w2i, w3i, u2i, u3i, u4i, t2i, u1r;
    static real u2r, u3r, w2r, w3r, u4r, t2r, t3r, t3i, t4r, t4i;
    static doublereal wrd, wid;
...
/*---------------------------------------------------------------------------
       ARGUMENTS :  
		...
		KSIGN - +1 FOR INVERSE TRANSFORM (FREQ TO TIME DOMAIN)     
		        -1 FOR FORWARD TRANSFORM (TIME TO FREQ DOMAIN)     
		IFORM - 0 REAL DATA                                        
			+1 COMPLEX DATA                                    
...
---------------------------------------------------------------------------*/
   ...
   /* Parameter adjustments */
    ...
    /* Function Body */
    wr = wi = wstpr = wstpi = (real)0.0;
    twopi = (real)6.283185307;
    if (*ndim - 1 >= 0) {
	goto L1;
    } else {
	goto L920;
...
L330:
	...
	for (i1 = 1; i1 <= i__3; i1 += 2) {
	    i__2 = non2;
	    i__6 = np1;
	    for (j3 = i1; i__6 < 0 ? j3 >= i__2 : j3 <= i__2; j3 +=  
i__6) {
		i__5 = ntot;
		i__4 = non2t;
		for (k1 = j3; i__4 < 0 ? k1 >= i__5 : k1 <= i__5; k1  
+= i__4) 
...
L380:
	theta = -twopi * (real) non2 / (real) (mmax <<  
2);
	if (*ksign >= 0) {
	    goto L390;
	} else {
...
L390:
	theta = -theta;
L400:
	sincos ((doublereal)theta, &wid, &wrd);
	wr = (real)wrd;
	wi = (real)wid;
	wstpr = (real)-2.0 * wi * wi;
	wstpi = (real)2.0 * wr * wi;
L405:
	i__4 = lmax;
	i__5 = non2t;
...
L420:
		...
		i__2 = non2;
		i__3 = np1;
		for (j3 = i1; i__3 < 0 ? j3 >= i__2 : j3 <= i__2; j3  
+= i__3) 
...
L615:
	i__5 = ifp1;
	i__4 = ifp2;
	for (j2 = j2min; i__4 < 0 ? j2 >= i__5 : j2 <= i__5; j2 +=  
i__4) {
	    theta = -twopi * (real) (j2 - 1) / (real)  
np2;
	    if (*ksign >= 0) {
		goto L620;
	    } else {
...
L620:
	    theta = -theta;
L625:
	    sinth = (real)sin((double)(0.5 * theta));
	    wstpr = sinth * (real)(-2. * sinth);
	    wstpi = (real)sin((double)theta);
	    wr = wstpr + (real)1.0;
	    wi = wstpi;
	    j1min = j2 + ifp1;
	    i__3 = j1rng;
	    i__2 = ifp1;
	    for (j1 = j1min; i__2 < 0 ? j1 >= i__3 : j1 <= i__3; j1  
+= i__2) {
		...
		for (i1 = j1; i1 <= i__6; i1 += 2) {
		    i__8 = ntot;
		    i__7 = np2;
		    for (i3 = i1; i__7 < 0 ? i3 >= i__8 : i3 <= i__8;  
i3 += 
			...
			i__9 = j3max;
			i__10 = np1;
			for (j3 = i3; i__10 < 0 ? j3 >= i__9 : j3 <=  
i__9; j3 

				+= i__10) {
			    tempr = data[j3];
			    data[j3] = data[j3] * wr - data[j3 + 1] *  
wi;
/* L630: */
			    data[j3 + 1] = tempr * wi + data[j3 + 1]  
* wr;
			}
		    }
		}
...
L650:
	...
	for (i1 = 1; i1 <= i__2; i1 += 2) {
	    i__3 = ntot;
	    i__4 = np2;
	    for (i3 = i1; i__4 < 0 ? i3 >= i__3 : i3 <= i__3; i3 +=  
i__4) {
		i__5 = krang;
		i__10 = kstep;
		for (kmin = 1; i__10 < 0 ? kmin >= i__5 : kmin <=  
i__5; kmin 
		    ...
		    i__9 = j1max;
		    i__7 = ifp1;
		    for (j1 = i3; i__7 < 0 ? j1 >= i__9 : j1 <= i__9;  
j1 += 
...
L692:
		...
		i__10 = i2max;
		i__5 = np1;
		for (i2 = i3; i__5 < 0 ? i2 >= i__10 : i2 <= i__10;  
i2 += 
...
L695:
		...
		i__5 = j3max;
		i__10 = np1;
		for (j3 = i3; i__10 < 0 ? j3 >= i__5 : j3 <= i__5; j3  
+= 
		    ...
		    i__6 = j2max;
		    i__8 = j2stp;
		    for (j2 = j3; i__8 < 0 ? j2 >= i__6 : j2 <= i__6;  
j2 += 
...
L710:
	...
	for (i = imin; i__3 < 0 ? i >= i__4 : i <= i__4; i += i__3) {
	    sumr = (real)0.5 * (data[i] + data[j]);
	    sumi = (real)0.5 * (data[i + 1] + data[j + 1]);
	    difr = (real)0.5 * (data[i] - data[j]);
	    difi = (real)0.5 * (data[i + 1] - data[j + 1]);
	...
	for (i3 = 1; i__3 < 0 ? i3 >= i__4 : i3 <= i__4; i3 += i__3)  
{
	    ...
	    i__2 = i2max;
	    i__9 = np1;
	    for (i2 = i3; i__9 < 0 ? i2 >= i__2 : i2 <= i__2; i2 +=  
i__9) {
		imin = i2 + i1rng;
		imax = i2 + np1 - 2;
		jmax = (i3 << 1) + np1 - imin;
...
L850:
		...
		i__7 = imax;
		i__8 = np0;
		for (i = imin; i__8 < 0 ? i >= i__7 : i <= i__7; i +=  
i__8) {
		    data[i] = data[j];
		    data[i + 1] = -data[j + 1];
/* L860: */
