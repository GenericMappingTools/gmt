...
GMT_LONG GMT_psmeca_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: psmeca <infiles> %s %s\n", GMT_J_OPT, GMT_Rgeo_OPT);
	...
	GMT_message (GMT, "\t<infiles> is one or more files. If no files are given, read standard input\n");
...
#define Return(code) {Free_psmeca_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_psmeca (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{	/* High-level function that implements the psmeca task */
	...
	char string[BUFSIZ], event_title[BUFSIZ], *line, col[15][GMT_TEXT_LEN];
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_psmeca_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_psmeca_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
