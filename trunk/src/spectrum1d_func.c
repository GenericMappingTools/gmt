/*--------------------------------------------------------------------
 *	$Id: spectrum1d_func.c,v 1.15 2011-06-08 19:21:49 guru Exp $
 *
...
#define Return(code) {Free_spectrum1d_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_spectrum1d (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_spectrum1d_usage (API, GMTAPI_USAGE));/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_spectrum1d_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
