...
/* gmt_core_module.c populates the external array of GMT core module parameters such as name
 ...
 *   void gmt_core_module_show_all(struct GMTAPI_CTRL *API); - Pretty print all core module names and their
 *           purposes
 *
...
/* sorted array with program parameters for all GMT core modules */
...
/* sorted array with shorter aliases for core modules starting with "gmt" */

struct Gmt_alias gmt_core_alias[] =
{	/* Alias:	Full name */
	{"2kml", 	"gmt2kml"},
	{"convert",	"gmtconvert"},
...
/* sorted array with replacement names for some core modules */
struct Gmt_alias gmt_core_oldname[] =
{	/* Old:		New: */
	{"gmtdp",	"gmtsimplify"},
	{NULL,		NULL}
};

/* Look out for core modules given by their aliases */
const char * gmt_core_formal_name (struct GMTAPI_CTRL *API, const char *module) {
	unsigned int k = 0;
	while (gmt_core_alias[k].alias != NULL) {
		if (!strcmp (module, gmt_core_alias[k].alias)) return gmt_core_alias[k].name;
		k++;
	}
	if (GMT_compat_check (API->GMT, 4)) {
		k = 0;
		while (gmt_core_oldname[k].alias != NULL) {
			if (!strcmp (module, gmt_core_oldname[k].alias)) return gmt_core_oldname[k].name;
			k++;
		}
	}
	return module;
...
/* Pretty print all core module names and their purposes */
