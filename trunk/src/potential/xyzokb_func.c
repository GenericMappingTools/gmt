/*--------------------------------------------------------------------
 *	$Id: xyzokb_func.c,v 1.2 2011-03-15 02:06:37 guru Exp $
 *
...
GMT_LONG GMT_xyzokb_usage (struct GMTAPI_CTRL *C, GMT_LONG level) {
	...
	GMT_message (GMT, "usage: xyzokb [-C<density>] [-G<outfile>] [-R<w/e/s/n>]\n");
	GMT_message (GMT, "\t[-Z<level>] [-T] [-L<z_observation>] [-S<radius>] [-D]\n");
	GMT_message (GMT, "\t[-F<polygon>] [z<dz>] [-Hf_dec/f_dip/m_int/m_dec/m_dip]\n");
...
/* ---------------------------------------------------------------------------- */
	...
	if (Ctrl->G.active) {
		...
		GMT_err_fail (GMT, GMT_init_newgrid (GMT, Gout, GMT->common.R.wesn, Ctrl->I.xinc, Ctrl->I.yinc, FALSE), Ctrl->G.file);
...
GMT_LONG read_xyz (struct GMT_CTRL *GMT, struct XYZOKB_CTRL *Ctrl, FILE *fp_xyz, double *central_long, double *central_lat) {
	...
	*central_long = 0.;	*central_lat = 0.;
	...
	if (Ctrl->M.active) {	/* take a first read just to compute the central logitude */
		while (fgets (line, GMT_LONG_TEXT, fp_xyz)) { 
...
GMT_LONG read_t (struct GMT_CTRL *GMT, FILE *fp_t, char *t_file) {
	...
	while (fgets (line, GMT_LONG_TEXT, fp_t)) { 
...
GMT_LONG read_raw (struct GMT_CTRL *GMT, FILE *fp_r, char *raw_file, double z_dir) {
	...
	while (fgets (line, GMT_LONG_TEXT, fp_r)) { 
...
GMT_LONG read_stl (struct GMT_CTRL *GMT, FILE *fp_s, char *stl_file, double z_dir) {
	...
	double in[3];
	char line[GMT_LONG_TEXT], text[128], ver_txt[128];

	n_alloc = GMT_CHUNK;
	ndata_s = 0;
	raw_mesh = GMT_memory (GMT, NULL, (size_t) n_alloc, struct RAW);
	
	while (fgets (line, GMT_LONG_TEXT, fp_s)) { 
		sscanf (line, "%s", text);
		if (strcmp (text, "outer") == 0) {
			fgets (line, GMT_LONG_TEXT, fp_s); /* get first vertex */
			...
			fgets (line, GMT_LONG_TEXT, fp_s); /* get second vertex */
			if(sscanf (line, "%s %lg %lg %lg", ver_txt, &in[0], &in[1], &in[2]) !=4)
				GMT_message (GMT, "ERROR deciphering triangle %ld of %s\n", ndata_s+1, stl_file);
			raw_mesh[ndata_s].t2[0] = in[0];
			raw_mesh[ndata_s].t2[1] = -in[1];
			raw_mesh[ndata_s].t2[2] = in[2] * z_dir;
			fgets (line, GMT_LONG_TEXT, fp_s); /* get third vertex */
			if(sscanf (line, "%s %lg %lg %lg", ver_txt, &in[0], &in[1], &in[2]) !=4)
				GMT_message (GMT, "ERROR deciphering triangle %ld of %s\n", ndata_s+1, stl_file);
...
GMT_LONG read_poly (struct GMT_CTRL *GMT, FILE *fp, GMT_LONG switch_xy) {
	...
	while (fgets (line, GMT_LONG_TEXT, fp)) { 
