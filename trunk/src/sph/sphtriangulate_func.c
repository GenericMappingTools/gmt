...
GMT_LONG GMT_sphtriangulate_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "\t   If -bo is used then -N may be used to specify a separate file where the polygon\n");
	GMT_message (GMT, "\t   information normally is written.\n");
...
GMT_LONG GMT_sphtriangulate_parse (struct GMTAPI_CTRL *C, struct SPHTRIANGULATE_CTRL *Ctrl, struct GMT_OPTION *options)
{
		...
		switch (opt->option) {
			...
			case 'L':
				...
				if (!(opt->arg && strchr ("defkMn", opt->arg[0]))) {
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR.  Expected -Ld|e|f|k|M|n]\n");
					n_errors++;
				}
				else
	...
	n_errors += GMT_check_condition (GMT, GMT->common.b.active[GMT_OUT] && Ctrl->A.active && !Ctrl->N.active, "GMT SYNTAX ERROR.  Binary output does not support storing areas unless -N is used\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && Ctrl->T.active, "GMT SYNTAX ERROR -N:  Cannot be used with -T.\n");
	n_errors += GMT_check_condition (GMT, Ctrl->N.active && !Ctrl->N.file, "GMT SYNTAX ERROR -N:  Must specify output file\n");
