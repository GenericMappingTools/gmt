...
double get_intensity (struct GMT_CTRL *GMT, struct GMT_GRID *I, uint64_t k)
{
...
int quick_idist (struct GMT_CTRL *GMT, int x1, int y1, int x2, int y2)
...
double get_z_ave (struct GMT_CTRL *GMT, double v[], double next_up, uint64_t n) {
	uint64_t k;
	double z_ave;
...
int GMT_grdview_usage (struct GMTAPI_CTRL *C, int level)
{
		...
		switch (opt->option) {
			...
			case 'G':	/* One or Three grid files */
				...
				if (n_commas == 2) {	/* Three r,g,b grids for draping */
					...
					GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error option -G: Usage is -G<z.grd> | -G<r.grd>,<g.grd>,<b.grd>\n");
					n_errors++;
				}
				break;
			...
			case 'N':	/* Facade */
				if (opt->arg[0]) {
					...
					GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error option -N: Usage is -N<level>[/<color>]\n");
					n_errors++;
				}
				break;
			case 'Q':	/* Plot mode */
				...
				switch (opt->arg[0]) {
					...
					case 'c':	/* Image with colormask */
						...
						break;
					default:
						GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error option -Q: Unrecognized qualifier (%c)\n", opt->arg[0]);
						n_errors++;
						break;
				}
...
int GMT_grdview (void *V_API, int mode, void *args)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) bailout (GMT_grdview_usage (API, GMTAPI_USAGE));/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) bailout (GMT_grdview_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	xval = GMT_Get_Coord (API, GMT_IS_GRID, GMT_X, Topo);
	yval = GMT_Get_Coord (API, GMT_IS_GRID, GMT_Y, Topo);
	...
	if (get_contours) {	/* Need to find contours */
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Find contours\n");
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Trace and bin contours...\n");
		for (c = 0; c < P->n_colors+1; c++) {	/* For each color change */
			...
			GMT_report (GMT, GMT_MSG_VERBOSE, "Now tracing contour interval %8g\r", cval);
	...
	if (Ctrl->T.active) {	/* Plot image as polygonal pieces. Here, -JZ is not set */
		...
		GMT_grd_loop (GMT, Z, row, col, ij) {	/* Compute rgb for each pixel */
			...
			if (Ctrl->I.active && Ctrl->T.skip && GMT_is_fnan (Intens->data[ij])) continue;
			GMT_get_rgb_from_z (GMT, P, Topo->data[ij], fill.rgb);
			if (Ctrl->I.active) GMT_illuminate (GMT, Intens->data[ij], fill.rgb);
		...
		int done, layers, last_i, last_j, p;
		...
		if (Ctrl->C.active && P->has_pattern) GMT_report (GMT, GMT_MSG_VERBOSE, "Warning: Patterns in cpt file will not work with -Qi\n");
		GMT_report (GMT, GMT_MSG_VERBOSE, "Get and store projected vertices\n");
		...
		if (drape_resample) {
			...
			x_drape = GMT_Get_Coord (API, GMT_IS_GRID, GMT_X, Z);
			y_drape = GMT_Get_Coord (API, GMT_IS_GRID, GMT_Y, Z);
			...
			GMT_grd_loop (GMT, Z, row, col, ij) {	/* Get projected coordinates converted to pixel locations */
				...
				if (GMT_is_dnan (value))	/* Outside -R or NaNs not used */
					...
					ix[bin] = MAX(0, MIN(lrint (floor((xp - GMT->current.proj.z_project.xmin) * Ctrl->Q.dpi)), last_i));
					iy[bin] = MAX(0, MIN(lrint (floor((yp - GMT->current.proj.z_project.ymin) * Ctrl->Q.dpi)), last_j));
				}
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Start rasterization\n");
		for (j = j_start; j != j_stop; j += j_inc) {

			GMT_report (GMT, GMT_MSG_VERBOSE, "Scan line conversion at j-line %.6ld\r", j);
		...
		GMT_report (GMT, GMT_MSG_VERBOSE, "Creating PostScript image ");
		if (Ctrl->Q.monochrome) {
			if (GMT_is_verbose (GMT, GMT_MSG_VERBOSE)) GMT_message (GMT, "[B/W image]\n");
			...
			if (GMT_is_verbose (GMT, GMT_MSG_VERBOSE)) GMT_message (GMT, "[color image]\n");
