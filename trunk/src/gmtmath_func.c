...
/*OPERATOR: LMSSCL 1 1 LMS scale estimate (LMS STD) of A.  */
{
	...
	for (s = k = 0; s < info->T->n_segments; s++)  {
		if (info->local) {
			GMT_sort_array ((void *)T->segment[s]->coord[col], info->T->segment[s]->n_rows, GMT_DOUBLE_TYPE);
	...
	GMT_sort_array ((void *)z, info->T->n_records, GMT_DOUBLE_TYPE);
...
/*OPERATOR: PQUANT 2 1 The B'th Quantile (0-100%) of A.  */
{
	...
	for (s = k = 0; s < info->T->n_segments; s++)  {
		if (info->local) {
			GMT_sort_array ((void *)T_prev->segment[s]->coord[col], info->T->segment[s]->n_rows, GMT_DOUBLE_TYPE);
...
#define Return1(code) {GMT_Destroy_Options (API, &list); Free_gmtmath_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code); }
...
GMT_LONG GMT_gmtmath (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
...
#include "gmtmath_op.h"
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_gmtmath_usage (API, GMTAPI_USAGE));/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_gmtmath_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
