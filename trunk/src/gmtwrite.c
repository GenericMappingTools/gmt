 ...
 * Version:	5 API
 *
 * Brief synopsis: gmtwrite lets us (read from memory and) write any of the 5 GMT resources.
 *
 */
...
void *New_gmtwrite_Ctrl (struct GMT_CTRL *GMT) {	/* Allocate and initialize a new control structure */
...
void Free_gmtwrite_Ctrl (struct GMT_CTRL *GMT, struct GMTWRITE_CTRL *C) {	/* Deallocate control structure */
	if (C->IO.file[GMT_IN]) free ((void *)C->IO.file[GMT_IN]);
	if (C->IO.file[GMT_OUT]) free ((void *)C->IO.file[GMT_OUT]);
	GMT_free (GMT, C);
}

int GMT_gmtwrite_usage (struct GMTAPI_CTRL *API, int level) {
	GMT_show_name_and_purpose (API, NULL, THIS_MODULE_NAME, THIS_MODULE_PURPOSE);
	if (level == GMT_MODULE_PURPOSE) return (GMT_NOERROR);
	GMT_Message (API, GMT_TIME_NONE, "usage: gmtwrite <infile> <outfile> -Td|t|g|c|i [%s] [%s]\n", GMT_Rx_OPT, GMT_V_OPT);
...
int GMT_gmtwrite_parse (struct GMT_CTRL *GMT, struct GMTWRITE_CTRL *Ctrl, struct GMT_OPTION *options) {
...
int GMT_gmtwrite (void *V_API, int mode, void *args)
{
	...
	if (mode == GMT_MODULE_PURPOSE) return (GMT_gmtwrite_usage (API, GMT_MODULE_PURPOSE));	/* Return the purpose of program */
	options = GMT_Create_Options (API, mode, args);	if (API->error) return (API->error);	/* Set or get option list */

	if (!options || options->option == GMT_OPT_USAGE) bailout (GMT_gmtwrite_usage (API, GMT_USAGE));	/* Return the usage message */
	if (options->option == GMT_OPT_SYNOPSIS) bailout (GMT_gmtwrite_usage (API, GMT_SYNOPSIS));	/* Return the synopsis */
