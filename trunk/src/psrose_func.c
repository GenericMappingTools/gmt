/*--------------------------------------------------------------------
 *	$Id: psrose_func.c,v 1.2 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_psrose_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "usage: psrose <infile> [-A<sector_angle>[r]] [%s]\n", GMT_B_OPT);
...
GMT_LONG GMT_psrose_parse (struct GMTAPI_CTRL *C, struct PSROSE_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	for (opt = options; opt; opt = opt->next) {	/* Process all the options given */
		...
		switch (opt->option) {
			...
			case 'L':	/* Overwride default labeling */
				Ctrl->L.active = TRUE;
				n_errors += GMT_check_condition (GMT, sscanf (opt->arg, "%[^/]/%[^/]/%[^/]/%s", txt_a, txt_b, txt_c, txt_d) != 4, "GMT SYNTAX ERROR -L option.  Correct syntax:\n\t-L<westlabel/eastlabel/southlabel/<northlabel>>\n");
			...
			case 'M':	/* Get arrow parameters */
				...
				if (n != 4 || GMT_getrgb (GMT, txt_d, Ctrl->M.rgb)) {
					GMT_report (GMT, GMT_MSG_FATAL, "GMT SYNTAX ERROR -M option.  Correct syntax:\n\t-M<tailwidth/headlength/headwidth/<color>>\n");
					n_errors++;
				}
				else {
	...
	n_errors += GMT_check_condition (GMT, Ctrl->C.active && Ctrl->C.file && GMT_access (GMT, Ctrl->C.file, R_OK), "GMT SYNTAX ERROR -C:  Cannot read file %s!\n", Ctrl->C.file);
	n_errors += GMT_check_condition (GMT, Ctrl->S.scale <= 0.0, "GMT SYNTAX ERROR -S option:  radius must be nonzero\n");
	n_errors += GMT_check_condition (GMT, GMT_IS_ZERO (Ctrl->Z.scale), "GMT SYNTAX ERROR -Z option:  factor must be nonzero\n");
	n_errors += GMT_check_condition (GMT, Ctrl->A.inc < 0.0, "GMT SYNTAX ERROR -A option:  sector width must be positive\n");
	n_errors += GMT_check_condition (GMT, !GMT->common.R.active, "GMT SYNTAX ERROR:  Must specify -R option\n");
	n_errors += GMT_check_condition (GMT, !((GMT->common.R.wesn[YLO] == -90.0 && GMT->common.R.wesn[YHI] == 90.0) || (GMT->common.R.wesn[YLO] == 0.0 && GMT->common.R.wesn[YHI] == 360.0)), "GMT SYNTAX ERROR -R option:  theta0/theta1 must be either -90/90 or 0/360\n");
	n_errors += GMT_check_binary_io (GMT, 2);

	return (n_errors ? GMT_PARSE_ERROR : GMT_OK);
}

#define Return(code) {Free_psrose_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_psrose (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_psrose_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_psrose_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	if (Ctrl->C.active) {
		...
		for (i = 0; i < n_modes; i++) {
			...
			GMT_setfill (GMT, PSL, &f, TRUE);       /* Use fill structure */
	...
	if (GMT->current.map.frame.plot) {	/* Draw grid lines etc */
		...
		form = GMT_setfont (GMT, PSL, &GMT->current.setting.font_title);
