...
int GMT_gmtstitch (void *V_API, int mode, void *args)
{
	...
	if ((D[GMT_OUT] = GMT_Create_Data (GMT->parent, GMT_IS_DATASET, D[GMT_IN]->geometry, 0, dim_tscr, NULL, NULL, 0, 0, Ctrl->Out.file)) == NULL) {
	...
	if (Ctrl->C.active) {	/* Wish to return already-closed polygons via a separate file */
		...
		if ((C = GMT_Create_Data (GMT->parent, GMT_IS_DATASET, GMT_IS_POLY, 0, dim_tscr, NULL, NULL, 0, 0, Ctrl->C.file)) == NULL) {
	...
	if (Ctrl->L.active) {	/* Write out the link information */
		...
		if ((LNK = GMT_Create_Data (GMT->parent, GMT_IS_TEXTSET, GMT_IS_NONE, 0, dim_tscr, NULL, NULL, 0, 0, Ctrl->L.file)) == NULL) {
