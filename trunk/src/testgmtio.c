...
int main (int argc, char *argv[]) {
	...
	if ((API = GMT_Create_Session ("TEST", 0U)) == NULL) exit (EXIT_FAILURE);
	GMT = API->GMT;
	...
	do {	/* Keep returning records until we reach EOF */
		...
		if ((in = GMT_Get_Record (API, GMT_READ_DOUBLE | GMT_FILE_BREAK, &n_fields)) == NULL) {	/* Read next record, get NULL if special case */
			if (GMT_REC_IS_ERROR (GMT)) {	/* This check kicks in if the data has bad formatting, text etc */
				GMT_report (GMT, GMT_MSG_VERBOSE, "Error found in record %" PRIu64 "\n", GMT->current.io.rec_no);
				fprintf (stdout, "E: ");
			...
			if (GMT_REC_IS_FILE_BREAK (GMT)) {	/* End of a file but not end of all files */
				GMT_report (GMT, GMT_MSG_VERBOSE, "End of intermediate data file after record %" PRIu64 "\n", GMT->current.io.rec_no);
			...
			if (GMT_REC_IS_GAP (GMT)) {	/* Found a gap */
				GMT_report (GMT, GMT_MSG_VERBOSE, "A gap found in record %" PRIu64 "\n", GMT->current.io.rec_no);
				fprintf (stdout, "G: ");
			}
		}
		if (GMT_REC_IS_DATA (GMT)) {	/* Found a data record */
			...
			GMT_report (GMT, GMT_MSG_VERBOSE, "Data found in record %" PRIu64 "\n", GMT->current.io.rec_no);
			fprintf (stdout, "D: ");
		}
