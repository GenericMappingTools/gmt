/* Misc cunctions to find and read DCW polygons */
...
{	/* Given comma-separated names, return the corresponding polygons, or give error */
	int ks, j, kk, retval, ncid, varid, id, first, last;
	size_t np;
	unsigned int k, n_items = 0, pos = 0;
	bool done, want_state, outline = (E->mode & 4);
	...
	if (!dump) {
		...
		if (E->mode & 8) GMT_setfill (GMT, &E->fill, outline);
	}

	while (GMT_strtok (E->codes, ",", &pos, code)) {
		want_state = false;
		...
		retval = nc_inq_dimid (ncid, "time", &id);
		...
		k = 0;
			...
			k--;	/* Back to last 999,999 point */
			S->n_rows = last - first + 1;	/* Number of points in this segment */
			/* Allocate segment and its memory */
			/* Copy over the doubles */
			if (dump) {
				strcpy (GMT->current.io.segment_header, msg);
				GMT_Put_Record (GMT->parent, GMT_WRITE_SEGMENT_HEADER, NULL);
				for (kk = first; kk <= last; kk++) {
					out[GMT_X] = S->coord[GMT_X][kk];
					out[GMT_Y] = S->coord[GMT_Y][kk];
...
{
	GMT_message (GMT, "\t-%c Append comma-separated list of codes for countries to highlight with selected pen or fill\n", option);
	GMT_message (GMT, "\t	code1, code2 ,etc are 2-character country codes\n");
	GMT_message (GMT, "\t	To get a state of a country (if present), append .state, e.g, US.TX for Texas.\n");
	GMT_message (GMT, "\t	Append +l to display list of countries and their codes [no plotting takes place]\n");
...
unsigned int GMT_DCW_parse (struct GMT_CTRL *GMT, char option, char *args, struct F *E)
{	/* Parse the E in pscoast */
	...
	if ((c = strchr (a, '+'))) {	/* Handle modifiers */
		while ((GMT_strtok (c, "+", &pos, p))) {
			switch (p[0]) {
				/* Listings*/
				case 'l':  E->mode = 1;  break;	/* Country list */
				case 'L':  E->mode = 2;  break;	/* Country and state list */
				case 'p':
					if (GMT_getpen (GMT, &p[1], &E->pen)) {	/* Error decoding pen */
						GMT_pen_syntax (GMT, 'E', " ");
						n_errors++;
					}
					E->mode |= 4;
					break;
				case 'f':
					if (GMT_getfill (GMT, &p[1], &E->fill)) {
