/*--------------------------------------------------------------------
 *	$Id: originator_func.c,v 1.2 2011-03-15 02:06:37 guru Exp $
 *
...
GMT_LONG GMT_originator_parse (struct GMTAPI_CTRL *C, struct ORIGINATOR_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, GMT_native_binary (GMT, GMT_IN) && GMT->common.b.ncol[GMT_IN] < n_input, "GMT SYNTAX ERROR:  Binary input data (-bi) must have at least %ld columns\n", n_input);
	...
	n_errors += GMT_check_condition (GMT, !Ctrl->F.file, "GMT SYNTAX ERROR -F:  Must specify hotspot file\n");
	n_errors += GMT_check_condition (GMT, !Ctrl->E.file, "GMT SYNTAX ERROR -F:  Must specify Euler pole file\n");
	n_errors += GMT_check_condition (GMT, Ctrl->D.value <= 0.0, "GMT SYNTAX ERROR -D:  Must specify a positive interval\n");
	n_errors += GMT_check_condition (GMT, Ctrl->W.dist <= 0.0, "GMT SYNTAX ERROR -W:  Must specify a positive distance in km\n");
...
GMT_LONG GMT_originator (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (Ctrl->S.n <= 0 || Ctrl->S.n > nh) {
		GMT_message (GMT, "%s: GMT SYNTAX ERROR -S:  Give value between 1 and %ld\n", GMT->init.progname, nh);
		Return (EXIT_FAILURE);
	}
