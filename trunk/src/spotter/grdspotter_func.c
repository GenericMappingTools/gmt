...
GMT_LONG GMT_grdspotter_usage (struct GMTAPI_CTRL *C, GMT_LONG level)
{
	...
	GMT_message (GMT, "\t-D Sets optional output grids:\n");
	...
	GMT_message (GMT, "\t-N sets upper age in m.y. for nodes whose plate age is NaN [180]\n");
	...
	GMT_message (GMT, "\t-T Sets upper ages.  Repeatable, choose from\n");
...
GMT_LONG GMT_grdspotter_parse (struct GMTAPI_CTRL *C, struct GRDSPOTTER_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, Ctrl->M.active && (Ctrl->W.active || Ctrl->Z.mode), "GMT SYNTAX ERROR:  Cannot use -M with -B or -Z (slicing)\n");
...
#define Return(code) {Free_grdspotter_Ctrl (GMT, Ctrl); GMT_end_module (GMT, GMT_cpy); return (code);}

GMT_LONG GMT_grdspotter (struct GMTAPI_CTRL *API, struct GMT_OPTION *options)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) return (GMT_grdspotter_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) return (GMT_grdspotter_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	Ctrl = (struct GRDSPOTTER_CTRL *) New_grdspotter_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	...
	if (Ctrl->L.file) {
		...
		if (Ctrl->Q.mode == 1) {	/* Only doing one CVA with no extra restrictions */
			...
			if ((fp = fopen (Ctrl->Q.file, "r")) == NULL) {	/* Oh, oh... */
				GMT_message (GMT, "%s: GMT ERROR:  -Q info file unreadable/nonexistent\n", GMT->init.progname);
				exit (EXIT_FAILURE);
			}
