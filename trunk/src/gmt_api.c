...
{	/* Sets the alloc mode of the container, given the family type, to GMT_REFERENCE,
	...
	if (data == NULL) return;
	switch (family) {
		case GMT_IS_GRID:	G = data; if (G->alloc_mode == GMT_ALLOC_BY_OTHERS) G->alloc_mode = GMT_REFERENCE; break;
		case GMT_IS_DATASET:	D = data; if (D->alloc_mode == GMT_ALLOC_BY_OTHERS) D->alloc_mode = GMT_REFERENCE; break;
		case GMT_IS_TEXTSET:	T = data; if (T->alloc_mode == GMT_ALLOC_BY_OTHERS) T->alloc_mode = GMT_REFERENCE; break;
		case GMT_IS_CPT:	P = data; if (P->alloc_mode == GMT_ALLOC_BY_OTHERS) P->alloc_mode = GMT_REFERENCE; break;
		case GMT_IS_MATRIX:	M = data; if (M->alloc_mode == GMT_ALLOC_BY_OTHERS) M->alloc_mode = GMT_REFERENCE; break;
		case GMT_IS_VECTOR:	V = data; if (V->alloc_mode == GMT_ALLOC_BY_OTHERS) V->alloc_mode = GMT_REFERENCE; break;
	}
}
...
{	/* Count how many data sets of the given family are currently registered and unused for the given direction (GMT_IN|GMT_OUT).
 	 * Also return the ID of the first data object for the given direction (GMT_NOTSET if not found).
	 */
	unsigned int i, n;
...
{	/* Returns the ID of the first object whose data pointer matches ptr.
	 ...
	 */
	unsigned int i, family = 0;
	int object_ID = GMT_NOTSET;	/* Not found yet */
	
	if (sfamily != GMT_NOTSET) family = (unsigned int)sfamily;
...
int GMTAPI_is_registered (struct GMTAPI_CTRL *API, unsigned int family, unsigned int geometry, unsigned int direction, unsigned int mode, char *filename, void *resource)
...
{	/* Does the actual work of loading in a CPT palette table.
	...
	switch (S_obj->method) {	/* File, array, stream etc ? */
		...
		case GMT_IS_READONLY:	/* Just pass memory location, so nothing is allocated */
			GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Referencing read-only CPT table from GMT_PALETTE memory location\n");
			if ((P_obj = S_obj->resource) == NULL) return_null (API, GMT_PTR_IS_NULL);
...
{	/* Does the actual work of loading in the entire virtual data set (possibly via many sources)
	...
	int item, first_item = 0, this_item = GMT_NOTSET, last_item;
	unsigned int geometry;
	bool allocate = false, update = false, all_D, use_GMT_io;
	size_t n_alloc;
	...
	if (object_ID == GMT_NOTSET) {	/* More than one source: Merge all registered data tables into a single virtual data set */
	...
	D_obj->alloc_mode = GMT_ALLOC_BY_GMT;		/* So GMT_* modules can free this memory (may override below) */
	...
	for (item = first_item; item <= last_item; item++) {	/* Look through all sources for registered inputs (or just one) */
		...
		switch (S_obj->method) {	/* File, array, stream etc ? */
	 		case GMT_IS_FILE:	/* Import all the segments, then count total number of records */
				...
				if ((D_obj->table[D_obj->n_tables] = GMT_read_table (API->GMT, S_obj->filename, S_obj->method, false, &geometry, use_GMT_io)) == NULL) continue;		/* Ran into an empty file (e.g., /dev/null or equivalent). Skip to next item, */
			...
			case GMT_IS_STREAM:	/* Import all the segments, then count total number of records */
	 		case GMT_IS_FDESC:
				...
				if ((D_obj->table[D_obj->n_tables] = GMT_read_table (API->GMT, S_obj->fp, S_obj->method, false, &geometry, use_GMT_io)) == NULL) continue;		/* Ran into an empty file (e.g., /dev/null or equivalent). Skip to next item, */
				D_obj->table[D_obj->n_tables]->id = D_obj->n_tables;	/* Give sequential internal object_ID numbers to tables */
				D_obj->n_tables++;	/* Since we just read one */
				update = true;
				break;
...
{	/* Does the actual work of writing out the specified data set to one destination.
	...
	if (S_obj->family != GMT_IS_DATASET) return (GMTAPI_report_error (API, GMT_WRONG_KIND));			/* Called with wrong data type */
...
{	/* Does the actual work of loading in the entire virtual text set (possibly via many sources)
	...
	T_obj->alloc_mode = GMT_ALLOC_BY_GMT;	/* So GMT_* modules can free this memory (may override below) */
...
{	/* Handles the reading of a 2-D grid given in one of several ways.
	...
	switch (S_obj->method) {
		case GMT_IS_FILE:	/* Name of a image file on disk */
			...
			I_obj->alloc_mode = GMT_ALLOC_BY_GMT;
			break;
	 	...
	 	case GMT_IS_READONLY:	/* GMT grid and header in a GMT_GRID container object by exact reference (no changes allowed to grid values) */
			...
			if (GMTAPI_adjust_grdpadding (I_obj->header, API->GMT->current.io.pad)) return_null (API, GMT_PADDING_NOT_ALLOWED);
			GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Referencing image data from read-only GMT_IMAGE memory location\n");
	 	...
	 	case GMT_IS_REFERENCE + GMT_VIA_MATRIX:	/* The user's 2-D grid array of some sort, + info in the args [NOT YET FULLY TESTED] */
			...
			if (!(M_obj->shape == GMT_IS_ROW_FORMAT && M_obj->type == GMT_FLOAT && M_obj->alloc_mode == 0 && (mode & GMT_GRID_IS_COMPLEX_MASK))) 
...
{	/* Handles the reading of a 2-D grid given in one of several ways.
	...
	switch (S_obj->method) {
		case GMT_IS_FILE:	/* Name of a grid file on disk */
			...
			G_obj->alloc_mode = GMT_ALLOC_BY_GMT;
			break;
	 	...
	 	case GMT_IS_READONLY:	/* GMT grid and header in a GMT_GRID container object by exact reference (no changes allowed to grid values) */
			...
			GMT_BC_init (API->GMT, G_obj->header);	/* Initialize grid interpolation and boundary condition parameters */
			if (GMT_err_pass (API->GMT, GMT_grd_BC_set (API->GMT, G_obj, GMT_IN), "Grid memory")) return_null (API, GMT_GRID_BC_ERROR);	/* Set boundary conditions */
			if (GMTAPI_adjust_grdpadding (G_obj->header, API->GMT->current.io.pad)) return_null (API, GMT_PADDING_NOT_ALLOWED);
			GMT_Report (API, GMT_MSG_LONG_VERBOSE, "Referencing grid data from read-only GMT_GRID memory location\n");
	...
	switch (S_obj->method) {
		case GMT_IS_FILE:	/* Name of a grid file on disk */
	 	...
	 	case GMT_IS_REFERENCE:	/* GMT grid and header in a GMT_GRID container object - just pass the reference */
			...
			if (G_obj->alloc_mode != GMT_ALLOC_BY_OTHERS) G_obj->alloc_mode = GMT_REFERENCE;	/* So we dont accidentally free this later */
...
int GMTAPI_destroy_data_ptr (struct GMTAPI_CTRL *API, unsigned int family, void *ptr, unsigned int *alloc_mode)
{
	...
	*alloc_mode = 0;
	...
	if (*alloc_mode != GMT_ALLOC_BY_OTHERS) GMT_free (API->GMT, ptr);
	return (GMT_OK);	/* Null pointer */
}

void GMTAPI_update_allocmode (struct GMTAPI_CTRL *API, int level)
{
	/* Ensure that if any objects are flagged with alloc_mode = GMT_ALLOC_BY_OTHERS
	 * then objects with duplicate container pointers must have their alloc_mode
	 * reset to GMT_ALLOC_BY_OTHERS as well.  This can happen when an external API wishes
	...
	if (level == GMT_NOTSET) {	/* Time to change all GMT_ALLOC_BY_OTHERS to GMT_REFERENCE so they can be destroyed */
		for (i = 0; i < API->n_objects; i++) {
			if (API->object[i]->alloc_mode != GMT_ALLOC_BY_OTHERS) continue;	/* Not set to clobber so nothing to do */
		...
		for (i = 0; i < API->n_objects; i++) {
			...
			for (j = 0; j < API->n_objects; j++) {
				...
				if (API->object[j]->alloc_mode == GMT_REFERENCE && API->object[j]->data == API->object[i]->data) {	/* Same container */
					API->object[j]->alloc_mode = GMT_ALLOC_BY_OTHERS;
					GMT_Report (API, GMT_MSG_DEBUG, "GMTAPI_update_allocmode: Objects %u and %u share same container and has alloc_mode == GMT_ALLOC_BY_OTHERS\n", i, j);
				}
			}
		}
	}
...
void GMT_Garbage_Collection (struct GMTAPI_CTRL *API, int level)
{
	...
	while (i < API->n_objects) {	/* While there are more objects to consider */
		...
		else if (alloc_mode == GMT_ALLOC_BY_OTHERS && level != GMT_NOTSET) {	/* Something allocated outside of GMT, leave until final Destroy_Session */
			S_obj->alloc_mode = GMT_ALLOC_BY_OTHERS;
		}
	...
	while (i < API->n_objects) {	/* While there are more objects to consider */
		S_obj = API->object[i];	/* Shorthand for the the current object */
		if (S_obj && (level == GMT_NOTSET || (S_obj->level == u_level && S_obj->alloc_mode != GMT_ALLOC_BY_OTHERS)))	/* Yes, this object was added in this module (or we dont care), get rid of it; leave i where it is */
...
{	/* Fortran version: We pass the hidden global GMT_FORTRAN structure */
	return (GMTAPI_report_error (GMT_FORTRAN, *error));
}
#endif
...
int GMT_Encode_ID (void *V_API, char *filename, int object_ID)
{
	...
	if (object_ID == GMT_NOTSET) return_error (API, GMT_NOT_A_VALID_ID);	/* ID is nont set yet */
...
int GMT_Register_IO (void *V_API, unsigned int family, unsigned int method, unsigned int geometry, unsigned int direction, double wesn[], void *resource)
{
	...
	if (API == NULL) return_value (API, GMT_NOT_A_SESSION, GMT_NOTSET);
	...
	GMT_Report (API, GMT_MSG_DEBUG, message, object_ID);
...
void * GMT_Duplicate_Data (void *V_API, unsigned int family, unsigned int mode, void *data)
{
	...
	if ((object_ID = GMT_Register_IO (API, family, GMT_IS_REFERENCE, geometry, GMT_IN, NULL, new)) == GMT_NOTSET) return_null (API, API->error);	/* Failure to register */
	if ((item = GMTAPI_Validate_ID (API, family, object_ID, GMT_IN)) == GMT_NOTSET) return_null (API, API->error);
			...
			if (S_obj->family == GMT_IS_DATASET) {
				...
				switch (mode) {
					...
					case GMT_WRITE_DOUBLE:		/* Export a segment row */
						if (p[1] == 0) { GMT_Report (API, GMT_MSG_LONG_VERBOSE, "GMTAPI: Internal Warning: GMT_Put_Record (double) called before any segments declared\n"); p[1] = 0; T_obj->n_segments = 1;}
						...
						break;
					default:
						GMT_Report (API, GMT_MSG_NORMAL, "GMTAPI: Internal error: GMT_Put_Record (double) called with illegal mode\n");
						return_error (API, GMT_NOT_A_VALID_IO_MODE);	
						break;
				}
			}
				...
				switch (mode) {
					...
					case GMT_WRITE_TEXT:		/* Export a record */
						if (p[1] == 0) { GMT_Report (API, GMT_MSG_LONG_VERBOSE, "GMTAPI: Internal Warning: GMT_Put_Record (text) called before any segments declared\n"); p[1] = 0; T_obj->n_segments = 1;}
...
int GMT_Destroy_Data (void *V_API, void *object)
{
	...
	if ((object_ID = GMTAPI_ptr2id (API, object)) == GMT_NOTSET) return_error (API, GMT_OBJECT_NOT_FOUND);	/* Could not find it */
...
void * GMT_Create_Data (void *V_API, unsigned int family, unsigned int geometry, unsigned int mode, uint64_t dim[], double *range, double *inc, unsigned int registration, int pad, void *data)
{
	...
	if (!already_registered) {	/* Now register this dataset so it can be deleted by GMT_Destroy_Data */
		...
		GMT_Report (API, GMT_MSG_DEBUG, "Successfully added data array to previously registered %s\n", GMT_family[family]);
...
{	/* Initialize 1-D or 2-D FFT machinery and set up wavenumbers */
	...
	GMT_Report (API, GMT_MSG_NORMAL, "GMT FFT only supports dimensions 1 and 2, not %u\n", dim);
...
{	/* Message whose output depends on verbosity setting */
	...
	snprintf (message, GMT_BUFSIZ, "%s (%s): ",
			gmt_module_name(API->GMT), GMT_basename (source_line));
	source_info_len = strlen (message);
	source_info_len = strlen (message);
	va_start (args, format);
...
{	/* Parse any number of comma, space or slash-separated values.
	 ...
	 * Arc distasnces are returned in degrees.
	...
	if (arg == NULL || arg[0] == '\0') return_value (API, GMT_NO_PARAMETERS, GMT_NOTSET);
