...
size_t GMTAPI_set_grdarray_size (struct GMT_CTRL *C, struct GMT_GRID_HEADER *h, unsigned int mode, double *wesn)
	...
	if (mode == 'r' || mode == 'R') {	/* Open file for reading */
		if (mode == 'R') {	/* File has no header; can only work if G->header has been set already, somehow */
			...
			if (G->header->nx == 0 || G->header->ny == 0) {
				GMT_Report (C->parent, GMT_MSG_NORMAL, "Unable to read header-less grid file %s without a preset header structure\n", file);
...
{	/* Remove specified object ID from active list of objects */
	...
	if ((s_item = GMTAPI_Validate_ID (API, GMTAPI_NOTSET, object_ID, direction)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
...
{	/* Does the actual work of writing out the specified CPT to a destination.
	...
	if (object_ID == GMTAPI_NOTSET) return (GMTAPI_report_error (API, GMT_OUTPUT_NOT_SET));
	if ((item = GMTAPI_Validate_ID (API, GMT_IS_CPT, object_ID, GMT_OUT)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
...
{	/* Does the actual work of writing out the specified data set to one destination.
	...
	if (object_ID == GMTAPI_NOTSET) return (GMTAPI_report_error (API, GMT_OUTPUT_NOT_SET));
	if ((item = GMTAPI_Validate_ID (API, GMT_IS_DATASET, object_ID, GMT_OUT)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
	...
	switch (S_obj->method) {	/* File, array, stream etc ? */
		...
		case GMT_IS_FILE:
	 	case GMT_IS_FDESC:
			...
			if ((error = GMT_write_dataset (API->GMT, ptr, default_method, D_obj, true, GMTAPI_NOTSET))) return (GMTAPI_report_error (API, error));
			break;
...
{	/* Does the actual work of writing out the specified text set to one destination.
	...
	if (object_ID == GMTAPI_NOTSET) return (GMTAPI_report_error (API, GMT_OUTPUT_NOT_SET));
	if ((item = GMTAPI_Validate_ID (API, GMT_IS_TEXTSET, object_ID, GMT_OUT)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
	...
	switch (S_obj->method) {	/* File, array, stream etc ? */
		...
		case GMT_IS_FILE:
	 	case GMT_IS_FDESC:
			...
			if ((error = GMT_write_textset (API->GMT, dest, default_method, T_obj, GMTAPI_NOTSET))) return (GMTAPI_report_error (API, error));
			break;
...
{	/* Writes out a single grid to destination */
	...
	if (object_ID == GMTAPI_NOTSET) return (GMTAPI_report_error (API, GMT_OUTPUT_NOT_SET));
	if ((item = GMTAPI_Validate_ID (API, GMT_IS_GRID, object_ID, GMT_OUT)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
		...
		case GMT_IS_FILE:	/* Name of a grid file on disk */
	 	...
	 	case GMT_IS_DUPLICATE:	/* Duplicate GMT grid and header to a GMT_GRID container object. Subset allowed */
			...
			if (!S_obj->region) {	/* No subset, possibly same padding */
				...
				if (GMT_err_pass (API->GMT, GMT_grd_BC_set (API->GMT, G_copy), "Grid memory")) return (GMTAPI_report_error (API, GMT_GRID_BC_ERROR));	/* Set boundary conditions */
	 	...
	 	case GMT_IS_REFERENCE:	/* GMT grid and header in a GMT_GRID container object - just pass the reference */
			...
			GMT_BC_init (API->GMT, G_obj->header);	/* Initialize grid interpolation and boundary condition parameters */
			if (GMT_err_pass (API->GMT, GMT_grd_BC_set (API->GMT, G_obj), "Grid memory")) return (GMTAPI_report_error (API, GMT_GRID_BC_ERROR));	/* Set boundary conditions */
...
int GMTAPI_Export_Data (struct GMTAPI_CTRL *API, unsigned int family, int object_ID, unsigned int mode, void *data)
{
	...
	/* Get information about this resource first */
	if ((item = GMTAPI_Validate_ID (API, family, object_ID, GMT_OUT)) == GMTAPI_NOTSET) return (GMTAPI_report_error (API, API->error));
...
int GMT_Register_IO (void *V_API, unsigned int family, unsigned int method, unsigned int geometry, unsigned int direction, double wesn[], void *resource)
{
	...
	if ((object_ID = GMTAPI_is_registered (API, family, geometry, direction, mode, NULL, resource)) != GMTAPI_NOTSET) {	/* Registered before */
...
void * GMT_Read_Data (void *V_API, unsigned int family, unsigned int method, unsigned int geometry, unsigned int mode, double wesn[], char *input, void *data)
{
	...
	if ((family == GMT_IS_GRID || family == GMT_IS_IMAGE) && (mode & GMT_GRID_DATA_ONLY)) {	/* Case 4: Already registered when we obtained header, find object ID */
		if ((in_ID = GMTAPI_is_registered (API, family, geometry, GMT_IN, mode, input, data)) == GMTAPI_NOTSET) return_null (API, GMT_OBJECT_NOT_FOUND);	/* Could not find it */
...
struct GMT_FFT_WAVENUMBER * GMTAPI_FFT_init_2d (struct GMTAPI_CTRL *API, struct GMT_GRID *G, unsigned int mode, void *v_info)
{
	...
	grd_dump (C, G->header, G->data, false, "Read in FFT_Create");
	...
	grd_dump (C, G->header, G->data, false, "After detrend");
	gmt_fft_taper (C, G, F);				/* Taper data, if requested */
	grd_dump (C, G->header, G->data, false, "After Taper");
	K->dim = 2;	/* 2-D FFT */
	return (K);
}
...
{	/* Parse any number of comma, space or slash-separated values.
	...
	if (arg == NULL || arg[0] == '\0') return_value (API, GMT_NO_PARAMETERS, GMTAPI_NOTSET);
