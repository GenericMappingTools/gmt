/*	$Id$
 *
 *	Copyright (c) 1996-2013by P. Wessel, W. H. F. Smith, R. Scharroo, J. Luis and F. Wobbe
...
void *get_memory (void *prev_addr, size_t n, size_t size, char *progname)
{
	...
	if (prev_addr) {
		if ((tmp = realloc ( prev_addr, n * size)) == NULL) {
			fprintf (stderr, "Error: %s could not reallocate more memory, n = %" PRIuS "\n", progname, n);
			exit (EXIT_FAILURE);
		}
	}
	else {
		if ((tmp = calloc (n, size)) == NULL) {
			fprintf (stderr, "Error: %s could not allocate memory, n = %" PRIuS "\n", progname, n);
			exit (EXIT_FAILURE);
		}
	}
	return (tmp);
...
int main (int argc, char **argv)
{
	...
	x = get_memory (NULL, 1, sizeof (int), "gshhg_dp");
	y = get_memory (NULL, 1, sizeof (int), "gshhg_dp");
	index = get_memory (NULL, 1, sizeof (int), "gshhg_dp");
	
	n_read = fread (&h, sizeof (struct GSHHG), 1U, fp_in);
	...
	while (n_read == 1) {
		...
		x = get_memory (x, h.n, sizeof (int), "gshhg_dp");
		y = get_memory (y, h.n, sizeof (int), "gshhg_dp");
		index = get_memory (index, h.n, sizeof (int), "gshhg_dp");
		...
		if (n > 2) {
			...
			h.id = n_out;
			h.n = n;
			if (fwrite (&h, sizeof (struct GSHHG), 1U, fp_out) != 1) {
