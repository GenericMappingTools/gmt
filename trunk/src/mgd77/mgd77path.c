...
int GMT_mgd77path (struct GMTAPI_CTRL *API, int mode, void *args)
{
	...
	if (!options || options->option == GMTAPI_OPT_USAGE) bailout (GMT_mgd77path_usage (API, GMTAPI_USAGE));	/* Return the usage message */
	if (options->option == GMTAPI_OPT_SYNOPSIS) bailout (GMT_mgd77path_usage (API, GMTAPI_SYNOPSIS));	/* Return the synopsis */
	...
	if (n_paths == 0) {
		GMT_report (GMT, GMT_MSG_NORMAL, "No cruises found\n");
		Return (EXIT_FAILURE);
	}
	
	for (i = 0; i < n_paths; i++) {		/* Process each ID */
 		if (MGD77_Get_Path (GMT, path, list[i], &M))
			GMT_report (GMT, GMT_MSG_NORMAL, "Cannot find cruise %s\n", list[i]);
		else if (Ctrl->A.mode) {
			printf ("%s\n", list[i]);
			n_cruises++;
		}
	...
	GMT_report (GMT, GMT_MSG_VERBOSE, "Found %d cruises\n", n_cruises);
