		...
		switch (opt->option) {
			...
			case 'C':	/* Distance calculation flag */
				...
				if (!strchr ("cefg", (int)opt->arg[0])) {
					GMT_report (GMT, GMT_MSG_NORMAL, "ERROR -C: Flag must be c, f, g, or e\n");
					n_errors++;
				}
			...
			case 'N':	/* Distance and speed unit selection */
				switch (opt->arg[0]) {
					...
					case 's':	/* Speed unit selection */
						...
						break;
					default:
						GMT_report (GMT, GMT_MSG_NORMAL, "ERROR -N: Choose from -Nd and -Ns\n");
						n_errors++;
						break;
				}
				if (!n_errors) {
			...
			case 'W':
				switch (opt->arg[0]) {
					...
					case 't':	/* Get new timegap */
						break;
					default:
						GMT_report (GMT, GMT_MSG_NORMAL, "Syntax error: -Wt|d<width>\n");
						n_errors++;
						break;
				}
				if (!n_errors) {
...
int GMT_x2sys_init (void *V_API, int mode, void *args)
{
	...
	if (access (def_file, R_OK)) {	/* No such local *.def file */
		GMT_report (GMT, GMT_MSG_NORMAL, "Unable to find local definition file : %s\n", def_file);
		Return (EXIT_FAILURE);
	}
	else if ((fp_def = fopen (def_file, "r")) == NULL) {
		GMT_report (GMT, GMT_MSG_NORMAL, "Unable to open local definition file : %s\n", def_file);
		Return (EXIT_FAILURE);
	}
	...
	if (x2sys_access (GMT, Ctrl->In.TAG, R_OK)) {	/* No such dir */
		...
		if (mkdir (path)) {
#endif
			GMT_report (GMT, GMT_MSG_NORMAL, "Unable to create TAG directory : %s\n", path);
			Return (EXIT_FAILURE);
		}
	}
	else if (!Ctrl->F.active) {	/* Directory exists but -F not on */
		GMT_report (GMT, GMT_MSG_NORMAL, "TAG directory already exists: %s\n", path);
		Return (EXIT_FAILURE);
	}
	...
	if (!x2sys_access (GMT, tag_file, R_OK)) {
		GMT_report (GMT, GMT_MSG_NORMAL, "File exists: %s\n", tag_file);
		x2sys_path (GMT, tag_file, path);
		if (Ctrl->F.active) {
			if (remove (path))
				GMT_report (GMT, GMT_MSG_NORMAL, "Unable to remove %s\n", path);
			else
				GMT_report (GMT, GMT_MSG_NORMAL, "Removed file %s\n", path);
		}
		else 
			n_found++;
	}
	...
	if (n_found) {
		GMT_report (GMT, GMT_MSG_NORMAL, "Remove/rename old files or use -F to overwrite\n");
		Return (EXIT_FAILURE);
	}


	GMT_report (GMT, GMT_MSG_VERBOSE, "Initialize %s\n", tag_file);
	...
	if ((fp = x2sys_fopen (GMT, def_file, "w")) == NULL) {
		GMT_report (GMT, GMT_MSG_NORMAL, "Could not create %s\n", def_file);
		Return (EXIT_FAILURE);
	}
