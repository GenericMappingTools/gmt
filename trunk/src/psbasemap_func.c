/*--------------------------------------------------------------------
 *	$Id: psbasemap_func.c,v 1.17 2011-03-15 02:06:36 guru Exp $
 *
...
GMT_LONG GMT_psbasemap_parse (struct GMTAPI_CTRL *C, struct PSBASEMAP_CTRL *Ctrl, struct GMT_OPTION *options)
{
	...
	n_errors += GMT_check_condition (GMT, !(GMT->current.map.frame.plot || Ctrl->L.active || Ctrl->T.active || Ctrl->G.active), "GMT SYNTAX ERROR:  Must specify at least one of -B, -G, -L, -T\n");
	n_errors += GMT_check_condition (GMT, Ctrl->L.active && !GMT_is_geographic (GMT, GMT_IN), "GMT SYNTAX ERROR:  -L applies to geographical data only\n");
