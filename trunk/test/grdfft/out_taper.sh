...
grdimage t.nc -Jx${scl}i -Ct.cpt -P -Ba -BWSne -K > $ps
grd2xyz t_tmp2.nc | awk '{if ($2 == 100) print $1, $3}' > tmp
R=`minmax tmp -I10/3`
psxy $R -JX3.5i/1.6i -O -K -W1p,green -Bxaf -Byafg10 -BWSne -X3i tmp >> $ps
...
EOF
# mirror and taper
grdimage t_tmp1.nc -Jx${scl}i -Ct.cpt -Ba -BWSne -O -K -X-3i -Y${yoff}i >> $ps
...
grdimage t_tmp2.nc -J -Ct.cpt -Ba -BWSne -O -K -Y${yoffe}i >> $ps
