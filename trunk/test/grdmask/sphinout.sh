...
EOF
grdmask "$src"/2k.dat -Gmask.nc -N0/0.5/1 -I1 -Rg -fg -A
grdimage mask.nc -Jx0.015id -Cmask.cpt -B60/30WSne -K -P -Xc -Y1.5i > $ps
psxy -Rmask.nc -J -O -K "$src"/2k.dat -W0.25p,blue >> $ps
...
psxy -R -J -O -K "$src"/2k.dat -W0.5p,blue >> $ps
...
gmtmath -T "$src"/2k.dat FLOOR = tmp.txt
