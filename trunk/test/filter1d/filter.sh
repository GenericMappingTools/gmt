...
psxy -R -J -B100g50/100g50WSne -O "$src"/noise.txt -W0.25p,red,. -K > tmp.ps
psxy -R -J -O "$src"/noise.txt -Sc0.05i -Gred -K >> tmp.ps
cat tmp.ps >> $ps
# Median without -E
psxy -R -J -O -T -K -Y-2.5i >> $ps
cat tmp.ps >> $ps
filter1d -Fm100 -N0 "$src"/noise.txt | psxy -R -J -O -K -W2p,blue >> $ps
# Median with -E
psxy -R -J -O -T -K -Y-2.5i >> $ps
cat tmp.ps >> $ps
filter1d -Fm100 -E -N0 "$src"/noise.txt | psxy -R -J -O -K -W2p,blue >> $ps
# Median with -E then Gaussian
psxy -R -J -O -T -K -Y-2.5i >> $ps
cat tmp.ps >> $ps
filter1d -Fm100 -E -N0 "$src"/noise.txt | filter1d -Fg100 -E -N0 | psxy -R -J -O -K -W2p,blue >> $ps
psxy -R -J -O -T >> $ps
