#!/bin/bash
#	$Id: circles.sh,v 1.1 2011-07-20 23:40:37 guru Exp $
#
...
spole1=`grep "L1 N Hemisphere" s.txt | awk '{printf "%s/%s\n", $1, $2}'`
spole2=`grep "L2 N Hemisphere" s.txt | awk '{printf "%s/%s\n", $1, $2}'`
slat1=`grep "L1 Small Circle" s.txt | awk '{print $NF}'`
slat2=`grep "L2 Small Circle" s.txt | awk '{print $NF}'`
...
grep "Great Circle Pole" s.txt | psxy -R -J -O -K -Sa0.2i -Ggreen -W0.25p >> $ps
