...
DATA="${src:=.}"/points.txt
# NN analysis
gmtspatial -Aa0k -fg $DATAt > results.txt
gmtset MAP_FRAME_TYPE plain
psxy -R-0.5/10.5/-0.5/10.5 -JM4.5i -P -Bag1WSne $DATAt -Sc0.1i -Gred -K -Xc > $ps
awk '{print $1, $2, NR-1}' $DATAt | pstext -R -J -O -K -F+f12+jCB -Dj0.1i >> $ps
rm -f tmp
...
# NN averaging
gmtspatial -Aa75k -fg $DATAt > results.txt
psxy -R -J -O -K -Bag1Wsne points.txt -Sc0.1i -Glightgray -Y4.75i $DATAt >> $ps
