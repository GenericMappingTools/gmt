...
gmt surface -R0/360/-85/85 -I1 -fg -r data.txt -Gdatag.nc
# Grid with gmt surface so periodic boundaries are -180/+180
gmt surface -R-180/180/-85/85 -I1 -fg -r data.txt -Gdatad.nc
# Contour grid where Greenwhich was in the middle when gridding
gmt grdcontour datad.nc -C10 -A50 -JQ0/5.5i -P -Baf -BWSne -K -Xc -Y0.65i > $ps
...
gmt grdcontour datag.nc -C10 -A50 -JQ0/5.5i -O -Baf -BWsne -K -Y2.85i >> $ps
