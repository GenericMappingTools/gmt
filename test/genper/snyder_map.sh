#!/bin/sh
#	$Id: snyder_map.sh,v 1.3 2010-07-14 15:11:30 remko Exp $

. ../functions.sh
header "Test -JG (US East Coast 160 km Snyder)"

EARTH_MODEL=e
DEBUG=
X0=-Xc
Y0=-Yc
REGION=-Rg
latitude=41.5
longitude=-74.0
altitude=160.0
tilt=55
azimuth=210
twist=0
Width=30.0
Height=30.0

PSFILE=snyder_map.${EARTH_MODEL}
TITLE=:.${PSFILE}:

PROJ=-JG${DEBUG}${EARTH_MODEL}${longitude}/${latitude}/${altitude}/${azimuth}/${tilt}/${twist}/${Width}/${Height}/7i+

pscoast ${GMT_VERBOSE} $REGION $PROJ -P -Yc -Xc -B5g1/5g1${TITLE} -W -Ia -Di -Na --ANNOT_MIN_SPACING=0.5i > $PSFILE.ps

pscmp $PSFILE
