#!/bin/sh
#	$Id: xy2ij.sh,v 1.3 2007-05-26 02:56:52 pwessel Exp $
#
...
echo -n "GMT: Test blockmean's (x,y) <--> (i,j) conversions:		"
cat << EOF > pixel.d
0.0	4.1	0
0.65	0.7	1
1.33	1.8	2
...
diff pixel_xy.d pixel_xy.d.orig  > log
diff pixel_ij.d pixel_ij.d.orig >> log
diff grid_xy.d  grid_xy.d.orig  >> log
diff grid_ij.d  grid_ij.d.orig  >> log
if [ -s fail ]; then
	echo "[FAILED]"
else
	echo "[OK"]
...
# Examine plot for clues if things go wrong
#gv xy2ij.ps &
