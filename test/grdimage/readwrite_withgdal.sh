...
# RGB image
grdimage -D "$src"/gdal/needle.jpg -JX7c/0 -P -Y20c -K > $ps

# Same image as above but as idexed
grdimage -D "$src"/gdal/needle.png -JX7c/0 -X7.5c -O -K >> $ps

# Projected
grdimage -Dr "$src"/gdal/needle.jpg -Rd -JW10c -Bg30/g15 -X-5.0c -Y-7c -O -K >> $ps
...
grdimage -D "$src"/gdal/needle.jpg -Iillum.nc -JM8c -Y-10c -X-2c -O -K >> $ps

# A gray image (one band, no color map)
grdimage -D "$src"/gdal/vader.jpg -JX4c/0 -X9c -Y5c -K -O >> $ps
