...
gmt sphtriangulate "${src:-.}"/hotspots.d -Qv -T > tt.arcs
gmt makecpt -Ccategorical -T0/55/1 > t.cpt
# Make a grid with node numbers
awk '{print $1, $2, NR}' "${src:-.}"/hotspots.d | gmt sphdistance -Rg -I30m -En5 -Gn.nc
...
gmt psxy -R -J -O -K tt.arcs -W1p >> $ps
gmt psxy -R -J -O -K -SE-350 -Gwhite -Wfaint "${src:-.}"/hotspots.d >> $ps
