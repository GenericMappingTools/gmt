...
gmt math -A-stepdata.txt -N4/1 -C0 1 ADD -C1 T ADD -C2 0.5 STEPT ADD -Ca LSQFIT = solution.txt
...
gmt math -A-stepdata.txt -N4/1 -C0 1 ADD -C1 T ADD -C2 0.5 STEPT ADD -Ca SVDFIT = solution.txt
q=(`cat solution.txt`)
...
echo "Fit y(x) = a + b*cos(6@~p@~x - c) + @~e@~(x) = a + b@-1@-*cos(6@~p@~x) + b@-2@-*sin(6@~p@~x) + @~e@~(x)" | gmt pstext -R -J -O -K -F+12p+jRB+cRB -Dj0.2i >> $ps
gmt psxy -R -J -O -T >> $ps
