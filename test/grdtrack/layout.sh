...
ps=layout.ps
function plot_it {
	gmt grdtrack "${src:-.}"/Matthews_2016_subduction_subset.txt -Gdummy.nc -C500k/100/100${1} > sz_pol_left.gmt
	...
	gmt psxy -R -J -O -K -W1p "${src:-.}"/Matthews_2016_subduction_subset.txt
	...
	gmt psxy -R -J -O -K -Ss0.25i -Gblack "${src:-.}"/Matthews_2016_subduction_subset.txt
	gmt pstext -R -J -O -K "${src:-.}"/Matthews_2016_subduction_subset.txt -F+f12p,Helvetica,white+r0
