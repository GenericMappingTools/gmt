...
gmt originator "${src:-.}"/seamounts.d -S1 -D10m -E${POLES} -F"${src:-.}"/pac_hs.d | sed -f t.sed > spotter_3.d
...
grep -v '^#' "${src:-.}"/pac_hs.d | sed -f t.sed | gmt psxy -R -J -O -K -Sa0.2i -W0.25p -Ckey.cpt >> $ps
grep -v '^#' "${src:-.}"/pac_hs.d | cut -f1,2,3 | gmt pstext -R -J -O -K -F+jCT+f10p -Dj0.15i/0.15i -Gwhite -W0.5p >> $ps
gmt psscale -Ckey.cpt -D3i/-0.5i/5i/0.14ih -O -Li0.15i >> $ps
