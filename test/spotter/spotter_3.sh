...
grep -v '^#' pac_hs.d > tmp
awk '{printf "s/%s/%d/g\n", $3, NR}' tmp > t.sed
grep -v '^#' pac_hs.d | awk '{printf "s/%s/%d/g\n", $3, NR}' > t.sed
makecpt -Ccategorical -T1/11/1 -N > t.cpt
paste t.cpt tmp | awk '{printf "%s\t%s\t;%s\n", $1, $2, $5}' > key.cpt
originator seamounts.d -S1 -D10m -E${POLES} -Fpac_hs.d | sed -f t.sed > spotter_3.d
...
grep -v '^#' pac_hs.d | sed -f t.sed | psxy -R -J -O -K -Sa0.2i -W0.25p -Ckey.cpt >> $ps
grep -v '^#' pac_hs.d | cut -f1,2,3 | pstext -R -J -O -K -F+jCT+f10p -Dj0.15i/0.15i -Gwhite -W0.5p >> $ps
psscale -Ckey.cpt -D3i/-0.5i/5i/0.14ih -O -Li0.15i >> $ps

rm -f tmp t.sed t.cpt key.cpt spotter_3.d 
pscmp
