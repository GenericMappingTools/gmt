...
/*	Given rata die rd and double seconds, return 
	...
	double f_days;
	f_days = (rd - C->current.setting.time_system.rata_die - C->current.setting.time_system.epoch_t0);
...
void GMT_gcal_from_dt (struct GMT_CTRL *C, double t, struct GMT_gcal *cal) {
...
void gmt_small_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, GMT_LONG step_secs, GMT_LONG init) {
	...
	if (step_secs == GMT_DAY2SEC_I) {
		...
		if (p->sd[0] != 0.0) {	/* Floor it to start of day.  */
			p->dt[0] -= (p->sd[0] * C->current.setting.time_system.i_scale);
			p->sd[0] = 0.0;
		}
		/* Now we step to next day in rd first, and set dt from there.
			This will work even when leap seconds are implemented.  */
		p->rd[1] = p->rd[0] + 1;
		GMT_gcal_from_rd (C, p->rd[1], &(p->cc[1]) );
		p->sd[1] = 0.0;
		p->dt[1] = GMT_rdc2dt (C, p->rd[1], p->sd[1]);
	}
	else {
		if (init) {
			x = step_secs * floor (p->sd[0] / step_secs);
			if (x != p->sd[0]) {
				p->dt[0] -= ((p->sd[0] - x) * C->current.setting.time_system.i_scale);
				x = p->sd[0];
			}
		}
		...
		*/
		...
		if (x >= GMT_DAY2SEC_F) {	/* Should not be greater than  */
			...
			p->sd[1] = x;
			p->dt[1] = p->dt[0] + step_secs * C->current.setting.time_system.i_scale;
...
void GMT_moment_interval (struct GMT_CTRL *C, struct GMT_MOMENT_INTERVAL *p, double dt_in, GMT_LONG init) {
	...
	*/
	...
	if (init) {
		...
		GMT_dt2rdc (C, dt_in, &(p->rd[0]), &(p->sd[0]) );
	...
	switch (p->unit) {
		...
		case 'D':
			if (p->step == 1) {
			...
			else if (C->current.plot.calclock.date.day_of_year) {
				...
				if (k > kyd) {
					...
					p->rd[1] = GMT_rd_from_gymd (C, p->cc[0].year+1, 1, 1) - 1 + p->step;	/* So jjj will be multiples of step */
				}
				else {
				...
				if (p->cc[0].day_m + p->step > kml) {
					...
					p->rd[1] = GMT_rd_from_gymd (C, p->cc[1].year, p->cc[1].month, 1);
				}
		...
		case 'R':	/* Special Gregorian Days of the Month: Annotations start at start of week, not start of month */
			...
			assuming this numbering applies to C->current.setting.time_week_start, 
			...
			*/
			kws = (C->current.plot.calclock.date.iso_calendar) ? 1 : C->current.setting.time_week_start;
			if (init) {
		...
		case 'r':	/* Gregorian weeks.  Step size is 1.  */
			if (init) {
				...
				k = p->cc[0].day_w - C->current.setting.time_week_start;
				if (k) {
		...
		case 'U':
			if (init) {
				...
				p->rd[0] = GMT_rd_from_iywd (C, p->cc[0].iso_y, p->cc[0].iso_w, 1);
				GMT_gcal_from_rd (C, p->rd[0], &(p->cc[0]) );
				GMT_memcpy (&(p->cc[1]), &(p->cc[0]), 1, struct GMT_gcal);	/* Set to same as first calendar */
				p->dt[0] = GMT_rdc2dt (C, p->rd[0], p->sd[0]);
			...
			if (p->cc[1].iso_y != p->cc[0].iso_y) {
				k = p->cc[1].iso_y;
				p->rd[1] = GMT_rd_from_iywd (C, k, 1, 1);
		...
		case 'O':
			...
			p->rd[1] = GMT_rd_from_gymd (C, p->cc[1].year, p->cc[1].month, p->cc[1].day_m);
			GMT_gcal_from_rd (C, p->rd[1], &(p->cc[1]) );
			p->sd[1] = 0.0;
			p->dt[1] = GMT_rdc2dt (C, p->rd[1], p->sd[1]);
			break;
		...
		case 'Y':
			if (init) {
				...
				if (C->current.plot.calclock.date.iso_calendar) {
					p->sd[0] = 0.0;
					if (p->step > 1) p->cc[0].iso_y -= gmt_cal_imod (p->cc[0].iso_y, p->step);
					p->rd[0] = GMT_rd_from_iywd (C, p->cc[0].iso_y, 1, 1);
				}
				else {
					p->sd[0] = 0.0;
					if (p->step > 1) p->cc[0].year -= gmt_cal_imod (p->cc[0].year, p->step);
					p->rd[0] = GMT_rd_from_gymd (C, p->cc[0].year, 1, 1);
				}
				GMT_gcal_from_rd (C, p->rd[0], &(p->cc[0]) );
				GMT_memcpy (&(p->cc[1]), &(p->cc[0]), 1, struct GMT_gcal);	/* Set to same as first calendar */
				p->dt[0] = GMT_rdc2dt (C, p->rd[0], p->sd[0]);
			...
			if (C->current.plot.calclock.date.iso_calendar) {
				...
				p->rd[1] = GMT_rd_from_iywd (C, p->cc[1].iso_y, 1, 1);
			}
			else {
				p->cc[1].year = p->cc[0].year + p->step;
				p->rd[1] = GMT_rd_from_gymd (C, p->cc[1].year, 1, 1);
			}
			p->sd[1] = 0.0;
			p->dt[1] = GMT_rdc2dt (C, p->rd[1], p->sd[1]);
			GMT_gcal_from_rd (C, p->rd[1], &(p->cc[1]) );
...
{	/* Given the internal time representation dt and the formatting information
	...
	step = 0.5 / W->f_sec_to_int / C->current.setting.time_system.scale;	/* Precision desired in time units */
...
{	/* Assemble the annotation label given the formatting options presented */
	...
	GMT_gcal_from_dt (C, t, &calendar);			/* Convert t to a complete calendar structure */
