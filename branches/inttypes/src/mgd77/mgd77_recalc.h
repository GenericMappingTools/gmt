...
struct MGD77_E77_APPLY {
	...
	*/
	GMT_BOOLEAN apply_corrections;	/* TRUE if one or more corrections are requested */
	GMT_BOOLEAN correction_requested[N_E77_CORR_FIELDS];	/* TRUE for each field we must correct */
	GMT_BOOLEAN got_it[MGD77_SET_COLS];	/* TRUE for each original MGD77 column that was requested among the output columns */
