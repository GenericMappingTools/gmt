...
a 4-byte integer.  Al integers used in the API are 4-byte integers with the exception
of one function where a longer integer is used.  Since different operating systems have their
own way of defining 8-byte integers we use C99's \texttt{int64\_6} for this purpose; it is guaranteed to yield
...
\texttt{	int id;}		&       /* \emph{An identification number} */ \\
\texttt{	uint64_t n\_rows;}	&       /* \emph{Number of rows in each vector} */ \\
...
\begin{verbatim}
struct GMTAPI_CTRL * GMT_Create_Session (char *tag, int mode)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Create_Data (struct GMTAPI_CTRL *API, int family, int64_t par[])
\end{verbatim}
...
\begin{verbatim}
int GMT_Begin_IO (struct GMTAPI_CTRL *API, int family, int direction)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Read_Data (struct GMTAPI_CTRL *API, int family, int method, \
    int geometry, double wesn[], int mode, char *input, void *data)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Get_Data (struct GMTAPI_CTRL *API, int ID, int mode, \
	void *data)
\end{verbatim}
...
\begin{verbatim}
void * GMT_Get_Record (struct GMTAPI_CTRL *API, int mode, int *nfields)
\end{verbatim}
...
\begin{verbatim}
int GMT_End_IO (struct GMTAPI_CTRL *API, int direction, int mode)
\end{verbatim}
...
\begin{verbatim}
int GMT_Write_Data (struct GMTAPI_CTRL *API, int family, int method, \
    int geometry, double wesn[], int mode, void *output, void *data)
\end{verbatim}
...
format specifier for a int variable (e.g., ``New\_Table\_\%6.6ld.txt''), which will be
...
format specifiers for two int variables (e.g., ``New\_Table\_\%6.6ld\_Segment\_\%3.3ld.txt''),
...
\begin{verbatim}
int GMT_Put_Data (struct GMTAPI_CTRL *API, int ID, int mode, \
	void *data)
\end{verbatim}
...
\begin{verbatim}
int GMT_Put_Record (struct GMTAPI_CTRL *API, int mode, void *rec)
\end{verbatim}
...
\begin{verbatim}
int GMT_Destroy_Data (struct GMTAPI_CTRL *API, int mode, void *data)
\end{verbatim}
