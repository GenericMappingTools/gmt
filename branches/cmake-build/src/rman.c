...
enum command {
	...
	/*BEGINCHARTAGS, */
	CHARTAB = '\t',
	CHARPERIOD = '.', CHARLSQUOTE = '`', CHARRSQUOTE = '\'', CHARGT =
		'>', CHARLT = '<',
	CHARAMP = '&', CHARBACKSLASH = '\\', CHARDASH = '-', CHARHAT =
		'^', CHARVBAR = '|',
	CHARNBSP = 0xa0, CHARCENT = 0xa2, CHARSECT = 0xa7, CHARCOPYR =
		0xa9, CHARNOT = 0xac,
	CHARDAGGER = 0xad, CHARREGTM = 0xae, CHARDEG = 0xb0, CHARPLUSMINUS =
		0xb1,
	CHARACUTE = 0xb4, CHARBULLET = 0xb7, CHAR14 = 0xbc, CHAR12 =
		0xbd, CHAR34 = 0xbe,
	...
	/*BEGINFONTTAGS, */
	BEGINBOLD, ENDBOLD, BEGINITALICS, ENDITALICS, BEGINBOLDITALICS,
	ENDBOLDITALICS,
	BEGINSC, ENDSC, BEGINY, ENDY, BEGINCODE, ENDCODE, BEGINMANREF,
	ENDMANREF,
	FONTSIZE,
	/*ENDFONTTAGS*/
	/*BEGINLAYOUTTAGS, */
	...
	/*BEGINSTRUCTTAGS, */
	BEGINDOC, ENDDOC, BEGINCOMMENT, ENDCOMMENT, COMMENTLINE, BEGINBODY,
	ENDBODY,
	BEGINHEADER, ENDHEADER, BEGINFOOTER, ENDFOOTER, BEGINLINE, ENDLINE,
	SHORTLINE,
	BEGINSECTION, ENDSECTION, BEGINSUBSECTION, ENDSUBSECTION,
	BEGINSECTHEAD, ENDSECTHEAD, BEGINSUBSECTHEAD, ENDSUBSECTHEAD,
	BEGINBULPAIR, ENDBULPAIR, BEGINBULLET, ENDBULLET, BEGINBULTXT,
	ENDBULTXT,
	BEGINTABLE, ENDTABLE, BEGINTABLELINE, ENDTABLELINE, BEGINTABLEENTRY,
	ENDTABLEENTRY,
...
const unsigned char trouble[] = { CHARTAB, CHARPERIOD, CHARLSQUOTE, CHARRSQUOTE,
	CHARGT, CHARLT, CHARAMP, CHARBACKSLASH, CHARDASH, CHARHAT, CHARVBAR,
	CHARCENT,
	CHARSECT, CHARCOPYR, CHARNOT, CHARDAGGER, CHARREGTM, CHARDEG,
	CHARPLUSMINUS,
...
enum { NAME, SYNOPSIS, DESCRIPTION, SEEALSO, FILES, AUTHOR,
	RANDOM /*last! */  };
char *sectheadname[] = {
	"NAME:NOMBRE", "SYNOPSIS", "DESCRIPTION:INTRODUCTION",
	"SEE ALSO:RELATED INFORMATION", "FILES", "AUTHOR:AUTHORS", "RANDOM"
};
...
int lcexceptionscmp(const void *a, const void *b)
{
	return stricmp(a, b);
}
...
int strcoloncmp2(char *candidate, int end, const char *list, int sen)
{
	const char *l = list;
	char *c, c2;
	...
	while (*l) {
		...
		for (c = candidate; *c && *l; c++, l++)
			if ((sen && *c != *l)
					|| (!sen && tolower((int)*c) != tolower((int)*l)))
				break;
...
int strcoloncmp(char *candidate, int end, const char *list)
{
	int sen = 1;
	const char *l = list;
...
void casify(char *p)
{
	...
	for (fuc = 1; *p; p++) {
		if (isspace(*p) || strchr("&/", *p) != NULL)
			...
			*q = '\0';
			exp =
				(char **)bsearch(&p, lcexceptions, lcexceptionslen,
												 sizeof(char *), lcexceptionscmp);
			*q = tmpch;
			if (exp != NULL) {
				for (q = *exp; *q; q++)
					*p++ = *q;
...
/* add an attribute tag to a range of characters */
	...
	void
tagadd(int /*enum tagtype--abused in source parsing */ type, int first,
			 int last)
{
	assert(type != NOTAG);
...
void manrefextract(char *p)
{
	...
	if (strincmp(p, "http", 4) == 0) {
		...
		manrefname = p;
		p += 4;
		while (*p && !isspace(*p) && !strchr(nonhref, *p))
			p++;
	} else {
		href = manRef;
...
{
	fprintf(stderr,
					"The output formats for Tk and TkMan require nroff-formatted input\n");
	exit(1);
}
...
void TkMan(enum command cmd)
{
	...
	switch (cmd) {
		...
		case ENDDOC:
			...
			printf("set manx(tabcnts) {");
			for (i = 1; i < CurLine; i++)
				printf("%d ", linetabcnt[i]);
			printf("}\n");
			printf("set manx(clo) {");
			for (i = 0; i < clocnt; i++)
				printf("%d ", clo[i]);
			printf("}\n");
			printf("set manx(para) {");
			for (i = 0; i < paracnt; i++)
				printf("%d ", para[i]);
			printf("}\n");
			printf("set manx(reb) {");
			for (i = 0; i < rebuscnt; i++)
				printf("%d ", rebus[i]);
			printf("}\n");
		...
		case BEGINLINE:
			...
			c = plain[0];
			if (linelen >= 2 && ((c == '-' || c == '%' || c == '\\' || c == '$' /**/ /* not much talk of money in man pages so reasonable */ ) && (isalnum(plain[1])	/*<= plain[1]!='-' */
																																																																						 /*no dash */
																																																																						 ||ncnt
																																																																						 /*GNU long option */
																																																																						)
			...
			if (rebuspatcnt && scnt >= 5 /* not sect or subsect heads */ ) {
				for (p = plain; *p && *p != ' '; p++)	/*empty */
					;	/* never first word */
				while (*p) {
					for (i = 0; i < rebuspatcnt; i++) {
						if (tolower((int)*p) ==
								tolower((int)*rebuspat[i])
								&& strincmp(p, rebuspat[i],
														rebuspatlen[i]) == 0) {
			...
			if (fReflow && !ncnt && (finflow || lastlinelen > 50)
					&& (abs(scnt - lastscnt) <= 1
							|| abs(scnt - hanging) <= 1)) {
				finflow = 1;
				putchar(' ');
...
void pod(enum command cmd)
{
	...
	if (cmd == BEGINBULPAIR) {
		...
		if (curindent && hanging != curindent)
			printf("\n=back\n\n");
...
void HTML(enum command cmd)
{
	...
	switch (cmd) {
		...
		case CHARBULLET:
			if (I > 0 || !finlist)
				printf("&#183;"
							 /*"&middot;" *//*&sect;--middot hardly visible */
							);
			break;
		case CHARPLUSMINUS:
			printf("&plusmn;");
	...
	switch (cmd) {
		...
		case ENDDOC:
			...
			if (!tocc) {
				...
				for (i = 0, lasttoc = BEGINSECTION; i < tocc;
					...
					printf
						("<li><a name='toc%d' href='#sect%d'>%s</a></li>\n",
						 i, i, toc[i].text);
				}
				if (lasttoc == BEGINSUBSECTION)
					printf("</ul>");
				printf("</ul>\n");
			}
			printf("</body>\n</html>\n");
		...
		case BEGINSECTION:
			break;
		case ENDSECTION:
			if (sectheadid == NAME && message != NULL)
				printf(message);
			break;
		case BEGINSUBSECTION:
			break;
		...
		case BEGINCODEBLOCK:
			printf("<pre>");
			pre = 1;
			break;		/* wrong for two-column lists in kermit.1, pine.1, perl4.1 */
		case ENDCODEBLOCK:
			printf("</pre>");
			pre = 0;
			break;
		...
		case BEGINMANREF:
			...
			if (fmanRef) {
				printf("<a href='");
				printf(href, manrefname, manrefsect);
				printf("'>");
			} else
				printf("<i>");
			break;
...
void XML(enum command cmd)
{
	...
	*/
	...
	switch (cmd) {
		...
		case BEGINSECTHEAD:
		case BEGINSUBSECTHEAD:
			if (sectheadid != NAME && sectheadid != SYNOPSIS)
				printf("<title>");
			break;
		case ENDSECTHEAD:
		case ENDSUBSECTHEAD:
...
{
	fprintf(stderr,
					"FrameMaker comes with filters that convert from roff to MIF.\n");
	exit(1);
}
...
{
	assert(cmd == ENDINDENT || cmd == BEGINBULLET || cmd == BEGINBULTXT
				 || cmd == BEGINBULPAIR || cmd == BEGINBODY || cmd == BEGINSECTION
				 || cmd == BEGINSUBSECTION || cmd == ENDDOC);
	/*
		 int i;
		 int p;
		 int match;
	...
	if (cmd == BEGINBULTXT || cmd == BEGINBULLET)
		return;

	if (Pb && cmd == BEGINBULPAIR) {
		(*fn) (ENDBODY);
		Pb = 0;
	}			/* special */
	if (Pbp) {
	...
	if (cmd == BEGINBULPAIR || cmd == ENDINDENT)
		return;
...
char *la_gets(char *buf)
{
	...
	if (fla) {
		...
		if ((fPara || sectheadid == SEEALSO || fNOHY) && p > buf
				&& p[-1] == '-' && isspace(lookahead)) {
			p--;	/* zap hyphen */
			...
			while (lookahead && isspace(lookahead)
						 && lookahead != '\n') {
				getchartab();
				hy++;
			}
			...
			while (lookahead && !isspace(lookahead) && i++ < MAXBUF)
				*p++ = getchartab();
...
void filterline(char *buf, char *plain)
{
	char *p, *q, *r;
	char *ph;
	int iq;
	...
	for (scnt = 0, p = plain; *p == ' '; p++)
		scnt++;		/* initial space count */
	if (scnt > 200)
		scnt = 130 - (q - p);
	...
	if (fChangeleft)
		for (; q - 40 > plain && *q == '|'; q--) {	/* change bars */
			if (fChangeleft != -1)
				ccnt++;
			while (q - 2 >= plain && q[-1] == '\b' && q[-2] == '|')
				q -= 2;	/* boldface changebars! */
		}
	...
	if (q - plain > 100 && *q == '|') {
		...
		while ((*q == ' ' || *q == '_' || *q == '-') && q > plain)
			q--;
	}

	for (r = q; (*r & 0xff) == CHARDAGGER; r--)
		*r = '-';	/* convert daggers at end of line to hyphens */
	...
	if (indent >= 0 && scnt >= indent)
		scnt -= indent;
	if (!fPara && !fIQS) {
		if (fChangeleft)
			I += (scnt > ccnt) ? scnt : ccnt;
		else
			I += scnt;
	}
	I0 = I;
	...
	for (q = plain; *p; p++) {
		...
		if (*p == ' ') {
			...
			/* reduce strings of <3 spaces to 1 */
		...
		if (*p == '\b' && p[1] == '_' && q > plain && q[-1] == '+') {
			...
			if ((*p == '_' && p[1] == '\b' && p[2] != '_'
				...
				tag = ITALICS;
				p += 2;
			} else if (*p == '_' && p[2] == p[4] && p[1] == '\b'
								 && p[3] == '\b' && p[2] != '_') {
				/* bold italics (for Solaris) */
				for (p += 2; *p == p[2] && p[1] == '\b';)
					p += 2;
				if (tag != BOLDITALICS && hl >= 0) {
					tagadd(tag, hl, I + iq);
					hl = -1;
				}
				if (hl == -1)
		...
		if (*p == '(' && q > plain
			...
			while (tagc > 0 && tags[tagc - 1].last > hl2)
				tagc--;
		...
		/*assert(*p!='\0'); */
		if (!*p)
			break;	/* not just safety check -- check out sgmls.1 */
	...
	*q = *ph = '\0';
	...
	for (i = tagci; i < tagc; i++) {
		...
		for (j = i + 1; j < tagc; j++) {
			assert(tags[i].last <=
						 tags[j].
						 first /*|| tags[i].first>=tags[j].last */ );
		}
	}
}
...
void preformatted_filter(void)
{
	...
	while (la_gets(buf) != NULL) {
		...
		finTable = fTable &&
			((!ncnt && fotable) ||
			 (ncnt && bs_cnt >= 2 && bs_cnt <= 5
				&& ((float)bs_sum / (float)bs_cnt) > 3.0));
		if (finTable) {
			if (!fotable)
				(*fn) (BEGINTABLE);
		} else if (fotable) {
			...
			I = I0;
			tagc = 0;
			filterline(buf, plain);	/* rescan first line out of table */
		}
#endif
		...
		if (s_cnt >= 2) {
			...
			if (scnt2)
				s_avg =
					(float)(s_sum - scnt2) / (float)(s_cnt - 1);
			else
		...
		if ( /*fMan && ( */ !fHead || indent == -1 /*) */ ) {
			...
			if (!*header) {
				...
				if (stricmp(p, "NAME") == 0 || stricmp(p, "NOMBRE") == 0) {	/* works because line already filtered */
					indent = scnt;	/*filterline(buf,plain); */
					scnt = 0;
					I = I0;
					fHead = 1;
				} else {
					...
					strcpy(header, p);
					if ((header_m = HEADFOOTSKIP) > linelen)
						header_m = 0;
					strcpy(head, phrase);
					headlen = phraselen;
					la_gets(buf);
		...
		if ( /*fMan && ( */ scnt == 0 || scnt > MINMID /*) */ ) {
			...
			if (!fFoot && !isspace(*p)
					...
					(((ncnt >= 2
						 && AbsLine /*+ncnt */  >= 61 /*was 58 */ 
						 && AbsLine /*+ncnt */  < 70)
						|| (ncnt >= 4 && AbsLine /*+ncnt */  >= 59
								&& AbsLine /*+ncnt */  < 74)
						|| (ncnt && AbsLine /*+ncnt */  >= 61
								&& AbsLine /*+ncnt */  <= 66))
					 && ( /*lookahead!=' ' || */
								(s_cnt >= 1 && s_avg > 1.1) || !falluc))
				 ) {
				(*fn) (BEGINFOOTER);
				/* grab footer and its first word */
				strcpy(footer, p);
				/*                              if ((footer_m=linelen-HEADFOOTSKIP)<0) footer_m=0; */
				if ((footer_m = HEADFOOTSKIP) > linelen)
					footer_m = 0;
				/*grabphrase(p); */ strcpy(foot, phrase);
				footlen = phraselen;
				...
				if (!fFoot && *header == '\0')
					fHead = 0;	/* this is dangerous */
				fFoot = 1;
				continue;
			} else
				/* a lot of work, but only for a few lines (about 4%) */
				if (fFoot
						&& (scnt == 0 || scnt + indent > MINMID)
						&&
						((headlen && strncmp(head, p, headlen) == 0)
						 ...
						 || (header_m && header_m < linelen &&
								 strncmp(&header[header_m],
												 &p[header_m],
												 HEADFOOTMAX) == 0)
						 || (footer_m && footer_m < linelen
								 && strncmp(&footer[footer_m],
														&p[footer_m],
														HEADFOOTMAX) == 0)
						 ...
						 || (headlen && headlen < linelen &&
								 strncmp(head, &p[linelen - headlen],
												 headlen) == 0)
						 || (footlen && footlen < linelen
								 && strncmp(foot, &p[linelen - footlen],
														footlen) == 0)
						)) {
					tagc = 0;
					title = 1;
		...
		/*tabgram[scnt]++; */
		if (title)
			ncnt = (scnt != oscnt || ( /*scnt<4 && */ isupper(*p)));
		itabcnt = scnt / 5;
		if (CurLine == 1) {
			ncnt = 0;
			tagc = 0;
		}		/* gobble all newlines before first text line */
		sect = (scnt == 0 && isupper(*p));
		subsect = (fSubsections && (scnt == 2 || scnt == 3));
		if ((sect || subsect) && ncnt > 1)
			ncnt = 1;	/* single blank line between sections */
		(*fn) (BEGINLINE);
		if ( /*fPara && */ ncnt)
			Pnew = 1;
		...
		for (i = 0, p = plain, curtag = 0, fcont = 0; *p;
				 p++, i++, fcont = 0) {
			...
			if (curtag < tagc && i + I0 + off == tags[curtag].first) {
				for (r = hitxt, j =
						 tags[curtag].last - tags[curtag].first,
						 hitxt[j] = '\0'; j; j--)
					hitxt[j - 1] = p[j - 1];
		...
		else if (scnt + linelen + spcsqz < MINRM /*&& ncnt */ 
						 && lookahead != '\n'
						 && prevcmd != BEGINBULTXT && prevcmd != ENDSUBSECTHEAD
	...
	while (1) {
		...
		*p = '\0';
		...
		if (*p == '\\' && *(p + 1) == '"') {
			if (!fTclTk
					&& strstr(p + 1,
										"supplemental macros used in Tcl/Tk") !=
					NULL)
				fTclTk = 1;
			p += 2;
		}
		...
		while (*p) {
			...
			if (*p == '\\') {
				...
				if (*p == 'n') {
					...
					*q = '0';
					*(q + 1) = '\0';	/* defaults to 0, in case doesn't exist */
					for (i = 0; i < regcnt; i++) {
...
void source_flush(void)
{
	...
	if (sI >= 4 /*+1 */ 
		...
		if (*q == '(' && strcoloncmp(&q[1], ')', vollist)) {
			...
			if (*r == ' '
					&& (sectheadid == SEEALSO
							|| /*single letter volume */ *(q + 2) == ')'
							|| *(q + 3) == ')'))
				r--;	/* permitted single intervening space */
	...
	for (i = 0, j = 0, p = plain; i < sI && *p; i++, p++) {
		if (!linelen)
			(*fn) (BEGINLINE);	/* issue BEGINLINE when know will be chars on line */
...
char *source_out0(char *p, char end)
{
		...
		if (*p == '\\') {	/* escape character */
			switch (*++p) {
				...
				case 's':	/* font size change */
					...
					intArg = sign * ((*p++) - '0');
					if (intArg == 0)
						intArg = -fontdelta;	/* s0 returns to normal height */
					if (fontdelta || intArg) {
						source_flush();
						(*fn) (FONTSIZE);
					}
				...
				case '(':	/* multicharacter macros */
					...
					for (i = 0; i < speccnt; i++) {
						if (p[0] == spec[i].key[0]
								&& p[1] == spec[i].key[1]) {
							p += 2;
							for (j = 0; spec[i].subst[j];
									 j++)
								sputchar(spec[i].subst[j]);
							break;
						}
					}
					break;
	...
	if (*p && *p != ' ')
		p++;		/* skip over end character */
	return p;
	}

	/* oh, for function overloading.  inlined by compiler, probably */
	char *source_out(const char *p)
	{
		return source_out0((char *)p, '\0');
	}
...
#define checkcmd(str)   strcmp(cmd,str)==0
	...
	void source_list(void)
	{
		...
		q = plain;
		plain[sI] = '\0';
		if ( /*c==CHARBULLET || q=='-' -- command line opts! || */ *q == '.'
				 || *q & 0x80) {
			listtype = UL;
			q++;
		} else {
			...
			while (isdigit(*q)) {
				listtype = OL;
				q++;
			}		/* I hope this gives the right number */
			if (*q == '.')
				q++;
			if (strchr(")]", *q))
				q++;
		...
		i = sI;
		sI = 0;
		if (!Pbp || listtype != oldlisttype)
			poppush(BEGINBULPAIR);
		poppush(BEGINBULLET);
		/*if (tphp) source_line(p); else source_out_word(p); */
		/*if (listtype!=OL && listtype!=UL) */ sI = i;
		source_struct(ENDBULLET);
		Pbul = 0;		/* handled immediately below */
		poppush(BEGINBULTXT);
	...
	void source_command(char *p)
	{
		...
		if (isComment) {
			...
			if (*p) {
				...
				finnf = 0;
				for (j = 0; (sectheadid = j) < RANDOM; j++)
					if (strcoloncmp2(p, '\0', sectheadname[j], 0))
						break;
				if (!finitDoc) {
					/* handle missing .TH */
				...
				while (isspace(plain[--sI]))	/*nada */
					;
				plain[++sI] = '\0';
				tocadd(plain, BEGINSECTION, CurLine);	/* flushed with source_struct above */
				fFlush = 1;
			...
			if (*p) {
				...
				source_flush();
				poppush(BEGINSUBSECTION);
				source_struct(BEGINSUBSECTHEAD);
				fFlush = 0;
		...
		} else if ((tphp = checkcmd("TP")) || (tphp = checkcmd("HP"))
							 || checkcmd("IP") || checkcmd("LI")) {
			...
			if (*p == 'n') {
				cond = 1;
				p++;
			} /* masquerading as nroff the right thing to do? */
			else if (*p == 't') {
				cond = 0;
				p++;
				...
				*q = '\0';
				...
				cond = 0;	/* a guess, seems to be right bettern than half the time */
			if (invcond)
				cond = 1 - cond;
			while (isspace(*p))
				p++;
			...
			if (strncmp(p, "\\{", 2) == 0) {	/* rather than handle groups here, have turn on/off output flag? */
				...
				while (isspace(*p))
					p++;
				while (strncmp(p, ".\\}", 3) != 0
							 || strncmp(p, "\\}", 2) != 0 /*Solaris */ ) {
					if (cond)
						source_line(p);
					if ((p = source_gets()) == NULL)
						break;
				}
			} else if (cond)
				source_line(p);
			...
			if (ie)
				source_line(source_gets());	/* do else part with prevailing lastif */
			...
			while ((p = source_gets()) != NULL) {
				if (strcmp(p, endig) == 0)
					break;
				if (!lastif)
					source_line(p);	/* usually ignore line, except in one weird case */
			}
			...
			*q = '\0';
			...
			for (insertat = 0; insertat < macrocnt; insertat++) {
				if (strcmp(p, macro[insertat].key) == 0)
					break;
			}
			...
			*q = '\0';
			...
			if (fTclTk) {
				...
				if (sobuf != NULL) {
					...
					fid = open(p, O_RDONLY);
					if (fid != -1) {
						if (read(fid, sobuf, fileinfo.st_size)
								== fileinfo.st_size) {
		...
		if (err) {
			fprintf(stderr,
							"%s: couldn't read in referenced file %s.\n",
							argv0, p);
			if (strchr(p, '/') == NULL) {
				fprintf(stderr,
								"\tTry cd'ing into same directory of man page first.\n");
			} else if (strchr(p, '/') == strrchr(p, '.')) {
				fprintf(stderr,
								"\tTry cd'ing into parent directory of man page first.\n");
			} else {
				fprintf(stderr,
		...
		tblc = 0;	/*tblspanmax=0; */
		tableSep = '\0';	/* need to reset each time because tabbed lines (.ta) made into tables too */
		while ((p = source_gets()) != NULL) {
			...
			/* HP has a prequel terminated by ';' */
			for (i = 0; *p; i++, p = q) {
				if (*p == '.')
					break;
				if (*p == 'f')
					p += 2;	/* DEC sets font here */
				q = p + 1;
		...
		while ((p = source_gets()) != NULL) {
			...
			insertat = 0;
			for (j = 0; *tbl[tbli][j]; j++)
				if (*tbl[tbli][j] != 's')
					insertat++;
			if (!tableSep && insertat > 1) {
				if (fsourceTab)
					tableSep = '\t';
				else
					tableSep = '@';
			...
			for (i = 0; *tbl[tbli][i] && *p; i++) {
				...
				if (*tblcellformat == '^') {	/* vertical span => blank entry */
					...
					/* stagadd(VBAR); */
					continue;
				} else if (strchr("lrcn", *tblcellformat) ==
									 NULL) {
					tblcellformat = "l";
					/*continue; */
				}

				while (strncmp(tbl[tbli][i + 1], "s", 1) == 0) {
					tblcellspan++;
					i++;
				}

				source_struct(BEGINTABLEENTRY);
				if (toupper(tblcellformat[1]) == 'B')
					stagadd(BEGINBOLD);
				else if (toupper(tblcellformat[1]) == 'I')
					stagadd(BEGINITALICS);
		...
		*q = '\0';
		...
		for (insertat = 0; insertat < regcnt; insertat++) {
			if (strcmp(reg[insertat].name, p) == 0)
				break;
		}
		if (insertat == regcnt) {
			regcnt++;
		...
		p = q;
		if (*q == '+' || *q == '-')
			q++;	/* accept signed, floating point numbers */
		if (*q == '.')
			q++;
		if (!*q || isspace(*q)) {
			*q++ = '0';
		...
		*q = '\0';	/* ignore units */
		...
		source_struct(BEGINSECTHEAD);
		source_out("STANDARD OPTIONS");
		source_struct(ENDSECTHEAD);
		tblc = 1;
		tbl[0][0] = tbl[0][1] = tbl[0][2] = "l";
		tbl[0][3] = "";
		source_struct(BEGINTABLE);
		while (1) {
			p = source_gets();
		...
		if (i == macrocnt) {
			...
			for (j = 0; j < macnotcnt; j++)
				if (strcmp(macnotfound[j], cmd) == 0)
					break;
			if (j == macnotcnt) {
				if (!fQuiet)
					fprintf(stderr,
									"macro \"%s\" not recognized -- ignoring\n",
									cmd);
				q = malloc(strlen(cmd) + 1);
				strcpy(q, cmd);
...
void source_line(char *p)
{
	...
	char *cmd = p;
	if (p == NULL)
		return;		/* bug somewhere else, but where? */

	isComment = ( /*checkcmd("") || */ checkcmd("\\\"")
								|| /*DEC triple dot */ checkcmd(".."));
	if (inComment && !isComment) {
		source_struct(ENDCOMMENT);
		inComment = 0;
	}
	/* special case to handle transition */
#if 0
	if (*p != '.' && *p != '\'' && !finlist) {
		if (fsourceTab && !fosourceTab) {
			...
			tbli = 0;
			tableSep = '\t';
			tbl[0][0] = tbl[0][1] = tbl[0][2] = tbl[0][3] =
				tbl[0][4] = tbl[0][5] = tbl[0][6] = tbl[0][7] =
				tbl[0][8] = "l";
			source_struct(BEGINTABLE);
			finTable = 1;
	...
	if (*p == '.' /*|| *p=='\''  -- normalized */ ) {	/* command == starts with "." */
		...
		/*source_command("P"); */
		ncnt = 1;
		source_struct(BEGINLINE);
		ncnt = 0;	/* empty line => paragraph break */
...
void source_filter(void)
{
	...
	*p = '\n';
	...
	while (fDiff && fgets(diffline, MAXBUF, difffd) != NULL) {
		...
		for (q = diffline;; q++) {
			diffcmd = *q;
			if (diffcmd == 'a' || diffcmd == 'c' || diffcmd == 'd')
				break;
		}
		if (sscanf(diffline, "%d,%d", &on1, &on2) == 1)
			on2 = on1 - 1 + (diffcmd == 'd' || diffcmd == 'c');
		if (sscanf(++q, "%d,%d", &nn1, &nn2) == 1)
			nn2 = nn1 - 1 + (diffcmd == 'a' || diffcmd == 'c');
				...
				*shiftp = tmpc;	/* includes space */
				if (shiftq != NULL) {
					...
					*shiftq = tmpc;
					...
					shiftp--;
					*shiftp = '\0';	/* squash the trailing space */
					stagadd(BEGINDIFFD);
					source_line(p);
					stagadd(ENDDIFFD);
					source_line(" ");
		...
		while (insertcnt--)
			fgets(diffline, MAXBUF, difffd);	/* eat duplicate text of above */
		while ( /*insertcnt-- */ AbsLine < nextDiffLine
						&& (p = source_gets()) != NULL) {
			if (first && *p != '.') {
				stagadd(BEGINDIFFA);
				first = 0;
...
int main(int argc, char *argv[])
{
	...
	*p = '\0';
	...
	for (i = 0; i < argc; i++)
		argvch[i] = argv[i];	/* need argvch[0] for getopt? */
	...
	while ((c = getopt(argc, argvch, strgetopt)) != -1) {
		...
		switch (c) {
			...
			case 'n':
				strcpy(manName, optarg);
				fname = 1;
				break;	/* name & section for when using stdin */
			case 's':
				strcpy(manSect, optarg);
				break;
			...
			case 'R':
				...
				for (; *p; oldp = p, p++) {
					if (*oldp == '\0')
						rebuspat[rebuspatcnt++] = p;
					if (*p == '|')
						*p = '\0';
				}
				for (i = 0; i < rebuspatcnt; i++)
					rebuspatlen[i] = strlen(rebuspat[i]);	/* for strnlen() */
				break;
	...
	if (optind < argc) {
		...
		if (!fname) {	/* if no name given, create from file name */
			...
			if ((p = strrchr(argvch[optind], '/')) != NULL)
				p++;
			else
				p = argvch[optind];
			strcpy(manName, p);
	...
	if (fDiff && (!fSource || fn != HTML)) {
		fprintf(stderr,
						"diff incorporation supported for man page source, generating HTML\n");
		exit(1);
	}
