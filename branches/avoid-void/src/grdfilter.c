...
GMT_LONG init_area_weights (struct GMT_CTRL *GMT, struct GMT_GRID *G, GMT_LONG mode, struct GMT_GRID *A, char *file)
{
	...
	if (file) {	/* For debug purposes: Save the area weight grid */
		...
		if ((error = GMT_Put_Data (GMT->parent, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, 0, file, A))) return (error);
...
GMT_LONG GMT_grdfilter (struct GMTAPI_CTRL *API, GMT_LONG mode, void *args)
{
	...
	A = GMT_Create_Data (API, GMT_IS_GRID, NULL);
#ifdef DEBUG
...
#pragma omp for schedule(dynamic,1) private(row_out,y_out,lat_out,j_origin,y,col_out,wt_sum,value,n_in_median,ij_out,ii,col_in,jj,row_in,ij_in,ij_wt) firstprivate(y_shift) nowait
	...
	if (Ctrl->F.highpass) {
		if (GMT->common.R.active || Ctrl->I.active || GMT->common.r.active) {	/* Must resample result */
			...
			if (GMT_Register_IO (API, GMT_IS_GRID, GMT_IS_REF, GMT_IS_SURFACE, GMT_IN, &Gout, NULL, &object_ID)) Return (EXIT_FAILURE);
			GMT_Encode_ID (API, in_string, object_ID);	/* Make filename with embedded object ID for grid Gout */
			if (GMT_Register_IO (API, GMT_IS_GRID, GMT_IS_REF, GMT_IS_SURFACE, GMT_OUT, &L, NULL, &object_ID)) Return (EXIT_FAILURE);
			...
			GMT_Destroy_Data (API, GMT_ALLOCATED, &Gout);
	...
	if (Ctrl->A.active) {	/* Save the debug output instead */
		...
		fprintf (fp, "%ld\n", n_conv);
		fclose (fp);
		if ((error = GMT_Put_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, 0, Ctrl->G.file, Gin))) Return (error);
		GMT_free_grid (GMT, &Gout, TRUE);	/* Was never used due to testing */
	}
	else if (Ctrl->F.highpass && L) {
		if ((error = GMT_Put_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, 0, Ctrl->G.file, L))) Return (error);
	}
	else {
		if ((error = GMT_Put_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, NULL, 0, Ctrl->G.file, Gout))) Return (error);
	}
#else
