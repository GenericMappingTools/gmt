...
\texttt{ptr} is the address of the pointer to the named resource.
...
resources will be accessed:  Select GMT\_BY\_SET to read entire data or text tables, CPT files, or grids, and use
GMT\_BY\_REC when you need to access a dataset (or text set) record by record.  The latter selection involves determining
...
\begin{tabular}{ll} \hline
\multicolumn{2}{l}{\texttt{struct GMT\_VECTOR \{}} \\ 
\texttt{GMT_REC_IS_TBL_HEADER(API)}	&       TRUE if we read a table header \\
\texttt{GMT_REC_IS_SEG_HEADER(API)}	&       TRUE if we read a segment header \\
\texttt{GMT_REC_IS_ANY_HEADER(API)}	&       TRUE if we read either header type \\
\texttt{GMT_REC_IS_ERROR(API)}		&       TRUE if we had a read or conversion failure \\
\texttt{GMT_REC_IS_EOF(API)}		&       TRUE if we reached the end of the file (EOF) \\
\texttt{GMT_REC_IS_NAN(API)}		&       TRUE if we only read NaNs \\
\texttt{GMT_REC_IS_GAP(API)}		&       TRUE if this record implies a data gap \\
\texttt{GMT_REC_IS_NEW_SEGMENT(API)}	&       TRUE if we enter a new segment \\
\texttt{GMT_REC_IS_LINE_BREAK(API)}	&       TRUE if we encountered a segment header, EOF, NaNs or gap \\
\texttt{GMT_REC_IS_FILE_BREAK(API)}	&       TRUE if we finished one file but not the last \\
\texttt{GMT_REC_IS_DATA(API)}		&       TRUE if we read a data record \\
\texttt{\};}	&        \\  \hline
\end{tabular}
...
\begin{verbatim}
long GMT_Put_Data (struct GMTAPI_CTRL *API, long family, long method, \
