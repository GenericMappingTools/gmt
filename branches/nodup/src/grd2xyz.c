...
#define Return(code) {Free_grd2xyz_Ctrl (GMT, Ctrl); GMT_end_module (GMT); bailout (code);}
...
int GMT_grd2xyz (void *V_API, int mode, void *args)
{
	...
	if (GMT_Init_IO (API, GMT_IS_DATASET, GMT_IS_POINT, GMT_OUT, GMT_ADD_STDIO_IF_NONE, 0, options) != GMT_OK) {	/* Registers stdout, unless already set */
		Return (API->error);
	}
