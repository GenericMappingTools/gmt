...
documentation tests are located in [doc/scripts/](/doc/scripts) directory. The majority of GMT tests are plot-based, with
each test requiring a bash script for generating the plot and a reference PostScript file. These tests pass if the
...
To enable testing, you need *uncomment* following lines in your `ConfigUserAdvanced.cmake` when configuring GMT:
...
Pull requests should avoid needing to change PostScript files in the `test/` and `doc/scripts/` directories. However, if
this is unavoidable, new PostScript reference files can be generated by running `ctest -R <test-script-name>` in the
build directory after following the [building guide](BUILDING.md) and the [Configuring CMake for testing GMT](MAINTENANCE.md#configuring-cmake-for-testing-gmt) instructions. The new PostScript file can be copied from the appropriate
subdirectory within `build/test/` or `build/doc/scipts/`.
